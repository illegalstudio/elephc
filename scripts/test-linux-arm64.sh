...
if [ ${#TEST_ARGS[@]} -eq 0 ]; then
    echo "Running all tests on Linux ARM64 with RUST_TEST_THREADS=$TEST_THREADS using target volume '$TARGET_VOLUME'..."
    docker run \
        --platform "$PLATFORM" \
        --name "$CONTAINER_NAME" \
        --init \
    ...
    echo "Running tests matching '${TEST_ARGS[*]}' on Linux ARM64 with RUST_TEST_THREADS=$TEST_THREADS using target volume '$TARGET_VOLUME'..."
    docker run \
        --platform "$PLATFORM" \
        --name "$CONTAINER_NAME" \
        --init \
