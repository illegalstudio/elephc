...
# libelephc_tz.a exist in the target dir — `cargo test` alone never emits the
...
if [ ${#TEST_ARGS[@]} -eq 0 ]; then
    ...
    docker run \
        ...
        -w /app \
        "$IMAGE" \
        sh -c 'cargo build -p elephc-tls -p elephc-pdo -p elephc-crypto -p elephc-tz && cargo test'
else
    ...
    docker run \
        ...
        -w /app \
        "$IMAGE" \
        sh -c 'cargo build -p elephc-tls -p elephc-pdo -p elephc-crypto -p elephc-tz && cargo test "$@"' sh "${TEST_ARGS[@]}"
fi
