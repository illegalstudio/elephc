...
# libelephc_bcmath.a / libelephc_phar.a / libelephc_tz.a / libelephc_image.a / libelephc_web.a /
# libelephc_pcntl.a / libelephc_magician.a exist in the target dir —
...
if [ "$TEST_ARG_COUNT" -eq 0 ]; then
    ...
    docker run \
        ...
        -w /app \
        "$IMAGE" \
        sh -c 'cargo build -p elephc-tls -p elephc-pdo -p elephc-crypto -p elephc-bcmath -p elephc-phar -p elephc-tz -p elephc-image -p elephc-web -p elephc-pcntl -p elephc-magician && cargo test'
else
    ...
    docker run \
        ...
        -w /app \
        "$IMAGE" \
        sh -c 'cargo build -p elephc-tls -p elephc-pdo -p elephc-crypto -p elephc-bcmath -p elephc-phar -p elephc-tz -p elephc-image -p elephc-web -p elephc-pcntl -p elephc-magician && cargo test "$@"' sh "${TEST_ARGS[@]}"
fi
