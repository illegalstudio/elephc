...
if [ ${#TEST_ARGS[@]} -eq 0 ]; then
    echo "Running all tests on Linux x86_64..."
    docker run --platform "$PLATFORM" --rm -v "$PROJECT_DIR:/app" -w /app "$IMAGE" cargo test
else
    echo "Running tests matching '${TEST_ARGS[*]}' on Linux x86_64..."
    docker run --platform "$PLATFORM" --rm -v "$PROJECT_DIR:/app" -w /app "$IMAGE" cargo test "${TEST_ARGS[@]}"
fi
