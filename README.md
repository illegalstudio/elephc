...
- **Constant propagation after type checking**: forwards scalar local values through straight-line code and across simple agreeing `if` merges, which in turn unlocks more folding in later expressions such as `$x ** $y`.
