...
- **Constant propagation after type checking**: forwards scalar local values through straight-line code, across conservative agreeing `if` / `switch` / `try` merges, and through uniform local `?:` / `match` assignments, which in turn unlocks more folding in later expressions such as `$x ** $y`.
