...
- [x] Curated native dependencies v1 — `elephc native add/install/update/remove/list/doctor`, exact comment-preserving manifests and deterministic locks, content-addressed target/ABI/toolchain cache, transactional verified source builds, and read-only compile-time resolution. PCRE2 10.47 is the first package and links through an opaque Elephc shim with no production system-library fallback. This remains separate from Composer packages, Rust bridge crates, and toolchain installation.
