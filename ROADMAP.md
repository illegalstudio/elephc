...
- [x] Extended JSON surface: `json_encode($value, $flags, $depth)`, `json_decode($json, $associative, $depth, $flags)`, `json_last_error_msg()`, `json_validate()` (signatures + Phase 2 stubs; structural decode and flag observance pending)
...
- [x] `JSON_THROW_ON_ERROR` for `json_validate()` (throws `JsonException` with the PHP-compatible message instead of returning `false`) and `json_encode()` Inf/NaN (raises `JsonException` for `JSON_ERROR_INF_OR_NAN`); `_json_active_flags` + `__rt_json_throw_error` plumbing ready for additional encoder/decoder hooks
...
- [x] Inf/NaN detection in `json_encode()` (sets `JSON_ERROR_INF_OR_NAN`, substitutes `0` without the throw flag)
...
- [x] `JSON_PARTIAL_OUTPUT_ON_ERROR` semantics: elephc returns partial JSON output for every encoder error (Inf/NaN → `0`, malformed UTF-8 → bytes dropped or replaced) because string-typed builtins cannot return `false`; the flag is therefore effectively always observed
...
- [ ] PHP attributes runtime introspection — implement `ReflectionClass::getAttributes()`, `ReflectionMethod::getAttributes()`, `ReflectionProperty::getAttributes()`, plus `ReflectionAttribute::newInstance()`. Class declarations currently expose limited metadata through `class_attribute_names()`, `class_attribute_args()`, and `class_get_attributes()` with `ReflectionAttribute::getName()` / `getArguments()`, but Reflection lookups (`new ReflectionClass(...)->getAttributes()`) return `Undefined class: ReflectionClass`.
