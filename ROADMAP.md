...
Broad OOP + procedural date/time surface (`DateTime`, `DateTimeImmutable`, `DateTimeInterface`, `DateTimeZone`, `DateInterval`, `DatePeriod`, and ~45 procedural functions/aliases), plus `ext/calendar`, implemented as synthetic classes/methods over the date runtime. Residual php-src differences are tracked in `docs/php/datetime.md` and `DATETIME_PHP_SRC_COMPLIANCE_SPEC.md`.
...
- [x] `idate()` returns `false` for an empty or unrecognized literal or runtime-computed format (no `E_WARNING` emitted — documented limitation)
...
- [x] `DatePeriod` PHP 8.2+ mirror properties with PHP recurrence-count semantics; `implements IteratorAggregate`; `getStartDate(): DateTimeInterface` / `getEndDate(): ?DateTimeInterface` return types (independent iterator and readonly enforcement remain documented gaps)
...
- [x] `getLastErrors()` shares state across mutable/immutable classes, returns `false` when clean, and reports the principal byte-positioned errors/warnings; the exhaustive timelib message/count table remains documented
