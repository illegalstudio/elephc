...
- [x] `date()` / `gmdate()` specifiers including `O P Z T e I c r u v p B`; `createFromFormat()` specifiers including `D l S F M z v O P Z T e` and the `! | # ? * +` metas with strict trailing-data rejection
