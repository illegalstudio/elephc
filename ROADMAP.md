...
- [x] Portable HTTPS without per-handle configuration — libcurl bakes the build machine's CA path in (and nothing at all when cross-compiling), so the bridge instead resolves a bundle at run time and sets it as `CURLOPT_CAINFO`: `$CURL_CA_BUNDLE` if set, else nothing when the baked-in path still exists, else the first of seven fixed, root-owned distro root-store locations that does. An explicit `CURLOPT_CAINFO` replaces it; a `CURLOPT_CAPATH` composes with it, exactly as a capath composes with libcurl's own baked-in bundle in a stock build. Never relaxes verification (see `docs/php/curl.md`, including the `$CURL_CA_BUNDLE` threat model)
...
- [x] The 170 `tests/codegen/curl/*` fixtures running in CI — excluded from the ordinary sharded `codegen-tests-*` jobs (which still self-skip if run standalone) and instead run in dedicated `curl-codegen-tests-<platform>` jobs (4 shards × 3 platforms) that materialize managed native curl/openssl/zlib first via `elephc native install --locked`, cached across runs by an `actions/cache` keyed on `examples/curl-get/elephc.lock` and the recipe sources. Those jobs also assert the tests actually ran rather than silently skipping (grep the managed-native skip-gate message out of the shard's `--nocapture` output and fail if it appears) — the exact failure mode that made this line easy to leave unchecked before
