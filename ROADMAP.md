...
- [x] Phase 32 (Tier 2) — TLS client certificates (mutual TLS): the `elephc-tls` crate gains `client_cert_config` plus the options-aware attach/connect entry points. Later Windows-compliance work superseded the original limitations: `local_pk` may fall back to a combined `local_cert`, and `ssl.passphrase` decrypts encrypted PKCS#8 plus legacy OpenSSL RSA/EC PEM keys. Cipher/security/compression policy now fails closed when rustls cannot represent it; the remaining OpenSSL-policy gap is tracked above. Tests cover valid/missing identities, wrong passphrases, the official php-src legacy RSA fixture, and client/server option construction. ARM64 + x86_64.
...
- [ ] **Full OpenSSL TLS-policy parity** — `ssl.ciphers` now has a strict,
  case-sensitive subset for rustls-backed ECDHE AES-GCM/ChaCha20 TLS 1.2
  suites, while TLS 1.3 remains at its PHP-compatible default policy. OpenSSL
  aliases/operators and every explicit `ssl.security_level` fail closed rather
  than acting as no-ops; `ssl.disable_compression=false` also fails because
  rustls cannot enable TLS compression. Full parity still requires an OpenSSL
  policy backend or equivalent implementation of its cipher grammar and
  security levels. `ssl.passphrase` is implemented for encrypted PKCS#8 and
  legacy encrypted RSA/EC PEM keys.
...
- [x] `xml` and `xmlwriter` extensions with their complete PHP surface INVENTORY — every function, class and constant, which is not the same as behavioural parity (see "Differences from PHP" in `docs/php/xml.md`) — in native compilation and `eval()`: the 22 `xml_*` functions with `XMLParser`, the 42 `xmlwriter_*` functions with `XMLWriter`, and the 28 `XML_*` constants through the `elephc-xml` bridge (`--with-xml`) over the managed `libxml2` 2.15.3 catalog package (`elephc native add libxml2`) — real libxml2 push parsing and `xmlwriter`, statically linked on every supported target; the runtime's per-handler-invocation heap cost and the `xml_parse_into_struct()` footprint are documented under "Runtime limits" in `docs/php/xml.md`
...
- [x] `declare` directives (PR #459) — `declare(strict_types=1);` and the block form `declare(ticks=1) { ... }` parse and are validated syntactically; elephc compiles an always-strict subset, so directives are treated as no-ops
...
- [x] PHP-compatible `defined('Class::CONST')` lookup — public class/interface/trait-imported constants and enum cases, case-sensitive member names, inherited and implemented-interface constants, scope-aware private/protected visibility, lexical `self::`/`parent::`, and runtime called-class dispatch for `static::`
