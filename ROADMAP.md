...
- [x] `mb_strtoupper()` — Unicode full case mapping with the same nullable optional `$encoding` contract as `mb_strlen()` (UTF-8 default, `8bit`/`binary`/`7bit` ASCII fold, iconv-backed encodings, callable dispatch, catchable `ValueError`)
