...
- [ ] **`phar://` advanced writes** — native PHAR writes now preserve and update
  ...
  `file_put_contents()` supports runtime-built `phar://` URLs. Deferred:
  dynamic `fopen()` write URLs, compressed-entry writes, tar/zip writes,
  OpenSSL/private-key signing, and concurrent PHAR write streams.
