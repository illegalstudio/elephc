...
- [x] Portable HTTPS without per-handle configuration — libcurl bakes the build machine's CA path in (and nothing at all when cross-compiling), so the bridge instead resolves a bundle at run time and sets it as `CURLOPT_CAINFO`: `$CURL_CA_BUNDLE` if set, else nothing when the baked-in path still exists, else the first of eight fixed distro root-store locations that does. Never overrides an explicit `CURLOPT_CAINFO`/`CURLOPT_CAPATH`, never relaxes verification (see `docs/php/curl.md`)
