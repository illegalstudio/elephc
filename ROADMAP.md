...
- [x] `mb_convert_case()` — PHP 8.5 UTF-8 `MB_CASE_*` conversion (including `MB_CASE_TITLE` for Termwind `capitalize`), simple vs full mappings, Case_Ignorable title state, Greek final sigma, 8bit/iconv encodings, Magician/eval parity, and catchable `ValueError` for invalid mode or encoding
