...
- [x] `mb_strtolower()` — PHP 8.5 UTF-8 full lowercase on AOT and Magician `eval()`, including Final_Sigma and 1:N maps, `8bit`/`binary`/`7bit` ASCII-byte mode, callable dispatch, and catchable `ValueError` for unknown encodings
