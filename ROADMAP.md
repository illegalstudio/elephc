...
- [x] Phase 4 — `SplStack`, `SplQueue`, `SplDoublyLinkedList`, `SplFixedArray`. Synthesised as PHP-userland classes (Strategy A), wrapped in a root-namespace block so they coexist with user `namespace App;` declarations. Each class is backed by a private `array $items`, implements `Iterator` + `Countable`, and rounds-trips scalar values cleanly. `SplFixedArray::offsetGet` / `offsetSet` throw `RuntimeException` on out-of-range access. Bracket-syntax `ArrayAccess` dispatch is not yet supported — call `offsetGet` / `offsetSet` explicitly
