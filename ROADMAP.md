...
- [x] String offset writes (`$s[$i] = $v`, issue #851) on string variables, including `mixed` and `global` storage — first byte only, space padding past the end, negative offsets, PHP's `Illegal string offset` / first-byte warnings and empty-value `Error`, copy-on-write, and PHP's refusals for compound, `++`/`--`, and `[]` forms; property and array-element targets are still compile errors
