...
| Single-evaluation non-local compound targets | Medium | Lower compound assignments such as `$items[f()] += 1` and `getBox()->x += 1` through target temporaries so receiver/index expressions with side effects are evaluated exactly once. Current support accepts side-effect-free array/property/static-property targets and rejects effectful targets instead of generating PHP-incompatible double evaluation. |
