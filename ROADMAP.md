...
- [x] Residual PHP 8.5 diagnostics and declaration parity — value-discard `(void)` at statement roots and `for` init/update clauses, Zend-equivalent unary-plus conversion/warnings/TypeErrors, profile-correct `E_ALL`/`E_STRICT`, `@error_reporting()` fatal-mask visibility, complete class/interface/trait `__set_state` contract diagnostics, and runtime-tagged relational comparison through `PhpRelCmp`
- [x] Parser-free production declarations — DateTime/DatePeriod and timelib helper surfaces are checked-in direct Rust AST with test-only PHP generation oracles. The pre-rebase evidence is 324 DateTime codegen tests, 679/679 executable php-src `ext/date` PHPTs plus 13 matching skips, five-target assembly, and linked iOS device/simulator static libraries; the post-rebase candidate is fail-closed pending fresh audit locks and the same rerun.
