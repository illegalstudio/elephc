...
- [x] The 34 PHP-visible `curl_*` contracts as ordinary citizens of the shared builtin catalog — a root `curl` feature relays `elephc-magician/curl` and `elephc-builtin-contract/curl`, so the workspace cross-backend parity suite audits them with the same per-contract assertions as every other builtin (name sets, catalog-vs-eval signature shapes, eval coverage, execution route), plus a compiler-side test that compares each prelude-provided contract with the PHP declaration `src/curl_prelude.rs` actually injects. Both suites run in the always-on `curl-feature-contract` CI job and need no native libcurl
- [x] The curl surface in the generated builtin documentation — `--features curl` is the single canonical documentation configuration for `scripts/docs/builtin_registry.json` and the generated pages (621 rows, `docs/php/builtins/network/`), and the `builtins-docs-sync` drift gate runs in that same configuration; the PHP comparison page's curl coverage row goes from `0 / 33` to `32 / 33`, with `curl_multi_get_handles()`/`curl_share_init_persistent()` reported separately as PHP 8.5 additions the vendored 8.4 baseline cannot count
