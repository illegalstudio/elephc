...
/// Verifies the default PHP 8.5 compatibility mode exposes compact fetch flags and the remaining
...
"#,
    ...
    assert_eq!(out, "12,32,64,19,20,1");
}
...
/// FETCH_LAZY, and PHP 8.5 reports the expanded method-specific ValueError.
...
"#,
    ...
    assert_eq!(
        out,
        "threw:PDOStatement::fetchAll(): Argument #1 ($mode) PDO::FETCH_LAZY cannot be used with PDOStatement::fetchAll()"
    );
}

/// Verifies PHP 8.4 and older retain the shorter FETCH_LAZY ValueError wording.
#[test]
fn test_pdo_php84_fetch_all_lazy_uses_legacy_value_error() {
    let out = compile_and_run_with_php_version(
        r#"<?php
$db = new PDO("sqlite::memory:");
try {
    $db->query("SELECT 1")->fetchAll(PDO::FETCH_LAZY);
    echo "no-throw";
} catch (ValueError $e) {
    echo $e->getMessage();
}
...
echo ($typedFirst === $typedSecond ? "same" : "different"), ":", $typedFirst->id, ":", $typedSecond[1], "|";
    ...
    echo "unexpected:", get_class($error), ":", $error->getMessage();
}
"#,
    );
    assert_eq!(
...
/// PHP 8.4 accepts `FETCH_PROPS_LATE` with a non-CLASS base mode as a silent
/// no-op; PHP 8.5 tightens this combination in the versioned regression above.
#[test]
fn test_pdo_php84_fetch_props_late_flag_with_other_mode_is_not_rejected() {
    let out = compile_and_run_with_php_version(
        r#"<?php
$db = new PDO("sqlite::memory:");
$db->exec("CREATE TABLE t (id INTEGER)");
...
#[test]
fn test_pdo_php84_fetch_classtype_flag_throws_with_non_class_mode() {
    let out = compile_and_run_with_php_version(
        r#"<?php
$db = new PDO("sqlite::memory:");
$db->exec("CREATE TABLE t (id INTEGER)");
