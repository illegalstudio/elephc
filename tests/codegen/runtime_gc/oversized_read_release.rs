...
/// Resource result boxing must not keep a pinned filename argument alive after `fopen()`.
#[test]
fn test_fopen_releases_owned_path_argument_after_resource_boxing() {
    let out = compile_and_run_with_heap_debug(
        r#"<?php
$path = "elephc-fopen-owned-path-release.bin";
$sink = fopen($path, "w");
fclose($sink);
$source = fopen($path, "r");
fclose($source);
