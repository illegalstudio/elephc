...
/// Skips codegen tests that exercise the local `elephc monitor` transport.
///
/// Windows deliberately supports only remote TCP/TLS monitor endpoints; its
/// local inherited-channel implementation is a hard diagnostic in
/// `crate::monitor`. Keeping this gate in the shared test support also avoids
/// prepending the cross-target compiler flags to a `monitor` subprocess, where
/// the top-level parser would correctly reject monitor's `--out` exporter.
pub(crate) fn skip_windows_local_monitor() -> bool {
    if target().platform == Platform::Windows {
        eprintln!(
            "skipping local monitor codegen fixture: Windows supports remote monitor endpoints only"
        );
        return true;
    }
    false
