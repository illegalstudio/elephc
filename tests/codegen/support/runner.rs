...
pub(crate) fn link_binary(
    ...
    // The elephc-tls, elephc-pdo, elephc-crypto, and elephc-tz bridge staticlibs
    // all live in `<target>/debug` alongside the test binaries; surface that
    // directory on the linker search path automatically whenever a compiled
    // program links any bridge, so PDO/crypto/tz tests get the same robust,
    ...
    let needs_bridge_staticlib = actual_link_libs.iter().any(|l| {
        *l == "elephc_tls" || *l == "elephc_pdo" || *l == "elephc_crypto" || *l == "elephc_tz"
    });
