...
/// `stream_socket_client("tls://...")` must complete a TLS handshake before
/// returning the stream, exactly as php-src does by setting
/// `sslsock->enable_on_connect` in the `tls`/`ssl` transport factory
/// (ext/openssl/openssl.c registers them through `php_stream_xport_register`).
///
/// Against a real rustls server, a plaintext socket cannot produce a valid HTTP
/// response: the server aborts the connection on the malformed ClientHello. So a
/// readable body here is proof that the handshake actually ran.
#[test]
...
/// `stream_socket_client("tls://...")` must hand back a stream, without reading it.
///
/// The other crypto-transport fixtures conflate two things: that the handshake
/// completed, and that the encrypted stream then reads and writes correctly. When
/// they fail together there is no way to tell which half broke, because a failed
/// handshake returns `false` and the `fwrite()` that follows takes the program down
/// before it can print anything. This asserts only the first half, so the two
/// failure modes separate.
#[test]
fn test_stream_socket_client_tls_scheme_returns_a_stream() {
    let (_server, port) = spawn_https_server(b"handshake only");
    let out = compile_and_run(&format!(
        r#"<?php
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer", "0");
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer_name", "0");
...
/// `fwrite()` on a `tls://` stream reports the byte count, without reading back.
///
/// The companion above proves the handshake completes; this one covers the write
/// half of the encrypted path on its own. Between them the three data fixtures --
/// which write, then read, then compare -- stop being a single indivisible signal:
/// whichever of connect, write and read is at fault, exactly one of these fails.
#[test]
fn test_stream_socket_client_tls_scheme_writes_without_reading() {
    let (_server, port) = spawn_https_server(b"write only");
    let out = compile_and_run(&format!(
        r#"<?php
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer", "0");
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer_name", "0");
$fp = stream_socket_client("tls://127.0.0.1:{port}");
$written = fwrite($fp, "GET /x HTTP/1.0\r\nHost: 127.0.0.1\r\n\r\n");
echo $written === false ? "false" : "wrote:" . $written;
"#
    ));
    // 36 bytes, per php's own strlen of the same literal.
    assert_eq!(out, "wrote:36");
}

/// Reads one small block from a TLS stream, to separate the read from the loop.
#[test]
fn test_stream_socket_client_tls_scheme_reads_a_single_block() {
    // The three compound TLS fixtures all fail on linux-x86_64 while connect and write
    // pass there, so the fault is somewhere in the read. They cannot say where: each
    // one calls stream_get_contents, which is a loop around __rt_fread plus its own
    // accumulation, and a fault in either looks identical from outside.
    //
    // This calls __rt_fread exactly once and never touches the accumulation loop, so
    // the two become distinguishable: failing here puts the fault in the TLS read
    // itself, passing here puts it in the loop around it. Only the length is asserted,
    // because what matters is whether the call survives, not what the server sent.
    let (_server, port) = spawn_https_server(b"single block body");
    let out = compile_and_run(&format!(
        r#"<?php
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer", "0");
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer_name", "0");
...
fwrite($fp, "GET /x HTTP/1.0\r\nHost: 127.0.0.1\r\n\r\n");
$chunk = fread($fp, 16);
echo $chunk === false ? "false" : "read:" . strlen($chunk);
"#
    ));
    assert_eq!(out, "read:16");
}

/// Reads twice from a TLS stream, to separate the first read from any later one.
#[test]
fn test_stream_socket_client_tls_scheme_reads_twice() {
    // One TLS read succeeds on linux-x86_64 while the fixtures that drain the stream
    // fail there, so the fault appears somewhere after the first read. That leaves two
    // candidates: the second read itself, or the accumulation wrapped around it.
    //
    // Two plain reads answer the first half. Failing here means a TLS read only works
    // once and the fault is in what the previous one leaves behind; passing here means
    // repeated TLS reads are fine and the fault belongs to the loop that drives them.
    let (_server, port) = spawn_https_server(b"two reads body");
    let out = compile_and_run(&format!(
        r#"<?php
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer", "0");
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer_name", "0");
...
$first = fread($fp, 16);
$second = fread($fp, 16);
echo "read:" . strlen($first) . "/read:" . strlen($second);
"#
    ));
    assert_eq!(out, "read:16/read:16");
}

/// Closes a TLS stream after reading, to test the one step no passing fixture reaches.
#[test]
fn test_stream_socket_client_tls_scheme_closes_after_reading() {
    // Every TLS fixture that fails on linux-x86_64 ends in fclose(), and every one
    // that passes stops short of it -- connect, write and a single read all return
    // without closing. The plaintext fixture closes the same way and passes, so
    // closing is not broken in general; closing a *TLS* stream is simply the one step
    // nothing green has ever executed.
    //
    // This reaches it with everything else kept minimal: one read rather than a drain,
    // so a failure here cannot be blamed on the accumulation loop. The read length is
    // asserted alongside the close, to keep a silently empty read from passing as a
    // successful close.
    let (_server, port) = spawn_https_server(b"closed after reading");
    let out = compile_and_run(&format!(
        r#"<?php
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer", "0");
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer_name", "0");
...
$chunk = fread($fp, 16);
$closed = fclose($fp);
echo strlen($chunk) . "/" . ($closed ? "closed" : "close-false");
"#
    ));
    assert_eq!(out, "16/closed");
}

/// Drains a TLS stream by hand, to separate repeated reads from stream_get_contents.
#[test]
fn test_stream_socket_client_tls_scheme_drains_by_hand() {
    // The counterpart to the two-read fixture: this drains the stream to exhaustion
    // exactly as stream_get_contents does, but through PHP-level concatenation rather
    // than the runtime helper's own accumulation. Passing here while the three
    // stream_get_contents fixtures keep failing puts the fault in that helper's
    // accumulation, not in the TLS reads it is built on.
    //
    // The round counter bounds the loop, so a read that stops reporting the stream
    // exhausted fails the assertion instead of hanging the shard.
    let (_server, port) = spawn_https_server(b"hand drained body");
    let out = compile_and_run(&format!(
        r#"<?php
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer", "0");
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer_name", "0");
...
"#
    ...
    assert_eq!(out, "[hand drained body]");
}

/// `ssl://` is the same transport under its legacy name, so it must handshake too.
#[test]
...
/// A `tls://` connection to a peer that speaks plain HTTP must fail, because the
/// handshake cannot complete. php-src returns `false` from
/// `stream_socket_client()` in that case rather than handing back a usable
/// plaintext stream. This is the control for the two tests above: without it, a
/// helper that silently skipped TLS would still pass them against a lenient peer.
#[test]
...
/// `tcp://` must stay plaintext, so the handshake-on-connect path cannot leak into
/// transports PHP defines as unencrypted.
///
/// The peer speaks plain HTTP and echoes the request back. A `tcp://` client that
/// started negotiating TLS would send a ClientHello instead, get no usable reply,
/// and fail the connect — so a successful echo is the control that the crypto path
/// stays confined to the crypto schemes.
#[test]
...
/// A default stream context must refuse the harness's self-signed certificate.
///
/// This is the case `peer_verification.phpt` guards in php-src: with no `ssl`
/// options at all, `verify_peer` is on for clients (xp_ssl.c derives
/// `must_verify_peer` from `sslsock->is_client`), so an untrusted certificate makes
/// `stream_socket_client()` return `false`. Every other test here disables
/// verification to reach the data path, which means none of them would notice a
/// handshake that negotiated TLS but authenticated nobody.
#[test]
...
/// Every crypto transport php-src registers must negotiate, not just `tls://`.
///
/// `tls_wrapper.phpt` exercises the same set in one test. Pointing each scheme at a
/// peer that closes without answering proves the scheme was recognised: a
/// recognised scheme fails the handshake and yields `false`, while an unrecognised
/// one would hand back a working plaintext stream. `tcp://` is the negative case.
#[test]
...
"#
        ...
        assert_eq!(out, "false", "scheme {scheme} was not treated as a crypto transport");
    }
}

/// `sslv2://` is not a transport php-src accepts, and neither is an unknown scheme,
/// so both must stay on the plaintext path rather than attempting a handshake.
#[test]
...
/// Writing to a socket whose peer has gone away must fail the write, not the
/// process.
///
/// PHP installs `SIG_IGN` for SIGPIPE before running a script
/// (`php_ignore_sigpipe`), so `fwrite()` returns a short count or false and
/// execution continues. A compiled program that inherits the default disposition
/// dies from signal 13 with no output at all, losing the whole script — which is
/// how this was found: a TLS handshake against a peer that sent RST killed the
/// binary with exit 141 and an empty stdout.
#[test]
...
/// A crypto transport requested from inside `eval()` must fail rather than open an
/// unencrypted socket.
///
/// The interpreter fallback has no TLS engine — `elephc-magician` does not link the
/// `elephc-tls` bridge — and it used to strip `ssl://` and `tls://` off the address
/// and connect in the clear, handing back a plaintext stream under a name that
/// promises confidentiality. Refusing is the honest answer; the compiled path
/// negotiates these transports normally.
#[test]
fn test_eval_crypto_transport_refuses_instead_of_downgrading() {
    let (_server, port) = spawn_http_echo_server();
    let out = compile_and_run(&format!(
        r#"<?php
$plain = 'return stream_socket_client("tcp://127.0.0.1:{port}") === false ? "false" : "stream";';
$crypto = 'return stream_socket_client("tls://127.0.0.1:{port}") === false ? "false" : "stream";';
echo eval($plain), "|", eval($crypto);
"#
    ));
    assert_eq!(out, "stream|false");
}

/// Verifies `eval()` reports the same platform constants as the compiled program
/// around it.
///
/// The interpreter is cross-compiled with the program, but its constant table had
/// no Windows arm, so a Windows binary answered PHP_OS "Linux" and
/// DIRECTORY_SEPARATOR "/" from inside eval() while its own compiled constants said
/// "WINNT" and "\". The assertion is that the two agree, which holds on every
/// target without naming any of them.
#[test]
fn test_eval_platform_constants_match_the_compiled_ones() {
    let out = compile_and_run(
        r#"<?php
$same = PHP_OS === eval('return PHP_OS;')
     && PHP_OS_FAMILY === eval('return PHP_OS_FAMILY;')
     && PHP_EOL === eval('return PHP_EOL;')
     && DIRECTORY_SEPARATOR === eval('return DIRECTORY_SEPARATOR;')
     && PATH_SEPARATOR === eval('return PATH_SEPARATOR;')
     && sys_get_temp_dir() === eval('return sys_get_temp_dir();');
echo $same ? "agree" : "disagree";
"#,
    );
    assert_eq!(out, "agree");
}

/// Verifies a TCP server can rebind its port right after a connection closed.
///
/// php enables SO_REUSEADDR on every listener and treats the
/// `socket.so_reuseaddr` context option purely as an opt-out
/// (main/streams/xp_socket.c: "SO_REUSEADDR is enabled by default so this option is
/// just to disable it if set to false"). Without it the port sits in TIME_WAIT and
/// the rebind fails, so a server could not restart on its own port. elephc's IPv6
/// listener already set it; the IPv4 one did not.
#[test]
fn test_stream_socket_server_rebinds_its_port_after_a_connection() {
    // The program picks its own port and reads the assignment back, so no window
    // exists in which another process could claim a number reserved here.
    let out = compile_and_run(
        r#"<?php
$srv = stream_socket_server("tcp://127.0.0.1:0");
$addr = stream_socket_get_name($srv, false);
...
fclose($cli);
fclose($srv);
$again = @stream_socket_server("tcp://" . $addr);
echo $again === false ? "refused" : "rebound";
"#,
    );
    assert_eq!(out, "rebound");
}

/// Verifies `microtime()` honours its argument inside `eval()` exactly as the
/// compiled path does.
///
/// php returns the `"<usec fraction> <seconds>"` string unless the argument is
/// truthy (ext/standard/microtime.c `_php_math_microtime`). The interpreter
/// evaluated the argument only for its side effects and always returned a float, so
/// an eval'd `microtime()` handed back a double where the compiled builtin -- which
/// already has the float, string and mixed variants -- returned a string.
#[test]
fn test_eval_microtime_honours_its_as_float_argument() {
    let out = compile_and_run(
        r#"<?php
echo gettype(eval('return microtime();')), "|",
     gettype(eval('return microtime(true);')), "|",
     gettype(eval('return microtime(false);')), "|",
     gettype(microtime()), "|",
     gettype(microtime(true));
"#,
    );
    assert_eq!(out, "string|double|string|string|double");
}

/// Compiles a program and returns the size of the linked binary in bytes.
///
/// Used to observe which bridges the link actually pulled in: a bridge is
/// megabytes, so its presence or absence is unmistakable in the artifact size.
/// The linked artifact is `test.exe` on windows and `test` everywhere else, which
/// is what `target_binary_path` encodes -- hardcoding `test` measured a file that
/// does not exist on windows. A missing binary is a hard failure rather than a
/// zero, so a link that never happened cannot masquerade as "the bridge was not
/// linked", which is exactly the conclusion this helper exists to draw.
fn compiled_binary_size(source: &str) -> u64 {
    let (_output, dir) = compile_and_run_in_dir(source);
    let binary = target_binary_path(&dir.join("test"));
    let size = fs::metadata(&binary)
        .unwrap_or_else(|error| {
            panic!("linked binary {} could not be measured: {error}", binary.display())
        })
        .len();
    let _ = fs::remove_dir_all(&dir);
    size
...
/// Verifies a literal plaintext address does not drag the TLS bridge into the
/// binary, while a crypto scheme and a runtime address both still do.
///
/// The scheme normally lives in a runtime string, so the handshake path — and with
/// it the bridge exports — has to be emitted. Reading a `ConstStr` address back at
/// lowering time lets a program that only ever says `tcp://` skip both: measured
/// here at 155 KiB against 5.7 MiB, a 37x difference that is entirely rustls. The
/// lowering and `stream_socket_client_requirements` decide on the same literal, so
/// they cannot disagree; were they to, the bridge exports would be undefined at
/// link time, which is loud rather than a silent plaintext connection.
#[test]
fn test_literal_plain_address_does_not_link_the_tls_bridge() {
    let plain = compiled_binary_size(r#"<?php $fp = @stream_socket_client("tcp://127.0.0.1:9"); var_dump($fp);"#);
    let crypto = compiled_binary_size(r#"<?php $fp = @stream_socket_client("tls://127.0.0.1:9"); var_dump($fp);"#);
    let dynamic = compiled_binary_size(
        r#"<?php $a = "tcp://127.0.0.1:9"; $fp = @stream_socket_client($a); var_dump($fp);"#,
    );
    // The bridge is megabytes; any sane threshold separates "linked" from "not".
    let bridge_floor = 2 * 1024 * 1024;
    assert!(plain < bridge_floor, "a literal tcp:// address must not link the TLS bridge, got {plain} bytes");
    assert!(crypto > bridge_floor, "a literal tls:// address must link the TLS bridge, got {crypto} bytes");
    assert!(dynamic > bridge_floor, "a runtime address must link the TLS bridge, got {dynamic} bytes");
}

/// Verifies `fsockopen()` negotiates the crypto transports, as php does.
///
/// php accepts a crypto scheme in the hostname — `fsockopen("tls://host", 443)`
/// returns an encrypted stream — because fsockopen and stream_socket_client share
/// one transport factory. elephc reaches the socket through the `__rt_fsockopen`
/// runtime helper, which tail-calls `__rt_stream_socket_client`, so it bypassed the
/// crypto-on-connect block that lives in the stream_socket_client lowering and kept
/// handing back a plaintext socket for a `tls://` hostname.
#[test]
fn test_fsockopen_tls_hostname_performs_handshake() {
    let (_server, port) = spawn_https_server(b"fsockopen over tls");
    let out = compile_and_run(&format!(
        r#"<?php
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer", "0");
stream_context_set_option(stream_context_get_default(), "ssl", "verify_peer_name", "0");
$errno = 0; $errstr = "";
$fp = fsockopen("tls://127.0.0.1", {port}, $errno, $errstr, 5);
fwrite($fp, "GET /x HTTP/1.0\r\nHost: 127.0.0.1\r\n\r\n");
$resp = stream_get_contents($fp);
fclose($fp);
$pos = strpos($resp, "\r\n\r\n");
...
"#
    ...
    assert_eq!(out, "[fsockopen over tls]");
}

/// Verifies `fsockopen()` with a plain hostname stays plaintext and does not link
/// the TLS bridge, the same literal-driven split `stream_socket_client` uses.
#[test]
fn test_fsockopen_plain_hostname_stays_plaintext() {
    let (_server, port) = spawn_http_echo_server();
    let out = compile_and_run(&format!(
        r#"<?php
$errno = 0; $errstr = "";
...
"#
    ...
    assert_eq!(out, "echoed");
    let plain = compiled_binary_size(
        r#"<?php $e=0;$s=""; $fp = @fsockopen("127.0.0.1", 9, $e, $s, 1); var_dump($fp);"#,
    );
    assert!(plain < 2 * 1024 * 1024, "a plain hostname must not link the TLS bridge, got {plain} bytes");
}
