...
/// part of the cross-platform assertion. The stale key also proves proc_open
/// replaces the by-reference output instead of mutating the caller's old array.
#[test]
...
if ($process === false) {
    ...
    fclose($pipes[1]);
    proc_close($process);
    echo substr($output, 0, 16) === "binary-direction"
        && count($pipes) === 2
        && !array_key_exists(99, $pipes)
        ? "ok"
        : "missing";
}
"#
...
fn test_proc_close_drains_unread_full_pipe_before_waiting() {
    let command = if target().platform.php_os_family_name() == "Windows" {
...
/// Verifies a child exit code above 0x7FFFFFFF comes back as a negative integer.
///
/// Windows exit codes span the whole DWORD range, and php stores the value
/// `GetExitCodeProcess()` produces into an `int` — `FG(pclose_ret)` for
/// `proc_close()` and `proc->exit_code` for `proc_get_status()` — so
/// `cmd /c exit -1` reads back as `-1`. Loading the DWORD with a zero-extending
/// move instead reported 4294967295, which compares wrong against every negative
/// exit-code check. POSIX shells clamp the status to 0..255, so the oversized case
/// only exists on Windows and the other targets assert the clamped value.
#[test]
fn test_proc_close_reports_negative_exit_code() {
    let (command, expected) = if target().platform == Platform::Windows {
        ("cmd /c exit -1", "-1")
    } else {
        ("sh -c 'exit 255'", "255")
    };
    let out = compile_and_run(
        &r#"<?php
$pipes = [];
$process = proc_open("__COMMAND__", [1 => ["pipe", "w"]], $pipes);
fclose($pipes[1]);
...
/// Verifies `proc_get_status()` reports the same signed exit code as `proc_close()`,
/// since both read the value through `GetExitCodeProcess()` on Windows.
#[test]
fn test_proc_get_status_reports_negative_exit_code() {
    let (command, expected) = if target().platform == Platform::Windows {
        ("cmd /c exit -1", "-1")
    } else {
        ("sh -c 'exit 255'", "255")
...
fclose($pipes[1]);
$status = proc_get_status($process);
while ($status["running"]) { $status = proc_get_status($process); }
