...
/// A nested array value is refused with a catchable `\TypeError`, the documented
/// divergence from a real `ext/curl`'s surprising one-level flattening (see
/// `__elephc_curl_build_multipart()`'s doc comment for what was measured).
#[test]
fn nested_array_value_is_refused() {
    if skip_without_curl_native("nested_array_value_is_refused") {
        return;
    }
    ...
    assert_eq!(
        out,
        "curl_setopt(): CURLOPT_POSTFIELDS array value must not be an array\n"
    );
}
