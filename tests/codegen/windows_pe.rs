...
//! - Every skip goes through `windows_pe_fixture_may_run`, so a CI job that sets
//!   `ELEPHC_REQUIRE_WINDOWS_TOOLCHAIN=1` fails loudly instead of reporting a
//!   vacuous pass when its MinGW or Wine install degraded.
...
/// Exercises PHP's Windows-only SAPI surface and proves PCNTL disappears from
/// the same native symbol/constant registries on a PE target.
#[test]
fn test_windows_sapi_surface_and_pcntl_absence() {
...
foreach ($names as $name) { echo function_exists($name) ? '1' : '0'; }
echo defined('SIGCHLD') ? '|sig' : '|nosig';
echo extension_loaded('pcntl') ? '|pcntl' : '|nopcntl';
echo sapi_windows_cp_get() > 0 ? '|cp' : '|bad-cp';
$text = "h\xc3\xa9";
echo sapi_windows_cp_conv('utf-8', 'utf-8', $text) === $text ? '|conv' : '|bad-conv';
echo sapi_windows_cp_conv(65001, 65001, $text) === $text ? '|int-conv' : '|bad-int-conv';
"#,
        "1111111|nosig|nopcntl|cp|conv|int-conv",
    );
}

/// Verifies eval bridge calls with fifth and sixth C ABI words cross-compile to PE32+.
///
/// The source exercises both `__elephc_eval_execute` and the post-eval function-call bridge,
/// whose output result pointer follows the four positional MS x64 register slots.
#[test]
fn test_windows_eval_bridge_overflow_words_compile_to_pe() {
    compile_windows_pe(
        r#"<?php
$source = 'function windows_eval_abi_probe($value) { return $value + 1; }';
eval($source);
echo windows_eval_abi_probe(41);
"#,
    );
}

/// Verifies eval can invoke an AOT optional function through an associative
/// descriptor container on PE. Its hidden `func_get_args()` collector prepends
/// the actual count through the six-word internal `__rt_hash_set` ABI.
#[test]
fn test_windows_eval_native_function_assoc_collector_compiles_to_pe() {
    compile_windows_pe(
        r#"<?php
function windows_eval_assoc_collector_probe(string $first = "D"): string {
    return func_num_args() . ":" . implode(",", func_get_args());
}
echo eval('return windows_eval_assoc_collector_probe(first: "N");');
"#,
    );
}

/// Verifies the GD desktop-capture declarations exist on Windows without
/// requiring an interactive desktop capture in headless CI.
#[test]
fn test_windows_gd_capture_functions_are_visible() {
    compile_and_run_windows(
        "<?php echo function_exists('imagegrabscreen') ? '1' : '0'; \
         echo function_exists('imagegrabwindow') ? '1' : '0';",
        "11",
    );
}

/// Compiles a PHP source string to a Windows PE32+ binary and verifies the output.
/// Skips the test if MinGW-w64 is not installed, unless the job requires the
/// toolchain (see `windows_pe_fixture_may_run`), in which case it fails instead.
fn compile_windows_pe(source: &str) {
    if !windows_pe_fixture_may_run("Windows PE test", false) {
        return;
    }
...
fn test_windows_web_bridge_compiles_and_exports() {
    if !windows_pe_fixture_may_run("Windows web bridge test", false) {
        return;
    }
    let id = TEST_ID.fetch_add(1, Ordering::SeqCst);
    ...
    fs::write(
        ...
        "<?php header('Content-Type: text/plain'); if (!isset($_GET['name'])) { \
         header('X-Debug-Uri: ' . $_SERVER['REQUEST_URI']); \
         header('X-Debug-Query: ' . $_SERVER['QUERY_STRING']); \
         header('X-Debug-Count: ' . count($_GET)); \
    ...
    let compile = elephc_cli_command(&dir)
        ...
        .arg("windows-x86_64")
        .arg("--web")
        // The compiler strips executable symbol tables by default. Keep this
        // one intentionally so the PE-level bridge ABI assertion can inspect
        // the resolved staticlib symbols rather than mistaking a size
        // optimization for a missing web export.
        .arg("--keep-symbols")
        .arg(&php_path)
        .output()
    ...
    // Symbol export is asserted above on any host; serving a real HTTP request
    // additionally needs a PE execution host, so it is gated separately rather
    // than skipping the whole test.
    if windows_pe_fixture_may_run("Windows web bridge HTTP probe", true) {
        ...
        let output = child.wait_with_output().expect("collect Windows web PE output");
        let stderr = String::from_utf8_lossy(&output.stderr).into_owned();
        let fault_address = stderr
            .split_once("at address ")
            .and_then(|(_, tail)| tail.split_whitespace().next())
            .map(str::to_string);
        let fault_symbol = fault_address
            .as_deref()
            .and_then(|address| Command::new("x86_64-w64-mingw32-addr2line")
                .args(["-f", "-C", "-e"])
                .arg(&exe_path)
                .arg(address)
                .output()
                .ok()
                .map(|symbol| String::from_utf8_lossy(&symbol.stdout).trim().to_string()))
            .filter(|symbol| !symbol.contains("??"))
            .or_else(|| {
                let address = u64::from_str_radix(
                    fault_address.as_deref()?.trim_start_matches("0x"),
                    16,
                ).ok()?;
                let symbols = Command::new("x86_64-w64-mingw32-nm")
                    .args(["-n", "-C"])
                ...
                String::from_utf8_lossy(&symbols.stdout)
                    ...
                    .filter_map(|line| {
                        ...
                        (kind != "U" && value <= address && !name.is_empty())
                            .then_some((value, name))
                    })
                    .max_by_key(|(value, _)| *value)
                    .map(|(value, name)| format!("{name}+{:#x}", address - value))
            })
            .unwrap_or_else(|| "(symbolization unavailable)".to_string());
        assert!(
            output.status.success(),
            "Windows web PE exited with {}; stderr: {stderr}\nfault symbol: {fault_symbol}",
            output.status,
        );
...
fn test_windows_pe_hardening_and_generated_unwind_metadata() {
    if !windows_pe_fixture_may_run("Windows PE hardening test", false) {
        return;
    }
    let id = TEST_ID.fetch_add(1, Ordering::SeqCst);
    let dir = std::env::temp_dir().join(format!(
...
/// Verifies a runtime-name dynamic-property write reaches the six-argument
/// SysV `__rt_hash_set` helper without trying to use a fifth MSx64 register.
#[test]
fn test_windows_runtime_dynamic_property_write_compiles_to_pe() {
    compile_windows_pe(
        r#"<?php
#[\AllowDynamicProperties]
class WindowsDynamicBag {}
$property = "answer";
$bag = new WindowsDynamicBag();
$bag->{$property} = 42;
echo $bag->{$property};
"#,
    );
...
/// Verifies HTTPS transports every TLS options ABI v6 field through the
/// Windows runtime and links the pure-Rust bridge without network execution.
#[test]
fn test_windows_https_tls_v6_options_compile_to_pe() {
    compile_windows_pe(
        r#"<?php
stream_context_create(["ssl" => [
    ...
    "SNI_enabled" => false,
    "no_ticket" => true,
    "peer_fingerprint" => ["sha256" => "00"],
]]);
@fopen("https://127.0.0.1:9/", "r");"#,
    );
}
...
fn test_windows_stream_timeout_sets_metadata_without_eof() {
    if !windows_pe_fixture_may_run("Windows timeout execution test", true) {
        return;
    }
...
/// Verifies Windows method callback wrappers carry an environment after two
/// string descriptors through the native fifth MSx64 argument slot, then call
/// the generated method with both overflow words and caller shadow space.
#[test]
fn test_windows_string_comparator_method_callback_overflow_compiles_to_pe() {
    compile_windows_pe(
        r#"<?php
class WindowsStringComparator {
    public function __construct(private int $direction) {}
    public static function ascending(string $left, string $right): int {
        return strcmp($left, $right);
    }
    public function descending(string $left, string $right): int {
        return $this->direction * strcmp($left, $right);
    }
}

$static = ["c", "a", "b"];
usort($static, WindowsStringComparator::ascending(...));
$instance = ["a", "c", "b"];
$comparator = new WindowsStringComparator(-1);
usort($instance, $comparator->descending(...));
"#,
    );
}
...
unset($box);
echo "H|";
echo $alias[1], count($filtered), count($staticFiltered), "|I";
"#,
        "A|B|C|D|E|F|stackG|1H|1122|I",
    );
}
...
fn compile_and_run_windows_expect_code_with_helpers(
    ...
    if !windows_pe_fixture_may_run("Windows PE execution test", false) {
        return;
    }
    let id = TEST_ID.fetch_add(1, Ordering::SeqCst);
    ...
    if !windows_pe_fixture_may_run("Windows PE execution (binary compiled successfully)", true) {
        let _ = fs::remove_dir_all(&dir);
        return;
    }
...
$fiber->resume("resume");
"#,
        "main-before|before-start|fiber-entry|payload|number|128:1234.50|after-start|resumed|resume|decoded|done",
    );
}
...
"#,
        "raw=4|write=11|blocking=1/1|echo=raw|fbpxrg-bx|child|raw|fbpxrg-bx\r\n|err=child=raw|fbpxrg-bx\r\n:0|start=started|file=ran=15:raw|fbpxrg-bx\r\n;out=21;err=21",
        0,
        &[("proc-socket-helper", "<?php file_put_contents('proc-socket-start.txt', 'started'); $input=fread(STDIN,64); echo 'echo=' . $input . '|'; $out=fwrite(STDOUT, 'child|' . $input); $err=fwrite(STDERR, 'child=' . $input); file_put_contents('proc-socket-debug.txt', 'ran=' . strlen($input) . ':' . $input . ';out=' . $out . ';err=' . $err);")],
    );
}

/// Control for `proc_open(["socket"])`: direct CRT pipe descriptors must carry
/// the same parent-to-child bytes before the socket-specific path is accepted.
#[test]
fn test_windows_proc_open_pipe_stdin_round_trip() {
    compile_and_run_windows_expect_code_with_helpers(
        r#"<?php
$pipes = [];
$process = proc_open(
    ['__WINDOWS_HELPER_PROC_PIPE_STDIN__'],
...
"#,
        "write=12|blocking=1/1|echo=pipe-input|child|pipe-input\r\n|err=child=pipe-input\r\n:0|start=started|file=ran=12:pipe-input\r\n;out=18;err=18",
        0,
        &[("proc-pipe-stdin", "<?php file_put_contents('proc-pipe-start.txt', 'started'); $input=fread(STDIN,64); echo 'echo=' . $input . '|'; $out=fwrite(STDOUT, 'child|' . $input); $err=fwrite(STDERR, 'child=' . $input); file_put_contents('proc-pipe-debug.txt', 'ran=' . strlen($input) . ':' . $input . ';out=' . $out . ';err=' . $err);")],
    );
}
