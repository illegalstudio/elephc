...
echo (is_array($meta) ? count($meta) : 0) . ":" . $meta["pdo_type"] . "|";
$date = $db->query("SELECT CAST('2024-02-03' AS DATE) AS d FROM RDB\$DATABASE")->fetchColumn();
echo $date . "|";
$db->exec("RECREATE GLOBAL TEMPORARY TABLE ELEPHC_PDO_FB_TEST (VAL INTEGER) ON COMMIT PRESERVE ROWS");
$db->beginTransaction();
$db->exec("INSERT INTO ELEPHC_PDO_FB_TEST (VAL) VALUES (42)");
$db->rollBack();
