...
///
/// `/` is readable and not writable on every unix, so it separates the two modes without
/// creating anything. VERIFIED on 8.5.10 BOTH ways: read-write fatals, read-only runs.
/// php-src asks `access()` for `R_OK | W_OK` normally and `R_OK` alone when read-only.
#[test]
