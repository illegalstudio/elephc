...
- Fixed reassigning a scalar-array local to a literal rebuilt from its own elements inside a loop (issue #594): `$r = [$r[0] - 1, 0]` or a `[$r[1], $r[0]]` swap now reads back correctly instead of surfacing heap addresses (and looping forever when the corrupted read sat in a `while` condition). The loop-entry array widening now also promotes such full rebinds whose element storage becomes boxed `mixed` or tagged `int|null`, matching the existing in-place growth widening, so every read of the local uses the boxed representation the back edge produces.
