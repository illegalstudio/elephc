...
- wasm32-wasi AOT backend: `--target wasm32-wasi` compiles a PHP program to a WebAssembly module (`.wat`/`.wasm`) through a dedicated EIR-driven backend that runs under any WASI host, with `--emit npm` packaging the module as an NPM package. The target supports a growing subset of the language (an unimplemented EIR operation aborts the whole module); current higher-order array builtins include `array_map`, `array_filter`, `usort`, `uasort`, `uksort`, `array_reduce`, and `array_walk`, plus `call_user_func`/`call_user_func_array`.
