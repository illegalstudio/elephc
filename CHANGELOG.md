...
- Fixed chained container reads after a missing outer array offset (issue #526): indexed, associative, and mixed-key consumers now recognize null-container receivers before dereferencing them, preventing segfaults across every supported target. Direct reads warn and yield null, `isset()` and null coalescing stay silent across the full subscript chain, and `foreach` skips a missing container instead of crashing.
