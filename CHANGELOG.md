...
## [Unreleased]
- Fixed `buffer<T>` indexing with checked-arithmetic results (issue #497): non-constant integer add/subtract/multiply expressions are boxed as `mixed` so they can promote to `float` on overflow, and buffer reads and writes now accept and runtime-narrow those indices to `int` before bounds checks instead of failing type checking or EIR backend lowering.
