...
## [Unreleased]
- Fixed `count()` on a boxed `mixed` receiver whose payload is the null-container sentinel segfaulting (issue #602): a missed array read forwarded through a ternary merge boxes the sentinel as an indexed-array cell, and `__rt_mixed_count` dereferenced the sentinel payload past a plain-zero guard. The runtime helper now guards the in-band sentinel — carried as the cell or as the tag-4/5 payload pointer — before every dereference and signals it back so the call site raises PHP's `TypeError: count(): Argument #1 ($value) must be of type Countable|array, null given`, matching the concrete-array path. A plain null pointer (e.g. an uninitialized off-web superglobal), a boxed null cell, and empty containers keep their existing counts, and the catchable error path stays heap-clean.
