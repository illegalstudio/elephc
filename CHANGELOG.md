...
## [Unreleased]
- Fixed `disk_free_space()` and `disk_total_space()` answering `0.0` for a directory that cannot be stat'ed, where PHP returns `false`. `0.0` is a legitimate byte count — a full filesystem reports zero bytes available — so "out of space" and "no such directory" were the same answer and `=== false` never matched. Both functions share one runtime helper, which checked the syscall and then reported the failure through the payload instead of a flag; the flag now travels beside it and the results box as `float|false`, on the compiled path and in the `eval()` runtime alike. Widening the return also required relaxing one checker rule: relational operators accepted a union only when every member supported integer dispatch, which excludes `float`, so `disk_free_space(".") > 0` — valid PHP that existing code and tests use — would have become a compile error. Comparison now accepts any union whose members are numeric-or-false, which the lowering already handled: a boxed value holding a float compares correctly today. A directory that genuinely has zero bytes free still answers `0.0`, and successful results keep behaving as floats under arithmetic, `is_float()` and ordering.
