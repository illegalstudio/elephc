...
- Added `elephc monitor`, a PHP-level profiler that works the same way in every environment. Build with `--with-monitoring` and point it at what you have: a `.php` source (built first), a binary, or a service already serving traffic at `host:port`, `http://` or `https://`. Every target yields the same **exact** measurements — a recursion-safe shadow stack reports inclusive/self time, allocation counts, retained objects (allocated minus freed, signed), DB queries, blocked-in-the-driver wait time and call counts, with the self values partitioning the root's inclusive for every dimension — and the same exports: a per-function table whose runtime time is broken down by cause (heap allocation, Mixed cell boxing, reference counting, dynamic Mixed arithmetic), Speedscope, pprof, Graphviz DOT, and a self-contained interactive HTML call graph with flame, bottom-up, critical-path and queries views. It prints Blackfire-style recommendations — a hotspot is named, and an N+1 is only called one when the call fan-out and the exact query counts both say so — writes a Chrome/Perfetto timeline, and compares two captures through `--save`/`--baseline`. The capability is dormant until asked, so the artifact you profile is the one you ship; a binary built without it is refused rather than profiled approximately. `--live` and `--attach` are the one exception: they read a process from the outside through `/usr/bin/sample`, so they are sampled and macOS-only, and `--live` keeps the last ten captures navigable with a timeline scrubber and a diff-vs-previous mode, served in place with `--serve`.
- Added authenticated profiling, so turning a profiler on is something only a key holder can do. `--with-monitoring` generates a 32-byte build key, embeds it, writes a `<binary>.key` sidecar and prints its public fingerprint. A program `monitor` launches is asked over a control channel on fd 3 — possession of the channel is the credential, so there is nothing to copy or replay and no environment variable anyone else could set. A running service is asked over its endpoint, after a mutual HMAC handshake in which each side proves it holds the key before a sample crosses; over `https://` the certificate must validate against the system roots first. A `--web` service also answers a signed `X-Elephc-Query` header, which profiles that one request and leaves every other one untouched; the value carries a timestamp, so a captured header stops working within five minutes.
- Added exact per-request profiling and distributed tracing to `--web --with-monitoring`: each request is profiled as its own slice carrying a W3C Trace Context identity, so a profile joins whatever distributed trace its caller already belongs to rather than forming an island. An inbound `traceparent` continues that trace as a child span and anything malformed starts a fresh one; the runtime republishes the current value as `ELEPHC_TRACEPARENT`, so userland propagates it with `getenv()` and an `http.header` stream context and needs no new builtin. `elephc monitor --stitch <log>...` correlates service logs into span trees on stdout and as a self-contained page. Propagation reaches the HTTP stream layer only — elephc has no `curl` yet — and a slice records its duration but not its start time, so the view compares durations rather than placing spans on a shared axis.
- Added a per-project performance budget: `elephc monitor` reads assertions from the nearest `.elephc` file, found by walking up from the profiled source so one file at a project root covers everything under it, and gates the build with exit status 2. Assertions cover every measured dimension (`calls`, `allocs`, `retained`, `queries`, `self_ms`, `incl_ms`, `wait_ms`, `time_pct`), take `*` as the function to assert on the whole run, and carry a label after `#` explaining why the budget exists. The report lists failures first, and distinguishes a failure from an assertion that could not be evaluated — one naming a function the run never reached, or a metric that does not exist, is wrong in the budget rather than in the code, though it still fails the gate.
- Fixed `$i++` allocating a heap cell on every iteration of every loop, even with an explicit `int $i` declaration. The integer-narrowing pass could not reach a loop counter — a store counts as an integer sink only once the slot is `I64`, and the slot stayed `Mixed` precisely because its only writer was the boxed checked producer, so neither side could move first. A second phase now proves the producer and the slot together, removing the per-iteration allocation and the `mixed_cast_int` on every read; a counting loop measured about 31% less CPU.
- Fixed a declared `: int` return coercing a dynamically-typed value through the plain integer path, which silently truncated a float. A function whose counter overflowed — where PHP promotes the result to float — returned `int(-9223372036854775808)` instead of throwing `TypeError: f(): Return value must be of type int, float returned`. Declared integer returns now apply PHP's coercive rules at runtime, matching what argument binding already did.
- Fixed the PDO methods that php declares as `T|false` being declared `T|bool` in elephc's prelude, which made the standard guard-then-use idiom impossible to compile: a `bool` member is not the literal-false subtype an `=== false` guard strips, so `$q = $pdo->quote('x'); if ($q === false) { … } return strtoupper($q);` was rejected. `exec`, `quote`, `lastInsertId`, `getColumnMeta`, `pgsqlLOBCreate`, `lobCreate`, `cubrid_schema`, `copyToArray`, and `pgsqlCopyToArray` now narrow through both guard shapes. `prepare()` and `query()` deliberately keep the wider contract: they build their statement through a dynamic-class allocation whose result is `Mixed`, and narrowing them would let that reach a parameter typed as a class, which codegen cannot unbox.
...
- Added Linux support to `elephc monitor`, which previously refused to run anywhere without `/usr/bin/sample`. The default capture needs no external sampler at all — it is measured inside the process — so Linux and macOS now return the same exact numbers through the same table, Speedscope, pprof, DOT and HTML call graph, with nothing to install and no elevated `perf_event_paranoid`, working unchanged inside a container. Only `--live` and `--attach`, which read a process from the outside, remain macOS-only, and they now name the working alternative (`ELEPHC_PROBE_ADDR` plus `elephc monitor <address>`) instead of reporting a platform requirement.
- Fixed every `--web` and PDO binary failing to link on Linux with `undefined reference to 'elephc_probe_route_fn'`. Six runtime `.comm` function-pointer slots are resolved by the Rust bridge crates rather than by the emitted assembly, so they follow the platform's C ABI — `_name` on Mach-O, `name` on ELF — but all twelve sites naming them spelled a hardcoded leading underscore. The assembly was valid on both formats and only the ELF link failed, which is why macOS stayed green throughout. The neighbouring function symbols already resolved through the platform mangling; the slots they wrote into did not.
- Fixed `--debug-info` failing to assemble on Linux for any program containing a closure or a callable. Two lowering paths emit a global thunk inside a function body and branch around it, and neither restored the section afterwards; on ELF every global entry opens its own `.text.<name>`, so the enclosing function's tail — epilogue and `ret` included — was filed under the thunk's section. Mach-O keeps one flat `__text` and never noticed. The `.quad Lelephc_fend_N - <fn>` extents that `--debug-info` writes then spanned two sections and GNU as refused them (`can't resolve`); measured on a PDO service, 7 of 204 extents crossed a boundary and the assembler reported exactly those 7. Beyond debug info the split left each affected function at the mercy of `--gc-sections`, which is free to keep one of its two sections and drop the other.
- Fixed `elephc monitor` spawning the program it had just compiled by a bare name, which makes the OS search `PATH` rather than the working directory. It only appeared to work on machines whose `PATH` carries an empty entry — POSIX reads that as the current directory — so the exact profile died with `No such file or directory` immediately after reporting a successful compile anywhere else, macOS included. Compile outputs now resolve to an absolute path; a target naming no local file is still left to `PATH`.
- Changed the exact profiler's shadow-stack cap from 4,096 to 65,536 frames. The old figure rested on the claim that it "comfortably exceeds any hand-written PHP call depth", which holds for nesting and not for recursion — a naive tree walk or an unbounded `fib` passes it easily. The stack is grown on demand, so the cap reserves nothing and a deeper ceiling costs nothing until it is approached; the guard itself stays, so a runaway recursion still degrades and is still reported instead of growing without bound.
- Changed the distributed trace view into a real waterfall: each profiled slice now records the wall-clock time it opened, so correlated spans are placed on a shared axis instead of all starting flush left. Sequential hops step rightwards and concurrent ones overlap — the distinction the correlated view exists to show, and which comparing durations alone could not express. A trace whose spans are not all dated (a capture taken before slices carried timestamps) keeps the previous duration-only layout rather than pinning the undated span at the origin and implying it started first. Being wall clock, the axis inherits the usual caveat: two hosts' clocks can disagree, so a hop may appear to start slightly before its parent.

- Added a per-service request summary to `elephc monitor --stitch`: request count, nearest-rank p50/p90/p95/p99, request rate, mean queries per request, and the share of time spent waiting on a database. Rows split per endpoint when every slice names a route, because a service-wide p95 averages away the one slow route worth finding. The percentiles are nearest-rank so each is a duration some request actually took rather than an interpolation nobody experienced; below 20 requests the report states outright that its upper percentiles are that service's slowest requests and not a distribution, and the rate is omitted rather than invented when the slices carry no timestamps or all share one instant.
- Added `--otlp <endpoint>`, exporting correlated request slices to an OTLP/HTTP collector as OpenTelemetry spans. elephc already carried the W3C Trace Context identity, so a service belonged to its caller's trace; the spans themselves never reached a backend, leaving an elephc hop as a gap in someone else's waterfall. Traces only and deliberately so: the OTel Profiles signal is alpha and its SIG advises against depending on it, while OTLP Profiles round-trips losslessly with pprof and the Collector ships a pprof receiver, so the existing `--pprof` output already reaches an OTel backend. Plain HTTP to a local agent is the intended deployment and an https endpoint is refused with that advice, which keeps a TLS stack and a credential store out of the compiler; slices without a timestamp are skipped and counted, since a span needs both ends of an interval.
- Added `--prometheus <file>`, writing the per-service stats in the Prometheus text exposition format for a textfile collector — a file rather than an endpoint, because `monitor` runs and exits and leaves nothing to scrape. Percentiles are exposed as a `summary` rather than a histogram, since the capture holds exact per-request values and buckets would invent a resolution it does not have. Route labels are escaped, as they are built from untrusted HTTP paths and the format ends a label value at an unescaped quote.
- Fixed `elephc-instr` and `elephc-probe` never being built by CI or the Docker test scripts, so any program compiled with `--with-monitoring` failed to link there with `required Elephc bridge elephc_probe could not be found`. The bridge archives come from an explicit build list duplicated in six places outside Rust, and a test now derives that list from the bridge table itself so the next one is covered automatically.
- Fixed the release tarball and the Homebrew formula carrying eight of the eleven bridge archives, so an elephc installed from a release answered `elephc --with-monitoring app.php` with `required Elephc bridge elephc_instr could not be found`. A bridge is resolved from the directory the compiler lives in (or its sibling `lib/`), so a compiler ships with exactly what was packed beside it — and nothing that runs inside a checkout can notice a short packing list, because every archive is always present in `target/`. `elephc_magician`, the `eval()` interpreter, had been missing since before 0.26.4 on the same footing: a released elephc refused `--with-eval`, and any `eval()` it could not fold at compile time — one defining a function or a class, or reading its body from a variable — with the same linker error. Only an `eval()` of a constant expression escaped, because that never reaches the interpreter. The build list, the tar list and the formula are now each checked against the bridge table.

- Added selective monitoring: `--with-monitoring=<names>` hooks only the functions you name, with a trailing `*` matching by prefix and `@file` reading one name per line. Everything else runs at full speed, which is what makes exact numbers affordable on a service under load — on the demo service, profiling all 35 functions cost +3% against +1% for three. The trade is reported rather than hidden: with a subset, an uninstrumented callee runs inside its instrumented caller's frame, so its time lands in that caller's self and self values stop summing to the root's inclusive. Every selective run prints a note saying exactly that, because presenting the same columns as the full mode would quietly redefine each one.
- Added sampled allocation attribution to the sampling path, which previously had no memory dimension at all: each sample charges the allocation counter's delta since the previous one to the stack it caught. The total is exact by construction, since consecutive deltas telescope back to the counter, while the attribution is statistical — the report distinguishes the two rather than presenting one number.
- Changed the sampling path to report exact DB query counts and I/O wait per route. Those dimensions looked like they belonged to the per-call hooks, but they never came from them: the PDO bridge emits an event per driver call, and attributing one needs only a notion of where you are, which the sampler already has. A run the sampler observed 17 times still reported the same 551 queries the exact profile reports, since an event is not a sample.
