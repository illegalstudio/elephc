...
- Fixed appending to a boxed `mixed` value whose payload is a null container segfaulting (issue #592): a missed indexed read forwarded through a ternary/branch merge boxed the null-container sentinel, and `$r[] = v` then read the array header inline before the sentinel was filtered and faulted (exit 139). The Mixed-cell append path now guards the null pointer and the null-container sentinel and auto-vivifies a fresh single-element array — matching PHP's `$x[] = v` on `null` — for null, sentinel, and unset payloads on every supported target, staying heap-clean.
