...
## [Unreleased]
- Fixed several PHP conformance gaps: `Exception`/`Error` constructors now accept and store `$previous` (`?Throwable`) so `getPrevious()` round-trips instead of always returning null; interface method returns may narrow `self` covariantly (including PSR-7-style wither call sites and parent `static` overrides); soft-keyword `enum` is accepted as a class/interface/trait name (`class Enum`, `implements Enum`); `use const` imports lexer-tokenized globals and aliases; and `foreach` over unknown `array` values widens keys to `int|string` instead of treating them as integers only.
