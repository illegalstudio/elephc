...
- `--web` worker CPU affinity: `--worker-affinity` pins each worker to CPU getpid()%ncpus (round-robin; Linux hard pin via sched_setaffinity, macOS advisory thread_policy_set tag). Best-effort, default off, no hot-path cost.
- `--web` static-asset fast path: opt-in `--static-dir DIR` serves files from `DIR` on the I/O thread before the PHP handler runs, so a static asset never occupies the PHP runtime. `--static-prefix PATH` (default `/assets`) triggers the intercept; other paths fall through to PHP unchanged. Files are read via `spawn_blocking` + `std::fs` (no `tokio::fs` dependency) and cached in a per-worker LRU sized by `--static-cache-size MIB` (default 64; `0` rejected when `--static-dir` is set). `--static-max-age SECS` (default 3600) sets `Cache-Control`, `--static-max-file-size BYTES` (default 1 MiB; `0` = no cap) bounds file size, and `If-None-Match` returns `304` (ETag is `"<size>-<mtime_secs>"`). Path traversal is rejected with `404`. Off by default; the hot path is byte-for-byte unchanged when `--static-dir` is absent. All three web modes.
- `--web` metrics endpoint: `--metrics` exposes a per-worker JSON snapshot at `--metrics-path` (default `/_status`) with rps, latency p50/p99, status classes, 503 rate, RSS, active connections, and h2 stream counts. Per-worker (SO_REUSEPORT: scrape repeatedly); off by default; the endpoint is not recorded as a request. All three web modes.
- `--web` zero-downtime SIGHUP reload: sending SIGHUP to the master triggers a rolling worker reload — each worker is SIGUSR1'd, drains in-flight requests within `--reload-grace` SECS (default 10, 0 = wait forever), exits cleanly, and the master respawns a fresh worker (N-1 always serve). SIGINT/SIGTERM stay hard-kill (unchanged). All three web modes.
- `--web` RSS-based worker recycle: opt-in `--max-rss MiB` makes a worker self-measure its resident set and exit cleanly when over the cap, so the master respawns a fresh worker (bounding memory growth over time); `0` = off (default, byte-for-byte unchanged). All three web modes; measurement gated to at most once per 64 accepts.
- HTTP/2 over TLS via ALPN negotiation (`--http2` + `--tls-cert`): TLS now advertises `h2` ahead of `http/1.1` when `--http2` is set, so capable clients negotiate h2 over TLS; plaintext h2c prior-knowledge unchanged.
- Global-state audit tool + mutable-symbol classification baseline: new `elephc-globals-audit` crate extracts and classifies every `.comm`/`.globl`/`static mut` symbol into CONST/RO_BOOT/SHARED_LOCK/TLS, with per-target versioned baselines and a `--check` gate that fails on new unclassified globals or changed classifications. Raw `adrp` symbol loads across `src/codegen/` and `src/codegen_ir/` are centralized onto `emit_symbol_address` (and the paged-load helper), with an audit test forbidding new raw `adrp` on mutable symbols outside the central seam. An addressing-mode microbench harness measures baseline vs native TLS vs context-register access costs on macos-aarch64, with cross-compiled asm verification for linux targets. No user-visible change.
- `--web` HTTP/2 opt-in: `--http2` makes the compiled `--web`, `--web-worker`, and `--web-worker=script` server speak HTTP/2 (h2c prior-knowledge on plaintext) in addition to HTTP/1.1, with per-connection concurrent streams, GOAWAY-based graceful shutdown, and HPACK header-bomb clamping. **Requires `--handler-offload`** (without offload, multiplexed h2 streams all stall on the single inline handler; exit 2 otherwise) and is off by default (the server speaks h1 only via one `http1_only()` code path when the flag is absent). `--http2-max-streams N` (default 8) caps concurrent streams per connection and acts as the per-connection stream budget (fallback to `--max-requests`/`--max-requests-per-connection`); `--http2-max-header-size N` (default 64 KiB) clamps the HPACK header block; `N < 1` for max-streams is rejected (exit 2). Connection-level headers are stripped from h2 responses (RFC 7540 §8.1.2.2). h2 over TLS (ALPN `h2`) is a follow-up; ALPN still advertises `http/1.1` only today.
- `--web` handler offload: opt-in `--handler-offload` runs the PHP handler on a dedicated `php-handler` thread per worker, fed by a bounded job queue by the I/O thread, so request/response I/O of other connections overlaps handler execution (handlers still never overlap — one consumer thread; the N=1 case of the future ZTS work). `--max-pending N` (default 16; `0` → exit 2) bounds the queue and returns `503` + `Retry-After: 1` when full; queued-body memory is bounded by `N × --max-body-size`. `exit`/`die` and `--max-execution-time` are unchanged (the bailout anchor rides `handler()` to the handler thread; SIGALRM is delivered there deterministically). Applies to all three web modes; default off keeps the synchronous inline handler path byte-for-byte unchanged.
- `--web` master fd-dispatch: opt-in `--dispatch master` makes the master process accept every connection and hand it (as a raw fd, before any TLS handshake) to the first idle worker over a per-worker socketpair, a real shared queue that removes head-of-line blocking between workers — a slow request occupies one worker while others keep serving, and the distribution is uniform on macOS too. Applies to all three web modes; the default `--dispatch kernel` (per-worker `SO_REUSEPORT`) is unchanged. Each worker serves one connection at a time (slot = 1), so keep-alive connections stay pinned — pair with a short `--max-requests-per-connection` to rebalance. `--dispatch-backlog N` (default 1024) bounds the master's queue; when full it applies kernel SYN backpressure rather than rejecting (never a 503). If the master dies, workers exit cleanly on EOF.
- `--web` server-side TLS: the compiled `--web`, `--web-worker`, and `--web-worker=script` server can now terminate HTTPS itself via opt-in `--tls-cert FILE`/`--tls-key FILE` (PEM cert chain + unencrypted key), serving HTTP/1.1 over TLS 1.2/1.3 on `--listen` in place of plaintext. Over TLS `$_SERVER['HTTPS']` is `'on'` and `REQUEST_SCHEME` is `'https'` (the `HTTPS` key stays absent on plaintext, PHP-FPM-exact). A missing flag pair or an unreadable/malformed cert/key fails fast (exit 2) before any worker forks; certificates are read once at startup (restart to rotate).
- `--web` keep-alive rotation: opt-in `--max-requests-per-connection` and `--idle-timeout` keep-alive controls for the compiled `--web` server (default off; no behavior change). Both apply in all three web modes and let an operator rebalance keep-alive connections across workers on demand — `--max-requests-per-connection N` closes a connection after N responses so the client reconnects and `SO_REUSEPORT` re-picks a worker, and `--idle-timeout SECS` closes an idle connection after SECS seconds. A worker at its `--max-requests` recycle cap now also drains its keep-alive connections with `Connection: close` instead of cutting them at exit.
...
- `--web` throughput: the request superglobals (`$_SERVER`, `$_GET`, `$_POST`, `$_FILES`, `$_COOKIE`, `$_REQUEST`, `$_ENV`) are now built only when the program references them, so a handler that reads none of them pays no per-request superglobal cost. Detection covers references inside included and autoloaded files and is transparent — a program that reads all of them behaves exactly as before. Dependent superglobals are pulled in automatically (`$_REQUEST` → `$_GET`+`$_POST`; `$_POST`/`$_COOKIE` → `$_SERVER`).
- `--web-worker` (trampoline mode): `$_ENV` is now read from the process environment once at worker boot and kept for the worker's lifetime instead of being rebuilt on every request (the environment is fixed at fork). A per-request `$_ENV` mutation therefore persists into the next request in this mode; use `getenv()` for the live process environment. Classic `--web` and `--web-worker=script` are unchanged (`$_ENV` stays per-request fresh there).
- `--web` performance: fixed a per-request memory leak in the request bridge (each request's parsed method/URI/headers/body were overwritten without being freed, growing worker memory under load); disabled Nagle (`TCP_NODELAY`) on accepted connections to cut keep-alive tail latency; response gzip now uses the fast compression level (level 1) since it runs on the worker's single thread; each header's `$_SERVER` key (`HTTP_*`) is precomputed in Rust so the prelude no longer runs `strtoupper`/`str_replace` per header per request; the request body is kept as a reference-counted buffer instead of being copied into a fresh `Vec` per request; the HTTP connection builder is constructed once per worker rather than per connection; and the per-request path avoids redundant header/protocol string copies. No behavior change.
