...
## [Unreleased]
- Fixed `count()` on a boxed `mixed` receiver whose payload is the null-container sentinel segfaulting (issue #602): a missed array read forwarded through a ternary merge boxes the sentinel as an indexed-array cell, and `__rt_mixed_count` dereferenced the sentinel payload past a plain-zero guard. The runtime helper now recognizes every null form of the receiver — a null pointer, the in-band sentinel (as the cell or as the tag-4/5 payload), and a boxed null cell (tag 8) — and returns a null signal that the call site turns into PHP's `TypeError: count(): Argument #1 ($value) must be of type Countable|array, null given`, matching the concrete-array path. Real containers, including empty ones, still count correctly, and the catchable error path stays heap-clean.
