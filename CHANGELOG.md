...
## [Unreleased]
- Fixed `sprintf()`/`printf()` silently formatting `0` for every argument when the format string is not a compile-time literal and the argument is a `mixed` value — the shape produced by iterating over a heterogeneous array, as in `foreach ([["%5d", 42]] as $c) echo sprintf($c[0], $c[1]);`, which printed `    0` instead of `   42`. `echo` rendered the same value correctly, so the loss looked like a formatting bug when it was argument marshalling: with no literal format there is no known conversion category, and the packer's catch-all arm pushed a zero payload tagged as an integer for every shape it did not name, `mixed` included. A `mixed` operand is now packed from its real runtime type, which is enough because the formatter already coerces a record whose tag disagrees with the conversion character. Also fixed `vsprintf()`/`vprintf()`/`vfprintf()` rendering a `null` array element as `9223372036854775806` — the raw internal null sentinel — where PHP renders `""` for `%s` and `0` for `%d`; `null` now packs as a zero-length string, which both conversions already handle. The conversion ladder the two paths had separately is now shared, so `sprintf($fmt, $v)` and `vsprintf($fmt, [$v])` cannot drift apart again.
