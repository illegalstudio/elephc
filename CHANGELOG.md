...
- Changed the remaining wrapper slots that read a raw scalar — `stream_open`, `dir_opendir`, and the path-op family (`unlink`, `rename`, `mkdir`, `rmdir`) — to consult the same boxed-result mask as the stream slots, completing the family. This is defensive completion rather than a fix with a reproducer, and the distinction is worth stating: no wrapper body written so far makes those methods infer a `Mixed` return, so their mask bit is never set and removing the conversion changes no observable output. A behavioural test therefore cannot tell whether their wiring is present or has rotted, which is why the guard is structural — it asserts every such helper routes through `__rt_wrapper_unbox_int` on both architectures, and it fails when any one of them stops. What made the difference for `stream_tell` was the body returning an untyped property, not the missing annotation on its own; the same shape reaching these slots would have been wrong in the same way.
