...
- Experimental wasm32-wasi AOT backend: `--target wasm32-wasi` compiles a supported PHP/EIR subset to a WebAssembly module (`.wat`/`.wasm`), with `--emit npm` packaging the module as a Node.js WASI package. Portability is gated on the pinned Node.js, Wasmer, and Wasmtime versions documented in the target guide; arbitrary WASI hosts are not yet claimed. Unsupported operations and callable shapes are rejected by the pre-emission capability audit. Higher-order array and `call_user_func*` support is shape-limited and remains part of the explicit WASM parity campaign rather than a complete PHP surface.
