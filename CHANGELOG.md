...
- Fixed post-guard type narrowing being dropped when the guard body diverges through structure the previous last-statement check ignored (issue #590): the checker now reuses the compiler's structural termination analysis, so a terminal statement before unreachable trailing code, and nested `if`/`switch`/`try` branches that all terminate, keep the narrowed non-null type for the following statements (for example, list-unpacking a guarded `?array`). `break`/`continue` and `exit()`/`die()`/declared-`never` calls remain valid non-fallthrough effects.
