...
## [Unreleased]
- Fixed `count()` answering 0 and continuing for a value that is not countable, where PHP raises a `TypeError` and stops. `count(false)` printed `0`; it now dies with `count(): Argument #1 ($value) must be of type Countable|array, false given`, php-src's own wording, and the type is named exactly as PHP names it — including a boolean by its VALUE (`false given` / `true given`, never `bool given`), plus `int`, `float`, `string`, `resource` and `null`. The quiet zero was PHP 7.2's behaviour and the runtime documented it as intentional, which is why nobody had reason to look. It mattered beyond its own wrongness: `count()`'s checker hook refuses a union unless EVERY member is countable, which reads as gratuitous strictness but was the only thing catching `count(file($p))` at compile time in place of this missing throw — so it was also what blocked giving `file()` its `array|false` return type. An object is left as it was: PHP also throws for one that does not implement `Countable`, and deciding that needs the interface check at run time. One test asserted the old zero for `count(json_decode("42"))` and has been turned around against reference PHP.
