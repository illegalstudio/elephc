    ...
    pub fn execute(&mut self, sql: &str, params: Vec<SqlType>) -> Result<FirebirdResult, String> {
        ...
        if keyword == "SELECT" || keyword == "WITH" {
            match self.connection.query::<_, Row>(sql, params) {
                Ok(rows) => {
                    let result = FirebirdResult::from_rows(rows, self, sql);
            ...
            match self.connection.execute_returnable::<_, Row>(sql, params) {
                Ok(row) => {
                    let result = FirebirdResult::from_rows(vec![row], self, sql);
                    self.changes = 1;
                    Ok(result)
                }
                Err(error) => Err(self.record_error(error)),
...
impl FirebirdResult {
    ...
    fn from_rows(rows: Vec<Row>, connection: &FirebirdConn, sql: &str) -> Self {
        let temporal_types = result_temporal_types(sql);
        let columns = rows.first().map_or_else(Vec::new, |row| {
            row.cols
                .iter()
                .enumerate()
                .map(|(index, column)| FirebirdColumn {
                    name: column.name.clone(),
                    raw_type: i64::from(
                        temporal_types
                            .get(index)
                            .and_then(|value| *value)
                            .unwrap_or(column.raw_type),
                    ),
                })
                .collect()
        });
        let rows = rows
            ...
            .map(|row| {
                row.cols
                    ...
                    .enumerate()
                    .map(|(index, column)| {
                        let temporal_type = temporal_types.get(index).and_then(|value| *value);
                        let raw_type = temporal_type.unwrap_or(column.raw_type);
                        decode_cell(raw_type, column.value, connection, temporal_type)
                    })
                    .collect()
            })
            .collect::<Vec<_>>();
...
/// Recovers DATE/TIME/TIMESTAMP result types that rsfbclient's pure-Rust
/// protocol layer normalizes to TIMESTAMP before returning row metadata.
fn result_temporal_types(sql: &str) -> Vec<Option<u32>> {
    top_level_result_expressions(sql)
        .into_iter()
        .map(|expression| {
            let words = sql_keywords(expression);
            words.windows(2).find_map(|pair| match pair {
                [as_word, type_word] if as_word == "AS" && type_word == "DATE" => Some(570),
                [as_word, type_word] if as_word == "AS" && type_word == "TIME" => Some(560),
                [as_word, type_word] if as_word == "AS" && type_word == "TIMESTAMP" => Some(510),
                _ => None,
            }).or_else(|| match words.first().map(String::as_str) {
                Some("DATE") => Some(570),
                Some("TIME") => Some(560),
                Some("TIMESTAMP") => Some(510),
                _ => None,
            })
        })
        .collect()
...
/// Splits an outer SELECT or DML RETURNING projection without treating nested
/// expressions, quoted text, or comments as projection delimiters.
fn top_level_result_expressions(sql: &str) -> Vec<&str> {
    let bytes = sql.as_bytes();
    let mut expressions = Vec::new();
    let mut projection_start = None;
    let mut stops_at_from = false;
    ...
    while index < bytes.len() {
        ...
        match bytes[index] {
            ...
            b',' if depth == 0 && projection_start.is_some() => {
                expressions.push(sql[expression_start..index].trim());
                expression_start = index + 1;
            }
            _ if depth == 0
                && projection_start.is_none()
                && (keyword_at(bytes, index, b"SELECT")
                    || keyword_at(bytes, index, b"RETURNING")) =>
            {
                stops_at_from = keyword_at(bytes, index, b"SELECT");
                let keyword_len = if stops_at_from {
                    "SELECT".len()
                } else {
                    "RETURNING".len()
                };
                let start = index + keyword_len;
                projection_start = Some(start);
    ...
    if projection_start.is_some() && expression_start < sql.len() {
        expressions.push(sql[expression_start..].trim());
    }
    expressions
}

/// Reports whether an ASCII SQL keyword starts at one token boundary.
fn keyword_at(bytes: &[u8], index: usize, keyword: &[u8]) -> bool {
    let Some(candidate) = bytes.get(index..index + keyword.len()) else {
        return false;
    };
    let boundary = |byte: Option<&u8>| {
        byte.map_or(true, |byte| !byte.is_ascii_alphanumeric() && *byte != b'_')
    };
    boundary(index.checked_sub(1).and_then(|previous| bytes.get(previous)))
        && boundary(bytes.get(index + keyword.len()))
...
fn decode_cell(
    raw_type: u32,
    value: SqlType,
    connection: &FirebirdConn,
    temporal_type: Option<u32>,
) -> FirebirdCell {
    match value {
        ...
        SqlType::Timestamp(value) => {
            let format = match temporal_type.map(|raw_type| raw_type & !1) {
                Some(570) => &connection.date_format,
                Some(560) => &connection.time_format,
                Some(510) => &connection.timestamp_format,
                _ => match raw_type & !1 {
                    570 => &connection.date_format,
                    560 => &connection.time_format,
                    510 if value.time() == chrono::NaiveTime::default() => {
...
mod tests {
    ...
    /// Recovers normalized temporal types from each outer SELECT expression.
    #[test]
    fn recovers_temporal_cast_types_from_select_projection() {
        assert_eq!(
            result_temporal_types(
                "WITH seed AS (SELECT 1 FROM RDB$DATABASE) \
                 SELECT CAST('2024-02-03' AS DATE) AS d, \
                        COALESCE(CAST('12:34:56' AS TIME), CAST('00:00:00' AS TIME)) AS t, \
                        CAST('2024-02-03 00:00:00' AS TIMESTAMP) AS ts, \
                        'FROM, AS DATE' AS label \
                 FROM seed",
            ),
            vec![Some(570), Some(560), Some(510), None]
        );
        assert_eq!(
            result_temporal_types(
                "UPDATE events SET touched = CURRENT_TIMESTAMP \
                 RETURNING DATE '2024-02-03', TIME '12:34:56', \
                           CAST(touched AS TIMESTAMP)",
            ),
            vec![Some(570), Some(560), Some(510)]
        );
    }
