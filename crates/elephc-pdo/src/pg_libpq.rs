    ...
    (0..result.nfields())
        .map(|column| {
            ...
            match result.field_type(column) as i64 {
                ...
                700 | 701 => std::str::from_utf8(value)
                    .ok()
                    .and_then(|value| value.parse().ok())
                    .map(Cell::Float)
                    .unwrap_or_else(|| Cell::Text(String::from_utf8_lossy(value).into_owned())),
                // PostgreSQL numeric is text in PDO so its declared scale and
                // arbitrary precision survive the bridge unchanged.
                1700 => Cell::Text(String::from_utf8_lossy(value).into_owned()),
...
impl PgConn {
    ...
    /// Executes one transaction-control command through libpq, returning `1`/`0`.
    pub fn exec_simple(&mut self, sql: &str) -> i64 {
        (self.exec(sql) >= 0) as i64
    }
    ...
    pub fn last_insert_id_text(&mut self, name: Option<&str>) -> String {
        ...
        let result = self.client.exec(&format!("{query}::text"));
        if !result_ok(&result) {
        ...
        self.errmsg.clear();
        self.sqlstate = "00000".to_string();
        result.value(0, 0).map(|value| String::from_utf8_lossy(value).into_owned()).unwrap_or_default()
    }
    ...
    pub fn client_version(&self) -> String {
        format!("postgres libpq {}", format_server_version(libpq::version()))
    }
