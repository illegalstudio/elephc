...
extern "C" {
    ...
    fn dbsetlshort(login: *mut LoginRecord, value: c_int, which: c_int) -> c_int;
    ...
    pub fn open(dsn: &str) -> Result<Self, String> {
        ...
        unsafe {
            ...
            if let Some(port) = options.port {
                if configured.is_ok() && dbsetlshort(login, port, DBSETPORT) == FAIL {
                    dbloginfree(login);
                    return Err("PDO_DBLIB: failed to set port".to_string());
                }
            }
            ...
            let host = CString::new(options.host)
...
impl DblibStmt {
    ...
    pub fn new(conn_id: i64, sql: &str) -> Result<Self, String> {
        let (translated_sql, named_map, order, mixed) =
            crate::my::translate_placeholders(sql, false);
        if mixed {
...
mod tests {
    ...
    fn interpolates_dblib_bindings() {
        let (sql, _, order, _) = crate::my::translate_placeholders(
            "SELECT :name, :name, ?",
            false,
        );
