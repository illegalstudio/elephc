...
fn store_open_failure(dsn: &str, message: &str) {
    ...
    let (sqlstate, native_code) = if dsn.starts_with("cubrid:") {
        ...
        #[cfg(not(any(feature = "odbc", feature = "informix", feature = "ibm", feature = "sqlsrv")))]
        {
            (String::new(), 0)
        }
    } else {
...
thread_local! {
    /// Per-thread byte buffer for the most recent `elephc_pdo_column_data_ptr` result.
...
/// Per-thread buffer for the most recent `elephc_pdo_mysql_charset` result.
fn mysql_charset_cell() -> &'static LocalKey<RefCell<CString>> {
    thread_local! {
        static C: RefCell<CString> = RefCell::new(CString::default());
    }
    &C
...
/// data pointer call on that thread; elephc copies it immediately through
/// `ptr_read_string`.
fn store_bytes(bytes: Vec<u8>) -> *const c_char {
    COLDATA_CELL.with(|cell| {
...
fn open_conn_for_dsn(
    ...
    match driver::DriverKind::from_dsn(dsn) {
        ...
        #[cfg(feature = "ibm")]
        Some(driver::DriverKind::Ibm) => odbc::OdbcConn::open_ibm(dsn).map(Conn::Odbc),
        #[cfg(feature = "sqlsrv")]
        Some(driver::DriverKind::Sqlsrv) => odbc::OdbcConn::open_sqlsrv(dsn).map(Conn::Odbc),
        #[cfg(feature = "oci")]
...
/// v56 adds PDO_IBM 1.7.0 connection attributes and metadata, and completes the
/// common PDO core `name`/`len`/`precision` metadata for CLI-backed drivers.
/// v57 adds Microsoft PDO_SQLSRV 5.13.1 through Microsoft ODBC Driver 17/18,
/// including its statement attributes, connection information, datetime flag,
/// and sensitivity-classification metadata accessors.
#[no_mangle]
...
pub extern "C" fn elephc_pdo_available_driver_name(index: i64) -> *const c_char {
    ffi_guard(static_cstr(b"\0"), || {
        ...
        match kind {
            ...
            #[cfg(feature = "ibm")]
            driver::DriverKind::Ibm => static_cstr(b"ibm\0"),
            #[cfg(feature = "sqlsrv")]
            driver::DriverKind::Sqlsrv => static_cstr(b"sqlsrv\0"),
            #[cfg(feature = "oci")]
...
/// is `0`. `my_driver_config` (v41) is the packed pdo_mysql connection-option
/// string carrying LOCAL INFILE controls, compression, IGNORE_SPACE,
/// multi-statements, buffering, CAPATH, cipher suites, and an authentication
/// public key. A caught panic degrades to the same `-1` failure sentinel as a failed open.
///
/// # Safety
/// `dsn` and, when non-null, `my_init_command`/`my_ssl_config`/`persistent_key`/
/// `my_driver_config` must each point to a NUL-terminated string valid for the
...
pub unsafe extern "C" fn elephc_pdo_last_insert_id(conn_id: i64, name: *const c_char) -> i64 {
    ffi_guard(0, || {
        ...
        match guard.get_mut(&conn_id) {
            ...
            #[cfg(any(feature = "odbc", feature = "informix", feature = "ibm", feature = "sqlsrv"))]
            Some(Conn::Odbc(c)) => c.last_insert_id(cstr_arg(name)).parse().unwrap_or(0),
            #[cfg(feature = "oci")]
            Some(Conn::Oci(_)) => 0,
    ...
    ffi_guard(static_cstr(b"\0"), || {
        let text = {
            ...
            match guard.get_mut(&conn_id) {
                ...
                #[cfg(any(feature = "odbc", feature = "informix", feature = "ibm", feature = "sqlsrv"))]
                Some(Conn::Odbc(c)) => c.last_insert_id(cstr_arg(name)),
                #[cfg(feature = "oci")]
...
/// Applies one PDO_SQLSRV runtime-settable statement attribute.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_stmt_set_attribute(
    stmt_id: i64,
    attribute: i64,
    value: i64,
) -> i64 {
    ffi_guard(0, || {
        #[cfg(feature = "sqlsrv")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get_mut(&stmt_id) {
            return statement.set_sqlsrv_attribute(attribute, value) as i64;
        }
        let _ = (stmt_id, attribute, value);
        0
    })
}

/// Applies one PDO_SQLSRV prepare-only statement option.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_stmt_configure(
    stmt_id: i64,
    attribute: i64,
    value: i64,
) -> i64 {
    ffi_guard(0, || {
        #[cfg(feature = "sqlsrv")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get_mut(&stmt_id) {
            return statement.configure_sqlsrv_prepare_option(attribute, value) as i64;
        }
        let _ = (stmt_id, attribute, value);
        0
    })
...
/// Reads one PDO_SQLSRV statement attribute, or `-1` when unsupported.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_stmt_attribute(stmt_id: i64, attribute: i64) -> i64 {
    ffi_guard(-1, || {
        #[cfg(feature = "sqlsrv")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get(&stmt_id) {
            return statement.sqlsrv_attribute(attribute).unwrap_or(-1);
        }
        let _ = (stmt_id, attribute);
        -1
    })
}

/// Reports whether SQLSRV's datetime-fetch mode should create a `DateTime` object.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_column_is_datetime(stmt_id: i64, column: i64) -> i64 {
    ffi_guard(0, || {
        #[cfg(feature = "sqlsrv")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get(&stmt_id) {
            return statement.column_is_datetime(column) as i64;
        }
        let _ = (stmt_id, column);
        0
    })
...
/// Returns one PDO_SQLSRV client/server information field as text.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_info(conn_id: i64, field: i64) -> *const c_char {
    ffi_guard(static_cstr(b"\0"), || {
        let value = {
            #[cfg(feature = "sqlsrv")]
            if let Some(Conn::Odbc(connection)) = lock_recover(conns()).get_mut(&conn_id) {
                connection.sqlsrv_info(field)
            } else {
                String::new()
            }
        ...
        let _ = (conn_id, field);
        store_cstr(server_info_cell(), &value)
    })
}

/// Returns the number of SQLSRV sensitivity pairs for one result column.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_classification_pair_count(
    stmt_id: i64,
    column: i64,
) -> i64 {
    ffi_guard(-1, || {
        #[cfg(feature = "sqlsrv")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get_mut(&stmt_id) {
            return statement.sqlsrv_classification_pair_count(column);
        }
        let _ = (stmt_id, column);
        -1
    })
...
/// Returns one SQLSRV sensitivity label or information-type string.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_classification_text(
    stmt_id: i64,
    column: i64,
    pair: i64,
    field: i64,
    ...
    ffi_guard(static_cstr(b"\0"), || {
        let value = {
            #[cfg(feature = "sqlsrv")]
            if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get_mut(&stmt_id) {
                statement.sqlsrv_classification_text(column, pair, field)
            } else {
                String::new()
            }
            #[cfg(not(feature = "sqlsrv"))]
        ...
        let _ = (stmt_id, column, pair, field);
        store_cstr(sqlsrv_classification_cell(), &value)
    })
}

/// Returns one SQLSRV column sensitivity rank, or `-1` when absent.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_classification_pair_rank(
    stmt_id: i64,
    column: i64,
    pair: i64,
) -> i64 {
    ffi_guard(-1, || {
        #[cfg(feature = "sqlsrv")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get_mut(&stmt_id) {
            return statement.sqlsrv_classification_pair_rank(column, pair);
        }
        let _ = (stmt_id, column, pair);
        -1
    })
}

/// Returns the SQLSRV result-set sensitivity rank, or `-1` when absent.
#[no_mangle]
pub extern "C" fn elephc_pdo_sqlsrv_classification_query_rank(stmt_id: i64) -> i64 {
    ffi_guard(-1, || {
        #[cfg(feature = "sqlsrv")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get_mut(&stmt_id) {
            return statement.sqlsrv_classification_query_rank();
        }
        let _ = stmt_id;
        -1
    })
...
/// Writes one string-valued PDO_IBM client/trusted-context connection attribute.
#[no_mangle]
pub unsafe extern "C" fn elephc_pdo_ibm_set_attribute_text(
    conn_id: i64,
    attribute: i64,
    value: *const c_char,
) -> i64 {
    ffi_guard(0, || {
        #[cfg(feature = "ibm")]
        if let (Some(value), Some(Conn::Odbc(connection))) =
            (cstr_arg(value), lock_recover(conns()).get_mut(&conn_id))
        {
            return connection.set_ibm_attribute_text(attribute, value) as i64;
        }
        let _ = (conn_id, attribute, value);
        0
    })
...
/// Reads one string-valued PDO_IBM client/trusted-context connection attribute.
#[no_mangle]
pub extern "C" fn elephc_pdo_ibm_attribute_text(
    conn_id: i64,
    attribute: i64,
) -> *const c_char {
    ffi_guard(static_cstr(b"\0"), || {
        let value = {
            #[cfg(feature = "ibm")]
            if let Some(Conn::Odbc(connection)) = lock_recover(conns()).get_mut(&conn_id) {
                connection.ibm_attribute_text(attribute).unwrap_or_default()
            } else {
                String::new()
            }
            #[cfg(not(feature = "ibm"))]
        ...
        let _ = (conn_id, attribute);
        store_cstr(ibm_attribute_cell(), &value)
    })
}

/// Reads PDO_IBM's integer-valued trusted-context enablement attribute.
#[no_mangle]
pub extern "C" fn elephc_pdo_ibm_attribute_int(conn_id: i64, attribute: i64) -> i64 {
    ffi_guard(-1, || {
        #[cfg(feature = "ibm")]
        if let Some(Conn::Odbc(connection)) = lock_recover(conns()).get_mut(&conn_id) {
            return connection.ibm_attribute_int(attribute).unwrap_or(-1);
        }
        let _ = (conn_id, attribute);
        -1
    })
        ...
        let prepared: Result<Stmt, ()> = if let Some(db) = sqlite_db {
            ...
            match guard.get_mut(&conn_id) {
                ...
                Some(Conn::Odbc(connection)) => match cstr_arg(sql) {
                    Some(sql) => odbc::OdbcStmt::new(connection, conn_id, sql, emulated)
                        .map(Stmt::Odbc)
                        .map_err(|_| ()),
                    None => Err(()),
                },
...
/// Reports whether one completed CLI output bind used PDO_IBM's native integer path.
#[no_mangle]
pub extern "C" fn elephc_pdo_output_is_numeric(stmt_id: i64, idx: i64) -> i64 {
    ffi_guard(0, || {
        let _ = idx;
        let guard = lock_recover(stmts());
        match guard.get(&stmt_id) {
            ...
            Some(Stmt::Odbc(statement)) => statement
                .output_value(idx)
                .map_or(0, |value| value.numeric as i64),
            _ => 0,
        }
    })
}
...
/// Returns PDO_IBM's `SQLDescribeCol` scale, or zero for another driver.
#[no_mangle]
pub extern "C" fn elephc_pdo_ibm_column_scale(stmt_id: i64, column: i64) -> i64 {
    ffi_guard(0, || {
        #[cfg(feature = "ibm")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get(&stmt_id) {
            return statement.column_scale(column);
...
/// Returns PDO_IBM's metadata `pdo_type`, including its upstream BOOLEAN fallthrough.
#[no_mangle]
pub extern "C" fn elephc_pdo_ibm_column_pdo_type(stmt_id: i64, column: i64) -> i64 {
    ffi_guard(2, || {
        #[cfg(feature = "ibm")]
        if let Some(Stmt::Odbc(statement)) = lock_recover(stmts()).get(&stmt_id) {
            return statement.column_pdo_type(column);
...
pub extern "C" fn elephc_pdo_column_len(stmt_id: i64, i: i64) -> i64 {
    ffi_guard(-1, || {
        ...
        match guard.get(&stmt_id) {
            ...
            #[cfg(any(feature = "odbc", feature = "informix", feature = "ibm", feature = "sqlsrv"))]
            Some(Stmt::Odbc(s)) => s.column_len(i),
...
pub extern "C" fn elephc_pdo_column_precision(stmt_id: i64, i: i64) -> i64 {
    ffi_guard(-1, || {
        ...
        match guard.get(&stmt_id) {
            ...
            Some(Stmt::Firebird(_)) => -1,
            #[cfg(any(feature = "odbc", feature = "informix", feature = "ibm", feature = "sqlsrv"))]
            Some(Stmt::Odbc(s)) => s.column_precision(i),
...
/// The pointer remains valid until the next `elephc_pdo_column_data_ptr` call on the
/// current thread. An empty column — and a caught panic — report a NULL pointer,
/// which the prelude reads as the empty string (it pairs every read with
/// `elephc_pdo_column_data_len`). Calls on another thread use an independent buffer.
#[no_mangle]
...
mod tests {
    ...
    /// each thread has copied them out of the bridge-owned return buffer.
    #[test]
    fn column_data_buffers_are_isolated_between_threads() {
        let barrier = std::sync::Arc::new(std::sync::Barrier::new(2));
        let handles = [b"Ada".to_vec(), b"Grace".to_vec()].map(|expected| {
            let barrier = std::sync::Arc::clone(&barrier);
            std::thread::spawn(move || {
                let pointer = store_bytes(expected.clone());
                barrier.wait();
                let actual = unsafe { read_bytes(pointer, expected.len() as i64) };
    ...
    /// history is enumerated on `elephc_pdo_version`'s own docblock. v57 exposes
    /// PDO_SQLSRV and its driver-specific statement/configuration surface on top of v56.
    #[test]
    fn version_is_v57() {
        assert_eq!(elephc_pdo_version(), 57);
    }
