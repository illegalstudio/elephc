...
/// Returns the end of a comment recognized by php-src's generic PDO scanner.
///
/// Unlike the MySQL-specific scanner, the generic scanner accepts every `--`
/// line comment and does not treat `#` as a comment opener. That distinction is
/// required for SQL Server temporary-table identifiers such as `#events`.
fn scan_pdo_comment(bytes: &[u8], i: usize) -> Option<usize> {
    let len = bytes.len();
    match *bytes.get(i)? {
        b'-' if i + 1 < len && bytes[i + 1] == b'-' => {
            let mut j = i + 2;
...
pub fn translate_placeholders(
    ...
    translate_placeholders_impl(sql, no_backslash_escapes, true)
}

/// Translates placeholders with php-src's generic PDO scanner rules.
///
/// PDO_DBLIB, PDO_FIREBIRD, PDO_ODBC, PDO_INFORMIX, PDO_IBM, and PDO_SQLSRV use
/// the shared scanner, where `#` is ordinary SQL text and backslashes do not
...
pub(crate) fn translate_pdo_placeholders(
    ...
    translate_placeholders_impl(sql, true, false)
}

/// Implements placeholder translation for either the MySQL or generic PDO dialect.
fn translate_placeholders_impl(
    sql: &str,
    no_backslash_escapes: bool,
    mysql_rules: bool,
) -> (String, HashMap<String, i64>, Vec<i64>, bool) {
    ...
    while i < len {
        let c = bytes[i];
        // Copy the active dialect's comments verbatim so markers inside them are
        // never mistaken for placeholders.
        let comment_end = if mysql_rules {
            scan_my_comment(bytes, i)
        } else {
            scan_pdo_comment(bytes, i)
        };
...
mod tests {
    ...
    /// Generic PDO scanning keeps SQL Server `#temp` identifiers visible and
    /// still rewrites a named marker that follows them.
    #[test]
    fn generic_pdo_placeholders_support_sqlsrv_temp_tables() {
        let (sql, named, order, mixed) = translate_pdo_placeholders(
            "INSERT INTO #elephc_sqlsrv_bridge(label) VALUES (:label)",
        );
        assert_eq!(
            sql,
            "INSERT INTO #elephc_sqlsrv_bridge(label) VALUES (?)"
        );
        assert_eq!(named.get("label"), Some(&1));
