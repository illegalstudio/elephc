...
//!
//! Called from:
//! - The AOT and Magician lexers when decoding byte escapes and literal source text.
//! - AOT codegen/optimization and Magician parsing when recovering exact PHP bytes.
//!
//! Key details:
...
pub fn push_escaped_byte(byte: u8, out: &mut String) {
    if byte.is_ascii() {
...
/// - `out`: The growing literal string buffer.
pub fn push_literal_char(ch: char, out: &mut String) {
...
pub fn literal_bytes(value: &str) -> Vec<u8> {
...
pub fn literal_byte_len(value: &str) -> usize {
    value
        .chars()
        .map(|ch| {
            let codepoint = ch as u32;
