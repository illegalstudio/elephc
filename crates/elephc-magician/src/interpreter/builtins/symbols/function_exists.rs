...
pub(in crate::interpreter) fn eval_function_probe_exists(
    ...
    // exist on that profile) must still be reported. The xml surface is filtered the same
    // way: its names exist only when the host program linked the bridge.
    !name.contains("::")
        && (context.has_function(name)
            || (eval_php_visible_builtin_exists(name)
                && !eval_builtin_hidden_by_php_version(name)
                && eval_xml_builtin_linked(context, name))
