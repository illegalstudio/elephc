...
/// Evaluates `fnmatch($pattern, $filename, $flags = 0)`.
pub(in crate::interpreter) fn eval_fnmatch_declared_call(
    ...
    eval_builtin_fnmatch(args, context, scope, values)
}

/// Evaluates `fnmatch()` from already evaluated arguments.
pub(in crate::interpreter) fn eval_fnmatch_declared_values_result(
    ...
    match evaluated_args {
        ...
        [pattern, filename, flags] => eval_fnmatch_result(*pattern, *filename, Some(*flags), values),
