//! Purpose:
//! Declarative eval registry entry for `socket_set_block`, PHP's alias of `stream_set_blocking`.
//!
//! Called from:
...
/// Dispatches direct eval calls for `socket_set_block` through the canonical `stream_set_blocking` implementation.
pub(in crate::interpreter) fn eval_socket_set_block_declared_call(
    args: &[EvalExpr],
...
/// Dispatches evaluated-argument calls for `socket_set_block` through the canonical `stream_set_blocking` implementation.
pub(in crate::interpreter) fn eval_socket_set_block_declared_values_result(
