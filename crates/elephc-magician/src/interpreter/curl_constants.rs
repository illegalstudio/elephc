...
//!   `scripts/docs/curl_surface.json`'s `constants` map (already alphabetically sorted, no
//!   duplicates — the same frozen source `src/types/curl_constants.rs` generates from) by a
//!   one-off script, then cross-checked byte-for-byte against a fresh parse of that same
//!   JSON. The `#[cfg(test)]` module below re-parses the JSON at test time and asserts the
//!   two never drift apart, mirroring `src/types/curl_constants.rs`'s own
//!   `curl_constants_match_frozen_surface` test.
//! - Values come from libcurl 8.21.0 plus PHP's own extra options (the plan's locked
//!   decision 1), never from a developer's local `php -r` — identical to the root table.

/// Every `ext/curl` integer constant, alphabetically sorted, generated from
/// `scripts/docs/curl_surface.json`.
pub(in crate::interpreter) const EVAL_CURL_INT_CONSTANTS: &[(&str, i64)] = &[
    ("CURLALTSVC_H1", 8),
    ("CURLALTSVC_H2", 16),
    ("CURLALTSVC_H3", 32),
...
mod tests {
    ...
    /// Re-parses the frozen JSON surface at test time and asserts this table matches it
    /// exactly: same entry count, every name/value pair present on both sides. Mirrors
    /// `src/types/curl_constants.rs`'s `curl_constants_match_frozen_surface` so the two
    /// forked tables cannot silently drift from each other OR from the frozen source.
    #[test]
    fn eval_curl_constants_match_frozen_surface() {
        let json_path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../scripts/docs/curl_surface.json"
        );
        let json_text = std::fs::read_to_string(json_path)
            .unwrap_or_else(|error| panic!("failed to read {json_path}: {error}"));
        let parsed: serde_json::Value =
            serde_json::from_str(&json_text).expect("curl_surface.json must be valid JSON");
        let constants = parsed
            .get("constants")
            .and_then(serde_json::Value::as_object)
            .expect("curl_surface.json must carry a `constants` object");

        assert_eq!(
            constants.len(),
            EVAL_CURL_INT_CONSTANTS.len(),
            "EVAL_CURL_INT_CONSTANTS entry count diverges from curl_surface.json"
        );

        let table: HashMap<&str, i64> = EVAL_CURL_INT_CONSTANTS.iter().copied().collect();
        for (name, value) in constants {
            let expected = value.as_i64().unwrap_or_else(|| {
                panic!("curl_surface.json constant {name} is not representable as i64")
            });
            let actual = *table
                .get(name.as_str())
                .unwrap_or_else(|| panic!("{name} is missing from EVAL_CURL_INT_CONSTANTS"));
            assert_eq!(actual, expected, "{name} value diverges from curl_surface.json");
        }
    }

    /// Guards against a hand-edit introducing an accidental duplicate name.
    #[test]
    fn no_duplicate_constant_names() {
        let mut seen = std::collections::HashSet::new();
        for (name, _) in EVAL_CURL_INT_CONSTANTS {
            assert!(seen.insert(*name), "duplicate constant name: {name}");
        }
    }

    /// Eyeball-obvious regression trip wire for the two constants the TDD step exercises.
    #[test]
    fn curlopt_url_matches_frozen_json() {
        let table: HashMap<&str, i64> = EVAL_CURL_INT_CONSTANTS.iter().copied().collect();
        assert_eq!(table.get("CURLOPT_URL"), Some(&10002));
        assert_eq!(table.get("CURLOPT_RETURNTRANSFER"), Some(&19913));
    }
}
