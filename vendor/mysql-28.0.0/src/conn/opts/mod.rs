...
impl SslOpts {
    /// Restricts rustls to the named IANA/OpenSSL-compatible cipher suites.
    pub fn with_cipher_suites(mut self, suites: Option<Vec<String>>) -> Self {
        self.cipher_suites = suites;
        self
    }

    /// Returns the requested TLS cipher-suite names.
...
pub(crate) struct InnerOpts {
    ...
    /// Trusted server RSA key for non-TLS caching_sha2_password authentication.
    server_public_key_path: Option<Cow<'static, Path>>,
...
impl Opts {
    ...
    /// Returns the caller-supplied authentication RSA public-key path.
...
impl OptsBuilder {
    ...
    /// Supplies a trusted server RSA key for caching_sha2_password authentication.
    pub fn server_public_key_path<T: Into<Cow<'static, Path>>>(
        mut self,
        path: Option<T>,
    ) -> Self {
        self.opts.0.server_public_key_path = path.map(Into::into);
        self
    }
