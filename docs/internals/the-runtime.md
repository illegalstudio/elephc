...
Argument unpacking (`...`) is supported for eval-declared function calls inside eval when the unpacked value is an array. Indexed elements bind positionally, string-keyed elements bind by parameter name, and unpacking after a named argument is rejected before the unpacked expression is evaluated.
