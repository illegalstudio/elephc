...
Programs that use PHP's `eval()` link one extra static bridge library, `elephc_eval`, in addition to the normal runtime object. Programs without `eval()` do not link that library and keep the ordinary fully native runtime path.
