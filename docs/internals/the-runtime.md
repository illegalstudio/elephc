...
The runtime can also be emitted in **position-independent mode** for `--emit cdylib` builds: the emitter's `pic_data_refs` flag makes the `abi::symbols` helpers route every global data reference through the GOT (`@GOTPCREL` on x86_64, `:got:`/`:got_lo12:` on AArch64) instead of direct PC-relative addressing. Every internal ELF global receives `.hidden`; Mach-O uses `.private_extern`. The PIC and non-PIC variants produce different assembly text, so they cache as separate runtime objects.

The cdylib owned-string boundary also makes recoverable unwinding explicit. Each PIC user function publishes an exception-activation record that links the current frame to its generated cleanup callback. If an exception escapes an exact `string -> string` export, the existing Throwable machinery walks those records, releases function-owned values, and returns through the boundary handler. Heap and concat allocation exhaustion use the same active boundary to report `ELEPHC_STATUS_ALLOCATION_FAILURE`; the executable fatal paths remain unchanged. The wrapper then copies a successful PHP string into independently owned runtime-heap storage for the C caller, who releases it through `elephc_free`, and resets `_concat_off` before returning to keep repeated host calls independent. Diagnostic presence is stored separately from diagnostic length, so an empty Throwable message remains distinguishable from “no error”. See [The Codegen](the-codegen.md) and [Shared Libraries](../beyond-php/cdylib.md).
