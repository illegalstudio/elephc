...
Eval namespace declarations are resolved while the bridge parser builds EvalIR. The parser qualifies function declarations, empty class declarations, object construction names, and qualified references against the active namespace; for unqualified function calls and constant fetches it emits a namespaced lookup plus global fallback so PHP builtin functions and predefined constants remain visible from namespaced eval fragments. `__NAMESPACE__` is lowered to the active namespace string for the parsed block.
