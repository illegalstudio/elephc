...
| `__rt_mb_strwidth` | PHP 8.5 East Asian Width for `mb_strwidth()` (emitted with the same optional mbstring helper family) | `x1`/`x2` | `x0` |
...
pub(crate) fn emit_runtime(emitter: &mut Emitter, features: RuntimeFeatures) {
    ...
    // search/replace, explode/implode, hashing, encoding, sscanf, mb_strlen/mb_strwidth (gated), ...
