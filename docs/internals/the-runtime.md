...
Eval `preg_match()`, count-only `preg_match_all()`, `preg_replace()`, `preg_replace_callback()`, and `preg_split()` compile stripped PHP-delimited patterns through Rust's `regex` crate inside the eval bridge. The implementation is deliberately separate from the native PCRE2 runtime helpers: eval supports common modifiers and capture/replacement/split array construction through `RuntimeValueOps`, while unsupported PCRE syntax or offset-capture result shapes surface as eval runtime fatals instead of falling back to a partial native ABI.
