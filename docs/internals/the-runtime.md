...
| `__rt_phar_write_open` / `__rt_phar_write_append` / `__rt_phar_write_finalize` / `__rt_file_put_contents_maybe_phar` | Buffer one `phar://` write entry, then finalize it through the `elephc-phar` bridge so native archives preserve existing entries; runtime-built `file_put_contents()` URLs call a bridge variant that splits the full `phar://` URL; the assembly fallback still emits a single-entry SHA1-signed native archive |
