...
### `__rt_ftoa_coerce` — Float to string, as a string coercion

**File:** `arrays/nan_bool_coercion_warning.rs`

The entry every float-to-string COERCION calls instead of `__rt_ftoa`: `echo`, `(string)`,
interpolation, concatenation, a string parameter or builtin argument, `print_r()` and the
boxed-Mixed string cast. On the PHP 8.5 profile a NAN first raises `Warning: unexpected NAN
value was coerced to string` through the clobber-free `__rt_warn_nan_coerced_string`; every
value then tail-jumps into `__rt_ftoa`. Formatting that is not a coercion (a comparison
against a non-numeric string, a `debug_print_backtrace()` argument) keeps calling `__rt_ftoa`,
because php-src does not warn there. On older profiles the entry is a bare jump.
