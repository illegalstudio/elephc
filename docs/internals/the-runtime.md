...
Eval `iterator_count()` and `iterator_to_array()` currently use the `RuntimeValueOps` array iteration hooks for array inputs. `iterator_to_array()` copies keys and values in iteration order by default, and delegates to the indexed `array_values()` projection when `preserve_keys` is false; Traversable object dispatch for eval `iterator_apply()` remains future work.
