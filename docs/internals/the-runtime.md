...
Eval `sort()` and `rsort()` reuse the same direct-variable write-back path. The interpreter snapshots array values through `RuntimeValueOps`, builds homogeneous numeric or byte-string sort keys, reconstructs an indexed replacement array, and returns `true`; dynamic callable dispatch validates the array and reports PHP's by-reference warning without mutating the original caller value.
