...
| `__rt_float_compare_slots` / `__rt_float_compare_slots_desc` | Comparator `sort()`/`rsort()` hand to `__rt_usort` for an `array<float>`. A float slot holds the VALUE rather than a pointer, so neither the integer helper (which would order the bit patterns, inverting the negatives) nor the string one can order it. The comparator decides nothing itself: it tags both payloads as floats and defers to `__rt_php_compare`, the table `<` and `<=>` use, so `NAN` and `-0.0` cannot disagree between them. The descending twin negates that answer rather than swapping the operands, because swapping would reverse EQUAL elements too |
