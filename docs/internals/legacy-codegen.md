---
title: "Legacy Codegen"
description: "Historical notes for the removed direct AST to assembly backend."
sidebar:
  order: 7
---

**Status:** historical. The direct AST to assembly backend is no longer a
selectable compiler backend. Current codegen is documented in
[The Code Generator](the-codegen.md).

**Historical source:** before removal, the active EIR path lived under
`src/codegen_ir/` and the direct AST emitter lived under `src/codegen/`.

This page preserves the old direct AST emitter notes for archaeology and for
understanding helper code that was later moved into `src/codegen_support/`.
Most `src/codegen/...` paths below are historical paths from before
`src/codegen/` became the EIR backend.

During the transition, elephc supported more than one backend. AArch64 was the
clearest reference path in the old direct emitter and in this document, while
Linux `x86_64` went through the same high-level lowering pipeline.
...
In the old transition-era default path, `src/ir_lower/` walked the checked optimized AST, produced validated EIR, and `src/codegen_ir/` emitted assembly for each EIR function, instruction, and terminator. The CLI's main output was the **user program assembly**; the shared runtime helpers were usually assembled separately and reused from the runtime object cache. The user-facing `.s` file had this structure:
...
Before removal, the legacy library-style `codegen::generate(...)` entry point exposed both pieces explicitly as `(user_asm, runtime_asm)`. The normal CLI path lowered to EIR, called the EIR backend's `generate_user_asm_from_ir_with_options(...)`, and linked against the runtime-object cache so repeated compiles did not have to reassemble the same shared runtime text every time.
...
The EIR-era codegen/runtime handoff split output into three distinct artifacts:
...
The source-map file was intentionally simple. It stored a list of `(asm_line, php_line, php_col)` entries so tools and humans could correlate generated assembly back to the original PHP statements without needing full DWARF debug info.

The AST optimizer intentionally ran before backend selection. By the time the EIR backend ran, constant expressions and some dead control-flow had already been removed, and EIR added the function-wide value and control-flow shape that the linear-scan register allocator (`src/ir_passes/`, see [The IR](the-ir.md)) and IR optimizations relied on. The legacy AST backend saw the same optimized AST when `--ast-backend` was still selectable.
...
The frozen `--ast-backend` path used `emit_expr()` to take an expression node and emit code that left the result in the standard registers. The EIR backend reached the same ABI/runtime helpers through EIR lowering in `src/ir_lower/` and instruction lowering in the historical `src/codegen_ir/` tree. The top-level legacy `expr.rs` file mainly dispatched into focused helpers under `expr/` such as `scalars.rs`, `variables.rs`, `binops/`, `arrays.rs`, `compare/`, `calls/`, and `objects/`.
...
`src/codegen/reflection.rs` owns the shared materialization path. It allocates each synthetic `ReflectionAttribute`, writes the resolved `__name`, builds the `array<mixed>` `__args` payload from supported literal attribute arguments, and stores a deterministic `__factory` id. In the removed direct AST backend, `ReflectionAttribute::newInstance()` was generated by the class-method emitter as a branch table over those factory ids; each branch constructed the real attribute class with the captured literal args, and the fallback returned `null` when no defined attribute class could be materialized.
