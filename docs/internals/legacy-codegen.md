...
The legacy library-style `codegen::generate(...)` entry point exposed both pieces explicitly as `(user_asm, runtime_asm)`. The normal CLI path lowered to EIR, called `codegen_ir::generate_user_asm_from_ir_with_options(...)`, and linked against the runtime-object cache so repeated compiles did not have to reassemble the same shared runtime text every time.
