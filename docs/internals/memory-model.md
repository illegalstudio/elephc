...
- **Variable reassignment**: when a heap-backed local/global/static slot is overwritten, codegen releases the previous owner through the appropriate runtime path (`__rt_heap_free_safe` for persisted strings, `__rt_decref_*` for refcounted arrays / hashes / objects). When the new value *borrows* its storage from a function that returns one of its own never-rebound by-value parameters (e.g. `$x = f($x)` where `function f($s) { return $s; }`), the call result is lowered as `Borrowed`: the new value is retained/copied **before** the previous owner is released — so the aliased block is not read after free — and the borrowed result is never separately freed, avoiding a double-free of the same block
- **Stringified `mixed` temporaries**: coercing a `mixed`/union value to string (concatenation, interpolation, or an explicit `(string)` cast) persists a fresh owned copy through `__rt_mixed_cast_string`. That copy is an owning temporary and is released once the consuming concat or store completes — unlike string/array casts, which alias their source, and int/float casts, which write through scratch storage and own nothing
