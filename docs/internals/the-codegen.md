...
For the in-progress `linux-x86_64` backend, this write path is now the first real end-to-end vertical slice: string results are written with Linux SysV syscall registers (`rsi`/`rdx`/`edi`/`eax`), integer echo goes through the x86_64 `__rt_itoa` routine, float echo goes through the x86_64 `__rt_ftoa` routine backed by `snprintf`, and `_main` only initializes `$argc` / `$argv` when the program actually references them. The current minimal x86_64 runtime also emits `__rt_build_argv`, which materializes a tiny argv array header with `malloc` plus borrowed `ptr+len` entries so `count($argv)` works before the full heap/array runtime is ported. The user-data emitter also filters class metadata down to the classes emitted for that target slice, so built-in class tables do not pull in unported method bodies while trivial x86_64 programs are being brought up.
