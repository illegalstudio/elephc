...
After checking, an exports scan (`src/exports.rs`) collects every top-level function marked `#[Export]`, validates its signature against the C-ABI marshaling rules, and assigns a collision-checked C identifier (`Demo\add` becomes `Demo_add`). The post-lowering call-graph check covers every export and traverses free functions, fixed constructors/destructors, all include variants, methods, closures, and statically resolved runtime callbacks. It rejects reachable fatal terminators, process-fatal builtin subsets, `exit`/`die`, `eval`, dynamic construction, foreign calls, and other opaque invocation paths that cannot prove process termination unreachable. The check also runs for exported code under `--check` and `--emit-ir`; in a normal executable build the attributes are reported with a warning and ignored. See [Shared Libraries](../beyond-php/cdylib.md).
