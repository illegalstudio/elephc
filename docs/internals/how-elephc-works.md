...
- preserve unrelated scalar locals across simple loops when the loop's local writes are conservatively known, including simple nested `switch`, `try/catch/finally`, and `foreach` shapes, and keep stable scalar values introduced by `for` init clauses
