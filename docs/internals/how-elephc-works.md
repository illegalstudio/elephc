...
- preserve unrelated scalar locals across simple loops when the loop's local writes are conservatively known, including simple nested `switch`, `try/catch/finally`, `foreach`, other simple nested loop shapes, and local array writes such as `$items[] = $i` / `$items[0] = $i`, while also keeping stable scalar values introduced by `for` init clauses
