...
`src/types/fibers.rs` owns the additional static checks for Fiber callbacks. `new Fiber(...)` must receive a closure, a known closure/callable variable, or a known first-class callable; arbitrary runtime callables are rejected. The visible callback parameter count is capped at seven start arguments, and by-reference callback start parameters are rejected. Closure captures and first-class-callable receiver environments live in callable descriptor capture slots rather than in Fiber's visible start-argument area. Values moving through `start()`, `resume()`, `suspend()`, and `getReturn()` are typed as boxed `mixed`.
