...
- Emits the typed EIR target `runtime.__elephc_curl_setopt_unsupported_warning` through `BuiltinLoweringContext`.
...
- **Typed EIR target**: `runtime.__elephc_curl_setopt_unsupported_warning`
- **Backend boundary**: `src/codegen/lower_inst/runtime_calls.rs` resolves the typed target without PHP-name dispatch.

## Signature summary

```php
function __elephc_curl_setopt_unsupported_warning(int $option): void
```
