...
- Emits the typed EIR target `runtime.__elephc_curl_easy_setopt_long` through `BuiltinLoweringContext`.
...
- **Typed EIR target**: `runtime.__elephc_curl_easy_setopt_long`
- **Backend boundary**: `src/codegen/lower_inst/runtime_calls.rs` resolves the typed target without PHP-name dispatch.

## Signature summary

```php
function __elephc_curl_easy_setopt_long(mixed $handle, int $option, int $value): bool
```
