...
- Emits the typed EIR target `runtime.__elephc_var_dump_object_property_count` through `BuiltinLoweringContext`.
...
- **Typed EIR target**: `runtime.__elephc_var_dump_object_property_count`
- **Backend boundary**: `src/codegen/lower_inst/runtime_calls.rs` resolves the typed target without PHP-name dispatch.

## Signature summary

```php
function __elephc_var_dump_object_property_count(mixed $object): int
```
