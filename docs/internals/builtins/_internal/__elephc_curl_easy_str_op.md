...
- Emits the typed EIR target `runtime.__elephc_curl_easy_str_op` through `BuiltinLoweringContext`.
...
- **Typed EIR target**: `runtime.__elephc_curl_easy_str_op`
- **Backend boundary**: `src/codegen/lower_inst/runtime_calls.rs` resolves the typed target without PHP-name dispatch.

## Signature summary

```php
function __elephc_curl_easy_str_op(mixed $handle, int $op, string $text, int $number): mixed
```
