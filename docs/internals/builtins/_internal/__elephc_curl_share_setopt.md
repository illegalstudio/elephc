...
- Emits the typed EIR target `runtime.__elephc_curl_share_setopt` through `BuiltinLoweringContext`.
...
- **Typed EIR target**: `runtime.__elephc_curl_share_setopt`
- **Backend boundary**: `src/codegen/lower_inst/runtime_calls.rs` resolves the typed target without PHP-name dispatch.

## Signature summary

```php
function __elephc_curl_share_setopt(mixed $share, int $option, int $value): int
```
