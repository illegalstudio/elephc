...
- A `null` variable passed to a **non-nullable by-reference parameter** is rejected at compile time (`must be of type int, null given`). That is php-src's own rule for a *userland* `function f(int &$x)`, which throws a `TypeError`, and it applies here to elephc-PHP builtins written with a by-reference parameter too. The one place it bites in practice is `curl_multi_exec($mh, $still_running)`: php.net's examples often seed `$still_running = null`, which php-src accepts only because `curl_multi_exec()` is an *internal* function, where the coercion happens on the way in. Seed it with an `int` — `$still_running = 1;` — which is valid in both. The same applies to any other elephc builtin declared `int &$out`, and to your own by-reference parameters.
...
- String offset writes (`$s[$i] = $v`) follow PHP for string variables, with these gaps (see [Strings](./strings.md#writing-to-a-string-offset)): a write through a property (`$o->s[0] = "x"`, `C::$s[0] = "x"`) or an array element (`$a["k"][0] = "x"`) is rejected at compile time; the offset must be an integer, a float, or a numeric string literal (PHP also accepts `bool`, `null`, and runtime strings, warning or throwing at run time); compound assignment, `++`/`--`, `$s[] = $v`, and `$s[0][0] = $v`, which always throw `Error` in PHP, are compile errors instead; and when an illegal negative offset writes nothing, the expression `($s[-9] = "x")` evaluates to `""` where PHP gives `null`. On a string held in `mixed` storage, the offset goes through array-key normalization, so a non-numeric string key throws `TypeError: Cannot access offset of type string on string` as in PHP, and a leading-numeric one (`"1x"`) throws the same `TypeError` where PHP warns `Illegal string offset "1x"` and writes at its numeric prefix.
