...
- `array_push()` onto an **associative** receiver requires each pushed value to fit the receiver's existing entry type: pushing a `string` onto an `array<string, int>` is a compile error, where PHP accepts it. `$arr[] = $v` has no such limit, because the checker widens that receiver's entry type before lowering; `array_push()`'s receiver is a by-reference builtin argument, which keeps the type it was declared with. Append with `$arr[] = $v` when the value type differs.
