...
- `min()`/`max()` with a boxed `mixed` operand (e.g. an element read from a heterogeneous array) coerce that operand to a number for the comparison and return a `float`-typed result, even when the selected value is an integer. The displayed value matches PHP, but a strict `=== <int>` against the result can differ; cast or compare loosely if needed.
- `NAN` ordering differs from PHP. The spaceship operator yields `0` when either operand is `NAN` (`$x <=> NAN` and `NAN <=> $x` both give `0`), whereas PHP yields `1`. `min()`/`max()` propagate `NAN` whenever any argument is `NAN`, while PHP's result is argument-order dependent. `NAN` is unordered, so do not rely on a defined comparison result for it; use `is_nan()` to detect it explicitly. Equality and the strict/loose comparison operators against `NAN` are still always false, matching PHP.
