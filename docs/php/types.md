...
- Appending a reference (`$a[] = &$x;`, `$a['k'][] = &$x;`, or `($a[] = &$x)`) is rejected at compile time for the same reason: in PHP the new element is a reference cell aliasing `$x`, and elephc's array elements cannot alias a variable's storage. Append the value instead, or alias an existing element with `$b =& $a[0]`.
