...
`call_user_func()` and `call_user_func_array()` also accept PHP callable arrays (`[$object, "method"]`, `[ClassName::class, "method"]`) and invokable objects. Dynamic string callback dispatch resolves user functions, supported builtin wrappers such as `STRLEN`, and public static method strings such as `"Formatter::wrap"`; `call_user_func_array()` applies associative arrays as named arguments against the resolved descriptor signature.
