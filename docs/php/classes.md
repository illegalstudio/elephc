...
- Attribute arguments materialized by reflection today include: string, int, float, bool, null, negation (`-N`) of a numeric literal, arrays (positional and associative, nested, with heterogeneous element types), **named arguments**, and **symbolic constant references** — a global constant (`#[A(SOME_CONST)]`) or a class/interface constant (`#[A(C::BAR)]`). `ReflectionClass`/`ReflectionMethod`/`ReflectionProperty::getAttributes()` → `getArguments()` returns named arguments under their string keys and positional arguments under their integer keys, matching PHP, with each constant resolved to its value, and `newInstance()` constructs the attribute with those arguments.
- A symbolic reference that elephc cannot resolve to a user constant — for example a built-in class constant such as `Attribute::TARGET_CLASS`, which is not registered — is treated as unsupported metadata: the attribute still parses and compiles and `class_attribute_names()` still lists it, but its arguments are not reflectable through `getAttributes()`/`class_get_attributes()`/`class_attribute_args()`. An **enum-case argument** (`#[A(E::Case)]`) is handled the same way for now — it compiles and the attribute name is reflectable, but materializing the enum case object through reflection is a planned follow-up.
