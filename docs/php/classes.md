...
- Attribute arguments materialized by reflection today include: string, int, float, bool, null, negation (`-N`) of a numeric literal, arrays (positional and associative, nested, with heterogeneous element types), and **named arguments**. `ReflectionClass`/`ReflectionMethod`/`ReflectionProperty::getAttributes()` → `getArguments()` returns named arguments under their string keys and positional arguments under their integer keys, matching PHP, and `newInstance()` constructs the attribute with those arguments. Constant and enum references in attribute arguments are not yet materialized; an attribute that uses them is parsed and compiled, `class_attribute_names()` still lists the attribute name, but `getAttributes()`/`class_get_attributes()`/`class_attribute_args()` report an error rather than dropping the metadata.
- The flat `class_attribute_args()` helper returns a positional array only; it rejects attributes whose arguments are keyed (named arguments or associative arrays, at any depth). Use `ReflectionClass::getAttributes()->getArguments()` for those.
