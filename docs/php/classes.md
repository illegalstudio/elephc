...
- `ReflectionMethod::invoke()` and `invokeArgs()` are supported for inline or straight-line tracked generated/AOT reflectors when the reflected method has declared parameter types. The lowered path supports instance and static methods, method-name case-insensitivity, defaults, named arguments, and static `invokeArgs()` argument arrays. Invoke targets whose parameter types only come from call-site inference and runtime-only argument arrays still require richer runtime/typechecker support.
