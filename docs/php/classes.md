...
- `ReflectionClass` supports `getName()`, `getAttributes()`, `isFinal()`, and `isAbstract()`. `ReflectionMethod` and `ReflectionProperty` currently support `getAttributes()` only; broader APIs such as `getProperties()`, `getMethods()`, and object construction through `ReflectionClass::newInstance()` are not yet available.
