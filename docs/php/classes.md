...
| `ReflectionParameter::getAttributes()` | `ReflectionAttribute[]` | The reflected method parameter's attributes |
...
and `isCallable()` are supported for retained parameter metadata.
...
union declarations report `false`.
