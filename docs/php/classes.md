...
- **Literal** positional arguments are materialized by reflection helpers today: string, int, float, bool, null, negation (`-N`) of a numeric literal, and positional arrays of those (including nested arrays and heterogeneous element types). Other legal PHP attribute arguments (named arguments, associative arrays, and constant/enum references) can still be parsed and compiled, and `class_attribute_names()` can still list the attribute name, but `class_attribute_args()`, `class_get_attributes()`, and Reflection `getAttributes()` report an error if they would need that not-yet-supported argument metadata.
