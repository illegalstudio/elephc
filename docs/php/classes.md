...
| `ReflectionUnionType::getTypes()` / `allowsNull()` / `__toString()` | `ReflectionParameter::getType()` | Return non-null union members as `ReflectionNamedType` objects, report whether `null` was part of the union, and stringify retained union metadata. Members print in PHP's order, not the declared one — class names first (in declared order), then `static`, `callable`, `object`, `array`, `iterable`, `string`, `int`, `float`, `bool`, `false`, `true`, `null` — so `int\|string` renders as `string\|int`, matching what `ReflectionMethod::__toString()` prints for the same signature |
...
| `ReflectionMethod::__toString()` | `new ReflectionMethod($class_name, $method_name)` or `ReflectionClass::getMethods()` | Format the method as PHP's `Method [ ... ] { ... }` dump, with its modifiers, prototype, parameters and return type |
| `ReflectionFunction::__toString()` | `new ReflectionFunction($function_name)` | Format the function as PHP's `Function [ ... ] { ... }` dump |
| `ReflectionParameter::__toString()` | Same construction forms as `ReflectionParameter::hasType()` | Format the parameter as PHP's `Parameter #0 [ <required> int $a ]` line |
...
| `ReflectionClassConstant::hasType()` | Same as `ReflectionClassConstant::getName()` | Return whether the reflected class constant has a declared PHP 8.3 type |
| `ReflectionClassConstant::getType()` | Same as `ReflectionClassConstant::getName()` | Return a retained `ReflectionNamedType`, `ReflectionUnionType`, or `ReflectionIntersectionType`, or `null` for an untyped constant |
...
- The `__toString()` dumps follow PHP's format with one line removed. PHP prints `  @@ <file> <line> - <line>` under the header, and a compiled binary cannot honestly answer it: the source it was built from need not exist where the binary runs, and baking the build machine's path in would be worse than leaving it out. The blank line that follows it stays, because PHP prints it for internal callables too, which have no such line. An internal callable names its module the way PHP registers it: `<internal:Core>`, `<internal:SPL>`, `<internal:standard>`. The eval bridge omits the same line. It still differs from the compiled path in two places where the compiled path matches PHP and it does not — it omits the prototype marker, and prints an empty parameters block where PHP prints an empty body. Union member order is no longer one of them: both paths now sort with the same PHP rank table (#1118). Everything else matches, including the `<user, prototype Interface>` marker on an override, the modifier order, `<required>` / `<optional>`, by-reference and variadic markers, array and constant defaults, and the empty body PHP prints for a callable with no parameters and no return type. The eval bridge also labels every callable `<user>`: an internal method reflected inside `eval()` prints `Method [ <user> public method count ]` where the compiled path prints `<internal:SPL, prototype Countable>`, and `isInternal()` answers false there (#1117). An object default prints its written arguments the way PHP exports them — `new \Foo(Foo::VALUE, LIMIT)` keeps both names, a named argument keeps its name, and an array made only of literals prints every key. Two spellings still differ: an internal constant written unqualified inside a namespace prints its value where PHP prints `N\PHP_INT_MAX`, and a global class written `\Foo::VALUE` prints without its leading backslash:
...
abstract class Base implements Shape {
    ...
    public function mixup(int|string $v, int &$out = 0, ...$rest): void {}
...
echo (new ReflectionMethod('Base', 'none'));
// Method [ <user> public method none ] {
// }

echo (new ReflectionMethod('Base', 'mixup'))->getParameters()[0];
// Parameter #0 [ <required> string|int $v ]
```

- A union type prints in PHP's order, not the declared one. PHP renders a union from its type mask, which is fixed: class names first in the order they were written, then `static`, `callable`, `object`, `array`, `iterable`, `string`, `int`, `float`, `bool`, `false`, `true`, `null`. So `int|string` comes back as `string|int` while `int|float` stays put — measured on 8.5.10.
