...
| `sscanf()` | `sscanf($str, $fmt): array\|null` | Parse a string with php's scanner, which is implemented in full: widths (`%5s`), `*` suppression (`%*d`), `%[...]` character classes with ranges and negation, the conversions `%c`, `%d`, `%D`, `%e`, `%E`, `%f`, `%g`, `%i`, `%n`, `%o`, `%s`, `%u`, `%x`, `%X`, the `l`/`h`/`L` length modifiers, and php's own `ValueError` wording for a format it refuses. Fields come back in php's TYPES — `%d` yields an `int` and `%f` a `float`, not the matched text — a conversion that did not match is `null`, and the array carries one entry per non-suppressed conversion in the format regardless of where scanning stopped. The result is `null` rather than an array when the scan reaches end of input before assigning anything, so `sscanf('', '%d')` is `null` while `sscanf('abc', '%d')` is `[null]`. Only the two-argument array-returning form is supported: php's by-ref `...$vars` form, which assigns each field through the reference and returns the field count instead, is refused at compile time. |
