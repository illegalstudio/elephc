...
| `mb_strimwidth()` | `mb_strimwidth($string, $start, $width, $trim_marker = "", $encoding = null): string` | Truncates a string to a display width, optionally appending `$trim_marker`. An omitted or `null` encoding uses UTF-8 East Asian Width (PHP 8.5); `8bit`/`binary`/`7bit` treat every byte as width 1. `$start` is a character offset (negative counts from the end). An unknown encoding or out-of-range `$start`/`$width` throws `\ValueError` |
