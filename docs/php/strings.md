...
| `mb_strtoupper()` | `mb_strtoupper($str, $encoding = null): string` | Unicode full uppercase (`ß` → `SS`). An omitted or `null` encoding uses UTF-8 and copies malformed bytes through unchanged; `8bit`/`binary`/`7bit` apply ASCII-only `a-z` folding; other encodings decode through system `iconv`. An unknown encoding name throws `\ValueError` |
