...
| `mb_strwidth()` | `mb_strwidth($str, $encoding = null): int` | Terminal display width in the given encoding using PHP 8.5 East Asian Width (halfwidth 1, fullwidth/wide 2). Encoding selection matches `mb_strlen()`; unknown encoding names throw `\ValueError` |
