...
| `mb_strtolower()` | `mb_strtolower($str, $encoding = null): string` | PHP 8.5 Unicode lowercase. An omitted or `null` encoding, plus `UTF-8`/`UTF8`, apply full lowercase (including Final_Sigma and 1:N maps such as `İ` → `i` + combining dot). `8bit`/`binary`/`7bit` lowercase ASCII `A-Z` per byte. An unknown encoding name throws `\ValueError` |
