...
the negative `function_exists()` branch. Guards accept string literals, string
constants, and variables with a statically known string value, including string
...
The compiler resolves the guard before type checking and does not reuse a
variable's old value across writes or reference aliases. Builtins available on
the selected target remain protected from redeclaration.
