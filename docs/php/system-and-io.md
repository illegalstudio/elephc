...
| Function | Signature | Description |
|---|---|---|
| `file_get_contents()` | `file_get_contents($filename): string\|false` | Read an entire file, or `false` if it cannot be opened. A literal `phar://` URL is decoded at compile time; a literal `http://`, `https://`, `ftp://`, or `ftps://` URL opens the wrapper, reads the whole body, and returns it (`false` on a failed open) — the same wrappers as `fopen()`. |
