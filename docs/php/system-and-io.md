...
Eval `json_encode()` supports zero-flags encoding for null, booleans, integers, floats, strings, indexed arrays, and associative arrays. The eval encoder uses default slash and control-character escaping and rejects non-zero flags until the full native JSON flag surface is bridged.
