...
Eval builtin dispatch also supports byte-string helpers such as `strrev()`, `bin2hex()`, and `base64_encode()`, plus floating-point math calls `fdiv()` and `fmod()`, including `fdiv()` zero handling through `INF` and `NAN` results.
