...
- The Linux x86_64 target's runtime is provided by a separate "minimal" emitter that mostly tracks macOS ARM64 now: the JSON helper registration, rax preservation across `__rt_json_depth_enter`, the PRETTY_PRINT post-processor, the transitive parent-class closure for the user-asm filter, and the always-emit builtin throwable hierarchy land in v0.20.x. The Linux x86_64 JSON suite passes 283/285 (99%); the remaining 2 failures are JsonSerializable-inside-array dispatches where the array's runtime value_type tag is not updated to 6 (object) on the typed-Array codegen path. Tracked separately.
