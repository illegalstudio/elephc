...
Eval `crc32()` computes the same non-negative CRC-32 integer as the static runtime helper. Eval `hash_algos()` returns the same indexed supported-algorithm name list as the native runtime helper.
...
Eval `gethostbyname()` resolves IPv4 host names through the host resolver and returns the original string when no IPv4 address is available.
...
Eval path component calls include `basename()`, `dirname()`, and `pathinfo()` with suffix trimming, repeated parent traversal, `PATHINFO_*` constants, named arguments, callable dispatch, and `function_exists()` probes.
