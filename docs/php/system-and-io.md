...
Eval builtin dispatch also supports byte-string helpers such as `strrev()` and `bin2hex()`, plus floating-point math calls `fdiv()` and `fmod()`, including `fdiv()` zero handling through `INF` and `NAN` results.
