...
Inside eval, `define()` stores dynamic constants that persist across later eval fragments, `defined()` probes them, and bare constant expressions fetch their retained boxed values. Duplicate eval `define()` calls return `false`; duplicate-constant warning emission from the eval bridge is still future work.
