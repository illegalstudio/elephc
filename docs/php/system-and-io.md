...
Eval `array_pop()`, `array_shift()`, `array_push()`, and `array_unshift()` support direct variable calls with write-back into the eval scope. `array_pop()` and `array_shift()` also accept named direct `array:` calls. When these by-reference builtins are reached through `call_user_func()` or `call_user_func_array()`, they follow PHP's by-value callback behavior for non-reference arguments: the return value is computed from the supplied array, but the caller's original array is not mutated.
