...
| `spl_classes()` | Returns the indexed array of SPL/core class and interface names shipped by the compiler today (currently 32 entries: SPL/core interfaces, throwable types, SPL exception classes, and the Phase 4 container metadata shells `SplDoublyLinkedList`, `SplStack`, `SplQueue`, `SplFixedArray`). The list grows as later phases ship more SPL types |
