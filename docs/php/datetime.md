...
- Format literals are byte-preserving: `date()`, `gmdate()`, and `DateTime::format()` retain embedded NUL and non-UTF-8 literal bytes; use `bin2hex()` when inspecting such results.
