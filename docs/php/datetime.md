...
The supported format characters are `Y y m n d j D l S F M z H G h g i s u v A a U O P Z T e`, plus the metas `! | # ? * + \`. In detail:
...
Available: `ATOM`, `COOKIE`, `ISO8601`, `ISO8601_EXPANDED`, `RFC822`, `RFC850`, `RFC1036`, `RFC1123`, `RFC7231`, `RFC2822`, `RFC3339`, `RFC3339_EXTENDED`, `RSS`, `W3C`. Note that `ISO8601_EXPANDED` contains the expanded-year `X` format specifier, which `format()` does not yet render specially.
...
| `f` | Fraction of a second (float). Always `0.0` — elephc works at second resolution. |
