...
| `array_map()` | `array_map($callback, $arr, ...$arrays): array` | Apply callback to each element. With two integer arrays it zips them element-wise (`array_map($cb, $a, $b)`), result length `max(len)`, padding the shorter with 0; the callback may be a named function or a closure (capturing closures included). |
