...
| `array_chunk()` | `array_chunk($arr, $size [, $preserve_keys]): array` | Split into chunks. A `$size` of `0` or less throws `\ValueError`. `$preserve_keys` must be a literal `true`/`false`; when `true` each chunk keeps its own window's source keys, and when `false` every key is dropped — **string keys included**, unlike `array_slice()` — and each chunk renumbers from `0`. Accepts an associative receiver in both modes, chunking it into associative chunks. |
