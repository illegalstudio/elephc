...
| `array_push()` | `array_push($arr, ...$values): int` | Append any number of elements and return the new count. `array_push($a, 3, 4)` appends both in source order, and `array_push($a)` with no values just reads the count back. An ASSOCIATIVE receiver is accepted and appends at PHP's next automatic integer key — `max(int keys) + 1`, or `0` when it has none — exactly as `$arr[] = $v` does. The pushed value has to fit the receiver's existing entry type: unlike `$arr[] = $v`, a by-reference builtin argument keeps the type it was declared with, so pushing a `string` onto an `array<string, int>` is a compile error rather than a widening. |
...
| `array_chunk()` | `array_chunk($arr, $size [, $preserve_keys]): array` | Split arrays, including associative arrays, into chunks. A `$size` of `0` or less throws a `\ValueError`. `$preserve_keys` must be a literal `true`/`false`; when `true` each chunk keeps its source keys, and when `false` every key is dropped — **string keys included**, unlike `array_slice()` — and each chunk renumbers from `0`. |
