...
names, and static-property arguments including dynamic receivers and dynamic
property names, write back fixed parameters after method execution, write back
mutated `&...$items` elements when the variadic container itself is not
rebound, and are reported through
