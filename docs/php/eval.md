...
| Constants | Predefined eval-visible constants, dynamic constants from `define()`, namespaced constant fallback, bare constant fetches, `$class::CONST`, braced dynamic class constant names (`ClassName::{$constant}` / `$class::{$constant}`), and `$object::class` are supported. |
