...
**An object read out of Mixed storage cannot be passed to a typed object parameter.** An object pulled out of an array, an array property, or any other `mixed`-typed storage reaches a parameter declared with its class as the boxed value rather than the object, so property reads inside the callee return wrong values (`$obj->prop` comes back `null`). Object identity (`===`) is unaffected, and the checker rejects the unguarded form, so the miscompile is only reachable through an `instanceof` narrowing. Functions that must accept such a value declare `mixed` plus a runtime `instanceof` guard instead — the curl multi surface (`curl_multi_add_handle()`, `curl_multi_remove_handle()`, `curl_multi_getcontent()`) does exactly that, because `curl_multi_info_read()` hands its handle back inside an array. Pinned by `test_mixed_sourced_object_through_typed_param_is_miscompiled`.
