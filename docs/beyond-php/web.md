...
- Session files are stored in `sys_get_temp_dir()` by default. Use
  `session_save_path($path)` to configure a custom directory.
...
- `session_set_save_handler()` is not supported — only the file-based save
  handler is provided.
