...
fn method_builder(method_decl: &ClassMethod, depth: usize, has_body: bool) -> String {
    ...
    if let Some(tail) = &method_decl.variadic {
        out.push_str(&variadic_call(
            tail,
            method_decl.variadic_by_ref,
            &method_decl.variadic_type,
            depth,
        ));
    } else {
        assert!(!method_decl.variadic_by_ref, "a by-ref flag without a variadic tail");
    }
    if let Some(ty) = &method_decl.return_type {
        out.push_str(&format!("\n{}.returns({})", pad(depth), ty_expr(ty)));
    }
    if has_body {
        if !unread_params(&method_decl.params, &method_decl.body).is_empty() {
            out.push_str(&format!("\n{}.keep_unread_params()", pad(depth)));
        }
