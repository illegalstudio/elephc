...
/// bound descriptor together with the direct-call binding. `None` unless the call has the
/// auto-captured `$this` shape, optionally followed by the compiler's hidden called-class capture.
...
/// compiled function. Its first capture is the receiver boxed as `Mixed`, the identical value the
/// direct call passes as the closure's hidden `$this` argument, followed by any hidden called-class
/// capture. The descriptor owns the receiver box and the call borrows it, so there is no second
/// receiver reference. The box, and with it the receiver, is freed exactly when the descriptor
/// retires, which makes the bound receiver's destructor run at the PHP-observable point.
///
