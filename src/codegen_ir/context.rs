...
impl<'a> FunctionContext<'a> {
    ...
    /// Stores the current result register(s) into a local slot or through its ref-cell pointer.
    ...
    fn store_current_result_to_ref_cell_local(&mut self, slot: LocalSlotId, target_ty: &PhpType) -> Result<()> {
        reject_multiword_ref_cell_local(target_ty, "store")?;
        let offset = self.local_offset(slot)?;
        let pointer_reg = abi::symbol_scratch_reg(self.emitter);
        abi::load_at_offset(self.emitter, pointer_reg, offset);
