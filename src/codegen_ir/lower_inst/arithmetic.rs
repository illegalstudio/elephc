...
/// Lowers a signed integer modulo operation, raising an uncatchable fatal when the divisor is zero.
///
/// PHP raises a catchable `DivisionByZeroError` on `% 0`; elephc has no exception unwinding, so a
/// zero divisor branches to a `Fatal error: modulo by zero` diagnostic and exits.
