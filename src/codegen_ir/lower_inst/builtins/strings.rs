...
/// Lowers `htmlspecialchars()` / `htmlentities()` — escapes the subject string (operand 0). The
...
pub(crate) fn lower_html_escape(ctx: &mut FunctionContext<'_>, inst: &Instruction) -> Result<()> {
    let ptr_reg = string_ptr_reg(ctx);
    let len_reg = string_len_reg(ctx);
    load_string_arg_to_regs(ctx, inst, 0, "htmlspecialchars", ptr_reg, len_reg)?;
