...
pub(super) fn lower_builtin_call(ctx: &mut FunctionContext<'_>, inst: &Instruction) -> Result<()> {
    ...
    match key.as_str() {
        ...
        "zval_pack" => pointers::lower_zval_pack(ctx, inst),
        "zval_unpack" => pointers::lower_zval_unpack(ctx, inst),
        "zval_type" => pointers::lower_zval_type(ctx, inst),
        "zval_free" => pointers::lower_zval_free(ctx, inst),
