//! Purpose:
//! Home of PHP's `xml_set_external_entity_ref_handler()`: installs the external entity reference handler, typing an unannotated
...
//!
//! Key details:
//! - Lowers to the xml prelude's `__elephc_xml_set_external_entity_ref_handler()` twin; see `super::handler_setters`.
...
pub(super) const SPEC: SetterSpec = SetterSpec {
    helper: "__elephc_xml_set_external_entity_ref_handler",
    handlers: &[handler_setters::EXTERNAL_ENTITY_REF],
};
...
builtin! {
    contract: "xml_set_external_entity_ref_handler",
    check: check,
    lazy_check: true,
