//! Purpose:
//! Home of PHP's `xml_set_processing_instruction_handler()`: installs the processing instruction handler, typing an unannotated
...
//!
//! Key details:
//! - Lowers to the xml prelude's `__elephc_xml_set_processing_instruction_handler()` twin; see `super::handler_setters`.
...
pub(super) const SPEC: SetterSpec = SetterSpec {
    helper: "__elephc_xml_set_processing_instruction_handler",
    handlers: &[handler_setters::PROCESSING_INSTRUCTION],
};
...
builtin! {
    contract: "xml_set_processing_instruction_handler",
    check: check,
    lazy_check: true,
