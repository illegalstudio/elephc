//! Purpose:
//! Home of PHP's `xml_set_element_handler()`: installs the start and end element handlers, typing an unannotated
...
//!
//! Key details:
//! - Lowers to the xml prelude's `__elephc_xml_set_element_handler()` twin; see `super::handler_setters`.
...
pub(super) const SPEC: SetterSpec = SetterSpec {
    helper: "__elephc_xml_set_element_handler",
    handlers: &[handler_setters::START_ELEMENT, handler_setters::PARSER_AND_STRING],
};

/// PHP parameter names, for named-argument binding in the checker hook.
const PARAMETERS: &[&str] = &["parser", "start_handler", "end_handler"];

builtin! {
    contract: "xml_set_element_handler",
    check: check,
    lazy_check: true,
