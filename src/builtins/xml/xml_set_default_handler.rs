//! Purpose:
//! Home of PHP's `xml_set_default_handler()`: installs the default handler, typing an unannotated
...
//!
//! Key details:
//! - Lowers to the xml prelude's `__elephc_xml_set_default_handler()` twin; see `super::handler_setters`.
...
pub(super) const SPEC: SetterSpec = SetterSpec {
    helper: "__elephc_xml_set_default_handler",
...
builtin! {
    contract: "xml_set_default_handler",
    check: check,
    lazy_check: true,
