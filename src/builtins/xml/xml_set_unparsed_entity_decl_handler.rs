//! Purpose:
//! Home of PHP's `xml_set_unparsed_entity_decl_handler()`: installs the unparsed entity declaration handler, typing an unannotated
...
//!
//! Key details:
//! - Lowers to the xml prelude's `__elephc_xml_set_unparsed_entity_decl_handler()` twin; see `super::handler_setters`.
...
pub(super) const SPEC: SetterSpec = SetterSpec {
    helper: "__elephc_xml_set_unparsed_entity_decl_handler",
    handlers: &[handler_setters::UNPARSED_ENTITY_DECL],
};
...
builtin! {
    contract: "xml_set_unparsed_entity_decl_handler",
    check: check,
    lazy_check: true,
