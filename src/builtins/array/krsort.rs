//! Purpose:
//! Home of the PHP `krsort` builtin: its declaration, type-check hook, and lowering.
//!
//! Called from:
...
//! - `lower` is a thin wrapper over the shared `arrays::lower_krsort` emitter.
...
/// Lowers a `krsort` call by dispatching to the shared array emitter.
fn lower(ctx: &mut FunctionContext, inst: &Instruction) -> Result<(), CodegenIrError> {
    crate::codegen_ir::lower_inst::builtins::arrays::lower_krsort(ctx, inst)
}
