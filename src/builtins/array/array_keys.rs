...
//! - A `Mixed` argument, or the exact packed-or-keyed union used by declared `array` properties,
//!   yields `Array<Mixed>` because the runtime key kind is known only once the box is opened.
//!   The backend unboxes and dispatches on the runtime tag, raising PHP's `TypeError` when a
//!   `Mixed` box does not hold an array.
