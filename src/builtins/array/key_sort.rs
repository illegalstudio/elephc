...
//!
//! Key details:
//! - Concrete arrays and exact PHP `array` declarations are accepted directly; boxed cells of
//!   supported array places defer runtime tag validation to shared nested key-sort lowering.
...
///
/// Concrete arrays and exact PHP `array` declarations are accepted statically. A boxed element
/// of a declared PHP array or heterogeneous concrete array place is also accepted because nested
/// lowering checks its runtime tag before mutation and raises the builtin-specific PHP `TypeError`
/// for invalid cells.
