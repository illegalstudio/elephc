...
//! - `check` accepts concrete indexed arrays, dynamic Mixed cells and declared PHP arrays.
//!   The backend validates that boxed values still use indexed storage.
...
///
/// Accepts concrete indexed arrays, dynamic Mixed cells or declared PHP arrays. Arity (exactly 2)
...
fn check(cx: &mut BuiltinCheckCtx) -> Result<PhpType, CompileError> {
    ...
    if (!matches!(&ty1, PhpType::Array(_) | PhpType::Mixed) && !ty1.is_php_array())
        || (!matches!(&ty2, PhpType::Array(_) | PhpType::Mixed) && !ty2.is_php_array())
    {
