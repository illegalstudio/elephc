//! Purpose:
//! Home of the PHP `urlencode` builtin: its declaration and lowering.
//!
//! Called from:
...
//!
//! Key details:
//! - No `check` hook is needed: `urlencode` is a pure-data builtin whose return
...
//!   passing the `__rt_urlencode` runtime helper.
...
/// Lowers a `urlencode` call by dispatching to the shared per-arch unary string runtime.
fn lower(
