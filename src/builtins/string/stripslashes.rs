//! Purpose:
//! Home of the PHP `stripslashes` builtin: its declaration and lowering.
//!
//! Called from:
...
//!
//! Key details:
//! - No `check` hook is needed: `stripslashes` is a pure-data builtin whose return
...
/// Lowers a `stripslashes` call by dispatching to the shared per-arch unary string runtime.
fn lower(
