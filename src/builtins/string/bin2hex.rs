//! Purpose:
//! Home of the PHP `bin2hex` builtin: its declaration and lowering.
//!
//! Called from:
...
//!
//! Key details:
//! - No `check` hook is needed: `bin2hex` is a pure-data builtin whose return
...
//!   passing the `__rt_bin2hex` runtime helper.
...
/// Lowers a `bin2hex` call by dispatching to the shared per-arch unary string runtime.
fn lower(
