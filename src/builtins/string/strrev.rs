//! Purpose:
//! Home of the PHP `strrev` builtin: its declaration and lowering.
//!
//! Called from:
...
//!
//! Key details:
//! - No `check` hook is needed: `strrev` is a pure-data builtin whose return
...
/// Lowers a `strrev` call by dispatching to the shared per-arch unary string runtime.
fn lower(
