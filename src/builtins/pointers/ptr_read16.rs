//! Purpose:
//! Home of the PHP `ptr_read16` builtin: its declaration, type-check hook, and lowering.
//!
//! Called from:
...
//! - `lower` is a thin wrapper over the shared `pointers::lower_ptr_read16` emitter.
...
/// Lowers a `ptr_read16` call by dispatching to the shared pointer emitter.
fn lower(ctx: &mut FunctionContext, inst: &Instruction) -> Result<(), CodegenIrError> {
    crate::codegen_ir::lower_inst::builtins::pointers::lower_ptr_read16(ctx, inst)
}
