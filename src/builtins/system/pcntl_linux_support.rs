...
//! - PHP's optional-looking `$cpu_ids` parameter must still be present and non-empty.
...
/// Requires a non-empty indexed integer mask while checking the optional process identifier.
pub(super) fn check_setcpuaffinity(cx: &mut BuiltinCheckCtx) -> Result<PhpType, CompileError> {
    ...
    if matches!(&cpu_ids.kind, ExprKind::ArrayLiteral(items) if items.is_empty()) {
        return Err(CompileError::new(
            cpu_ids.span,
            "pcntl_setcpuaffinity() parameter $cpu_ids must not be empty",
        ));
    ...
    match ty.codegen_repr() {
        PhpType::Array(element)
            if matches!(element.codegen_repr(), PhpType::Int | PhpType::Never) => {}
        other => {
            return Err(CompileError::new(
                cpu_ids.span,
                &format!(
