...
pub(crate) fn class_attribute_args_unsupported(
    ...
    class_info
        ...
        .is_some_and(|(idx, _)| match class_info.attribute_args.get(idx) {
            // The flat `class_attribute_args()` helper can materialize literal
            // positional, named, and array-keyed arguments. It cannot resolve
            // deferred symbolic references (global/class constants, enum cases)
            // on this echo path, so those stay rejected.
...
/// echo the captured entries: deferred symbolic references (global/class
/// constants, enum cases) are not materialized on this echo path. Literal keyed
/// arguments are supported and retain their keys.
