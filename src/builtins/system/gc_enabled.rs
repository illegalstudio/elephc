//! Purpose:
//! Registers PHP `gc_enabled()` with target-neutral EIR semantics.
//!
//! Called from:
//! - The builtin registry through `crate::builtins::system`.
//!
//! Key details:
//! - The returned boolean reflects the same runtime flag read by automatic safe points.
...
builtin! {
    ...
    semantics: BuiltinSemantics {
        ...
        effects: BuiltinEffects::Static(GcControlOp::Enabled.effects()),
...
/// Emits the automatic-collection enabled-state read.
fn lower(
    ...
    Ok(ctx.emit_value(
        Op::GcControl,
        Vec::new(),
        Some(Immediate::I64(GcControlOp::Enabled.as_i64())),
        call.result_type.clone(),
        GcControlOp::Enabled.effects(),
        Some(call.span),
    ))
}
