//! Purpose:
//! Home of the PHP `rewind` builtin: its declaration, type-check hook, and lowering.
//!
//! Called from:
...
//! - `lower` is a thin wrapper over `io::lower_rewind` in the EIR backend.
...
/// Lowers a `rewind` call by dispatching to the shared io emitter.
fn lower(ctx: &mut FunctionContext, inst: &Instruction) -> Result<(), CodegenIrError> {
    crate::codegen_ir::lower_inst::builtins::io::lower_rewind(ctx, inst)
}
