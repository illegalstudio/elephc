//! Purpose:
//! Home of the internal `__elephc_phar_get_signature_type` PHAR intrinsic: its declaration,
...
//!   call `infer_type`.
//! - `lower` is a thin wrapper over `io::lower_elephc_phar_get_signature_type` in the EIR backend.
...
fn lower(ctx: &mut FunctionContext, inst: &Instruction) -> Result<(), CodegenIrError> {
    crate::codegen_ir::lower_inst::builtins::io::lower_elephc_phar_get_signature_type(ctx, inst)
}
