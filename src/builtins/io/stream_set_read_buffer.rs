//! Purpose:
//! Home of the PHP `stream_set_read_buffer` builtin: its declaration and lowering.
//!
//! Called from:
...
//! - `lower` dispatches to `io::lower_stream_set_buffer`, shared with `stream_set_write_buffer`.
...
/// Lowers a `stream_set_read_buffer` call by dispatching to the shared io buffer emitter.
