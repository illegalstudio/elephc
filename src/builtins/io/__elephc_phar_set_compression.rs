//! Purpose:
//! Home of the internal `__elephc_phar_set_compression` PHAR intrinsic: its declaration,
...
//!   call `infer_type`.
//! - `lower` is a thin wrapper over `io::lower_elephc_phar_set_compression` in the EIR backend.
...
fn lower(ctx: &mut FunctionContext, inst: &Instruction) -> Result<(), CodegenIrError> {
    crate::codegen_ir::lower_inst::builtins::io::lower_elephc_phar_set_compression(ctx, inst)
}
