//! Purpose:
//! Home of the PHP `stream_get_filters` builtin: its declaration, type-check hook, and lowering.
//!
//! Called from:
...
//! - `lower` is a thin wrapper over `io::lower_stream_get_filters` in the EIR backend.
...
/// Lowers a `stream_get_filters` call by dispatching to the shared io emitter.
fn lower(ctx: &mut FunctionContext, inst: &Instruction) -> Result<(), CodegenIrError> {
    crate::codegen_ir::lower_inst::builtins::io::lower_stream_get_filters(ctx, inst)
}
