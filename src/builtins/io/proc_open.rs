...
builtin! {
    ...
    semantics: crate::builtins::semantics::with_argument_lowering(
        crate::builtins::semantics::host_only_runtime_fn_semantics(
...
fn valid_command_array_type(ty: &PhpType) -> bool {
    // A normal PHP `array` return annotation is represented as a packed-or-hash
    // union with mixed elements. php-src accepts that array at the parameter
    // boundary and lets its element-to-string conversion report any bad member
    // at runtime, so the checker must not reject the generic contract early.
    if ty.is_php_array() {
        return true;
    }
    match ty {
...
/// Returns whether `ty` is either a concrete runtime array layout or PHP's
/// declared generic `array` contract.
fn is_php_array_value_type(ty: &PhpType) -> bool {
...
mod tests {
    use super::{
        is_php_array_value_type, is_windows_cmd_program, quote_windows_argument,
        quote_windows_cmd_argument, static_windows_command_line, static_windows_environment_block,
        static_windows_options, valid_command_array_type, valid_environment_type,
        valid_options_type, WINDOWS_PROC_BLOCKING_PIPES, WINDOWS_PROC_BYPASS_SHELL,
        WINDOWS_PROC_CREATE_NEW_CONSOLE, WINDOWS_PROC_SUPPRESS_ERRORS,
    };
    ...
    /// Verifies a standard PHP `array` declaration remains valid for every
    /// proc_open argument php-src receives as an array, even before its dynamic
    /// members are marshalled at runtime.
    #[test]
    fn accepts_generic_php_array_contracts_for_dynamic_proc_arguments() {
        let generic_array = PhpType::php_array();
        assert!(valid_command_array_type(&generic_array));
        assert!(is_php_array_value_type(&generic_array));
        assert!(valid_environment_type(&generic_array));
        assert!(valid_options_type(&generic_array));
    }
