...
/// Requires the baked timelib identifier namespace for paths that can mutate
/// the process default timezone. Unlike offset formatting, validation is PHP
/// semantics on every supported target, not a Windows CRT workaround.
pub fn timezone_validation_requirements(
    _input: &BuiltinRequirementInput<'_>,
) -> Vec<BuiltinRequirement> {
    vec![BuiltinRequirement::Bridge("elephc_tz")]
}
...
mod tests {
    ...
    /// Verifies client and server socket contracts can share one precise TLS
    /// transport resolver without making plain TCP accept paths link rustls.
    #[test]
    fn socket_transport_requirements_link_tls_only_when_reachable() {
        let tcp = [Expr::new(
            ExprKind::StringLiteral("tcp://127.0.0.1:1234".to_string()),
            Span::dummy(),
        )];
        assert!(stream_socket_client_requirements(&input(&tcp)).is_empty());
        let tls = [Expr::new(
            ExprKind::StringLiteral("tls://127.0.0.1:1234".to_string()),
            Span::dummy(),
        )];
        assert_eq!(
            stream_socket_client_requirements(&input(&tls)),
            vec![BuiltinRequirement::Bridge("elephc_tls")]
        );
        let dynamic = [Expr::new(
            ExprKind::Variable("address".to_string()),
            Span::dummy(),
        )];
        assert_eq!(
            stream_socket_client_requirements(&input(&dynamic)),
            vec![BuiltinRequirement::Bridge("elephc_tls")]
        );
    }
}
