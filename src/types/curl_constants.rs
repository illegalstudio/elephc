//! Purpose:
//! Defines every `ext/curl` integer constant (`CURLOPT_*`, `CURLINFO_*`, `CURLE_*`,
//! `CURL_*`, and the rest of the `curl_*`-prefixed families) exposed as PHP integer
//! constants, generated from the frozen `scripts/docs/curl_surface.json` surface.
//!
//! Called from:
...
//! - `crate::name_resolver::names::is_builtin_global_constant`.
//!
//! Key details:
...
///
/// Example entries: `("CURLOPT_URL", 10002)`, `("CURLOPT_RETURNTRANSFER", 19913)`,
/// `("CURLINFO_LASTONE", 71)`.
pub(crate) const CURL_INT_CONSTANTS: &[(&str, i64)] = &[
    ("CURLALTSVC_H1", 8),
    ("CURLALTSVC_H2", 16),
    ("CURLALTSVC_H3", 32),
...
mod tests {
    ...
    /// Re-parses the frozen `scripts/docs/curl_surface.json` at test time and returns its
    /// `constants` map as `(name -> value)`. Keeps this test honest against the actual
    /// frozen source of truth rather than a second hand-copied expectation.
    fn frozen_surface_constants() -> HashMap<String, i64> {
        ...
        let constants = json
            ...
            .expect("curl_surface.json must have a `constants` object");
        constants
            .iter()
            .map(|(name, value)| {
                let value = value
                    .as_i64()
                    .unwrap_or_else(|| panic!("{name} value must fit in i64"));
                (name.clone(), value)
            })
            .collect()
    }

    /// Asserts no duplicate names exist in `CURL_INT_CONSTANTS`.
    #[test]
    fn no_duplicate_constant_names() {
        let mut names: Vec<&str> = CURL_INT_CONSTANTS.iter().map(|(n, _)| *n).collect();
        names.sort_unstable();
        let len_before = names.len();
        names.dedup();
        assert_eq!(names.len(), len_before, "duplicate CURL constant name");
    }

    /// `CURL_INT_CONSTANTS` is generated from `scripts/docs/curl_surface.json` and must never
    /// drift from it: same count, and every name maps to the same value in both directions.
    #[test]
    fn curl_constants_match_frozen_surface() {
        let frozen = frozen_surface_constants();
        assert_eq!(
            CURL_INT_CONSTANTS.len(),
            frozen.len(),
            "CURL_INT_CONSTANTS entry count must match scripts/docs/curl_surface.json \
             (regenerate the table from the JSON)"
        );
        for (name, value) in CURL_INT_CONSTANTS {
            let expected = frozen.get(*name).unwrap_or_else(|| {
                panic!("{name} is in CURL_INT_CONSTANTS but not in curl_surface.json")
            });
            assert_eq!(
                value, expected,
                "{name} value diverges from curl_surface.json"
            );
        }
        for name in frozen.keys() {
            assert!(
                CURL_INT_CONSTANTS.iter().any(|(n, _)| n == name),
                "{name} is in curl_surface.json but missing from CURL_INT_CONSTANTS"
            );
        }
    }

    /// `CURLOPT_URL` and `CURLOPT_RETURNTRANSFER` — the two constants Step 2's codegen test
    /// and the setopt fixtures depend on — match the frozen JSON exactly.
    #[test]
    fn curlopt_url_and_returntransfer_match_frozen_json() {
        let frozen = frozen_surface_constants();
        let value = |name: &str| {
            CURL_INT_CONSTANTS
                .iter()
                .find(|(n, _)| *n == name)
                .unwrap_or_else(|| panic!("{name} defined in CURL_INT_CONSTANTS"))
                .1
        };
        assert_eq!(value("CURLOPT_URL"), frozen["CURLOPT_URL"]);
        assert_eq!(value("CURLOPT_RETURNTRANSFER"), frozen["CURLOPT_RETURNTRANSFER"]);
        assert_eq!(value("CURLOPT_URL"), 10002);
        assert_eq!(value("CURLOPT_RETURNTRANSFER"), 19913);
    }
}
