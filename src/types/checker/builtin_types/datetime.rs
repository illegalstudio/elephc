...
/// `DateTimeZone::listIdentifiers(): array` — returns the embedded IANA timezone identifier list.
///
/// Static method; the body is a parsed `return [ ... ];` over the identifiers in
/// `timezone_ids::TIMEZONE_IDENTIFIERS_ARRAY` (captured from PHP). The optional `$timezoneGroup` /
/// `$countryCode` filter arguments are not supported. Returns an indexed array of strings.
...
if ($timezone === null) {
    if ($datetime === "now") {
        ...
        $this->timestamp = __elephc_strtotime_raw($datetime);
    }
...
/// Builds an `mktime(hour, minute, second, month, day, year)` call expression.
fn mktime_call(parts: [&str; 6]) -> Expr {
    Expr::new(
        ExprKind::FunctionCall {
            name: Name::unqualified("mktime"),
            args: parts
                .iter()
...
/// `setTime(int $hour, int $minute, int $second = 0)` — keeps the date, replaces the time-of-day.
...
if ($__rest === "") {
    ...
    $__ts = __elephc_strtotime_raw($__rest, $this->timestamp);
}
...
/// `Y y m n d j D l S F M z H G h g i s u v A a U O P Z T e` plus the metas `! | # ? * +`.
...
$jan4 = mktime($h, $mi, $se, 1, 4, $year);
$dow = (int)date("N", $jan4);
$day = 4 - ($dow - 1) + ($week - 1) * 7 + ($dayOfWeek - 1);
return $this->setTimestamp(mktime($h, $mi, $se, 1, $day, $year));
"#;
...
/// `is_localtime` slots. Built as `false` literals then conditionally overwritten, because an
/// int|false union flowing through a single variable would coerce to `0`.
...
$r = ["year" => false, "month" => false, "day" => false, "hour" => false, "minute" => false, "second" => false, "fraction" => false, "warning_count" => $warnings, "warnings" => [], "error_count" => $errors, "errors" => [], "is_localtime" => false];
if ($pY) { $r["year"] = $Y; }
if ($pmo) { $r["month"] = $mo; }
if ($pda) { $r["day"] = $da; }
if ($pH) { $r["hour"] = $H; }
...
/// PHP source backing `date_parse()`. elephc does not reimplement PHP's full free-form date grammar;
/// instead it tries a list of common formats (most specific first) via `date_parse_from_format` and
/// returns the first that consumes the whole string with no errors/warnings, else an error result.
/// Covers the common ISO-ish forms (`Y-m-d H:i:s`, `Y-m-d`, `H:i`, …).
const DATE_PARSE_SRC: &str = r#"<?php
$fmts = ["Y-m-d H:i:s", "Y-m-d H:i", "Y-m-d", "Y/m/d", "d-m-Y", "d/m/Y", "H:i:s", "H:i"];
$n = count($fmts);
$i = 0;
while ($i < $n) {
...
/// `date()` keeps them literal. Locale-dependent (`%c`/`%x`/`%X`) and week-number (`%U`/`%V`/`%W`)
/// specifiers are best-effort approximations; space-padded variants (`%e`/`%k`/`%l`) fall back to the
/// non-padded `date()` specifier.
...
/// trailing fractional second removed, so `strtotime()` can parse the remainder.
...
$sunrise = ($off["rc"] == 1) ? true : (($off["rc"] == -1) ? false : intval($off["hr"] * 3600 + $u));
$sunset = ($off["rc"] == 1) ? true : (($off["rc"] == -1) ? false : intval($off["hs"] * 3600 + $u));
$transit = intval($off["ts"] * 3600 + $u);
$cb = ($civ["rc"] == 1) ? true : (($civ["rc"] == -1) ? false : intval($civ["hr"] * 3600 + $u));
$ce = ($civ["rc"] == 1) ? true : (($civ["rc"] == -1) ? false : intval($civ["hs"] * 3600 + $u));
$nb = ($nau["rc"] == 1) ? true : (($nau["rc"] == -1) ? false : intval($nau["hr"] * 3600 + $u));
$ne = ($nau["rc"] == 1) ? true : (($nau["rc"] == -1) ? false : intval($nau["hs"] * 3600 + $u));
$ab = ($ast["rc"] == 1) ? true : (($ast["rc"] == -1) ? false : intval($ast["hr"] * 3600 + $u));
$ae = ($ast["rc"] == 1) ? true : (($ast["rc"] == -1) ? false : intval($ast["hs"] * 3600 + $u));
return [
    "sunrise" => $sunrise,
    "sunset" => $sunset,
    "transit" => $transit,
...
/// are accepted for signature compatibility but offset-based disambiguation is not performed; the
/// abbreviation's default zone is returned. The lookup is case-insensitive.
...
/// Supports `%Y %y %m %d %e %H %M %S %j %B %b %h %A %a %p %P`, the whitespace metas `%n`/`%t`,
/// `%%`, flexible spaces, and literal characters. Returns PHP's nine-key array
...
if ($gotY && $gotMon && $gotMday) {
    $ts = gmmktime($hour, $min, $sec, $mon, $mday, $year);
...
fn date_interval_constructor() -> ClassMethod {
    ...
    // M dispatch: minutes after T, months before.
    let m_branch = vec![
        Stmt::new(
            StmtKind::If {
                condition: binop(var("inTime"), BinOp::StrictEq, int(1)),
                then_body: vec![assign_this_property("i", var("num"))],
                elseif_clauses: Vec::new(),
                else_body: Some(vec![assign_this_property("m", var("num"))]),
            },
            dummy(),
        ),
        reset_num(),
    ...
    // if ($c === "T") {...} elseif ... unit letters ...
    let unit_if = Stmt::new(
        StmtKind::If {
            condition: is_c("T"),
            then_body: vec![Stmt::assign("inTime", int(1))],
            elseif_clauses: vec![
                (is_c("Y"), vec![assign_this_property("y", var("num")), reset_num()]),
                (
                    is_c("W"),
                    vec![
                ...
                (is_c("H"), vec![assign_this_property("h", var("num")), reset_num()]),
                (is_c("S"), vec![assign_this_property("s", var("num")), reset_num()]),
                (is_c("M"), m_branch),
            ],
            else_body: None,
        },
...
/// Builds an `int` `DateInterval` component property. All components default to `0` except `days`,
/// which defaults to `-1`: it is set only by `diff()`, and the sentinel lets `format("%a")` emit
/// PHP's `(unknown)` for intervals that were constructed directly rather than returned from `diff()`.
fn interval_property(name: &str) -> ClassProperty {
    let default = if name == "days" { -1 } else { 0 };
    property(name, TypeExpr::Int, Expr::new(ExprKind::IntLiteral(default), dummy()))
}
...
foreach ($parts as $p) {
    ...
    $n = $haveNum ? $num : 1;
    if ($p === "sec" || $p === "secs" || $p === "second" || $p === "seconds") { $iv->s = $iv->s + $n; }
    elseif ($p === "min" || $p === "mins" || $p === "minute" || $p === "minutes") { $iv->i = $iv->i + $n; }
    elseif ($p === "hour" || $p === "hours") { $iv->h = $iv->h + $n; }
    elseif ($p === "day" || $p === "days") { $iv->d = $iv->d + $n; }
    elseif ($p === "week" || $p === "weeks") { $iv->d = $iv->d + $n * 7; }
    elseif ($p === "fortnight" || $p === "fortnights") { $iv->d = $iv->d + $n * 14; }
    elseif ($p === "month" || $p === "months") { $iv->m = $iv->m + $n; }
    elseif ($p === "year" || $p === "years") { $iv->y = $iv->y + $n; }
    $haveNum = false;
    $num = 0;
}
return $iv;
    ...
    let dispatch = Stmt::new(
        StmtKind::If {
            ...
            elseif_clauses: vec![
                ...
                // %a: total days, or "(unknown)" for the -1 sentinel (interval not from diff()).
                (
                    spec_is("a"),
                    vec![Stmt::new(
                        StmtKind::If {
                            condition: binop(this_property("days"), BinOp::Lt, int(0)),
...
fn datetime_diff_method() -> ClassMethod {
    ...
    let mktime6 = |h: Expr, mi: Expr, s: Expr, mo: Expr, d: Expr, y: Expr| {
        Expr::new(
            ExprKind::FunctionCall { name: Name::unqualified("mktime"), args: vec![h, mi, s, mo, d, y] },
            dummy(),
        )
    };
