...
fn date_period_constructor() -> ClassMethod {
    ...
    let mut body = vec![assign_this("startTs", mcall(var("start"), "getTimestamp", Vec::new()))];
    for (store, part) in INTERVAL_PARTS {
...
/// `DatePeriod::current(): DateTime` — returns a fresh `DateTime` snapshot at the cursor.
fn date_period_current() -> ClassMethod {
    method(
        ...
        vec![
            assign("d", new_obj("DateTime", Vec::new())),
            expr_stmt(mcall(var("d"), "setTimestamp", vec![this_prop("curTs")])),
            ret(var("d")),
        ],
    )
}
...
/// Builds a getter that returns a fresh `DateTime` at the timestamp stored in `prop`.
fn date_getter(name: &str, prop: &str) -> ClassMethod {
    method(
        name,
        Vec::new(),
        Some(TypeExpr::Named(Name::unqualified("DateTime"))),
        vec![
            assign("d", new_obj("DateTime", Vec::new())),
            expr_stmt(mcall(var("d"), "setTimestamp", vec![this_prop(prop)])),
            ret(var("d")),
        ],
    )
}
...
///
/// Parses a subset of the RFC 5545 repeating-interval specification (`Rn/start[/interval[/end]]`,
/// with `n` ≥ 1) and forwards to the regular `(start, interval, end|recurrences)` constructor.
...
if ($iso[0] !== "R") { throw new DateMalformedPeriodStringException($bad); }
...
if ($start_str === "") { throw new DateMalformedPeriodStringException($bad); }
$has_end = ($end_str !== "") ? 1 : 0;
try {
...
fn date_period_methods() -> Vec<ClassMethod> {
    vec![
        ...
        date_period_key(),
        date_period_next(),
        date_getter("getStartDate", "startTs"),
        date_getter("getEndDate", "endTs"),
...
fn date_period_properties() -> Vec<ClassProperty> {
    let mut props = vec![int_property("startTs"), int_property("endTs")];
