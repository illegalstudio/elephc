...
impl Checker {
    ...
    /// Records the elephc-tz bridge requirement for the OOP datetime surface
    /// (`DateTime`/`DateTimeZone`/`DateTimeImmutable`/
    ...
    /// substitute. The bridge now also validates every timezone mutation
    /// transactionally, so this must apply on Unix and iOS as well as Windows.
    pub(crate) fn require_datetime_tz_bridge_if_needed(&mut self, class_name: &str) {
        if is_elephc_tz_backed_datetime_class(class_name) {
            self.require_builtin_library("elephc_tz");
        }
    }
