...
impl Checker {
    ...
    pub(crate) fn infer_new_object_type(
        ...
        if let Some(class_info) = self.classes.get(class_name.as_str()) {
            ...
            // Private constructors are not copied into a descendant's method map. Resolve the
            // owning ancestor for ordinary classes, while ext/date descendants deliberately use
            // their effective date base so the specialised overloads stay intact.
            ...
            let (constructor_class_info, constructor_owner_name) = if let Some(base) =
                date_constructor_base
            {
                (
                    self.classes
                        .get(base)
                        .expect("ext/date constructor base must be declared"),
                    Some(base),
                )
            } else if let Some((_, owner_name, owner_info)) = constructor_owner {
                (owner_info, Some(owner_name))
            } else {
                (class_info, None)
            };
    ...
    fn validate_reflection_owner_constructor(
        ...
        match class_name {
            ...
            "ReflectionEnumUnitCase" => {
                ...
                self.validate_reflection_enum_case_attrs(&reflected_class, &case_name, false, expr)
            }
            "ReflectionEnumBackedCase" => {
                ...
                self.validate_reflection_enum_case_attrs(&reflected_class, &case_name, true, expr)
            }
            _ => Ok(()),
        }
    }
        ...
        match receiver {
            ...
            StaticReceiver::Parent => {
                ...
                self.classes
                    ...
                    .ok_or_else(|| {
                        CompileError::new(span, &format!("Class '{}' has no parent class", current))
                    })
            }
        }
    }
    ...
    fn resolve_fiber_callable_sig(
        ...
        match &callback.kind {
            ExprKind::StringLiteral(name) => {
                self.resolve_fiber_string_callable_sig(name, callback.span, env)
            }
            ExprKind::ArrayLiteral(_) => {
                self.resolve_fiber_callable_array_literal_sig(callback, env)
            }
            ExprKind::Variable(name) => {
                if let Some(target) = self.callable_array_targets.get(name).cloned() {
                    return self.resolve_fiber_callable_array_variable_sig(
                        &target,
                        callback.span,
                        env,
                    );
    ...
    fn fiber_static_callable_receiver(
        ...
        let class_name = match &receiver.kind {
            ExprKind::StringLiteral(class_name) => self
                ...
                .map(str::to_string),
            ExprKind::ClassConstant { receiver } => {
                Some(self.resolve_fiber_callable_static_receiver_class(receiver, span)?)
            }
            _ => None,
        };
    ...
    fn specialize_callback_filter_function(
        ...
        let Some(sig) = self.functions.get_mut(name.as_str()) else {
            return Err(CompileError::new(
                span,
                &format!(
                    "Undefined function for CallbackFilterIterator callback: {}",
                    name
                ),
            ));
        };
...
/// (`ReflectionClass`, `ReflectionObject`, `ReflectionEnum`, `ReflectionMethod`, `ReflectionProperty`,
