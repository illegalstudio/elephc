...
fn widen_loop_grown_array_pushes(body: &[Stmt], update: Option<&Stmt>, env: &mut TypeEnv) {
    let names =
        crate::types::checker::loop_grown_mixed_array_pushes(body, update, &|name| {
            env.get(name).cloned()
        });
    for name in names {
    ...
    pub(crate) fn check_control_flow_stmt(
        ...
        match &stmt.kind {
            StmtKind::Foreach {
                ...
                widen_loop_grown_array_pushes(body, None, env);
            ...
            StmtKind::For {
                ...
                widen_loop_grown_array_pushes(body, update.as_deref(), env);
