...
//!
//! Called from:
//! - `crate::codegen_support::generate()` when class metadata contains methods
//!
//! Key details:
...
pub(super) fn emit_class_methods(
    ...
    for method in &class_info.method_decls {
        ...
        let generated_body = if class_name == "ReflectionAttribute" && method_key == "newinstance" {
            Some(crate::codegen_support::reflection::build_attribute_new_instance_body(classes))
            ...
            Some(crate::codegen_support::reflection::build_attribute_get_arguments_body(classes))
        } else {
            None
        };
