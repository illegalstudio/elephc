...
use crate::codegen_support::runtime::data::{RANDOM_BYTES_LENGTH_MSG, RANDOM_BYTES_SOURCE_MSG};
...
fn emit_random_bytes_x86_64(emitter: &mut Emitter) {
    ...
    let heap_kind = crate::codegen_support::sentinels::x86_64_heap_kind_word(1);
    emitter.instruction(&format!("mov r10, 0x{heap_kind:x}"));                  // load the owned-string heap kind with the x86_64 marker
