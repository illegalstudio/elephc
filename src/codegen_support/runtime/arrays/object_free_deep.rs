...
pub fn emit_object_free_deep(emitter: &mut Emitter, features: RuntimeFeatures) {
    ...
    //   [sp, #24] = loop index
    //   [sp, #32] = owned pending Throwable
    //   [sp, #40] = enclosing GC suppression
    //   [sp, #48] = saved x29
    //   [sp, #56] = saved x30
    ...
    emitter.instruction(&format!("str xzr, [x0, #{}]", crate::codegen_support::runtime::FIBER_STACK_BASE_OFFSET)); // null the stack_base so a double-free is a clean no-op
    emitter.instruction(&format!("str xzr, [x0, #{}]", crate::codegen_support::runtime::FIBER_STACK_SIZE_OFFSET)); // null the stack_size to mirror the cleared base pointer
    ...
    super::deep_cleanup::invoke(emitter, "__rt_decref_mixed", "x0");
    ...
    let user_arg_max_off = crate::codegen_support::runtime::FIBER_USER_ARG_MAX_OFFSET;
    let start_args_off = crate::codegen_support::runtime::FIBER_START_ARGS_OFFSET;
    ...
    emitter.instruction("ldr x10, [x0]");                                       // x10 = receiver class_id
    crate::codegen_support::abi::emit_load_symbol_to_reg(emitter, "x11", "_spl_dll_class_id", 0); // x11 = class id of SplDoublyLinkedList
    ...
    crate::codegen_support::abi::emit_load_symbol_to_reg(emitter, "x11", "_spl_stack_class_id", 0); // x11 = class id of SplStack
    ...
    crate::codegen_support::abi::emit_load_symbol_to_reg(emitter, "x11", "_spl_queue_class_id", 0); // x11 = class id of SplQueue
    ...
    super::deep_cleanup::invoke(emitter, "__rt_decref_array", "x0");
    ...
    emitter.instruction("ldr x10, [x0]");                                       // x10 = receiver class_id
    crate::codegen_support::abi::emit_load_symbol_to_reg(emitter, "x11", "_spl_fixed_array_class_id", 0); // x11 = class id of SplFixedArray
...
fn emit_object_free_deep_linux_x86_64(emitter: &mut Emitter, features: RuntimeFeatures) {
    ...
    emitter.instruction(&format!("mov rsi, QWORD PTR [rax + {}]", crate::codegen_support::runtime::FIBER_STACK_SIZE_OFFSET)); // rsi = total mapped length
    ...
    emitter.instruction(&format!("mov QWORD PTR [rax + {}], 0", crate::codegen_support::runtime::FIBER_STACK_BASE_OFFSET)); // null stack_base for double-free safety
    emitter.instruction(&format!("mov QWORD PTR [rax + {}], 0", crate::codegen_support::runtime::FIBER_STACK_SIZE_OFFSET)); // null stack_size to mirror the cleared base
    emitter.label("__rt_object_free_deep_fiber_no_stack");
    ...
    // -- SPL doubly-linked-list family: release custom internal storage, not PHP property slots --
    emitter.instruction("mov r10, QWORD PTR [rax]");                            // r10 = receiver class_id
    crate::codegen_support::abi::emit_load_symbol_to_reg(emitter, "r11", "_spl_dll_class_id", 0); // r11 = class id of SplDoublyLinkedList
    ...
    crate::codegen_support::abi::emit_load_symbol_to_reg(emitter, "r11", "_spl_stack_class_id", 0); // r11 = class id of SplStack
    ...
    crate::codegen_support::abi::emit_load_symbol_to_reg(emitter, "r11", "_spl_queue_class_id", 0); // r11 = class id of SplQueue
    ...
    super::deep_cleanup::invoke(emitter, "__rt_decref_array", "rax");
    ...
    // -- SplFixedArray custom storage: release the owned fixed-size Mixed array --
    emitter.instruction("mov r10, QWORD PTR [rax]");                            // r10 = receiver class_id
    crate::codegen_support::abi::emit_load_symbol_to_reg(emitter, "r11", "_spl_fixed_array_class_id", 0); // r11 = class id of SplFixedArray
...
fn emit_generator_coroutine_release_aarch64(emitter: &mut Emitter) {
    use crate::codegen_support::runtime::generators::coro;
    let stack_base = crate::codegen_support::runtime::FIBER_STACK_BASE_OFFSET;
    let stack_size = crate::codegen_support::runtime::FIBER_STACK_SIZE_OFFSET;
    ...
    for i in 0..crate::codegen_support::runtime::FIBER_START_ARGS_MAX as usize {
...
mod tests {
    ...
    fn test_object_free_deep_releases_callable_properties_aarch64() {
        ...
        assert!(asm.contains("__rt_callable_descriptor_release") && asm.contains("__rt_cleanup_invoke"));
    }
