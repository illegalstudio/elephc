...
pub fn emit_array_splice_refcounted(emitter: &mut Emitter) {
    ...
    emitter.instruction("b.ge __rt_array_splice_ref_len_clamp");                // a non-negative length only needs the upper clamp
    ...
    emitter.label("__rt_array_splice_ref_len_clamp");
...
fn emit_array_splice_refcounted_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("jge __rt_array_splice_ref_known_len_x86");             // a non-negative length only needs the upper clamp
    ...
    emitter.instruction("jge __rt_array_splice_ref_known_len_x86");             // the window still holds elements
