...
pub fn emit_random_u32(emitter: &mut Emitter) {
    ...
    match emitter.platform {
        ...
        Platform::Linux => {
            ...
            emitter.emit_call_c("time");                                               // get coarse wall-clock seconds from libc
...
/// in `eax`.
///
/// The random-range lowerings keep the caller's minimum in `r9` across the whole sampler
/// chain that funnels into this helper. The `syscall` instruction only clobbers `rcx`/`r11`,
/// but the libc `time()` fallback may clobber any SysV caller-saved register, so the
/// fallback path spills `r9` into the already-reserved frame slot around that call.
fn emit_random_u32_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov QWORD PTR [rbp - 16], r9");                        // spill r9 before the libc call: range lowerings keep the caller's minimum there
    ...
    emitter.instruction("mov r9, QWORD PTR [rbp - 16]");                        // restore r9: libc may clobber any SysV caller-saved register
