//! Purpose:
//! Emits the `__rt_array_free_deep`, `__rt_array_free_deep_done` runtime helper assembly for array free deep.
...
mod tests {
    ...
    /// The x86_64 helpers a released `CurlHandle`/`CurlMultiHandle` travels through on its
    /// way into the `elephc_curl` C bridge, in call order:
    /// `__rt_array_free_deep` (a handle held as an ARRAY ELEMENT, e.g. the array
    /// `curl_multi_get_handles()` returns) -> `__rt_decref_any`/`__rt_decref_mixed` (tail
    /// jumps, no frame of their own) -> `__rt_mixed_free_deep` -> `__rt_curl_*_free` ->
    /// `call r9`, the bridge entry pointer.
    ///
    /// Each of these OWNS a frame, so each one's `call`s must land on a 16-byte boundary or
    /// the misalignment travels the whole way down and reaches C.
    const DEEP_FREE_CHAIN: &[&str] = &[
    ...
    /// Returns the emitted body of one x86_64 runtime helper, from its label to the start
    /// of the next `.section .text.` block.
    fn body_of<'a>(asm: &'a str, label: &str) -> &'a str {
        let start = asm
            .find(&format!("\n{label}:\n"))
            .unwrap_or_else(|| panic!("{label} must be emitted for linux-x86_64"));
    ...
    /// SYSTEM V x86_64 REQUIRES `rsp % 16 == 0` AT EVERY `call`, and this asserts it for the
    /// helpers a curl handle is freed through.
    ///
    /// REGRESSION PIN. `__rt_array_free_deep` reserved 24 bytes for its three spill slots
    /// after `push rbp` had already landed rsp on a 16-byte boundary, so every `call` in its
    /// body ran 8 bytes off. Nothing noticed while the callees were hand-written assembly
    /// that touches only integer registers — but a `CurlHandle` released as an ARRAY ELEMENT
    /// walks `__rt_decref_any` -> `__rt_mixed_free_deep` -> `__rt_curl_easy_free` and calls
    /// `curl_easy_cleanup` through the bridge, where the first aligned SSE spill on that
    /// stack faults. That is exactly what
    /// `codegen::curl::multi::multi_get_handles_reports_add_order_and_tracks_removal`
    /// SIGSEGV'd on under linux-x86_64 (and only there: the aarch64 emitter already
    /// reserved 32).
    ///
    /// The walk is linear rather than control-flow-aware, which is sound for these helpers:
    /// each opens with a single `push rbp` + one `sub rsp, N` and closes with the matching
    /// `add`/`pop`, so the offset at any instruction does not depend on which branch reached
    /// it. `mov rsp, rbp` resets to "just the saved rbp", the shape `__rt_curl_*_free` uses.
    #[test]
    fn the_deep_free_chain_keeps_sysv_call_alignment_on_x86_64() {
        let asm = crate::codegen_support::generate_runtime_with_features(
            8_388_608,
            Target::parse("linux-x86_64").expect("supported target"),
            RuntimeFeatures::none(),
        );

        for label in DEEP_FREE_CHAIN {
            // rsp delta from the helper's entry, where rsp % 16 == 8 (the caller's `call`
            // pushed the return address onto an aligned stack).
            let mut offset: i64 = 0;
            for line in body_of(&asm, label).lines() {
                let instruction = line.split('#').next().unwrap_or("").trim();
                if instruction.starts_with("push ") || instruction == "pushfq" {
                    offset -= 8;
                } else if instruction.starts_with("pop ") || instruction == "popfq" {
                    offset += 8;
                } else if let Some(bytes) = instruction.strip_prefix("sub rsp, ") {
                    offset -= bytes.trim().parse::<i64>().expect("literal frame size");
                } else if let Some(bytes) = instruction.strip_prefix("add rsp, ") {
                    offset += bytes.trim().parse::<i64>().expect("literal frame size");
                } else if instruction == "mov rsp, rbp" {
                    offset = -8;
                } else if let Some(target) = instruction.strip_prefix("call ") {
                    assert_eq!(
                        offset.rem_euclid(16),
                        8,
                        "{label} calls {target} with rsp {} bytes past its entry, which is \
                         {} mod 16 — SysV needs 8 so the callee sees a 16-byte-aligned \
                         stack. A curl handle freed through here reaches the elephc_curl \
                         bridge, and C faults on a misaligned spill.",
                        offset,
                        offset.rem_euclid(16),
                    );
                }
