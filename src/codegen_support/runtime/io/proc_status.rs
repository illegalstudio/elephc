...
fn emit_proc_get_status_windows(emitter: &mut Emitter) {
    ...
    emitter.instruction("movsxd rax, DWORD PTR [rbp - 32]");                    // preserve the exit code as a SIGNED 32-bit value, matching
                                                                                // php storing the DWORD into `proc->exit_code` (an int)
