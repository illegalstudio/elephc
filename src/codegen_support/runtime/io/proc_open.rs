...
/// `x3` = reserved pipes placeholder; x86_64 `rdi`/`rsi`/`rdx`/`rcx`. Output: the raw child
...
/// result register. PHP replaces the by-reference argument, so the helper always
/// starts from a fresh Mixed-element array instead of retaining caller contents.
///
...
fn emit_proc_open_aarch64(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov x0, #16");                                         // proc_open replaces the caller's pipes value with a fresh hash
    emitter.instruction("mov x1, #7");                                          // hash value type 7 stores boxed Mixed resource cells
    abi::emit_call_label(emitter, "__rt_hash_new");
    emitter.instruction("str x0, [sp, #40]");                                   // save the fresh pipes hash pointer
    emitter.instruction("ldr x0, [sp, #16]");                                   // restore descriptor_spec after allocating the output container
    ...
    emitter.instruction("ldr x10, [x11, x9, lsl #3]");                          // parent_fd[j] (resource handle low payload)
    emitter.instruction("str x10, [sp, #72]");                                  // preserve the raw resource payload for the hash write
    emitter.instruction("add x11, sp, #416");                                   // base of descriptor_index bookkeeping
    emitter.instruction("ldr x1, [x11, x9, lsl #3]");                           // hash key low word = real integer descriptor key
    emitter.instruction("mov x2, #-1");                                         // hash key high word sentinel marks an integer key
    emitter.instruction("ldr x0, [sp, #40]");                                   // reload the current pipes hash pointer
    emitter.instruction("ldr x3, [sp, #72]");                                   // resource low payload = native stream fd
    emitter.instruction("mov x4, #1");                                          // resource high payload = kind 1 native stream
    emitter.instruction("mov x5, #9");                                          // runtime value tag 9 = resource
    abi::emit_call_label(emitter, "__rt_hash_set");                             // publish the resource at its real descriptor key
    emitter.instruction("str x0, [sp, #40]");                                   // persist a potentially grown pipes hash
...
fn emit_proc_open_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov rdi, 16");                                         // proc_open replaces the caller's pipes value with a fresh hash
    emitter.instruction("mov rsi, 7");                                          // hash value type 7 stores boxed Mixed resource cells
    abi::emit_call_label(emitter, "__rt_hash_new");
    emitter.instruction("mov QWORD PTR [rbp - 40], rax");                       // save the fresh pipes hash pointer
    emitter.instruction("mov rdi, QWORD PTR [rbp - 16]");                       // restore descriptor_spec after allocating the output container
    ...
    emitter.instruction("mov r10, QWORD PTR [r11 + r9 * 8]");                   // parent_fd[j] (resource handle low payload)
    emitter.instruction("mov QWORD PTR [rbp - 72], r10");                       // preserve the raw resource payload for the hash write
    emitter.instruction("lea r11, [rbp - 464]");                                // base of descriptor_index bookkeeping
    emitter.instruction("mov rsi, QWORD PTR [r11 + r9 * 8]");                   // hash key low word = real integer descriptor key
    emitter.instruction("mov rdx, -1");                                         // hash key high word sentinel marks an integer key
    emitter.instruction("mov rdi, QWORD PTR [rbp - 40]");                       // reload the current pipes hash pointer
    emitter.instruction("mov rcx, QWORD PTR [rbp - 72]");                       // resource low payload = native stream fd
    emitter.instruction("mov r8, 1");                                           // resource high payload = kind 1 native stream
    emitter.instruction("mov r9, 9");                                           // runtime value tag 9 = resource
    abi::emit_call_label(emitter, "__rt_hash_set");                             // publish the resource at its real descriptor key
    emitter.instruction("mov QWORD PTR [rbp - 40], rax");                       // persist a potentially grown pipes hash
...
/// lit_r, `[136]` cmdbuf pointer, `[144]`/`[152]`/`[160]` reserved legacy
/// standard-handle cleanup slots, `[168, 232)` parent_handle\[8\] (element `j` at
...
/// `socket` maps to a CRT-compatible pipe endpoint for child standard streams,
/// while the public AF_UNIX `socketpair` API remains unsupported; `pty` remains
/// outside this helper.
...
fn emit_proc_open_win32_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov QWORD PTR [rbp - 24], rsi");                       // save the command string pointer
    emitter.instruction("mov QWORD PTR [rbp - 32], rdx");                       // save the command string length
    emitter.instruction("mov QWORD PTR [rbp - 528], r8");                       // save optional cwd string pointer before internal calls clobber r8
    emitter.instruction("mov QWORD PTR [rbp - 536], r9");                       // save optional cwd string length before internal calls clobber r9
    emitter.instruction("mov rdi, 16");                                         // proc_open replaces the caller's pipes value with a fresh hash
    emitter.instruction("mov rsi, 7");                                          // hash value type 7 stores boxed Mixed resource cells
    abi::emit_call_label(emitter, "__rt_hash_new");
    emitter.instruction("mov QWORD PTR [rbp - 40], rax");                       // save the fresh pipes hash pointer
    ...
    // -- clear owned default-standard duplicates and cmdbuf before shared cleanup --
    emitter.instruction("mov QWORD PTR [rbp - 144], 0");                        // no inherited stdin duplicate is owned yet
    emitter.instruction("mov QWORD PTR [rbp - 152], 0");                        // no inherited stdout duplicate is owned yet
    emitter.instruction("mov QWORD PTR [rbp - 160], 0");                        // no inherited stderr duplicate is owned yet
    ...
    emitter.instruction("mov r10, QWORD PTR [rbp - 48]");                       // reload n
    emitter.instruction("cmp r9, r10");                                         // i < n?
    emitter.instruction("jae __rt_proc_open_std_defaults_win");                 // descriptor loop complete -> inherit php-src's std handles
    ...
    // -- open a CRT-compatible pipe pair: CreatePipe(&read_end, &write_end, &sa, 0) --
    emitter.label("__rt_proc_open_open_pipe_win");
    ...
    emitter.instruction("jne __rt_proc_open_cleanup_m0_win");                   // unsupported six-byte descriptor
    emitter.instruction("jmp __rt_proc_open_socket_descriptor_win");            // map the child standard stream onto a CRT-compatible pipe
    ...
    emitter.instruction("xor r9d, r9d");                                        // php-src opens its NUL blackhole with default handle attributes
    ...
    // -- descriptor `socket`: standard streams require CRT-compatible handles --
    ...
    emitter.instruction("mov r9, QWORD PTR [rbp - 64]");                        // descriptor position selects the child stdio direction
    emitter.instruction("lea r10, [rbp - 784]");                                // base of real descriptor-key bookkeeping
    emitter.instruction("mov r9, QWORD PTR [r10 + r9 * 8]");                    // actual descriptor key, not packed position
    emitter.instruction("cmp r9, 0");                                           // stdin means the child consumes the parent stream
    emitter.instruction("jne __rt_proc_open_socket_child_writes_win");          // stdout/stderr give the child the write end instead
    emitter.instruction("mov QWORD PTR [rbp - 80], 1");                         // child reads / parent writes through a CRT pipe
    emitter.instruction("jmp __rt_proc_open_open_pipe_win");                    // reuse pipe ownership, inheritance, and CRT adoption
    emitter.label("__rt_proc_open_socket_child_writes_win");
    emitter.instruction("mov QWORD PTR [rbp - 80], 0");                         // child writes / parent reads through a CRT pipe
    emitter.instruction("jmp __rt_proc_open_open_pipe_win");                    // preserve stream-filter and stdio behavior
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // PHP-compatible long-path conversion for CreateFileW
    ...
    emitter.instruction("mov r8, 7");                                           // allow PHP-compatible read, write, and delete sharing
    ...
    // -- php-src inherits the caller's std handles when a descriptor is absent --
    emitter.label("__rt_proc_open_std_defaults_win");
    emitter.instruction("cmp QWORD PTR [rbp - 408], 0");                        // si.hStdInput already wired by descriptor zero?
    emitter.instruction("jne __rt_proc_open_std_default_stdout_win");           // preserve the explicit stdin descriptor
    emitter.instruction("mov ecx, -10");                                        // STD_INPUT_HANDLE
    emitter.instruction("call GetStdHandle");                                   // php-src's inherited child stdin default
    emitter.instruction("test rax, rax");                                       // stdin handle is present?
    emitter.instruction("jz __rt_proc_open_capture_cleanup_win");               // absent stdin cannot be passed through STARTUPINFOW
    emitter.instruction("cmp rax, -1");                                         // stdin handle is not INVALID_HANDLE_VALUE?
    emitter.instruction("je __rt_proc_open_capture_cleanup_win");               // preserve GetLastError before generic cleanup
    emitter.instruction("mov rcx, -1");                                         // source process = current process pseudo-handle
    emitter.instruction("mov rdx, rax");                                        // source stdin handle
    emitter.instruction("mov r8, -1");                                          // target process = current process pseudo-handle
    emitter.instruction("lea r9, [rbp - 144]");                                 // retain the owned inheritable stdin duplicate
    emitter.instruction("mov QWORD PTR [rsp + 32], 0");                         // desired access ignored with DUPLICATE_SAME_ACCESS
    emitter.instruction("mov QWORD PTR [rsp + 40], 1");                         // make the child stdin copy inheritable
    emitter.instruction("mov QWORD PTR [rsp + 48], 2");                         // DUPLICATE_SAME_ACCESS
    emitter.instruction("call DuplicateHandle");                                // duplicate even a non-inheritable host stdin handle
    emitter.instruction("test eax, eax");                                       // stdin duplication succeeded?
    emitter.instruction("jz __rt_proc_open_capture_cleanup_win");               // capture the native failure before closing prior descriptors
    emitter.instruction("mov rax, QWORD PTR [rbp - 144]");                      // reload the child-owned stdin duplicate
    emitter.instruction("mov QWORD PTR [rbp - 408], rax");                      // install the inheritable stdin handle
    emitter.label("__rt_proc_open_std_default_stdout_win");
    emitter.instruction("cmp QWORD PTR [rbp - 400], 0");                        // si.hStdOutput already wired by descriptor one?
    emitter.instruction("jne __rt_proc_open_std_default_stderr_win");           // preserve the explicit stdout descriptor
    emitter.instruction("mov ecx, -11");                                        // STD_OUTPUT_HANDLE
    emitter.instruction("call GetStdHandle");                                   // php-src's inherited child stdout default
    emitter.instruction("test rax, rax");                                       // stdout handle is present?
    emitter.instruction("jz __rt_proc_open_capture_cleanup_win");               // absent stdout cannot be passed through STARTUPINFOW
    emitter.instruction("cmp rax, -1");                                         // stdout handle is not INVALID_HANDLE_VALUE?
    emitter.instruction("je __rt_proc_open_capture_cleanup_win");               // preserve GetLastError before generic cleanup
    emitter.instruction("mov rcx, -1");                                         // source process = current process pseudo-handle
    emitter.instruction("mov rdx, rax");                                        // source stdout handle
    emitter.instruction("mov r8, -1");                                          // target process = current process pseudo-handle
    emitter.instruction("lea r9, [rbp - 152]");                                 // retain the owned inheritable stdout duplicate
    emitter.instruction("mov QWORD PTR [rsp + 32], 0");                         // desired access ignored with DUPLICATE_SAME_ACCESS
    emitter.instruction("mov QWORD PTR [rsp + 40], 1");                         // make the child stdout copy inheritable
    emitter.instruction("mov QWORD PTR [rsp + 48], 2");                         // DUPLICATE_SAME_ACCESS
    emitter.instruction("call DuplicateHandle");                                // duplicate even a non-inheritable host stdout handle
    emitter.instruction("test eax, eax");                                       // stdout duplication succeeded?
    emitter.instruction("jz __rt_proc_open_capture_cleanup_win");               // capture the native failure before closing prior descriptors
    emitter.instruction("mov rax, QWORD PTR [rbp - 152]");                      // reload the child-owned stdout duplicate
    emitter.instruction("mov QWORD PTR [rbp - 400], rax");                      // install the inheritable stdout handle
    emitter.label("__rt_proc_open_std_default_stderr_win");
    emitter.instruction("cmp QWORD PTR [rbp - 392], 0");                        // si.hStdError already wired by descriptor two?
    emitter.instruction("jne __rt_proc_open_cmdline_win");                      // preserve the explicit stderr descriptor
    emitter.instruction("mov ecx, -12");                                        // STD_ERROR_HANDLE
    emitter.instruction("call GetStdHandle");                                   // php-src's inherited child stderr default
    emitter.instruction("test rax, rax");                                       // stderr handle is present?
    emitter.instruction("jz __rt_proc_open_capture_cleanup_win");               // absent stderr cannot be passed through STARTUPINFOW
    emitter.instruction("cmp rax, -1");                                         // stderr handle is not INVALID_HANDLE_VALUE?
    emitter.instruction("je __rt_proc_open_capture_cleanup_win");               // preserve GetLastError before generic cleanup
    emitter.instruction("mov rcx, -1");                                         // source process = current process pseudo-handle
    emitter.instruction("mov rdx, rax");                                        // source stderr handle
    emitter.instruction("mov r8, -1");                                          // target process = current process pseudo-handle
    emitter.instruction("lea r9, [rbp - 160]");                                 // retain the owned inheritable stderr duplicate
    emitter.instruction("mov QWORD PTR [rsp + 32], 0");                         // desired access ignored with DUPLICATE_SAME_ACCESS
    emitter.instruction("mov QWORD PTR [rsp + 40], 1");                         // make the child stderr copy inheritable
    emitter.instruction("mov QWORD PTR [rsp + 48], 2");                         // DUPLICATE_SAME_ACCESS
    emitter.instruction("call DuplicateHandle");                                // duplicate even a non-inheritable host stderr handle
    emitter.instruction("test eax, eax");                                       // stderr duplication succeeded?
    emitter.instruction("jz __rt_proc_open_capture_cleanup_win");               // capture the native failure before closing prior descriptors
    emitter.instruction("mov rax, QWORD PTR [rbp - 160]");                      // reload the child-owned stderr duplicate
    emitter.instruction("mov QWORD PTR [rbp - 392], rax");                      // install the inheritable stderr handle
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // PHP-compatible long-path conversion for the child cwd
    ...
    emitter.instruction("lea r8, [rbp - 384]");                                 // php-src shares inheritable SECURITY_ATTRIBUTES with the process handle
    emitter.instruction("lea r9, [rbp - 384]");                                 // php-src shares inheritable SECURITY_ATTRIBUTES with the initial thread handle
    ...
    emitter.instruction("mov r10, QWORD PTR [rbp - 48]");                       // reload n
    emitter.instruction("cmp r9, r10");                                         // j < n?
    emitter.instruction("jae __rt_proc_open_close_nul_win");                    // all child ends closed -> run the reserved-slot cleanup
    ...
    // -- retain the zeroed legacy cleanup slots without closing inherited std handles --
    emitter.label("__rt_proc_open_close_nul_win");
    emitter.instruction("mov rcx, QWORD PTR [rbp - 144]");                      // reserved stdin cleanup slot
    emitter.instruction("test rcx, rcx");                                       // legacy synthetic stdin handle present?
    emitter.instruction("jz __rt_proc_open_close_nul1_win");                    // inherited stdin must never be closed
    emitter.instruction("call CloseHandle");                                    // retain defensive cleanup for a future owned slot user
    emitter.label("__rt_proc_open_close_nul1_win");
    emitter.instruction("mov rcx, QWORD PTR [rbp - 152]");                      // reserved stdout cleanup slot
    emitter.instruction("test rcx, rcx");                                       // legacy synthetic stdout handle present?
    emitter.instruction("jz __rt_proc_open_close_nul2_win");                    // inherited stdout must never be closed
    emitter.instruction("call CloseHandle");                                    // retain defensive cleanup for a future owned slot user
    emitter.label("__rt_proc_open_close_nul2_win");
    emitter.instruction("mov rcx, QWORD PTR [rbp - 160]");                      // reserved stderr cleanup slot
    emitter.instruction("test rcx, rcx");                                       // legacy synthetic stderr handle present?
    emitter.instruction("jz __rt_proc_open_free_cmdbuf_win");                   // inherited stderr must never be closed
    emitter.instruction("call CloseHandle");                                    // retain defensive cleanup for a future owned slot user
    ...
    emitter.instruction("mov rax, QWORD PTR [rbp - 88]");                       // kind-1 resource low payload = CRT fd or raw SOCKET
    emitter.instruction("mov QWORD PTR [rbp - 72], rax");                       // preserve the raw resource payload for the hash write
    emitter.instruction("mov r9, QWORD PTR [rbp - 64]");                        // reload j after the preceding CRT/Win32 calls
    emitter.instruction("lea r10, [rbp - 784]");                                // base of real descriptor-key bookkeeping
    emitter.instruction("mov rsi, QWORD PTR [r10 + r9 * 8]");                   // hash key low word = actual integer descriptor key
    emitter.instruction("mov rdx, -1");                                         // hash key high word sentinel marks an integer key
    emitter.instruction("mov rdi, QWORD PTR [rbp - 40]");                       // reload the current pipes hash pointer
    emitter.instruction("mov rcx, QWORD PTR [rbp - 72]");                       // resource low payload = CRT fd or raw SOCKET
    emitter.instruction("mov r8, 1");                                           // resource high payload = kind 1 native stream
    emitter.instruction("mov r9, 9");                                           // runtime value tag 9 = resource
    abi::emit_call_label(emitter, "__rt_hash_set");                             // publish the resource at its real descriptor key
    emitter.instruction("mov QWORD PTR [rbp - 40], rax");                       // persist a potentially grown pipes hash
    ...
    // -- cleanup: close every opened child handle and pipe parent endpoint, then reserved slots + cmdbuf --
    ...
    emitter.instruction("jae __rt_proc_open_cleanup_nul_win");                  // all opened pipes closed -> run reserved-slot cleanup
    emitter.instruction("lea rax, [rbp - 352]");                                // base of is_pipe
    emitter.instruction("mov r11, QWORD PTR [rax + r9 * 8]");                   // is_pipe[j]
    ...
    emitter.instruction("mov rcx, QWORD PTR [rbp - 144]");                      // reserved stdin cleanup slot
    emitter.instruction("test rcx, rcx");                                       // legacy synthetic stdin handle present?
    emitter.instruction("jz __rt_proc_open_cleanup_nul1_win");                  // inherited stdin must never be closed
    emitter.instruction("call CloseHandle");                                    // retain defensive cleanup for a future owned slot user
    emitter.label("__rt_proc_open_cleanup_nul1_win");
    emitter.instruction("mov rcx, QWORD PTR [rbp - 152]");                      // reserved stdout cleanup slot
    emitter.instruction("test rcx, rcx");                                       // legacy synthetic stdout handle present?
    emitter.instruction("jz __rt_proc_open_cleanup_nul2_win");                  // inherited stdout must never be closed
    emitter.instruction("call CloseHandle");                                    // retain defensive cleanup for a future owned slot user
    emitter.label("__rt_proc_open_cleanup_nul2_win");
    emitter.instruction("mov rcx, QWORD PTR [rbp - 160]");                      // reserved stderr cleanup slot
    emitter.instruction("test rcx, rcx");                                       // legacy synthetic stderr handle present?
    emitter.instruction("jz __rt_proc_open_cleanup_cmdbuf_win");                // inherited stderr must never be closed
    emitter.instruction("call CloseHandle");                                    // retain defensive cleanup for a future owned slot user
...
mod tests {
    ...
    /// Win32 entry points for the command line, file descriptors, and explicit
    /// `null` descriptors.
    #[test]
    fn windows_proc_open_uses_strict_wide_process_apis() {
        ...
        assert_eq!(asm.matches("call __rt_win_utf8_to_utf16").count(), 1);
        assert_eq!(asm.matches("call __rt_win_utf8_path_to_utf16").count(), 2);
    ...
    fn windows_proc_open_checks_inheritance_and_cleans_every_resource() {
        ...
        assert!(asm.contains("__rt_proc_open_std_defaults_win:"));
        assert!(asm.contains("mov ecx, -10"));
        assert!(asm.contains("mov ecx, -11"));
        assert!(asm.contains("mov ecx, -12"));
        assert!(asm.contains("lea r8, [rbp - 384]"));
        assert!(asm.contains("lea r9, [rbp - 384]"));
        ...
        let defaults = asm
            .split("__rt_proc_open_std_defaults_win:")
            .nth(1)
            .and_then(|tail| tail.split("__rt_proc_open_cmdline_win:").next())
            .expect("Windows default-standard-handle emitter section");
        assert_eq!(
            defaults.matches("call DuplicateHandle").count(),
            3,
            "every inherited standard handle must receive an inheritable child copy",
        );
        for slot in [144, 152, 160] {
            assert!(
                defaults.contains(&format!("lea r9, [rbp - {slot}]")),
                "default standard handle must retain its owned duplicate in slot {slot}",
            );
        }
    ...
    fn windows_proc_open_marshals_direct_commands_and_environment() {
        ...
        let cwd_pointer = asm
            .find("mov QWORD PTR [rbp - 528], r8")
            .expect("cwd pointer must be captured at the runtime boundary");
        let cwd_length = asm
            .find("mov QWORD PTR [rbp - 536], r9")
            .expect("cwd length must be captured at the runtime boundary");
        let fresh_pipes = asm
            .find("call __rt_hash_new")
            .expect("proc_open must replace the by-reference pipes array");
        assert!(
            cwd_pointer < fresh_pipes && cwd_length < fresh_pipes,
            "volatile Windows cwd arguments must be saved before __rt_hash_new",
        );
    }
    ...
    /// process setup, publish resources into the fresh Mixed-valued hash, and
    /// return `$pipes` in the x86_64 secondary result register.
    #[test]
    fn windows_proc_open_preserves_sparse_descriptor_keys_and_pipes_result() {
        ...
        assert!(asm.contains("mov r9, 9"));
        assert!(asm.contains("call __rt_hash_set"));
    ...
    fn windows_proc_open_supports_child_only_descriptor_sources() {
        ...
        let file_descriptor = asm
            .split("__rt_proc_open_file_descriptor_win:")
            .nth(1)
            .and_then(|tail| tail.split("__rt_proc_open_child_handle_ready_win:").next())
            .expect("file descriptor emitter section");
        assert!(file_descriptor.contains("mov r8, 7"));
    ...
    /// Verifies Windows `proc_open(["socket"])` maps standard descriptors to
    /// CRT-compatible pipe endpoints rather than passing raw Winsock handles to
    /// a child `STDIN`/`STDOUT` slot.
    #[test]
    fn windows_proc_open_socket_descriptors_reuse_crt_pipe_endpoints() {
        let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
        emit_proc_open_win32_x86_64(&mut emitter);
        let asm = emitter.output();
        ...
        assert!(socket_descriptor.contains("__rt_proc_open_open_pipe_win"));
        assert!(socket_descriptor.contains("mov QWORD PTR [rbp - 80], 1"));
        assert!(socket_descriptor.contains("mov QWORD PTR [rbp - 80], 0"));
        assert!(!socket_descriptor.contains("call __rt_proc_open_socketpair_win"));
