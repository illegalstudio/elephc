...
fn emit_proc_close_win32_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("movsxd rax, DWORD PTR [rbp - 16]");                    // reload the exit code as a SIGNED 32-bit value: Windows exit
                                                                                // codes span the full DWORD range and php stores the DWORD into
                                                                                // an int, so `cmd /c exit -1` must read back as -1, not 4294967295
