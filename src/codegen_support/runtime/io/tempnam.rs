...
pub fn emit_tempnam(emitter: &mut Emitter) {
    ...
    emitter.instruction("str xzr, [sp, #40]");                                  // no system-temp fallback has been attempted yet

    // -- build template path: dir + "/" + prefix + "XXXXXX" in _cstr_buf --
    emitter.label("__rt_tempnam_build");
    ...
    emitter.emit_call_c("mkstemp");                                                    // mkstemp(template), x0=fd
    emitter.instruction("cmp w0, #0");                                          // mkstemp returns a C int: a 64-bit compare reads -1 as 0xffffffff and misses every failure
    ...
    // php retries in the system temporary directory when the requested one cannot
    // hold the file, and says so with a notice. Without the retry tempnam()
    // returned a path inside the unusable directory for a file it never created.
    emitter.label("__rt_tempnam_fail");
    emitter.instruction("ldr x9, [sp, #40]");                                   // has the system-temp fallback already been attempted?
    emitter.instruction("cbnz x9, __rt_tempnam_final_fail");                    // a failed retry is the terminal failure
    emitter.instruction("mov x9, #1");                                          // permit exactly one fallback attempt
    emitter.instruction("str x9, [sp, #40]");                                   // record that the fallback is now in flight
    emitter.instruction("bl __rt_php_temp_dir");                                // x1 = directory pointer, x2 = directory length
    emitter.instruction("cbz x2, __rt_tempnam_final_fail");                     // an empty temporary directory is unusable
    emitter.instruction("stp x1, x2, [sp, #0]");                                // retry against the system temporary directory
    abi::emit_symbol_address(emitter, "x1", "_tempnam_fallback_notice");        // php's notice explaining the fallback directory
    emitter.instruction(&format!("mov x2, #{}", TEMPNAM_FALLBACK_NOTICE.len())); // exact diagnostic byte length
    emitter.instruction("bl __rt_diag_warning");                                // emit the notice unless @ suppression is active
    emitter.instruction("b __rt_tempnam_build");                                // rebuild the template in the system directory

    emitter.label("__rt_tempnam_final_fail");
...
fn emit_tempnam_linux_x86_64(emitter: &mut Emitter) {
    ...
    // hold the file, and says so with a notice. Without the retry tempnam()
    // returned a path inside the unusable directory for a file it never created.
    let windows = emitter.target.platform == Platform::Windows;
    let empty_target = if windows {
        "__rt_tempnam_release_empty_fallback_x86"
    } else {
        "__rt_tempnam_final_fail_x86"
    };
    ...
    if windows {
        ...
        emitter.instruction("call __rt_php_temp_dir");                          // resolve TMPDIR, then P_tmpdir, as php does
    }
    emitter.instruction("test rax, rax");                                       // did the temp-directory query yield a path?
    emitter.instruction("jz __rt_tempnam_final_fail_x86");                      // a failed lookup leaves tempnam false
    emitter.instruction("test rdx, rdx");                                       // is the returned directory non-empty?
    emitter.instruction(&format!("jz {empty_target}"));                         // an empty temporary directory is unusable
    emitter.instruction("mov QWORD PTR [rbp - 8], rdx");                        // use the system-temp path length for the retry and return value
    emitter.instruction("mov QWORD PTR [rbp - 32], rax");                       // the directory bytes are copied by length, not by terminator
    if windows {
        emitter.instruction("mov QWORD PTR [rbp - 72], rax");                   // retain ownership until the directory bytes are copied
    } else {
        emitter.instruction("mov QWORD PTR [rbp - 72], 0");                     // __rt_php_temp_dir borrows: there is nothing to release
    }
