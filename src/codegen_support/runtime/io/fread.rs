...
/// through `__rt_concat_publish`, sets `_eof_flags[fd]` on EOF, and returns
/// (pointer, byte_count) in x1:x2.
///
...
pub fn emit_fread(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov x0, x14");                                         // allocate only what the stream can still supply
    ...
    emitter.instruction("str x9, [x0, #-8]");                                   // stamp the allocation before publishing it
    emitter.instruction("mov x12, x0");                                         // use the owned block as the read destination
    ...
    emitter.instruction("bl __rt_tls_session_get");                             // resolve the descriptor through the bounded session map
    emitter.instruction("cbz x0, __rt_fread_do_syscall");                       // no TLS attached → fall through to read syscall
    emitter.instruction("ldr x1, [sp, #16]");                                   // buf ptr
    ...
    emitter.instruction("ldr x1, [sp, #16]");                                   // buffer pointer for read
...
fn emit_fread_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov rax, rsi");                                        // request storage for the full requested read length
    ...
    emitter.instruction("mov rax, QWORD PTR [rbp - 24]");                       // return the reserved start pointer in the x86_64 elephc string-pointer result register
