//! Purpose:
//! Emits the `__rt_stream_socket_client` runtime helper assembly for the
...
//!
//! Key details:
//! - Opens a TCP connection: `socket` + `connect` on the parsed
//!   `[tcp://]A.B.C.D:port` address; returns the descriptor or -1.
...
pub fn emit_stream_socket_client(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_stream_socket_client_linux_x86_64(emitter);
        return;
    }
    ...
    //    catches `tcp://[::1]:port`, `udp://[::1]:port`, and bare
    //    `[::1]:port` before the unix:// / udg:// / IPv4 dispatch below.
    emitter.instruction("mov x9, #0");                                          // bracket scan index
    emitter.label("__rt_stream_socket_client_try_v6");
    emitter.instruction("cmp x9, x1");                                          // reached the end of the address?
    emitter.instruction("b.ge __rt_stream_socket_client_after_v6_probe");       // no '[' found → continue with unix:// / IPv4
    emitter.instruction("ldrb w10, [x0, x9]");                                  // load the candidate byte
    emitter.instruction("cmp w10, #91");                                        // ASCII '['
    emitter.instruction("b.eq __rt_stream_socket_client_v6_scheme");            // bracketed host: pick sock_type then tail-call
    emitter.instruction("add x9, x9, #1");                                      // keep scanning
    emitter.instruction("b __rt_stream_socket_client_try_v6");                  // continue

    emitter.label("__rt_stream_socket_client_v6_scheme");
    ...
    emitter.instruction("b.lt __rt_stream_socket_client_v6_dispatch");          // too short for udp:// → keep STREAM
    emitter.instruction("ldrb w10, [x0, #0]");                                  // load scheme byte 0
    emitter.instruction("cmp w10, #117");                                       // 'u'?
    emitter.instruction("b.ne __rt_stream_socket_client_v6_dispatch");          // not udp scheme
    ...
    emitter.label("__rt_stream_socket_client_v6_dispatch");
    emitter.instruction("b __rt_stream_socket_client_v6");                      // tail-call into the IPv6 helper
    emitter.label("__rt_stream_socket_client_after_v6_probe");
    ...
    emitter.instruction("b.lt __rt_stream_socket_client_try_udg");              // too short for unix://: try udg://
    emitter.instruction("ldrb w9, [x0, #0]");                                   // load scheme byte 0
    emitter.instruction("cmp w9, #117");                                        // is it 'u'?
    emitter.instruction("b.ne __rt_stream_socket_client_try_udg");              // not 'u': try udg:// (which also starts with 'u')
    emitter.instruction("ldrb w9, [x0, #1]");                                   // load scheme byte 1
    emitter.instruction("cmp w9, #110");                                        // is it 'n'?
    emitter.instruction("b.ne __rt_stream_socket_client_try_udg");              // not unix scheme: try udg://
    ...
    emitter.instruction("b __rt_unix_socket_client");                           // unix:// address: use the Unix-domain helper

    // -- redirect udg:// (SOCK_DGRAM) addresses to the Unix-domain helper --
    emitter.label("__rt_stream_socket_client_try_udg");
    emitter.instruction("cmp x1, #6");                                          // "udg://" needs at least six bytes
    emitter.instruction("b.lt __rt_stream_socket_client_inet");                 // too short for any Unix scheme
    emitter.instruction("ldrb w9, [x0, #0]");                                   // load scheme byte 0
    emitter.instruction("cmp w9, #117");                                        // is it 'u'?
    emitter.instruction("b.ne __rt_stream_socket_client_inet");                 // not the udg scheme
    emitter.instruction("ldrb w9, [x0, #1]");                                   // load scheme byte 1
    ...
    emitter.instruction("b __rt_unix_socket_client");                           // udg:// address: use the Unix-domain helper
    emitter.label("__rt_stream_socket_client_inet");
    ...
    emitter.instruction("bl __rt_apply_socket_bindto");                         // honor socket.bindto before connect (best-effort)
    ...
    emitter.instruction("ldr x0, [sp, #32]");                                   // load fd before the options helper
    emitter.instruction("bl __rt_apply_socket_client_opts");                    // apply tcp_nodelay etc. (best-effort, preserves nothing in x0)
    emitter.instruction("ldr x0, [sp, #32]");                                   // reload fd as the return value
    emitter.instruction("ldp x29, x30, [sp, #0]");                              // restore frame pointer and return address
    emitter.instruction("add sp, sp, #80");                                     // release the frame
    emitter.instruction("ret");                                                 // return the connected socket

    emitter.label("__rt_stream_socket_client_fail_close");
    emitter.instruction("ldr x0, [sp, #32]");                                   // reload the socket descriptor
    emitter.syscall(6);

    emitter.label("__rt_stream_socket_client_fail");
...
/// Emits the Linux x86_64 stream runtime helper for stream socket client.
fn emit_stream_socket_client_linux_x86_64(emitter: &mut Emitter) {
    emitter.blank();
    emitter.comment("--- runtime: stream_socket_client ---");
    emitter.label_global("__rt_stream_socket_client");
    ...
    emitter.label("__rt_stream_socket_client_try_v6_x86");
    emitter.instruction("cmp rcx, rsi");                                        // reached the end of the address?
    emitter.instruction("jae __rt_stream_socket_client_after_v6_probe_x86");    // no '[' found → continue with unix:// / IPv4
    emitter.instruction("movzx eax, BYTE PTR [rdi + rcx]");                     // load the candidate byte
    emitter.instruction("cmp eax, 91");                                         // ASCII '['
    emitter.instruction("je __rt_stream_socket_client_v6_scheme_x86");          // bracketed host: pick sock_type then tail-call
    emitter.instruction("inc rcx");                                             // keep scanning
    emitter.instruction("jmp __rt_stream_socket_client_try_v6_x86");            // continue

    emitter.label("__rt_stream_socket_client_v6_scheme_x86");
    // -- pick sock_type for the v6 helper: "udp://" prefix → SOCK_DGRAM,
    //    everything else (tcp://, bare bracketed host) → SOCK_STREAM.
    emitter.instruction("mov edx, 1");                                          // default: SOCK_STREAM
    emitter.instruction("cmp rsi, 6");                                          // "udp://" needs at least six bytes
    emitter.instruction("jl __rt_stream_socket_client_v6_dispatch_x86");        // too short for udp:// → keep STREAM
    emitter.instruction("movzx eax, BYTE PTR [rdi + 0]");                       // load scheme byte 0
    emitter.instruction("cmp eax, 117");                                        // 'u'?
    emitter.instruction("jne __rt_stream_socket_client_v6_dispatch_x86");       // not udp scheme
    ...
    emitter.label("__rt_stream_socket_client_v6_dispatch_x86");
    emitter.instruction("jmp __rt_stream_socket_client_v6");                    // tail-call into the IPv6 helper
    emitter.label("__rt_stream_socket_client_after_v6_probe_x86");

    // -- redirect unix:// (SOCK_STREAM) addresses to the Unix-domain helper --
    emitter.instruction("cmp rsi, 7");                                          // "unix://" needs at least seven bytes
    emitter.instruction("jl __rt_stream_socket_client_try_udg_x86");            // too short for unix://: try udg://
    emitter.instruction("movzx eax, BYTE PTR [rdi]");                           // load scheme byte 0
    emitter.instruction("cmp eax, 117");                                        // is it 'u'?
    emitter.instruction("jne __rt_stream_socket_client_try_udg_x86");           // not 'u': try udg://
    emitter.instruction("movzx eax, BYTE PTR [rdi + 1]");                       // load scheme byte 1
    emitter.instruction("cmp eax, 110");                                        // is it 'n'?
    emitter.instruction("jne __rt_stream_socket_client_try_udg_x86");           // not unix scheme: try udg://
    emitter.instruction("movzx eax, BYTE PTR [rdi + 2]");                       // load scheme byte 2
    ...
    emitter.instruction("jmp __rt_unix_socket_client");                         // unix:// address: use the Unix-domain helper

    // -- redirect udg:// (SOCK_DGRAM) addresses to the Unix-domain helper --
    emitter.label("__rt_stream_socket_client_try_udg_x86");
    emitter.instruction("cmp rsi, 6");                                          // "udg://" needs at least six bytes
    emitter.instruction("jl __rt_stream_socket_client_inet_x86");               // too short for any Unix scheme
    emitter.instruction("movzx eax, BYTE PTR [rdi]");                           // load scheme byte 0
    emitter.instruction("cmp eax, 117");                                        // is it 'u'?
    emitter.instruction("jne __rt_stream_socket_client_inet_x86");              // not the udg scheme
    emitter.instruction("movzx eax, BYTE PTR [rdi + 1]");                       // load scheme byte 1
    ...
    emitter.instruction("jmp __rt_unix_socket_client");                         // udg:// address: use the Unix-domain helper
    emitter.label("__rt_stream_socket_client_inet_x86");
    ...
    emitter.instruction("mov rax, QWORD PTR [rbp - 24]");                       // reload fd as the return value
    emitter.instruction("add rsp, 64");                                         // release the frame
    emitter.instruction("pop rbp");                                             // restore the caller frame pointer
    emitter.instruction("ret");                                                 // return the connected socket

    emitter.label("__rt_stream_socket_client_fail_close_x86");
    ...
    emitter.label("__rt_stream_socket_client_fail_x86");
