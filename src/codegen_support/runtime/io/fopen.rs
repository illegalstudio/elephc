...
fn emit_fopen_linux_x86_64(emitter: &mut Emitter) {
    ...
    // The surrounding 64-byte wrapper-dispatch frame is already 16-byte aligned.
    // On MSx64 it doubles as the required 32-byte shadow area plus the three
    // stack arguments; its scratch values are all consumed before the slots are
    // overwritten. The generic callback adapter intentionally stops at six
    // source registers, so this seven-argument site owns its explicit staging.
    ...
    if emitter.target.platform == Platform::Windows {
        emitter.instruction("mov QWORD PTR [rsp + 32], r8");                    // MSx64 arg 5 = mode length after consuming the object scratch slot
        emitter.instruction("mov QWORD PTR [rsp + 40], r9");                    // MSx64 arg 6 = options after consuming the handle-index scratch slot
        emitter.instruction("mov QWORD PTR [rsp + 48], r10");                   // MSx64 arg 7 = opened_path after consuming the method-pointer scratch slot
        emitter.remap_sysv_args_to_platform_for_callback(4);                     // map obj/path/mode SysV registers to MSx64 registers without collisions
        emitter.instruction("call r11");                                        // bespoke seven-argument generated callback; stack/shadow space is staged above
    } else {
        emitter.instruction("sub rsp, 16");                                     // reserve a 16-byte stack-arg slot for the 7th SysV int arg
        emitter.instruction("mov QWORD PTR [rsp], r10");                        // 7th SysV arg (opened_path address) at [rsp+0]
        emitter.emit_platform_callback_call("r11", 6);                           // first six generated-call args; the seventh is already SysV stack-staged
...
mod tests {
    ...
    fn test_windows_x86_64_fopen_stream_open_stages_stack_args_before_call() {
        ...
        let this_idx = asm.find("mov rcx, rdi").expect("expected MSx64 $this remap");
        ...
        assert!(asm.contains("mov QWORD PTR [rsp + 32], r8"), "expected the MSx64 fifth-argument stack store");
        assert_eq!(
            asm.matches("add rsp, 64").count(),
            2,
            "expected the two shared wrapper-dispatch releases"
        );
    }
