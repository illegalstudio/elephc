...
pub fn emit_https(emitter: &mut Emitter) {
    ...
    // Frame (320 bytes): response bookkeeping at 0..40, the stable 184-byte
    // ElephcTlsClientOptions v6 at 48..231, saved connection inputs at
    // 240..272, scalar scratch at 280, I/O progress/failure state at 288,
    // and the saved frame pair at 304..312.
    emitter.instruction("sub sp, sp, #320");                                    // allocate response, TLS v6 options, and connection spill storage
    emitter.instruction("stp x29, x30, [sp, #304]");                            // save frame pointer and return address
    emitter.instruction("add x29, sp, #304");                                   // establish the helper frame pointer
    emitter.instruction("str x3, [sp, #264]");                                  // save the HTTP request pointer
    emitter.instruction("str x4, [sp, #272]");                                  // save the HTTP request length
    emitter.instruction("str x0, [sp, #240]");                                  // save the connection host pointer
    emitter.instruction("str x1, [sp, #248]");                                  // save the connection host length
    emitter.instruction("str x2, [sp, #256]");                                  // save the connection port

    // -- initialize ElephcTlsClientOptions v6 (184 bytes at sp + 48) --
    for offset in (48..=224).step_by(8) {
        emitter.instruction(&format!("str xzr, [sp, #{offset}]"));              // zero one TLS options word
    }
    emitter.instruction("mov w9, #6");                                          // select ElephcTlsClientOptions ABI v6
    ...
    for (key, key_len, ptr_offset, len_offset) in [
        ...
        ("_ssl_alpn_protocols_key_str", 14, 136, 144),
        ("_ssl_passphrase_key_str", 10, 176, 184),
        ("_ssl_ciphers_key_str", 7, 192, 200),
    ] {
    ...
    // -- peer_fingerprint accepts PHP's scalar or associative-array forms --
    emitter.instruction("add x0, sp, #152");                                    // pass options.peer_fingerprint_ptr
    emitter.instruction("add x1, sp, #160");                                    // pass options.peer_fingerprint_len
    emitter.instruction("bl __rt_get_ssl_peer_fingerprint");                    // serialize the configured fingerprint policy

    // -- resolve the SNI and session-ticket switches into options.option_flags --
    emitter.instruction("mov x9, #1");                                          // SNI is enabled unless PHP explicitly disables it
    emitter.instruction("str x9, [sp, #280]");                                  // seed the scalar lookup output
    abi::emit_symbol_address(emitter, "x0", "_ssl_key_str");
    emitter.instruction("mov x1, #3");                                          // pass strlen("ssl")
    abi::emit_symbol_address(emitter, "x2", "_ssl_sni_enabled_key_str");
    emitter.instruction("mov x3, #11");                                         // pass strlen("SNI_enabled")
    emitter.instruction("add x4, sp, #280");                                    // pass the normalized boolean destination
    emitter.instruction("bl __rt_get_bool_context_option");                     // apply PHP scalar truthiness
    emitter.instruction("ldr x9, [sp, #280]");                                  // reload the normalized SNI switch
    emitter.instruction("cbnz x9, __rt_https_sni_done");                        // keep the zero-initialized flags when enabled
    emitter.instruction("ldr w10, [sp, #168]");                                 // load options.option_flags
    emitter.instruction("orr w10, w10, #1");                                    // set TLS_OPTION_SNI_DISABLED
    emitter.instruction("str w10, [sp, #168]");                                 // persist the SNI policy
    emitter.label("__rt_https_sni_done");
    emitter.instruction("str xzr, [sp, #280]");                                 // no_ticket defaults to false
    abi::emit_symbol_address(emitter, "x0", "_ssl_key_str");
    emitter.instruction("mov x1, #3");                                          // pass strlen("ssl")
    abi::emit_symbol_address(emitter, "x2", "_ssl_no_ticket_key_str");
    emitter.instruction("mov x3, #9");                                          // pass strlen("no_ticket")
    emitter.instruction("add x4, sp, #280");                                    // pass the normalized boolean destination
    emitter.instruction("bl __rt_get_bool_context_option");                     // apply PHP scalar truthiness
    emitter.instruction("ldr x9, [sp, #280]");                                  // reload the normalized ticket switch
    emitter.instruction("cbz x9, __rt_https_no_ticket_done");                   // leave tickets enabled by default
    emitter.instruction("ldr w10, [sp, #168]");                                 // load options.option_flags
    emitter.instruction("orr w10, w10, #2");                                    // set TLS_OPTION_NO_TICKET
    emitter.instruction("str w10, [sp, #168]");                                 // persist the ticket policy
    emitter.label("__rt_https_no_ticket_done");

    // -- resolve security_level with explicit presence (v6 offset 168) --
    abi::emit_symbol_address(emitter, "x0", "_ssl_key_str");
    emitter.instruction("mov x1, #3");                                          // pass strlen("ssl")
    abi::emit_symbol_address(emitter, "x2", "_ssl_security_level_key_str");
    emitter.instruction("mov x3, #14");                                         // pass strlen("security_level")
    emitter.instruction("add x4, sp, #208");                                    // pass the security_level value address
    emitter.instruction("bl __rt_get_int_context_option");                      // resolve integer/bool security level
    emitter.instruction("str w0, [sp, #216]");                                  // retain explicit security_level presence

    // -- resolve disable_compression with explicit presence (v6 offset 176) --
    abi::emit_symbol_address(emitter, "x0", "_ssl_key_str");
    emitter.instruction("mov x1, #3");                                          // pass strlen("ssl")
    abi::emit_symbol_address(emitter, "x2", "_ssl_disable_compression_key_str");
    emitter.instruction("mov x3, #19");                                         // pass strlen("disable_compression")
    emitter.instruction("add x4, sp, #220");                                    // pass the disable_compression value address
    emitter.instruction("bl __rt_get_bool_context_option");                     // resolve PHP scalar truthiness
    emitter.instruction("str w0, [sp, #224]");                                  // retain explicit disable_compression presence
    ...
    emitter.instruction("add x4, sp, #280");                                    // pass the verify_peer output address
    emitter.instruction("bl __rt_get_bool_context_option");                     // accept bool/int/string PHP truthiness
    emitter.instruction("ldr w9, [sp, #280]");                                  // load the normalized verify_peer value
    ...
    emitter.instruction("add x4, sp, #280");                                    // pass the verify_peer_name output address
    emitter.instruction("bl __rt_get_bool_context_option");                     // accept bool/int/string PHP truthiness
    emitter.instruction("ldr w9, [sp, #280]");                                  // load the normalized verify_peer_name value
    emitter.instruction("and w9, w9, #1");                                      // constrain the policy contribution to one bit
    ...
    emitter.instruction("str xzr, [sp, #280]");                                 // default allow_self_signed to false
    abi::emit_symbol_address(emitter, "x0", "_ssl_key_str");
    emitter.instruction("mov x1, #3");                                          // pass strlen("ssl")
    ...
    emitter.instruction("add x4, sp, #280");                                    // pass the allow_self_signed output address
    emitter.instruction("bl __rt_get_bool_context_option");                     // accept bool/int/string PHP truthiness
    emitter.instruction("ldr w9, [sp, #280]");                                  // load the normalized allow_self_signed value
    emitter.instruction("and w9, w9, #1");                                      // constrain the policy contribution to one bit
    ...
    emitter.instruction("ldr x0, [sp, #240]");                                  // restore the connection host pointer
    emitter.instruction("ldr x1, [sp, #248]");                                  // restore the connection host length
    emitter.instruction("ldr x2, [sp, #256]");                                  // restore the connection port
    ...
    emitter.instruction("str xzr, [sp, #288]");                                 // request write offset starts at zero
    emitter.label("__rt_https_open_write");
    emitter.instruction("ldr x9, [sp, #288]");                                  // reload bytes already sent through TLS
    emitter.instruction("ldr x10, [sp, #272]");                                 // reload the full request length
    ...
    emitter.instruction("ldr x1, [sp, #264]");                                  // reload the HTTP request pointer
    ...
    emitter.instruction("ldr x9, [sp, #288]");                                  // reload the completed write offset
    emitter.instruction("add x9, x9, x0");                                      // account for this partial TLS write
    emitter.instruction("str x9, [sp, #288]");                                  // retain progress for the next write iteration
    ...
    emitter.instruction("str xzr, [sp, #288]");                                 // mark clean TLS completion before closing the session
    ...
    emitter.instruction("str x9, [sp, #288]");                                  // preserve failure state across the TLS close call
    ...
    emitter.instruction("ldr x9, [sp, #288]");                                  // recover the pre-close I/O status
    ...
    emitter.instruction("str xzr, [sp, #288]");                                 // temporary-file write offset starts at zero
    emitter.label("__rt_https_open_body_write");
    emitter.instruction("ldr x9, [sp, #288]");                                  // reload bytes already copied to the temporary file
    ...
    emitter.instruction("ldr x9, [sp, #288]");                                  // reload the copied-byte count
    emitter.instruction("add x9, x9, x0");                                      // account for this partial tempfile write
    emitter.instruction("str x9, [sp, #288]");                                  // retain progress for the next copy iteration
    ...
    emitter.instruction("add sp, sp, #320");                                    // release the helper frame
...
fn emit_https_linux_x86_64(emitter: &mut Emitter) {
    ...
    // 184-byte ElephcTlsClientOptions v6 at -272..-89, and scalar scratch at -280.
    emitter.instruction("push rbp");                                            // preserve the caller frame pointer
    emitter.instruction("mov rbp, rsp");                                        // establish the helper frame pointer
    emitter.instruction("sub rsp, 288");                                        // reserve aligned response, TLS v6 options, and scalar scratch storage
    ...
    // -- initialize ElephcTlsClientOptions v6 (184 bytes at rbp - 272) --
    for offset in (96..=272).step_by(8) {
        emitter.instruction(&format!("mov QWORD PTR [rbp - {offset}], 0"));     // zero one TLS options word
    }
    emitter.instruction("mov DWORD PTR [rbp - 272], 6");                        // store options.abi_version at byte offset 0
    emitter.instruction("mov DWORD PTR [rbp - 268], 3");                        // default policy verifies the peer and its name

    // -- collect all combinable string options directly into the ABI struct --
    for (key, key_len, ptr_offset, len_offset) in [
        ("_ssl_peer_name_key_str", 9, 264, 256),
        ("_ssl_cafile_key_str", 6, 248, 240),
        ("_ssl_capath_key_str", 6, 232, 224),
        ("_ssl_local_cert_key_str", 10, 216, 208),
        ("_ssl_local_pk_key_str", 8, 200, 192),
        ("_ssl_alpn_protocols_key_str", 14, 184, 176),
        ("_ssl_passphrase_key_str", 10, 144, 136),
        ("_ssl_ciphers_key_str", 7, 128, 120),
    ] {
    ...
    // -- peer_fingerprint accepts PHP's scalar or associative-array forms --
    emitter.instruction("lea rdi, [rbp - 168]");                                // pass options.peer_fingerprint_ptr
    emitter.instruction("lea rsi, [rbp - 160]");                                // pass options.peer_fingerprint_len
    emitter.instruction("call __rt_get_ssl_peer_fingerprint");                  // serialize the configured fingerprint policy

    // -- resolve the SNI and session-ticket switches into options.option_flags --
    emitter.instruction("mov QWORD PTR [rbp - 280], 1");                        // SNI is enabled unless PHP explicitly disables it
    abi::emit_symbol_address(emitter, "rdi", "_ssl_key_str");
    emitter.instruction("mov rsi, 3");                                          // pass strlen("ssl")
    abi::emit_symbol_address(emitter, "rdx", "_ssl_sni_enabled_key_str");
    emitter.instruction("mov rcx, 11");                                         // pass strlen("SNI_enabled")
    emitter.instruction("lea r8, [rbp - 280]");                                 // pass the normalized boolean destination
    emitter.instruction("call __rt_get_bool_context_option");                   // apply PHP scalar truthiness
    emitter.instruction("cmp QWORD PTR [rbp - 280], 0");                        // is SNI enabled?
    emitter.instruction("jne __rt_https_sni_done_x86");                         // keep the zero-initialized flags when enabled
    emitter.instruction("or DWORD PTR [rbp - 152], 1");                         // set TLS_OPTION_SNI_DISABLED
    emitter.label("__rt_https_sni_done_x86");
    emitter.instruction("mov QWORD PTR [rbp - 280], 0");                        // no_ticket defaults to false
    abi::emit_symbol_address(emitter, "rdi", "_ssl_key_str");
    emitter.instruction("mov rsi, 3");                                          // pass strlen("ssl")
    abi::emit_symbol_address(emitter, "rdx", "_ssl_no_ticket_key_str");
    emitter.instruction("mov rcx, 9");                                          // pass strlen("no_ticket")
    emitter.instruction("lea r8, [rbp - 280]");                                 // pass the normalized boolean destination
    emitter.instruction("call __rt_get_bool_context_option");                   // apply PHP scalar truthiness
    emitter.instruction("cmp QWORD PTR [rbp - 280], 0");                        // did PHP disable tickets?
    emitter.instruction("je __rt_https_no_ticket_done_x86");                    // leave tickets enabled by default
    emitter.instruction("or DWORD PTR [rbp - 152], 2");                         // set TLS_OPTION_NO_TICKET
    emitter.label("__rt_https_no_ticket_done_x86");

    // -- resolve security_level and disable_compression with presence bits --
    abi::emit_symbol_address(emitter, "rdi", "_ssl_key_str");
    emitter.instruction("mov rsi, 3");                                          // pass strlen("ssl")
    abi::emit_symbol_address(emitter, "rdx", "_ssl_security_level_key_str");
    emitter.instruction("mov rcx, 14");                                         // pass strlen("security_level")
    emitter.instruction("lea r8, [rbp - 112]");                                 // pass options.security_level
    emitter.instruction("call __rt_get_int_context_option");                    // resolve the optional integer policy
    emitter.instruction("mov DWORD PTR [rbp - 104], eax");                      // retain explicit security_level presence
    abi::emit_symbol_address(emitter, "rdi", "_ssl_key_str");
    emitter.instruction("mov rsi, 3");                                          // pass strlen("ssl")
    abi::emit_symbol_address(emitter, "rdx", "_ssl_disable_compression_key_str");
    emitter.instruction("mov rcx, 19");                                         // pass strlen("disable_compression")
    emitter.instruction("lea r8, [rbp - 100]");                                 // pass options.disable_compression
    emitter.instruction("call __rt_get_bool_context_option");                   // resolve PHP scalar truthiness
    emitter.instruction("mov DWORD PTR [rbp - 96], eax");                       // retain explicit compression-option presence

    // -- resolve verify_peer with PHP scalar truthiness (default true) --
    emitter.instruction("mov QWORD PTR [rbp - 280], 1");                        // default verify_peer to true for TLS clients
    abi::emit_symbol_address(emitter, "rdi", "_ssl_key_str");                   // pass the TLS context wrapper name
    emitter.instruction("mov rsi, 3");                                          // pass strlen("ssl")
    ...
    emitter.instruction("lea r8, [rbp - 280]");                                 // pass the verify_peer output address
    emitter.instruction("call __rt_get_bool_context_option");                   // accept bool/int/string PHP truthiness
    emitter.instruction("mov r10d, DWORD PTR [rbp - 280]");                     // load the normalized verify_peer value
    emitter.instruction("and r10d, 1");                                         // constrain the policy contribution to one bit
    emitter.instruction("mov eax, DWORD PTR [rbp - 268]");                      // load the current TLS policy bitmask
    ...
    emitter.instruction("mov DWORD PTR [rbp - 268], eax");                      // persist the updated TLS policy

    // -- resolve verify_peer_name independently (default true) --
    emitter.instruction("mov QWORD PTR [rbp - 280], 1");                        // default verify_peer_name to true for TLS clients
    abi::emit_symbol_address(emitter, "rdi", "_ssl_key_str");                   // pass the TLS context wrapper name
    emitter.instruction("mov rsi, 3");                                          // pass strlen("ssl")
    ...
    emitter.instruction("lea r8, [rbp - 280]");                                 // pass the verify_peer_name output address
    emitter.instruction("call __rt_get_bool_context_option");                   // accept bool/int/string PHP truthiness
    emitter.instruction("mov r10d, DWORD PTR [rbp - 280]");                     // load the normalized verify_peer_name value
    emitter.instruction("and r10d, 1");                                         // constrain the policy contribution to one bit
    ...
    // -- resolve allow_self_signed independently (default false) --
    emitter.instruction("mov QWORD PTR [rbp - 280], 0");                        // default allow_self_signed to false
    abi::emit_symbol_address(emitter, "rdi", "_ssl_key_str");                   // pass the TLS context wrapper name
    emitter.instruction("mov rsi, 3");                                          // pass strlen("ssl")
    ...
    emitter.instruction("lea r8, [rbp - 280]");                                 // pass the allow_self_signed output address
    emitter.instruction("call __rt_get_bool_context_option");                   // accept bool/int/string PHP truthiness
    emitter.instruction("mov r10d, DWORD PTR [rbp - 280]");                     // load the normalized allow_self_signed value
    emitter.instruction("and r10d, 1");                                         // constrain the policy contribution to one bit
    ...
    emitter.instruction("lea rcx, [rbp - 272]");                                // arg 3 = stable TLS options struct address
    ...
    emitter.instruction("add rsp, 288");                                        // release the helper frame
...
mod tests {
    ...
    fn https_builds_policy_options_without_legacy_dispatch() {
        for target in [
            ...
            assert_eq!(asm.matches("__rt_get_bool_context_option").count(), 6);
            assert_eq!(asm.matches("__rt_get_int_context_option").count(), 1);
            assert_eq!(asm.matches("__rt_get_string_context_option").count(), 8);
            assert_eq!(asm.matches("__rt_get_ssl_peer_fingerprint").count(), 1);
            for key in [
    ...
    /// Pins every byte offset in the append-only 184-byte TLS client options ABI v6.
    #[test]
    fn https_tls_options_offsets_match_bridge_abi() {
        ...
        for offset in [
            48, 52, 56, 64, 72, 80, 88, 96, 104, 112, 120, 128, 136, 144, 152,
            160, 168, 176, 184, 192, 200, 208, 216, 224,
        ] {
            assert!(
                arm_asm.contains(&format!("[sp, #{offset}]")),
                "missing AArch64 TLS ABI offset {offset}"
            );
        }
        assert!(arm_asm.contains("add x4, sp, #220"));

        let mut x86 = Emitter::new(Target::new(Platform::Linux, Arch::X86_64));
        emit_https(&mut x86);
        let x86_asm = x86.output();
        for offset in [
            272, 268, 264, 256, 248, 240, 232, 224, 216, 208, 200, 192, 184,
            176, 168, 160, 152, 144, 136, 128, 120, 112, 104, 100, 96,
        ] {
            assert!(
                x86_asm.contains(&format!("[rbp - {offset}]")),
                "missing x86_64 TLS ABI offset {offset}"
            );
        }
    }
