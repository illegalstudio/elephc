...
pub(crate) fn emit_json_encode_bool(emitter: &mut Emitter) {
    ...
    // -- false --
    abi::emit_symbol_address(emitter, "x1", "_json_false");                     // load page of "false" string
    emitter.instruction("mov x2, #5");                                          // length of "false"
    emitter.instruction("ret");                                                 // return

    // -- true --
    emitter.label("__rt_json_encode_true");
    abi::emit_symbol_address(emitter, "x1", "_json_true");                      // load page of "true" string
...
fn emit_json_encode_bool_linux_x86_64(emitter: &mut Emitter) {
    ...
    abi::emit_symbol_address(emitter, "rax", "_json_false");                    // materialize the address of the static JSON false literal
    ...
    emitter.label("__rt_json_encode_true");
    abi::emit_symbol_address(emitter, "rax", "_json_true");                     // materialize the address of the static JSON true literal
