...
//!
//! Called from:
//! - `crate::codegen_support::runtime::system::date::emit_date()` for Linux x86_64 targets.
//!
//! Key details:
...
pub(super) fn emit_date_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("cmp al, 88");                                          // check whether the current token is 'X' (expanded year, always signed)
    emitter.instruction("je __rt_date_fmt_X_linux_x86_64");                     // handle the always-signed expanded-year token
    // x = expanded year (signed only for year < 0 or year >= 10000)
    emitter.instruction("cmp al, 120");                                         // check whether the current token is 'x' (expanded year, signed when out of range)
    emitter.instruction("je __rt_date_fmt_x_linux_x86_64");                     // handle the signed-when-out-of-range expanded-year token
    ...
    // -- helper: write an expanded ISO-8601 year (X/x) --
    //
    // Input: rax = year (signed), r11d = force_sign (1 => always sign; 0 => sign only for year<0 or year>=10000).
    // Emits a sign then the magnitude with at least 4 digits, reusing write_4digit (<=9999)
    // or write_int64 (>=10000). Clobbers caller-saved registers like the other writers.
    emitter.label("__rt_date_write_year_expanded_linux_x86_64");
    emitter.instruction("cmp rax, 0");                                          // is the year negative?
    emitter.instruction("jge __rt_date_yexp_nonneg_linux_x86_64");              // non-negative year keeps its magnitude
    emitter.instruction("mov r8, QWORD PTR [rbp - 40]");                        // reload the live output cursor
    emitter.instruction("mov BYTE PTR [r8], 45");                               // write the '-' sign for a negative year
    emitter.instruction("add r8, 1");                                           // advance the live output cursor
    emitter.instruction("mov QWORD PTR [rbp - 40], r8");                        // publish the advanced output cursor
    emitter.instruction("neg rax");                                             // magnitude = -year
    emitter.instruction("jmp __rt_date_yexp_mag_linux_x86_64");                 // emit the magnitude digits
    emitter.label("__rt_date_yexp_nonneg_linux_x86_64");
    emitter.instruction("test r11d, r11d");                                     // is force_sign set (X mode)?
    emitter.instruction("jne __rt_date_yexp_plus_linux_x86_64");                // X always prints '+'
    emitter.instruction("cmp rax, 9999");                                       // x mode: year within the unsigned 4-digit range?
    emitter.instruction("jle __rt_date_yexp_mag_linux_x86_64");                 // year in [0,9999] prints no sign
    emitter.label("__rt_date_yexp_plus_linux_x86_64");
    emitter.instruction("mov r8, QWORD PTR [rbp - 40]");                        // reload the live output cursor
    emitter.instruction("mov BYTE PTR [r8], 43");                               // write the '+' sign for an expanded non-negative year
    emitter.instruction("add r8, 1");                                           // advance the live output cursor
    emitter.instruction("mov QWORD PTR [rbp - 40], r8");                        // publish the advanced output cursor
    emitter.label("__rt_date_yexp_mag_linux_x86_64");
    emitter.instruction("cmp rax, 9999");                                       // does the magnitude fit in 4 digits?
    emitter.instruction("jle __rt_date_yexp_4_linux_x86_64");                   // <=9999 uses the 4-digit writer
    emitter.instruction("call __rt_date_write_int64_linux_x86_64");             // >=10000 writes the full decimal magnitude
    emitter.instruction("ret");                                                 // return to the format loop
    emitter.label("__rt_date_yexp_4_linux_x86_64");
    emitter.instruction("call __rt_date_write_4digit_linux_x86_64");            // write the zero-padded 4-digit magnitude
    emitter.instruction("ret");                                                 // return to the format loop
    ...
    emitter.label("__rt_date_fmt_X_linux_x86_64");
    emitter.instruction("mov r8, QWORD PTR [rbp - 32]");                        // reload the struct tm pointer before reading the year field
    emitter.instruction("movsxd rax, DWORD PTR [r8 + 20]");                     // load and sign-extend tm_year for full-range years
    emitter.instruction("add rax, 1900");                                       // convert the libc year-since-1900 encoding into a full year
    emitter.instruction("mov r11d, 1");                                         // force_sign = 1 (X always prints a sign)
    emitter.instruction("call __rt_date_write_year_expanded_linux_x86_64");     // write the signed expanded year
    emitter.instruction("jmp __rt_date_next_linux_x86_64");                     // continue with the next format byte
    ...
    emitter.instruction("movsxd rax, DWORD PTR [r8 + 20]");                     // load and sign-extend tm_year for full-range years
    emitter.instruction("add rax, 1900");                                       // convert the libc year-since-1900 encoding into a full year
    emitter.instruction("mov r11d, 0");                                         // force_sign = 0 (x omits the sign for [0,9999])
    emitter.instruction("call __rt_date_write_year_expanded_linux_x86_64");     // write the expanded year
    emitter.instruction("jmp __rt_date_next_linux_x86_64");                     // continue with the next format byte
