...
typedef struct { uint64_t count, capacity, value_type, head, tail; Entry entries[4]; } Hash;
...
static void initialize_graph(void) {
    ...
    graph_hash.entries[3] = (Entry){1, PTR("42"), 2, UINT64_C(0x7ff8000000000042), 0, 2, UINT64_MAX, 0};
    graph_hash.entries[0] = (Entry){1, 42, UINT64_MAX, 0, 0, 4, 3, 2};
    graph_hash.entries[2] = (Entry){1, PTR(binary_key), 2, PTR(&graph_indexed.length), 0, 4, 0, UINT64_MAX};
}
