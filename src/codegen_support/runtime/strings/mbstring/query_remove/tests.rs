//! Purpose:
//! Executes query root deletion with independent native ownership and destructor observations.
//!
//! Called from:
...
/// Verifies unlink-before-destruction, aliases, reentrant writes, nested owners, and pending cleanup.
#[test]
fn native_mbstring_query_remove_preserves_completed_mutations() {
    let nonce = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos();
    let directory = std::env::temp_dir().join(format!("mbstring-query-remove-{}-{nonce}", std::process::id()));
    ...
    std::fs::write(directory.join("fixture.c"), include_str!("native_remove.c")).unwrap();
    ...
    assert!(built.status.success(), "query removal fixture build failed: {}", String::from_utf8_lossy(&built.stderr));
    let result = Command::new(&executable).output().unwrap();
    assert!(result.status.success(), "query removal fixture {} failed: {}", directory.display(), String::from_utf8_lossy(&result.stderr));
    std::fs::remove_dir_all(directory).unwrap();
}

/// Checks actual instruction encodings and relocations for every supported query-removal target.
#[test]
#[ignore = "requires clang with ELF and Apple AArch64 assembler support"]
fn mbstring_query_remove_assembles_on_all_supported_targets() {
    let nonce = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos();
    let directory = std::env::temp_dir().join(format!("mbstring-query-remove-targets-{}-{nonce}", std::process::id()));
    std::fs::create_dir(&directory).unwrap();
    for (name, triple) in [
        ...
        eprintln!("query removal assembled: {name}");
    }
    std::fs::remove_dir_all(directory).unwrap();
}
