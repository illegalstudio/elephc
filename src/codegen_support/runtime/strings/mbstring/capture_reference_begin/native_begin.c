...
struct reference { uint64_t child, reserved; };
...
struct result { uint64_t ready; void *writer, *discarded; };
...
static struct value *replacement;
...
extern int begin(void *, struct reference *, uint64_t, struct result *) __asm__("__rt_mbstring_capture_reference_begin");
...
static struct reference *reference_owned(struct value *child) {
    struct reference *cell = fixture_allocate(sizeof(*cell));
    header(cell)->kind = HEAP_MAGIC | (7u << 8) | 8u;
    *cell = (struct reference){(uintptr_t)child, 0};
    return cell;
}
...
static void destruct_old(void) {
    ...
    if (mode) {
        struct hash *hash = array_value((void *)(uintptr_t)reference->child);
        CHECK(hash->length == 0 && header(hash)->refs == 1);
        if (actions & SNAPSHOT) { snapshot = fixture_retain(hash); }
    } else { CHECK(reference->child == 0); }
    if (actions & RECURSE) {
        ...
        CHECK(array_value((void *)(uintptr_t)reference->child)->length == 0);
        fixture_release(nested.writer);
    }
    if (actions & REPLACE) {
        struct value *old = (void *)(uintptr_t)reference->child;
        replacement = replacement_value();
        reference->child = (uintptr_t)replacement;
        fixture_release(old);
    }
...
void fixture_release(void *value) {
    ...
    if (kind == 3) {
        ...
        struct reference *cell = value;
        fixture_release((void *)(uintptr_t)cell->child);
...
static void publication_case(uint64_t typed, unsigned script) {
    ...
    snapshot = NULL;
    replacement = NULL;
    reference = reference_owned(box_owned(6, object(0), 0));
    ...
    struct value *current = (void *)(uintptr_t)reference->child;
...
static void alias_case(uint64_t typed, int share_box) {
    ...
    struct reference *root = reference_owned(old_box);
    struct result output;
    ...
    CHECK(array_value((void *)(uintptr_t)root->child)->length == 0);
    fixture_release(output.writer);
...
static void invalid_cases(void) {
    struct result output;
    struct value *ordinary = box_owned(0, NULL, 0);
    size_t before = allocated;
    memset(&output, 0xa5, sizeof(output));
    CHECK(begin(NULL, (struct reference *)ordinary, 0, &output) == 1);
    CHECK(output.ready == 0 && output.writer == NULL && output.discarded == NULL);
    CHECK(allocated == before && ordinary->tag == 0);
    fixture_release(ordinary);
    struct reference *root = reference_owned(box_owned(0, NULL, 0));
    uint64_t child = root->child;
    before = allocated;
    ...
    for (uint64_t i = 2; i < 4; ++i) {
        ...
        CHECK(output.ready == 0 && output.writer == NULL && output.discarded == NULL);
        CHECK(allocated == before && root->child == child && header(root)->refs == 1);
    }
