...
pub fn emit_ftoa(emitter: &mut Emitter) {
    ...
    emitter.emit_call_c("snprintf");                                            // format x at 14 significant digits through the target-aware C ABI
    ...
    emitter.emit_call_c("snprintf");                                            // format the decimal digits through the target-aware C ABI
...
/// Backward-compatible emitter entry point for the serialize-precision formatter.
///
/// Runtime assembly now exports `__rt_var_dump_ftoa`; callers retained this Rust
/// name while the var-dump-specific label was introduced, so both routes must emit
/// the same helper exactly once.
pub fn emit_ftoa_repr(emitter: &mut Emitter) {
    emit_var_dump_ftoa(emitter);
}
...
mod tests {
    ...
    /// Windows invokes C formatters through MSx64 shims. The shared x86 emitter still
    /// stages its internal calls with the SysV register convention, so raw imports
    /// would corrupt `%.*e` and exponent parsing on PE targets.
    #[test]
    fn test_emit_ftoa_windows_x86_64_routes_c_calls_through_shims() {
        let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
        emit_ftoa(&mut emitter);
        let asm = emitter.output();

        assert!(asm.contains("call __rt_sys_snprintf\n"));
        assert!(asm.contains("call __rt_sys_strtol\n"));
        assert!(!asm.contains("call snprintf\n"));
        assert!(!asm.contains("call strtol\n"));
    }

    /// Verifies that both targets use the complete php-src `zend_gcvt` route: a
    /// fixed 14-significant-digit scientific probe, trailing-zero suppression, and
    /// an explicit decimal/exponential split rather than CRT `%G` thresholds.
    #[test]
    fn test_emit_ftoa_applies_php_gcvt_fixups() {
        for arch in [Arch::AArch64, Arch::X86_64] {
            ...
            emit_ftoa(&mut emitter);
            let asm = emitter.output();
            assert!(asm.contains("__rt_ftoa_trim"), "missing trailing-zero trim for {:?}", arch);
            assert!(asm.contains("__rt_ftoa_exp"), "missing exponential layout for {:?}", arch);
            assert!(asm.contains("__rt_ftoa_nonfinite"), "missing non-finite classifier for {:?}", arch);
        }
    }

    /// Verifies that `__rt_var_dump_ftoa` delegates finite values to the shared
    ...
    fn test_emit_ftoa_repr_delegates_to_json_ftoa() {
        for arch in [Arch::AArch64, Arch::X86_64] {
            let mut emitter = Emitter::new(Target::new(Platform::Linux, arch));
            emit_var_dump_ftoa(&mut emitter);
            let asm = emitter.output();
            assert!(asm.contains("__rt_var_dump_ftoa:\n"), "missing entry point for {:?}", arch);
