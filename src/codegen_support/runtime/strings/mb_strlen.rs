...
fn emit_mb_strlen_aarch64(emitter: &mut Emitter) {
    ...
    emitter.emit_call_c("iconv_open"); // create the encoding-to-UTF-32LE conversion descriptor
    ...
    emitter.emit_call_c("iconv"); // decode as many complete characters as fit in the fixed output scratch
    ...
    emitter.emit_call_c("iconv"); // reset stateful decoders after substituting one malformed byte
    ...
    emitter.emit_call_c("iconv_close"); // release the conversion descriptor before returning
