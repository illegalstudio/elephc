...
fn test_win32_imports_declared() {
    ...
    assert!(asm.contains(".extern GetACP"));
    assert!(asm.contains(".extern RtlGetVersion"));
    assert!(asm.contains(".extern SetConsoleCP"));
    assert!(asm.contains(".extern SetConsoleOutputCP"));
    assert!(asm.contains(".extern SetConsoleMode"));
    ...
    assert!(asm.contains(".extern _putenv_s"));
...
/// Verifies POSIX `unsetenv` lowers to the Windows CRT removal operation rather
/// than leaving an undefined MinGW `unsetenv` reference in the PE link.
#[test]
fn test_windows_unsetenv_uses_putenv_s_shim() {
    let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
    emit_win32_shims(&mut emitter, RuntimeFeatures::all());
    let asm = emitter.output();
    let section = shim_section(&asm, "__rt_sys_unsetenv");
    assert!(section.contains("mov rcx, rdi"));
    assert!(section.contains("lea rdx, [rip + .Lelephc_empty_env_value]"));
    assert!(section.contains("call _putenv_s"));
    assert!(!section.contains("call unsetenv"));
    assert!(asm.contains(".Lelephc_empty_env_value:\n    .asciz \"\""));
}
...
/// Verifies that fd-to-HANDLE conversion prefers CRT fds and falls back for inherited std handles.
#[test]
fn test_fd_to_handle_emitted() {
    ...
    assert!(asm.contains("jmp .Lfd_to_handle_done"), "non-standard invalid descriptors remain invalid");
}
...
fn test_main_wrapper_shuffles_args() {
    ...
    assert!(asm.contains("call __rt_win_console_bootstrap"), "main wrapper must initialize attached consoles before user code");
    ...
    // Bootstrap and Winsock must both occur before __elephc_main.
    let console_pos = asm.find("call __rt_win_console_bootstrap");
    let init_pos = asm.find("call __rt_winsock_init");
    let main_pos = asm.find("call __elephc_main");
    assert!(console_pos.is_some() && init_pos.is_some() && main_pos.is_some() && console_pos < init_pos && init_pos < main_pos);
}

/// Verifies the CLI bootstrap changes only proven console handles and never
/// records optional setup failures in the runtime errno surface.
#[test]
fn test_console_bootstrap_is_handle_gated_and_preserves_output_modes() {
    let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
    emit_win_console_bootstrap(&mut emitter);
    let asm = emitter.output();
    let section = shim_section(&asm, "__rt_win_console_bootstrap");
    assert_eq!(section.matches("call GetConsoleMode").count(), 3);
    assert_eq!(section.matches("call SetConsoleOutputCP").count(), 2);
    assert_eq!(section.matches("call SetConsoleMode").count(), 2);
    assert!(section.contains("call SetConsoleCP"));
    assert!(section.matches("mov ecx, 65001").count() >= 3);
    assert!(section.matches("or edx, 4").count() == 2);
    assert!(!section.contains("__rt_errno"));
}

/// Verifies Windows uname uses the manifest-safe ntdll API and keeps PHP's
/// current Windows 10/11/Server product text inside the shared utsname shape.
#[test]
fn test_windows_uname_uses_real_version_product_and_native_identity_apis() {
    let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
    emit_shim_uname(&mut emitter);
    let asm = emitter.output();
    let section = shim_section(&asm, "__rt_sys_uname");
    assert!(section.contains("call RtlGetVersion"));
    assert!(section.contains("call GetComputerNameW"));
    assert!(section.contains("call GetNativeSystemInfo"));
    assert!(section.contains("call __rt_win_utf16_to_utf8"));
    assert!(section.contains("call __rt_win_uname_append_u32"));
    assert!(section.contains("cmp DWORD PTR [rsp + 52], 22000"));
    assert!(section.contains(".Luname_product_win11"));
    assert!(section.contains(".Luname_product_server_2025"));
    assert!(!section.contains("build 0"));
    assert!(section.contains("Unknown Windows version"));
}
...
fn test_open_shim_uses_wide_path_and_windows_call_layout() {
    ...
    assert_eq!(section.matches("call __rt_win_utf8_path_to_utf16").count(), 1);
...
fn test_utimensat_has_all_createfile_args() {
    ...
    assert!(asm.contains("call __rt_win_utf8_path_to_utf16"));
...
fn test_final_path_family_uses_unicode_and_extended_prefix_normalization() {
    ...
    for (label, api) in [
        ...
        let section = shim_section(&asm, label);
        assert!(section.contains("call __rt_win_utf8_path_to_utf16"), "{label}");
...
fn test_uname_uses_unicode_computer_name_layout() {
    ...
    let asm = emitter.output();
    let section = shim_section(&asm, "__rt_sys_uname");
    assert!(section.contains("sub rsp, 600"));
    assert!(section.contains("call GetComputerNameW"));
    assert!(!section.contains("call GetComputerNameA"));
    assert!(section.contains("lea rdi, [rsp + 336]"));
    assert!(section.contains("call __rt_win_utf16_to_utf8"));
...
fn test_access_shim_uses_get_file_attributes() {
    ...
    assert!(
        ...
        "a writable non-executable file must reach the DACL gate without falling through to X_OK"
    );
...
/// Verifies the ACL gate can only tighten a granted attribute answer, and that
/// every failure inside the check lands on the fail-open sentinel.
///
/// The counts below are the complete branch inventory of `__rt_win_acl_grants`:
/// four `jz` (the size probe reported nothing, the descriptor read failed, no
/// impersonation token, `AccessCheck` itself failed), two `je` (the two UNC
/// prefix forms) and one `ja` (a descriptor larger than the frame's fixed
/// buffer) all target `.Lacl_unavailable`, which is the ONLY producer of the -1
/// sentinel. Any new failure edge that does not land there would break the
/// fail-open invariant and change one of these numbers.
#[test]
fn test_access_acl_gate_fails_open_on_every_failure() {
    let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
    emit_win_acl_helpers(&mut emitter);
    emit_shim_access(&mut emitter);
    let asm = emitter.output();
    let gate = shim_section(&asm, "__rt_sys_access");
    assert!(gate.contains("call __rt_win_acl_grants"));
    assert_eq!(
        gate.matches("jz .Laccess_eacces").count(),
        1,
        "the DACL verdict must be the only new denial in the shim"
    );
    assert_eq!(
        gate.matches(".Laccess_acl_gate").count(),
        3,
        "both surviving attribute answers plus the gate's own label"
    );
    let acl = shim_section(&asm, "__rt_win_acl_grants");
    assert_eq!(acl.matches("jz .Lacl_unavailable").count(), 4);
    assert_eq!(acl.matches("je .Lacl_unavailable").count(), 2);
    assert_eq!(acl.matches("ja .Lacl_unavailable").count(), 1);
    assert_eq!(
        acl.matches("mov rax, -1").count(),
        1,
        "the fail-open sentinel has exactly one producer"
    );
    assert_eq!(acl.matches("sub rsp, 2232").count(), 1);
    assert_eq!(acl.matches("add rsp, 2232").count(), 2);
    assert_eq!(
        acl.matches("lea r8, [rsp + 176]").count(),
        1,
        "the descriptor lives in the frame's fixed buffer"
    );
    assert!(
        acl.contains("mov r9d, 2048") && acl.contains("cmp eax, 2048"),
        "the probed size must be bounded by the buffer actually passed"
    );
    assert!(
        !acl.contains("__rt_heap_alloc") && !acl.contains("__rt_heap_free"),
        "the ACL path must not touch the heap: __rt_heap_alloc never returns NULL"
    );
    assert!(!acl.contains("CloseHandle"), "the cached token is never closed here");
}

/// Verifies the impersonation token is opened at most once per process and that
/// a failed dance is remembered rather than retried.
#[test]
fn test_acl_token_cache_is_tri_state_and_reverts_only_its_own_impersonation() {
    let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
    emit_win_acl_helpers(&mut emitter);
    let asm = emitter.output();
    let token = shim_section(&asm, "__rt_win_acl_token_get");
    assert!(token.contains("mov QWORD PTR [rip + _win_acl_token], -1"));
    assert!(token.contains("mov QWORD PTR [rip + _win_acl_token], rax"));
    assert_eq!(token.matches("call ImpersonateSelf").count(), 1);
    assert_eq!(token.matches("call RevertToSelf").count(), 2);
    assert_eq!(token.matches("call OpenThreadToken").count(), 2);
    assert_eq!(token.matches("cmp QWORD PTR [rsp + 40], 0").count(), 2);
    assert_eq!(
        token.matches("mov edx, 8").count(),
        2,
        "TOKEN_QUERY, not TOKEN_ALL_ACCESS, on both OpenThreadToken calls"
    );
    assert_eq!(token.matches("sub rsp, 56").count(), 1);
    assert_eq!(token.matches("add rsp, 56").count(), 2);
    assert!(!token.contains("CloseHandle"), "the cached handle outlives every caller");
}

/// Verifies the advapi32 imports are declared alongside the rest of the table.
#[test]
fn test_win32_imports_declare_the_acl_family() {
    let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
    emit_win32_imports(&mut emitter);
    let asm = emitter.output();
    for symbol in [
        "GetCurrentThread",
        "OpenThreadToken",
        ...
        assert!(asm.contains(&format!(".extern {symbol}")), "{symbol} must be imported");
    }
}

/// Verifies the ACL helpers reach the emitted Windows runtime and that the
/// process-wide token cache slot is declared for the Windows data section.
#[test]
fn test_win32_shims_emit_acl_helpers_and_token_slot() {
    let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
    emit_win32_shims(&mut emitter, RuntimeFeatures::all());
    let asm = emitter.output();
    assert!(asm.contains(".globl __rt_win_acl_token_get\n"));
    assert!(asm.contains(".globl __rt_win_acl_grants\n"));
    let helpers = asm
        .find(".globl __rt_win_acl_grants\n")
        .expect("ACL helper emitted");
    let access = asm.find(".globl __rt_sys_access\n").expect("access shim emitted");
    assert!(
        helpers < access,
        "the helpers must precede __rt_sys_access so shim_section slices cleanly"
    );
    let data = crate::codegen_support::runtime::emit_runtime_data_fixed(
        1024 * 1024,
        Target::new(Platform::Windows, Arch::X86_64),
        crate::php_version::PhpVersion::Php85,
    );
    assert!(
        data.contains(".comm _win_acl_token, 8, 3\n"),
        "the tri-state token cache slot must exist on Windows"
    );
}
...
mod errno_translation {
    ...
    /// Verifies every mapping from PHP 8.5.6's frozen `php_win32_code_to_errno`
    /// table, including the deliberate `ERROR_HANDLE_EOF -> EINVAL` result.
    #[test]
    fn test_win32_errno_translate_common_codes_and_unknown_default() {
        ...
        let asm = emitter.output();
        for (code, target) in [
            ("1", ".Lw32err_einval"), ("4", ".Lw32err_emfile"),
            ("7", ".Lw32err_enomem"), ("10", ".Lw32err_e2big"),
            ("11", ".Lw32err_enoexec"), ("16", ".Lw32err_ecurdir"),
            ("17", ".Lw32err_exdev"), ("19", ".Lw32err_erofs"),
            ("20", ".Lw32err_enxio"), ("21", ".Lw32err_ebusy"),
            ("22", ".Lw32err_eio"), ("32", ".Lw32err_eagain"),
            ("33", ".Lw32err_eacces"), ("35", ".Lw32err_enfile"),
            ("37", ".Lw32err_einval"),
            ("38", ".Lw32err_einval"), ("50", ".Lw32err_enosys"),
            ("80", ".Lw32err_eexist"), ("109", ".Lw32err_epipe"),
            ("111", ".Lw32err_enametoolong"), ("112", ".Lw32err_enospc"),
            ("122", ".Lw32err_erange"), ("128", ".Lw32err_echild"),
            ("145", ".Lw32err_enotempty"), ("183", ".Lw32err_eexist"),
            ("206", ".Lw32err_enametoolong"), ("215", ".Lw32err_eagain"),
            ("258", ".Lw32err_etime"), ("267", ".Lw32err_enotdir"),
            ("336", ".Lw32err_eisdir"), ("996", ".Lw32err_eagain"),
            ("997", ".Lw32err_eagain"), ("1004", ".Lw32err_einval"),
            ("1113", ".Lw32err_einval"), ("1168", ".Lw32err_enoent"),
            ("1224", ".Lw32err_eacces"), ("1314", ".Lw32err_eacces"),
            ("1464", ".Lw32err_einval"), ("1816", ".Lw32err_enomem"),
            ("735", ".Lw32err_eio"), ("4390", ".Lw32err_einval"),
        ] {
            assert!(
                asm.contains(&format!("cmp ecx, {}\n    je {}", code, target)),
                "PHP 8.5.6 Win32 code {} must branch to {}", code, target,
            );
        }
        // The straight-line fallthrough before the first labeled terminal is
        // the "no compare matched" default path -> EINVAL(22).
        let first_label_pos = asm
            .find(".Lw32err_eagain:\n")
            .expect("first terminal label missing");
        let default_zone = &asm[..first_label_pos];
        assert!(
            default_zone.contains("mov eax, 22"),
            "unrecognized codes must default to EINVAL(22)"
        );
        assert!(
            default_zone.trim_end().ends_with("ret"),
            "the default EINVAL path must return immediately"
        );
    }
}
...
fn test_glob_and_fnmatch_use_wide_windows_apis() {
    ...
    assert!(glob.contains("call __rt_win_utf8_path_to_utf16"));
...
fn test_unicode_path_mutation_shims_use_strict_wide_boundary() {
    ...
    for (label, api) in [
        ...
        let section = shim_section(&asm, label);
        assert!(section.contains("call __rt_win_utf8_path_to_utf16"), "{label} must preserve UTF-8/UTF-16 boundary");
...
/// Verifies generic encoding remains byte-preserving while filesystem paths gain
/// PHP-compatible canonicalization and extended-length prefixes at their boundary.
#[test]
fn test_utf8_path_helper_keeps_generic_conversion_separate_from_long_path_policy() {
    let mut emitter = Emitter::new(Target::new(Platform::Windows, Arch::X86_64));
    emit_win32_encoding_helpers(&mut emitter);
    let asm = emitter.output();
    let section = shim_section(&asm, "__rt_win_utf8_to_utf16");
    assert!(section.contains(".Lutf8_to_utf16_ascii_scan:"));
    assert!(section.contains(".Lutf8_to_utf16_ascii_copy:"));
    assert!(section.contains("mov WORD PTR [rax + rcx * 2], dx"));
    assert!(section.contains("call GetACP"));
    assert!(section.contains(".Lutf8_to_utf16_acp_query:"));
    assert!(section.contains(".Lutf8_to_utf16_acp_free_fail:"));
    assert_eq!(section.matches("call __rt_heap_alloc").count(), 3);
    assert_eq!(section.matches("call __rt_heap_free").count(), 2);
    ...
    let path_section = shim_section(&asm, "__rt_win_utf8_path_to_utf16");
    assert!(path_section.contains("call __rt_win_utf8_to_utf16"));
    assert!(path_section.contains("call GetCurrentDirectoryW"));
    assert_eq!(path_section.matches("call GetFullPathNameW").count(), 1);
    assert!(path_section.contains("mov DWORD PTR [rsp + 88], eax"));
    assert!(!path_section.contains("mov rax, 65536"));
    assert!(path_section.contains("cmp WORD PTR [rax + 2], 0x3a"));
    assert!(path_section.contains("cmp WORD PTR [rax + 4], 0x5c"));
    assert!(path_section.contains("0x005c003f005c005c"));
    assert!(path_section.contains("0x005c0043004e0055"));
    assert!(path_section.contains("call __rt_heap_free"));
}
...
fn test_statfs_uses_wide_disk_space_layout_and_errno() {
    ...
    assert!(section.contains("sub rsp, 88"));
    assert!(section.contains("call __rt_win_utf8_path_to_utf16"));
