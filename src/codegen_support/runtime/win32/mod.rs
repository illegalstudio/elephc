...
pub(crate) fn emit_win32_shims(emitter: &mut Emitter, features: RuntimeFeatures) {
    ...
    emit_win_console_bootstrap(emitter);
    ...
    emit_shim_memory_and_errno(emitter);
    if features.zlib {
...
pub(crate) fn emit_fd_to_handle(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov QWORD PTR [rsp + 32], rdi");                      // retain the POSIX descriptor for the standard-handle fallback
    ...
    emitter.instruction("cmp rax, -1");                                         // did this process inherit a standard HANDLE without a CRT fd?
    emitter.instruction("jne .Lfd_to_handle_done");                             // ordinary CRT descriptors use their exact OS handle
    emitter.instruction("mov r10, QWORD PTR [rsp + 32]");                      // inspect only invalid descriptor lookups
    emitter.instruction("cmp r10, 0");                                          // standard input uses the STARTUPINFO handle
    emitter.instruction("je .Lfd_to_handle_stdin");                            // map descriptor zero through GetStdHandle
    emitter.instruction("cmp r10, 1");                                          // standard output uses the STARTUPINFO handle
    emitter.instruction("je .Lfd_to_handle_stdout");                            // map descriptor one through GetStdHandle
    emitter.instruction("cmp r10, 2");                                          // standard error uses the STARTUPINFO handle
    emitter.instruction("je .Lfd_to_handle_stderr");                           // map descriptor two through GetStdHandle
    emitter.instruction("jmp .Lfd_to_handle_done");                            // invalid non-standard descriptors remain invalid
    emitter.label(".Lfd_to_handle_stdin");
    emitter.instruction("mov ecx, -10");                                        // STD_INPUT_HANDLE
    emitter.instruction("jmp .Lfd_to_handle_standard");                        // query the process standard-handle table
    emitter.label(".Lfd_to_handle_stdout");
    emitter.instruction("mov ecx, -11");                                        // STD_OUTPUT_HANDLE
    emitter.instruction("jmp .Lfd_to_handle_standard");                        // query the process standard-handle table
    emitter.label(".Lfd_to_handle_stderr");
    emitter.instruction("mov ecx, -12");                                        // STD_ERROR_HANDLE
    emitter.label(".Lfd_to_handle_standard");
    emitter.instruction("call GetStdHandle");                                   // honor STARTUPINFOW handles for children without CRT fd slots
    emitter.label(".Lfd_to_handle_done");
...
pub(crate) fn emit_main_wrapper(emitter: &mut Emitter) {
    ...
    // The bootstrap changes only handles proven to be attached consoles. It
    // deliberately leaves pipes/files alone and does not touch `__rt_errno`.
    emitter.instruction("call __rt_win_console_bootstrap");                     // select UTF-8 console code pages and enable VT output where available
