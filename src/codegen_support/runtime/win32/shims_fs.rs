...
//! stat/rename/chmod/access/mkdir/rmdir/getcwd/chdir/statfs/getdents, the
//! temp-dir/dir-rewrite-stub helpers, and the php-parity Windows ACL check
//! (`GetFileSecurityW` + `AccessCheck`) that backs `is_readable()`/
//! `is_writable()`.
...
/// reports success.
///
/// Descriptors opened with `O_APPEND` are repositioned to end-of-file first.
/// `__rt_sys_open` records the Linux open flags in the per-descriptor status
/// cache and sets `_O_APPEND` on the CRT descriptor, but neither has any effect
/// here: `WriteFile` writes at the handle's own file pointer, which `OPEN_ALWAYS`
/// leaves at zero, and the CRT's append seek only runs inside `_write`, which
/// this shim deliberately bypasses. Without the seek, `fopen($f, 'a')` silently
/// overwrites the file from byte 0. php-src reaches the same end state through
/// the CRT (main/streams/plain_wrapper.c php_stdiop_write calls `_write`, and
/// win32/ioutil.c hands `_open_osfhandle` the `_O_APPEND` flag); the seek is
/// performed explicitly here because the write path is native.
pub(super) fn emit_shim_write(emitter: &mut Emitter) {
    emitter.label_global("__rt_sys_write");
    emitter.instruction("sub rsp, 72");                                         // allocate shadow(32) + written(4) + len/buf/fd spill slots
    emitter.instruction("mov QWORD PTR [rsp + 48], rdx");                       // spill len (rdx is volatile, clobbered by the call) to a safe slot
    emitter.instruction("mov QWORD PTR [rsp + 56], rsi");                       // spill the buffer across the status lookup, which clobbers rsi
    emitter.instruction("mov QWORD PTR [rsp + 64], rdi");                       // retain the CRT descriptor for the cached-status lookup
    ...
    emitter.instruction("mov QWORD PTR [rsp + 40], rax");                       // park the native HANDLE while the status cache is consulted
    emitter.instruction("mov rdi, QWORD PTR [rsp + 64]");                       // cached status is keyed by the CRT descriptor
    emitter.instruction("call __rt_win_fd_status_find");                        // inspect the recorded Linux open flags
    emitter.instruction("test rax, rax");                                       // did this descriptor receive explicit status flags?
    emitter.instruction("jz .Lsys_write_file");                                 // no cache entry: write at the current file pointer
    emitter.instruction("test QWORD PTR [rax + 16], 0x400");                    // is O_APPEND set in the cached Linux status flags?
    emitter.instruction("jz .Lsys_write_file");                                 // ordinary descriptors keep their file pointer
    emitter.instruction("mov rcx, QWORD PTR [rsp + 40]");                       // hFile = the parked native HANDLE
    emitter.instruction("xor edx, edx");                                        // liDistanceToMove = 0
    emitter.instruction("xor r8d, r8d");                                        // lpNewFilePointer = NULL
    emitter.instruction("mov r9d, 2");                                          // dwMoveMethod = FILE_END
    emitter.instruction("call SetFilePointerEx");                               // reposition to end-of-file so the write appends
    emitter.label(".Lsys_write_file");
    emitter.instruction("mov rcx, QWORD PTR [rsp + 40]");                       // handle
    emitter.instruction("mov rdx, QWORD PTR [rsp + 56]");                       // buffer
    ...
    emitter.instruction("mov rcx, QWORD PTR [rsp + 64]");                       // SOCKET
    emitter.instruction("mov rdx, QWORD PTR [rsp + 56]");                       // buffer
...
/// Stores the `st_blksize`/`st_blocks` values PHP reports on Windows.
///
/// Windows has neither `st_blksize` nor `st_blocks` in its stat structure, so
/// php-src compiles the `#else` arms of ext/standard/filestat.c php_stat and
/// reports -1 for both (`HAVE_STRUCT_STAT_ST_BLKSIZE` /
/// `HAVE_STRUCT_STAT_ST_BLOCKS` are undefined there). Leaving the zeroed buffer
/// alone reported 0, which reads as a real answer: `if ($s['blksize'] > 0)` takes
/// the wrong branch and a chunked-copy loop sized from it divides by zero.
///
/// Expects the caller's stat destination in `rsi`, which is where all three
/// Windows stat shims keep it after clearing the buffer.
fn emit_stat_unsupported_block_fields(emitter: &mut Emitter) {
    let blksize_off = emitter.platform.stat_blksize_offset();
    let blocks_off = emitter.platform.stat_blocks_offset();
    emitter.instruction(&format!("mov QWORD PTR [rsi + {}], -1", blksize_off)); // php reports -1 where the platform has no st_blksize
    emitter.instruction(&format!("mov QWORD PTR [rsi + {}], -1", blocks_off));  // php reports -1 where the platform has no st_blocks
}
...
pub(super) fn emit_shim_fstat(emitter: &mut Emitter) {
    ...
    emit_stat_unsupported_block_fields(emitter);
...
pub(super) fn emit_shim_open(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert the PHP UTF-8 path to an owned wide path
    ...
    emitter.instruction("mov r8, 7");                                           // PHP_WIN32_IOUTIL_DEFAULT_SHARE_MODE: FILE_SHARE_READ |
                                                                                // FILE_SHARE_WRITE | FILE_SHARE_DELETE (win32/ioutil.h). Without
                                                                                // the DELETE bit, unlink() and rename() fail while any handle to
                                                                                // the file is still open, where php on windows succeeds
...
fn emit_wide_path_bool_shim(
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert SysV rdi UTF-8 path strictly
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
...
pub(super) fn emit_shim_stat(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // strictly convert the PHP UTF-8 path
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
...
pub(super) fn emit_shim_rename(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert UTF-8 source path
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert UTF-8 destination path
...
pub(super) fn emit_shim_chmod(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert path strictly to UTF-16
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
...
/// Emits the Windows ACL helpers that back php-parity `is_readable()` /
/// `is_writable()`.
///
/// Must be emitted before [`emit_shim_access`]: its `.Laccess_acl_gate` calls
/// `__rt_win_acl_grants`.
pub(super) fn emit_win_acl_helpers(emitter: &mut Emitter) {
    emit_win_acl_token_get(emitter);
    emit_win_acl_grants(emitter);
}

/// Emits `__rt_win_acl_token_get() -> HANDLE`, the cached impersonation
/// token php's `tsrm_win32_access` keeps in `TWG(impersonation_token)`.
///
/// SysV: no arguments. Returns the borrowed token in rax, or NULL when no
/// token could be obtained. Ownership stays with the process-wide cache: the
/// handle is opened at most once and released by process teardown, so no
/// caller ever closes it and no path can close it twice.
///
/// `_win_acl_token` is tri-state: 0 = never attempted, -1 = attempted and
/// permanently unavailable, anything else = the cached token. A real token
/// handle can never be -1, so the sentinel is unambiguous. Caching the
/// failure is deliberate: a restricted token (low integrity, AppContainer, a
/// service token) fails the dance on every call, and retrying it per
/// `is_readable()` would pay two Win32 calls for a verdict that cannot
/// change. A cached -1 simply leaves every later call on the
/// `GetFileAttributesW` answer, which is exactly today's behavior.
///
/// php requests `TOKEN_ALL_ACCESS`; this asks only for `TOKEN_QUERY`, the
/// single right `AccessCheck` documents as required. That narrower mask is
/// the one deliberate divergence: it succeeds under restricted tokens that
/// refuse `TOKEN_ALL_ACCESS`, and when both succeed the verdict is identical.
///
/// A thread that was already impersonating (a SAPI host) is left exactly as
/// found. Only an impersonation this helper starts is undone with
/// `RevertToSelf`; the opened token object stays valid afterwards, which is
/// what lets the handle be cached instead of leaving the process
/// impersonating for the rest of its life the way php-src does.
///
/// Frame: `sub rsp, 56` — 32 bytes of MSx64 shadow space plus the
/// `OpenThreadToken` out-param (+32) and the "this call started the
/// impersonation" flag (+40). 56 mod 16 == 8, so the entry `rsp` (congruent
/// to 8 after the caller's `call` pushed a return address onto a 16-byte
/// aligned stack) becomes 16-byte aligned for every nested call.
fn emit_win_acl_token_get(emitter: &mut Emitter) {
    emitter.label_global("__rt_win_acl_token_get");
    emitter.instruction("mov rax, QWORD PTR [rip + _win_acl_token]");           // cached impersonation token, php's TWG(impersonation_token)
    emitter.instruction("test rax, rax");                                       // has the token dance ever run?
    emitter.instruction("jz .Lacl_token_acquire");                              // first call performs it
    emitter.instruction("cmp rax, -1");                                         // did an earlier attempt fail permanently?
    emitter.instruction("je .Lacl_token_none");                                 // never retry: stay on the attribute-only answer
    emitter.instruction("ret");                                                 // hand back the cached token without building a frame
    emitter.label(".Lacl_token_none");
    emitter.instruction("xor eax, eax");                                        // NULL tells the caller to fail open
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lacl_token_acquire");
    emitter.instruction("sub rsp, 56");                                         // shadow space plus the token and impersonation-flag locals
    emitter.instruction("mov QWORD PTR [rsp + 32], 0");                         // OpenThreadToken out-param starts empty
    emitter.instruction("mov QWORD PTR [rsp + 40], 0");                         // no impersonation has been started by this call
    emitter.instruction("call GetCurrentThread");                               // pseudo-handle for the calling thread
    emitter.instruction("mov rcx, rax");                                        // ThreadHandle
    emitter.instruction("mov edx, 8");                                          // TOKEN_QUERY: the only right AccessCheck requires
    emitter.instruction("mov r8d, 1");                                          // OpenAsSelf = TRUE, as php passes
    emitter.instruction("lea r9, [rsp + 32]");                                  // &TokenHandle
    emitter.instruction("call OpenThreadToken");                                // reuse a token the host already impersonates
    emitter.instruction("test eax, eax");                                       // did the thread already carry one?
    emitter.instruction("jnz .Lacl_token_have");                                // nothing to impersonate
    emitter.instruction("mov ecx, 2");                                          // SecurityImpersonation
    emitter.instruction("call ImpersonateSelf");                                // php's ERROR_NO_TOKEN recovery
    emitter.instruction("test eax, eax");                                       // impersonation refused by a restricted token?
    emitter.instruction("jz .Lacl_token_fail");                                 // remember the failure and fail open
    emitter.instruction("mov QWORD PTR [rsp + 40], 1");                         // this call now owes a RevertToSelf
    emitter.instruction("call GetCurrentThread");                               // pseudo-handle again after the Win32 call
    emitter.instruction("mov rcx, rax");                                        // ThreadHandle
    emitter.instruction("mov edx, 8");                                          // TOKEN_QUERY
    emitter.instruction("mov r8d, 1");                                          // OpenAsSelf = TRUE
    emitter.instruction("lea r9, [rsp + 32]");                                  // &TokenHandle
    emitter.instruction("call OpenThreadToken");                                // open the token the impersonation just attached
    emitter.instruction("test eax, eax");                                       // still no token?
    emitter.instruction("jz .Lacl_token_fail");                                 // remember the failure and fail open
    emitter.label(".Lacl_token_have");
    emitter.instruction("cmp QWORD PTR [rsp + 40], 0");                         // did this call start the impersonation?
    emitter.instruction("je .Lacl_token_publish");                              // a host impersonation must survive untouched
    emitter.instruction("call RevertToSelf");                                   // detach it; the opened token object stays valid
    emitter.label(".Lacl_token_publish");
    emitter.instruction("mov rax, QWORD PTR [rsp + 32]");                       // the opened impersonation token
    emitter.instruction("test rax, rax");                                       // defensive: a TRUE return with a NULL handle
    emitter.instruction("jz .Lacl_token_reverted");                             // treat it as a permanent failure
    emitter.instruction("mov QWORD PTR [rip + _win_acl_token], rax");           // cache it for every later access() call
    emitter.instruction("add rsp, 56");                                         // release the frame
    emitter.instruction("ret");                                                 // return the borrowed token
    emitter.label(".Lacl_token_fail");
    emitter.instruction("cmp QWORD PTR [rsp + 40], 0");                         // did this call start the impersonation?
    emitter.instruction("je .Lacl_token_reverted");                             // nothing to undo
    emitter.instruction("call RevertToSelf");                                   // leave the thread exactly as it was found
    emitter.label(".Lacl_token_reverted");
    emitter.instruction("mov QWORD PTR [rip + _win_acl_token], -1");            // remember the permanent failure
    emitter.instruction("xor eax, eax");                                        // NULL tells the caller to fail open
    emitter.instruction("add rsp, 56");                                         // release the frame
    emitter.instruction("ret");                                                 // return
    emitter.blank();
...
/// Emits `__rt_win_acl_grants(const WCHAR *path, DWORD desired) -> i64`, the
/// `GetFileSecurityW` + `AccessCheck` verdict php's `tsrm_win32_access` uses
/// for `R_OK`/`W_OK` on Windows.
///
/// SysV: rdi = borrowed UTF-16 path (the caller keeps ownership), esi =
/// desired access mask. Returns 1 when the DACL grants the request, 0 when it
/// denies it, and -1 when the check could not be performed at all.
///
/// -1 is the whole point of the helper: every failure — unsupported volume,
/// missing path, a descriptor too large for the frame's buffer, no
/// impersonation token, a failing `AccessCheck` — returns -1 so
/// `__rt_sys_access` keeps the `GetFileAttributesW` answer it already
/// computed. A denial is only ever reported when `AccessCheck` itself
/// succeeded and set `AccessStatus` to FALSE, so the worst case of this whole
/// path is exactly today's behavior.
///
/// UNC paths are refused up front: `GetFileSecurityW` returns the *remote*
/// descriptor while `AccessCheck` evaluates it against a *local* token, which
/// produces confident wrong denials on domain shares. php-src has that wart;
/// this helper fails open on `\\server\share` instead of inheriting it.
///
/// The helper owns no resources. The security descriptor lives in a
/// fixed 2048-byte buffer inside this frame, so there is nothing to release
/// on any path and no allocator to fail: a descriptor whose probed size
/// exceeds the buffer simply takes the fail-open exit. (An earlier draft
/// called `__rt_heap_alloc` and treated a NULL return as "fail open", but
/// that helper never returns NULL — it diverges through
/// `__rt_heap_exhausted` — so the guard was unreachable and exhaustion was
/// undefined control flow inside a Win32 shim.) The token is borrowed from
/// `__rt_win_acl_token_get`'s process-wide cache and is never closed here.
///
/// Frame: `sub rsp, 2232`, laid out as
/// `[rsp+0..31]` MSx64 shadow space, `[rsp+32..63]` the four stack arguments
/// `AccessCheck` needs beyond rcx/rdx/r8/r9, `[rsp+64]` lpFileName,
/// `[rsp+72]` the desired access mask `MapGenericMask` rewrites in place,
/// `[rsp+80]` the borrowed token, `[rsp+88]`/`[rsp+92]` lpnLengthNeeded and
/// GrantedAccess, `[rsp+96]`/`[rsp+100]` AccessStatus and PrivilegeSetLength,
/// `[rsp+104..151]` the 48-byte `PRIVILEGE_SET`, `[rsp+152..167]` the
/// `GENERIC_MAPPING`, `[rsp+168..175]` slack that lands the descriptor buffer
/// on a 16-byte boundary, `[rsp+176..2223]` the 2048-byte security
/// descriptor, and `[rsp+2224..2231]` the tail slack that makes the total
/// 2232 = 139 * 16 + 8. That residue of 8 is the alignment contract: entry
/// `rsp` is congruent to 8 mod 16 (the caller's `call` pushed a return
/// address onto a 16-byte aligned stack), so subtracting a size congruent to
/// 8 leaves `rsp` 16-byte aligned at every nested `call`, as MSx64 requires.
/// 2232 also stays under a 4 KiB page, so no `__chkstk` probe is needed to
/// step past the stack guard page.
fn emit_win_acl_grants(emitter: &mut Emitter) {
    emitter.label_global("__rt_win_acl_grants");
    emitter.instruction("sub rsp, 2232");                                       // shadow space, stack arguments, ACL locals, descriptor
    emitter.instruction("mov QWORD PTR [rsp + 64], rdi");                       // borrowed UTF-16 path
    emitter.instruction("mov eax, esi");                                        // zero-extend the requested access mask
    emitter.instruction("mov QWORD PTR [rsp + 72], rax");                       // MapGenericMask reads and rewrites this DWORD
    emitter.instruction("mov QWORD PTR [rsp + 80], 0");                         // no impersonation token is borrowed yet
    emitter.instruction("mov QWORD PTR [rsp + 88], 0");                         // lpnLengthNeeded and GrantedAccess out-params
    emitter.instruction("mov QWORD PTR [rsp + 96], 0");                         // AccessStatus and PrivilegeSetLength out-params
    emitter.instruction("mov QWORD PTR [rsp + 104], 0");                        // PRIVILEGE_SET, zeroed exactly like php's `= {0}`
    emitter.instruction("mov QWORD PTR [rsp + 112], 0");                        // PRIVILEGE_SET
    emitter.instruction("mov QWORD PTR [rsp + 120], 0");                        // PRIVILEGE_SET
    emitter.instruction("mov QWORD PTR [rsp + 128], 0");                        // PRIVILEGE_SET
    emitter.instruction("mov QWORD PTR [rsp + 136], 0");                        // PRIVILEGE_SET
    emitter.instruction("mov QWORD PTR [rsp + 144], 0");                        // PRIVILEGE_SET tail
    emitter.instruction("mov DWORD PTR [rsp + 152], 0x00120089");               // GENERIC_MAPPING.GenericRead = FILE_GENERIC_READ
    emitter.instruction("mov DWORD PTR [rsp + 156], 0x00120116");               // GENERIC_MAPPING.GenericWrite = FILE_GENERIC_WRITE
    emitter.instruction("mov DWORD PTR [rsp + 160], 0x001200A0");               // GENERIC_MAPPING.GenericExecute = FILE_GENERIC_EXEC
    emitter.instruction("mov DWORD PTR [rsp + 164], 0x001F01FF");               // GENERIC_MAPPING.GenericAll = FILE_ALL_ACCESS
    emitter.instruction("movzx ecx, WORD PTR [rdi]");                           // first UTF-16 code unit of the path
    emitter.instruction("cmp ecx, 0x5C");                                       // leading backslash?
    emitter.instruction("je .Lacl_unc_probe");                                  // may begin a UNC prefix
    emitter.instruction("cmp ecx, 0x2F");                                       // leading forward slash?
    emitter.instruction("jne .Lacl_local_path");                                // ordinary local path
    emitter.label(".Lacl_unc_probe");
    emitter.instruction("movzx ecx, WORD PTR [rdi + 2]");                       // second code unit; the NUL terminator at worst
    emitter.instruction("cmp ecx, 0x5C");                                       // a doubled backslash opens a UNC share
    emitter.instruction("je .Lacl_unavailable");                                // a remote DACL cannot be judged by a local token
    emitter.instruction("cmp ecx, 0x2F");                                       // a doubled forward slash opens the same share
    emitter.instruction("je .Lacl_unavailable");                                // fail open: the attribute answer is the safer one
    emitter.label(".Lacl_local_path");
    emitter.instruction("mov rcx, rdi");                                        // lpFileName
    emitter.instruction("mov edx, 7");                                          // OWNER | GROUP | DACL_SECURITY_INFORMATION
    emitter.instruction("xor r8d, r8d");                                        // pSecurityDescriptor = NULL for the size probe
    emitter.instruction("xor r9d, r9d");                                        // nLength = 0
    emitter.instruction("lea rax, [rsp + 88]");                                 // &lpnLengthNeeded
    emitter.instruction("mov QWORD PTR [rsp + 32], rax");                       // fifth argument travels on the stack
    emitter.instruction("call GetFileSecurityW");                               // php's first call: probe the descriptor size
    emitter.instruction("mov eax, DWORD PTR [rsp + 88]");                       // required byte count, still zero when unreported
    emitter.instruction("test eax, eax");                                       // did the probe report a size?
    emitter.instruction("jz .Lacl_unavailable");                                // unsupported volume or missing path: fail open
    emitter.instruction("cmp eax, 2048");                                       // does the descriptor fit the frame's fixed buffer?
    emitter.instruction("ja .Lacl_unavailable");                                // oversized descriptor: fail open, never allocate
    emitter.instruction("mov rcx, QWORD PTR [rsp + 64]");                       // lpFileName
    emitter.instruction("mov edx, 7");                                          // OWNER | GROUP | DACL_SECURITY_INFORMATION
    emitter.instruction("lea r8, [rsp + 176]");                                 // pSecurityDescriptor = the frame's fixed buffer
    emitter.instruction("mov r9d, 2048");                                       // nLength = that buffer's real capacity
    emitter.instruction("lea rax, [rsp + 88]");                                 // &lpnLengthNeeded
    emitter.instruction("mov QWORD PTR [rsp + 32], rax");                       // fifth argument travels on the stack
    emitter.instruction("call GetFileSecurityW");                               // php's second call: fill the descriptor
    emitter.instruction("test eax, eax");                                       // did the descriptor read succeed?
    emitter.instruction("jz .Lacl_unavailable");                                // fail open on a refused descriptor
    emitter.instruction("call __rt_win_acl_token_get");                         // borrow php's cached impersonation token
    emitter.instruction("test rax, rax");                                       // is a usable token available?
    emitter.instruction("jz .Lacl_unavailable");                                // fail open with no token to check against
    emitter.instruction("mov QWORD PTR [rsp + 80], rax");                       // borrowed: the cache keeps ownership
    emitter.instruction("lea rcx, [rsp + 72]");                                 // &desired access mask
    emitter.instruction("lea rdx, [rsp + 152]");                                // &GENERIC_MAPPING
    emitter.instruction("call MapGenericMask");                                 // resolve generic bits exactly as php does
    emitter.instruction("mov DWORD PTR [rsp + 100], 48");                       // PrivilegeSetLength = the reserved buffer size
    emitter.instruction("lea rcx, [rsp + 176]");                                // pSecurityDescriptor
    emitter.instruction("mov rdx, QWORD PTR [rsp + 80]");                       // ClientToken
    emitter.instruction("mov r8d, DWORD PTR [rsp + 72]");                       // DesiredAccess after generic mapping
    emitter.instruction("lea r9, [rsp + 152]");                                 // GenericMapping
    emitter.instruction("lea rax, [rsp + 104]");                                // PrivilegeSet
    emitter.instruction("mov QWORD PTR [rsp + 32], rax");                       // fifth argument
    emitter.instruction("lea rax, [rsp + 100]");                                // PrivilegeSetLength
    emitter.instruction("mov QWORD PTR [rsp + 40], rax");                       // sixth argument
    emitter.instruction("lea rax, [rsp + 92]");                                 // GrantedAccess
    emitter.instruction("mov QWORD PTR [rsp + 48], rax");                       // seventh argument
    emitter.instruction("lea rax, [rsp + 96]");                                 // AccessStatus
    emitter.instruction("mov QWORD PTR [rsp + 56], rax");                       // eighth argument
    emitter.instruction("call AccessCheck");                                    // php's DACL verdict for this token
    emitter.instruction("test eax, eax");                                       // did AccessCheck itself succeed?
    emitter.instruction("jz .Lacl_unavailable");                                // fail open on a check that could not run
    emitter.instruction("cmp DWORD PTR [rsp + 96], 0");                         // AccessStatus: FALSE means the DACL denies it
    emitter.instruction("setne al");                                            // 1 = granted, 0 = denied
    emitter.instruction("movzx eax, al");                                       // widen the verdict; the zero-extend clears rax
    emitter.instruction("add rsp, 2232");                                       // release the frame
    emitter.instruction("ret");                                                 // return 1 (granted) or 0 (denied)
    emitter.label(".Lacl_unavailable");
    emitter.instruction("mov rax, -1");                                         // the check could not run: the caller keeps its answer
    emitter.instruction("add rsp, 2232");                                       // release the frame
    emitter.instruction("ret");                                                 // return
    emitter.blank();
}
...
/// `GetBinaryTypeW` to decide `X_OK`.
///
/// `R_OK`/`W_OK` that survive the attribute rules are then handed to
/// `__rt_win_acl_grants`, php-src's `GetFileSecurityW` + `AccessCheck` pair
/// (TSRM/tsrm_win32.c). That check can only TIGHTEN a positive attribute
/// answer: it returns -1 whenever it cannot run, and the gate treats -1 the
/// same as "granted", so the worst case of the whole ACL path is the
/// attribute-only behavior this shim had before. `X_OK` never reaches the
/// gate — `GetBinaryTypeW` already answers it exactly as php does.
/// `GetFileAttributesW` returns `INVALID_FILE_ATTRIBUTES` (0xFFFFFFFF) when
...
pub(super) fn emit_shim_access(emitter: &mut Emitter) {
    ...
    emitter.instruction("jz .Laccess_acl_gate");                                // F_OK/R_OK: the DACL still owes php its verdict
    ...
    emitter.instruction("jmp .Laccess_acl_gate");                               // writable attributes still owe php its DACL verdict
    ...
    emitter.instruction("mov DWORD PTR [rip + __rt_errno], 13");                // EACCES: a readonly attribute or a denying DACL
    ...
    emitter.instruction("add rsp, 72");                                         // restore stack
    emitter.instruction("ret");                                                 // return
    emitter.label(".Laccess_acl_gate");
    emitter.instruction("mov edx, DWORD PTR [rsp + 40]");                       // reload the requested access bits
    emitter.instruction("xor esi, esi");                                        // php starts from desired_access = 0
    emitter.instruction("test edx, 4");                                         // R_OK requested?
    emitter.instruction("jz .Laccess_acl_write");                               // no read right to demand
    emitter.instruction("mov esi, 0x00120089");                                 // FILE_GENERIC_READ, exactly php's mask
    emitter.label(".Laccess_acl_write");
    emitter.instruction("test edx, 2");                                         // W_OK requested?
    emitter.instruction("jz .Laccess_acl_ready");                               // no write right to demand
    emitter.instruction("or esi, 0x00120116");                                  // FILE_GENERIC_WRITE, exactly php's mask
    emitter.label(".Laccess_acl_ready");
    emitter.instruction("test esi, esi");                                       // did the request name an ACL-checkable right?
    emitter.instruction("jz .Laccess_success");                                 // F_OK: the existence probe already answered it
    emitter.instruction("mov rdi, QWORD PTR [rsp + 32]");                       // borrowed UTF-16 path
    emitter.instruction("call __rt_win_acl_grants");                            // php's GetFileSecurityW + AccessCheck verdict
    emitter.instruction("test rax, rax");                                       // zero means the DACL denies the request
    emitter.instruction("jz .Laccess_eacces");                                  // php reports a denial as a failed access()
    emitter.instruction("jmp .Laccess_success");                                // granted, or unavailable: keep the attribute answer
    emitter.blank();
}
...
pub(super) fn emit_shim_statfs(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // strictly convert the filesystem path
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
...
pub(super) fn emit_shim_opendir(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // allocate a strict UTF-16 pattern
...
pub(super) fn emit_shim_mkstemp(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // allocate the native Unicode path
