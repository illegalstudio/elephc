...
pub(super) fn emit_shim_c_symbols(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert target
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert link path
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert existing path
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert new path
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert link path strictly
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert the link name for CreateFileW
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // strictly convert target path
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // allocate native Unicode wildcard pattern
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert input path strictly
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
...
/// -> EINVAL(22), following PHP's Windows table; `WSAEINTR`(10004) -> EINTR(4);
/// `ERROR_INSUFFICIENT_BUFFER`(122) -> ERANGE(34);
/// `ERROR_NO_UNICODE_TRANSLATION`(1113) -> EINVAL(22);
/// `WSAECONNRESET`(10054) -> ECONNRESET(104); `WSAETIMEDOUT`(10060) ->
/// ETIMEDOUT(110). Any other code -> EINVAL(22), matching php-src.
...
fn emit_shim_readlink_reparse(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert reparse-point path strictly
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
...
fn emit_shim_lstat_symlink_probe(emitter: &mut Emitter) {
    ...
    emitter.instruction("call __rt_win_utf8_path_to_utf16");                    // convert the SysV UTF-8 path without lossy ANSI fallback
    emitter.instruction("test rax, rax");                                       // conversion succeeded?
...
pub(super) fn emit_win32_errno_translate(emitter: &mut Emitter) {
    ...
    // Keep this table synchronized with PHP 8.5.6's `php_win32_code_to_errno`
    // (`win32/winutil.c`). The WSA-specific cases below are additional runtime
    // reachability, not replacements for PHP's Win32 mappings.
    emitter.instruction("mov ecx, eax");                                        // save the raw Win32/WSA code for the compare chain
    emitter.instruction("cmp ecx, 1");                                          // ERROR_INVALID_FUNCTION
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("cmp ecx, 4");                                          // ERROR_TOO_MANY_OPEN_FILES
    emitter.instruction("je .Lw32err_emfile");                                  // -> EMFILE
    emitter.instruction("cmp ecx, 7");                                          // ERROR_ARENA_TRASHED
    emitter.instruction("je .Lw32err_enomem");                                  // -> ENOMEM
    emitter.instruction("cmp ecx, 8");                                          // ERROR_NOT_ENOUGH_MEMORY
    emitter.instruction("je .Lw32err_enomem");                                  // -> ENOMEM
    emitter.instruction("cmp ecx, 9");                                          // ERROR_INVALID_BLOCK
    emitter.instruction("je .Lw32err_enomem");                                  // -> ENOMEM
    emitter.instruction("cmp ecx, 10");                                         // ERROR_BAD_ENVIRONMENT
    emitter.instruction("je .Lw32err_e2big");                                   // -> E2BIG
    emitter.instruction("cmp ecx, 11");                                         // ERROR_BAD_FORMAT
    emitter.instruction("je .Lw32err_enoexec");                                 // -> ENOEXEC
    emitter.instruction("cmp ecx, 12");                                         // ERROR_INVALID_ACCESS
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("cmp ecx, 13");                                         // ERROR_INVALID_DATA
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("cmp ecx, 14");                                         // ERROR_OUTOFMEMORY
    emitter.instruction("je .Lw32err_enomem");                                  // -> ENOMEM
    emitter.instruction("cmp ecx, 15");                                         // ERROR_INVALID_DRIVE
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 16");                                         // ERROR_CURRENT_DIRECTORY
    emitter.instruction("je .Lw32err_ecurdir");                                 // -> ECURDIR
    emitter.instruction("cmp ecx, 17");                                         // ERROR_NOT_SAME_DEVICE
    emitter.instruction("je .Lw32err_exdev");                                   // -> EXDEV
    emitter.instruction("cmp ecx, 18");                                         // ERROR_NO_MORE_FILES
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 19");                                         // ERROR_WRITE_PROTECT
    emitter.instruction("je .Lw32err_erofs");                                   // -> EROFS
    emitter.instruction("cmp ecx, 20");                                         // ERROR_BAD_UNIT
    emitter.instruction("je .Lw32err_enxio");                                   // -> ENXIO
    emitter.instruction("cmp ecx, 21");                                         // ERROR_NOT_READY
    emitter.instruction("je .Lw32err_ebusy");                                   // -> EBUSY
    emitter.instruction("cmp ecx, 22");                                         // ERROR_BAD_COMMAND
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 23");                                         // ERROR_CRC
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 24");                                         // ERROR_BAD_LENGTH
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 25");                                         // ERROR_SEEK
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 26");                                         // ERROR_NOT_DOS_DISK
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 27");                                         // ERROR_SECTOR_NOT_FOUND
    emitter.instruction("je .Lw32err_enxio");                                   // -> ENXIO
    emitter.instruction("cmp ecx, 28");                                         // ERROR_OUT_OF_PAPER
    emitter.instruction("je .Lw32err_ebusy");                                   // -> EBUSY
    emitter.instruction("cmp ecx, 29");                                         // ERROR_WRITE_FAULT
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 30");                                         // ERROR_READ_FAULT
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 31");                                         // ERROR_GEN_FAILURE
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 32");                                         // ERROR_SHARING_VIOLATION
    emitter.instruction("je .Lw32err_eagain");                                  // -> EAGAIN
    emitter.instruction("cmp ecx, 33");                                         // ERROR_LOCK_VIOLATION
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    emitter.instruction("cmp ecx, 34");                                         // ERROR_WRONG_DISK
    emitter.instruction("je .Lw32err_enxio");                                   // -> ENXIO
    emitter.instruction("cmp ecx, 35");                                         // ERROR_FCB_UNAVAILABLE
    emitter.instruction("je .Lw32err_enfile");                                  // -> ENFILE
    emitter.instruction("cmp ecx, 36");                                         // ERROR_SHARING_BUFFER_EXCEEDED
    emitter.instruction("je .Lw32err_enfile");                                  // -> ENFILE
    emitter.instruction("cmp ecx, 37");                                         // ERROR_HANDLE_EOF (legacy table slot)
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    ...
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL, matching php_win32_code_to_errno
    emitter.instruction("cmp ecx, 50");                                         // ERROR_NOT_SUPPORTED
    emitter.instruction("je .Lw32err_enosys");                                  // -> ENOSYS
    emitter.instruction("cmp ecx, 53");                                         // ERROR_BAD_NETPATH
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 65");                                         // ERROR_NETWORK_ACCESS_DENIED
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    emitter.instruction("cmp ecx, 67");                                         // ERROR_BAD_NET_NAME
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 80");                                         // ERROR_FILE_EXISTS
    emitter.instruction("je .Lw32err_eexist");                                  // -> EEXIST
    emitter.instruction("cmp ecx, 82");                                         // ERROR_CANNOT_MAKE
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    emitter.instruction("cmp ecx, 83");                                         // ERROR_FAIL_I24
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    emitter.instruction("cmp ecx, 87");                                         // ERROR_INVALID_PARAMETER
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("cmp ecx, 89");                                         // ERROR_NO_PROC_SLOTS
    emitter.instruction("je .Lw32err_eagain");                                  // -> EAGAIN
    emitter.instruction("cmp ecx, 108");                                        // ERROR_DRIVE_LOCKED
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    ...
    emitter.instruction("cmp ecx, 111");                                        // ERROR_BUFFER_OVERFLOW
    emitter.instruction("je .Lw32err_enametoolong");                            // -> ENAMETOOLONG
    emitter.instruction("cmp ecx, 114");                                        // ERROR_INVALID_TARGET_HANDLE
    emitter.instruction("je .Lw32err_ebadf");                                   // -> EBADF
    emitter.instruction("cmp ecx, 123");                                        // ERROR_INVALID_NAME
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 124");                                        // ERROR_INVALID_HANDLE (PHP's second mapping)
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("cmp ecx, 126");                                        // ERROR_MOD_NOT_FOUND
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 127");                                        // ERROR_PROC_NOT_FOUND
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 128");                                        // ERROR_WAIT_NO_CHILDREN
    emitter.instruction("je .Lw32err_echild");                                  // -> ECHILD
    emitter.instruction("cmp ecx, 129");                                        // ERROR_CHILD_NOT_COMPLETE
    emitter.instruction("je .Lw32err_echild");                                  // -> ECHILD
    emitter.instruction("cmp ecx, 130");                                        // ERROR_DIRECT_ACCESS_HANDLE
    emitter.instruction("je .Lw32err_ebadf");                                   // -> EBADF
    emitter.instruction("cmp ecx, 131");                                        // ERROR_NEGATIVE_SEEK
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("cmp ecx, 132");                                        // ERROR_SEEK_ON_DEVICE
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    emitter.instruction("cmp ecx, 145");                                        // ERROR_DIR_NOT_EMPTY
    emitter.instruction("je .Lw32err_enotempty");                               // -> ENOTEMPTY
    emitter.instruction("cmp ecx, 158");                                        // ERROR_NOT_LOCKED
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    emitter.instruction("cmp ecx, 161");                                        // ERROR_BAD_PATHNAME
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 164");                                        // ERROR_MAX_THRDS_REACHED
    emitter.instruction("je .Lw32err_eagain");                                  // -> EAGAIN
    emitter.instruction("cmp ecx, 167");                                        // ERROR_LOCK_FAILED
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    ...
    emitter.instruction("cmp ecx, 215");                                        // ERROR_NESTING_NOT_ALLOWED
    emitter.instruction("je .Lw32err_eagain");                                  // -> EAGAIN
    emitter.instruction("cmp ecx, 258");                                        // WAIT_TIMEOUT
    emitter.instruction("je .Lw32err_etime");                                   // -> ETIME
    emitter.instruction("cmp ecx, 336");                                        // ERROR_DIRECTORY_NOT_SUPPORTED
    emitter.instruction("je .Lw32err_eisdir");                                  // -> EISDIR
    emitter.instruction("cmp ecx, 996");                                        // ERROR_IO_INCOMPLETE
    emitter.instruction("je .Lw32err_eagain");                                  // -> EAGAIN
    emitter.instruction("cmp ecx, 1004");                                       // ERROR_INVALID_FLAGS
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("cmp ecx, 1168");                                       // ERROR_NOT_FOUND
    emitter.instruction("je .Lw32err_enoent");                                  // -> ENOENT
    emitter.instruction("cmp ecx, 1224");                                       // ERROR_USER_MAPPED_FILE
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    emitter.instruction("cmp ecx, 1314");                                       // ERROR_PRIVILEGE_NOT_HELD
    emitter.instruction("je .Lw32err_eacces");                                  // -> EACCES
    emitter.instruction("cmp ecx, 1464");                                       // ERROR_SYMLINK_NOT_SUPPORTED
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("cmp ecx, 1816");                                       // ERROR_NOT_ENOUGH_QUOTA
    emitter.instruction("je .Lw32err_enomem");                                  // -> ENOMEM
    emitter.instruction("cmp ecx, 735");                                        // ERROR_ABANDONED_WAIT_0
    emitter.instruction("je .Lw32err_eio");                                     // -> EIO
    emitter.instruction("cmp ecx, 4390");                                       // ERROR_NOT_A_REPARSE_POINT
    emitter.instruction("je .Lw32err_einval");                                  // -> EINVAL
    emitter.instruction("mov eax, 22");                                         // unknown code -> EINVAL
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_eagain");
    emitter.instruction("mov eax, 11");                                         // EAGAIN
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_emfile");
    emitter.instruction("mov eax, 24");                                         // EMFILE
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_enfile");
    emitter.instruction("mov eax, 23");                                         // ENFILE
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_enomem");
    emitter.instruction("mov eax, 12");                                         // ENOMEM
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_e2big");
    emitter.instruction("mov eax, 7");                                          // E2BIG
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_enoexec");
    emitter.instruction("mov eax, 8");                                          // ENOEXEC
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_ecurdir");
    emitter.instruction("mov eax, 16");                                         // ECURDIR, as exposed by PHP's Windows CRT
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_exdev");
    emitter.instruction("mov eax, 18");                                         // EXDEV
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_erofs");
    emitter.instruction("mov eax, 30");                                         // EROFS
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_enxio");
    emitter.instruction("mov eax, 6");                                          // ENXIO
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_ebusy");
    emitter.instruction("mov eax, 16");                                         // EBUSY
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_eio");
    emitter.instruction("mov eax, 5");                                          // EIO
    emitter.instruction("ret");                                                 // return
    ...
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_einval");
    emitter.instruction("mov eax, 22");                                         // EINVAL
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_enosys");
    emitter.instruction("mov eax, 38");                                         // ENOSYS
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_echild");
    emitter.instruction("mov eax, 10");                                         // ECHILD
    emitter.instruction("ret");                                                 // return
    ...
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_etime");
    emitter.instruction("mov eax, 62");                                         // ETIME
    emitter.instruction("ret");                                                 // return
    emitter.label(".Lw32err_eisdir");
    emitter.instruction("mov eax, 21");                                         // EISDIR
    emitter.instruction("ret");                                                 // return
    emitter.blank();
}
