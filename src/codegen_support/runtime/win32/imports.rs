...
const WIN32_IMPORTS: &[&str] = &[
    ...
    "GetComputerNameW",
    "GetNativeSystemInfo",
    // `RtlGetVersion` is manifest-independent, unlike GetVersionEx. `ntdll` is
    // already part of the Windows linker input set (see `linker/command.rs`).
    "RtlGetVersion",
    ...
    "SetErrorMode",
    // Windows ACL family (advapi32, except GetCurrentThread which is
    // kernel32): the security-descriptor + AccessCheck path php-src's
    // TSRM/tsrm_win32.c uses for access(R_OK)/access(W_OK). Consumed by
    // `emit_win_acl_helpers` in shims_fs.rs; advapi32 is added to the Windows
    // link set in src/linker.rs and tests/codegen/support/runner.rs.
    "GetCurrentThread",
    "OpenThreadToken",
    "ImpersonateSelf",
    "RevertToSelf",
...
pub(crate) fn windows_c_shim_name(symbol: &str) -> Option<&'static str> {
    match symbol {
        "memcpy" => Some("__rt_sys_memcpy"),
        "strlen" => Some("__rt_sys_strlen"),
        "_errno" => Some("__rt_sys_errno"),
        ...
        "unsetenv" => Some("__rt_sys_unsetenv"),
