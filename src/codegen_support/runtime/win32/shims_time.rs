...
/// Emits the Windows implementation of POSIX `unsetenv(name)`.
///
/// The compiler's runtime uses the SysV internal ABI, while MinGW's `_putenv_s`
/// import follows the Windows x64 ABI. `_putenv_s(name, "")` is the CRT-defined
/// removal operation and avoids depending on the absent MinGW `unsetenv` export.
pub(super) fn emit_shim_unsetenv(emitter: &mut Emitter) {
    emitter.label_global("__rt_sys_unsetenv");
    emitter.instruction("sub rsp, 40");                                         // reserve Windows shadow space
    emitter.instruction("mov rcx, rdi");                                        // name: SysV rdi -> MSx64 rcx
    emitter.instruction("lea rdx, [rip + .Lelephc_empty_env_value]");           // empty value removes the variable
    emitter.instruction("call _putenv_s");                                      // MSVCRT _putenv_s(name, "") -> int status
    emitter.instruction("add rsp, 40");                                         // restore caller stack
    emitter.instruction("ret");                                                 // return status in rax
    emitter.blank();

    emitter.raw(".data");
    emitter.raw(".Lelephc_empty_env_value:");
    emitter.raw("    .asciz \"\"");
    emitter.raw(".text");
    emitter.blank();
}
