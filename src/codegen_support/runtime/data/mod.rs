...
/// Catchable `\ValueError` message when `mb_strwidth()` receives an unknown encoding name.
pub(crate) const MB_STRWIDTH_UNKNOWN_ENCODING_MSG: &str =
    "mb_strwidth(): Argument #2 ($encoding) must be a valid encoding";
