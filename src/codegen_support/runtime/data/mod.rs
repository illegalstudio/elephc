...
/// Fatal error message when `random_bytes()` receives a length below one.
pub(crate) const RANDOM_BYTES_LENGTH_MSG: &str =
    "Fatal error: random_bytes(): Argument #1 ($length) must be greater than 0\n";
/// Fatal error message when the operating-system CSPRNG cannot fill the request.
pub(crate) const RANDOM_BYTES_SOURCE_MSG: &str =
    "Fatal error: random_bytes(): Unable to gather sufficient random data\n";
