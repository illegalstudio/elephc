...
/// Catchable `\ValueError` message when `mb_strtolower()` receives an unknown encoding name.
pub(crate) const MB_STRTOLOWER_UNKNOWN_ENCODING_MSG: &str =
    "mb_strtolower(): Argument #2 ($encoding) must be a valid encoding";
