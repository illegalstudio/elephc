...
pub(super) fn emit_platform_runtime(emitter: &mut Emitter, features: RuntimeFeatures) {
    ...
    io::emit_stream_pending_append(emitter);
    io::emit_stream_pending_take(emitter);
    io::emit_stream_pending_clear(emitter);
    io::emit_stream_pending_held(emitter);
    ...
    io::emit_wrapper_context_notice(emitter);
    io::emit_uw_post_read_eof(emitter);
