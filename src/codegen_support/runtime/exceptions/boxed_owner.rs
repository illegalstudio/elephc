...
//! - The destination owner is acquired before the source representation is released.
...
pub fn emit_throwable_boxed_owners(emitter: &mut Emitter) {
    ...
    match emitter.target.arch {
        Arch::AArch64 => emitter.instruction("mov x0, x1"),                     // retain the raw object before consuming its box
        Arch::X86_64 => emitter.instruction("mov rax, rdi"),                    // retain the raw object before consuming its box
    }
...
mod tests {
    ...
    /// Both conversions acquire the destination owner before releasing the source on every target.
    #[test]
    fn throwable_box_transfers_balance_both_representations_on_all_targets() {
        for name in [
            ...
            let (take, give) = assembly.split_once("__rt_throwable_box_owned:").unwrap();
