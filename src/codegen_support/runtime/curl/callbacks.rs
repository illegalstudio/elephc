...
pub fn emit_curl_invoke_callback(emitter: &mut Emitter) {
    ...
    emitter.instruction("b __rt_curl_invoke_cb_release_return");                // release the owned integer-return cell
    ...
    emitter.instruction("b __rt_curl_invoke_cb_cleanup");                       // continue with argument-container cleanup
...
fn emit_curl_invoke_callback_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("jnz __rt_curl_invoke_cb_string_x86");                  // decode the boxed return as a string
    ...
    emitter.instruction("jmp __rt_curl_invoke_cb_release_return_x86");          // release the owned integer-return cell
    ...
    emitter.instruction("jz __rt_curl_invoke_cb_cleanup_x86");                  // no return cell, continue with argument cleanup
    emitter.instruction("call __rt_decref_mixed");                              // release it (bytes already copied out)
    emitter.instruction("jmp __rt_curl_invoke_cb_cleanup_x86");                 // continue with argument-container cleanup
    ...
    emitter.instruction("jz __rt_curl_invoke_cb_done_x86");                     // no argument container was allocated
