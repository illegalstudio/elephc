...
use super::*;
use crate::codegen_support::abi::calls::compact_windows_c_abi_stack_args;
...
/// Keeps generated Windows ABI registers distinct from internal SysV runtime-helper registers.
#[test]
fn test_windows_runtime_helpers_keep_sysv_argument_registers() {
    let windows = Target::parse("windows-x86_64").expect("Windows PE target");
    let emitter = crate::codegen_support::emit::Emitter::new(windows);
    let generated = (0..4)
        .map(|index| int_arg_reg_name(windows, index))
        .collect::<Vec<_>>();
    let runtime = (0..6)
        .map(|index| runtime_helper_int_arg_reg(&emitter, index))
        .collect::<Vec<_>>();

    assert_eq!(generated, ["rcx", "rdx", "r8", "r9"]);
    assert_eq!(runtime, ["rdi", "rsi", "rdx", "rcx", "r8", "r9"]);
}
