    ...
    match emitter.target.arch {
        ...
        Arch::X86_64 => {
            if offset == 0 {
                ...
                emitter.instruction(&format!(                                   // materialize the local-slot address relative to the frame pointer
                    "lea {}, [{} - {}]",
                    dest,
...
pub fn emit_store_zero_to_local_slot(emitter: &mut Emitter, offset: usize) {
    match emitter.target.arch {
        ...
        Arch::X86_64 => {
            if offset == 0 {
                emitter.instruction(&format!(                                   // zero-initialize the frame-base slot directly through the frame pointer
                    "mov QWORD PTR [{}], 0",
                    frame_pointer_reg(emitter)
                )); // zero-initialize the frame-base slot directly through rbp
            } else {
                emitter.instruction(&format!(                                   // zero-initialize the requested local slot relative to the frame pointer
                    "mov QWORD PTR [{} - {}], 0",
                    frame_pointer_reg(emitter),
                    offset
