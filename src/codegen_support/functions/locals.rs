...
//!
//! Called from:
//! - `crate::codegen_support::functions` before prologue emission
//!
//! Key details:
...
fn collect_assignment_expr_vars(expr: &Expr, ctx: &mut Context, sig: &FunctionSig) {
    match &expr.kind {
        ...
        ExprKind::Pipe { value, callable } => {
            ...
            let temp_name = crate::codegen_support::expr::calls::pipe_value_temp_name(expr.span);
            let static_ty = infer_local_type(value, sig, Some(ctx));
            let slot_ty = static_ty.codegen_repr();
            if ctx.variables.contains_key(&temp_name) {
                ctx.ensure_var_slot_capacity_for_type(&temp_name, &slot_ty);
        ...
        ExprKind::FirstClassCallable(CallableTarget::Method { object, .. }) => {
            ...
            if !matches!(&object.kind, ExprKind::Variable(_) | ExprKind::This) {
                let temp_name =
                    crate::codegen_support::expr::calls::first_class_method_receiver_temp_name(object.span);
...
fn collect_named_call_temps_for_sig(
    ...
    if plan.has_spread_args() {
        ...
        let prefix_name =
            crate::codegen_support::expr::calls::args::named_call_prefix_temp_name(call_span);
...
fn collect_call_arg_temp(
    ...
    let temp_name = crate::codegen_support::expr::calls::args::named_call_arg_temp_name(call_span, arg_idx);
