...
pub fn emit_stmt(stmt: &Stmt, emitter: &mut Emitter, ctx: &mut Context, data: &mut DataSection) {
    ...
    if let Some(slot) = ctx.concat_base_offset {
        let scratch = crate::codegen_support::abi::temp_int_reg(emitter.target);
        crate::codegen_support::abi::load_at_offset(emitter, scratch, slot);
        crate::codegen_support::abi::emit_store_reg_to_symbol(emitter, scratch, "_concat_off", 0);
    } else {
        crate::codegen_support::abi::emit_store_zero_to_symbol(emitter, "_concat_off", 0);
    }
