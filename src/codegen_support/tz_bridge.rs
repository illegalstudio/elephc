...
/// Publishes the exact PHP/timelib timezone-identifier validator before a
/// runtime path can call `__rt_date_default_timezone_set`.
///
/// The runtime itself is emitted for every binary, so it must never carry a
/// direct external relocation to `elephc_tz_is_valid`: that would link the
/// managed bridge into programs that never touch timezone mutation. This slot
/// is instead published at the relevant EIR lowering call site, after the
/// builtin requirement has selected `elephc_tz` for that program.
pub(crate) fn publish_elephc_tz_validation_function_pointer(emitter: &mut Emitter) {
    let (register, symbol_register) = match emitter.target.arch {
        Arch::AArch64 => ("x10", "x10"),
        Arch::X86_64 => ("r9", "r9"),
    };
    let extern_sym = emitter.target.extern_symbol("elephc_tz_is_valid");
    abi::emit_extern_symbol_address(emitter, register, &extern_sym);
    abi::emit_store_reg_to_symbol(emitter, symbol_register, "_elephc_tz_validate_fn", 0); // publish exact IANA identifier validation
}
