...
use crate::codegen_support::context::{Context, DeferredClosure, DeferredRuntimeCallableInvoker};
...
fn runtime_static_method_wrappers(ctx: &Context) -> Vec<(String, String, FunctionSig)> {
    ...
    for (class_name, class_info) in &ctx.classes {
        ...
        if crate::codegen_support::expr::objects::known_dynamic_new_builtin_class_names()
