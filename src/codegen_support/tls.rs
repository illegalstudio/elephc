...
mod tests {
    ...
    fn stream_crypto_lowering_forwards_method_presence_and_source_session() {
        ...
        let source = [
            "src/codegen/lower_inst/builtins/io/stream_sockets.rs",
            "src/codegen/lower_inst/builtins/io/close_crypto_arch.rs",
        ]
        .into_iter()
        .map(|relative| {
            fs::read_to_string(root.join(relative)).expect("read stream crypto lowering")
        })
        .collect::<String>();
        for fragment in [
            "str x9, [sp, #32]",
