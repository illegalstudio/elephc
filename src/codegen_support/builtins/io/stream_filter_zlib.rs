//! Purpose:
//! Emits the `zlib.deflate` write-direction stream filter attachment for
//! `stream_filter_append` / `stream_filter_prepend`.
//!
//! Called from:
//! - `crate::codegen_support::builtins::io::stream_filter::emit_attach()` when the
//!   filter-name literal is `"zlib.deflate"`.
//!
//! Key details:
...
/// Emits the `zlib.deflate` write-filter attachment. Returns the stream
/// re-boxed as a resource, matching `stream_filter_append`'s contract.
pub fn emit_zlib_deflate_attach(
    args: &[Expr],
    emitter: &mut Emitter,
    ctx: &mut Context,
    data: &mut DataSection,
) -> Option<PhpType> {
    emitter.comment("stream_filter_append(zlib.deflate)");
    emit_stream_fd_arg("stream_filter_append", &args[0], emitter, ctx, data);

    // PHP's 4th `$params` arg sets the deflate compression level (-1..9), as
    // either a bare int (`$rw, 6`) or the canonical array form (`['level' => 6]`).
    ...
    match emitter.target.arch {
        Arch::AArch64 => emit_arm64(emitter, &fwrite_label, &close_label, &skip_label, level),
        Arch::X86_64 => emit_x86_64(emitter, &fwrite_label, &close_label, &skip_label, level),
    }
    Some(PhpType::Mixed)
}
