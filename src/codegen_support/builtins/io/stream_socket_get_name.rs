//! Purpose:
//! Emits PHP `stream_socket_get_name` calls.
//! Yields the local or peer address of a socket as an `A.B.C.D:port` string.
//!
//! Called from:
//! - `crate::codegen_support::builtins::io::emit()`.
//!
//! Key details:
//! - Marshals the descriptor and the remote flag into the two
//!   `__rt_stream_socket_get_name` argument registers.
...
pub fn emit(
    ...
    emitter.comment("stream_socket_get_name()");
    emit_stream_fd_arg("stream_socket_get_name", &args[0], emitter, ctx, data);
    abi::emit_push_reg(emitter, abi::int_result_reg(emitter)); // preserve the descriptor
    emit_expr(&args[1], emitter, ctx, data);
    match emitter.target.arch {
        Arch::AArch64 => {
            emitter.instruction("mov x1, x0");                                  // remote flag into argument 1
            abi::emit_pop_reg(emitter, "x0"); // descriptor into argument 0
        }
        Arch::X86_64 => {
            emitter.instruction("mov rsi, rax");                                // remote flag into argument 1
            abi::emit_pop_reg(emitter, "rdi"); // descriptor into argument 0
        }
    }
    abi::emit_call_label(emitter, "__rt_stream_socket_get_name");
...
fn box_string_or_false(emitter: &mut Emitter, ctx: &mut Context) {
    let false_label = ctx.next_label("stream_socket_get_name_false");
    let done_label = ctx.next_label("stream_socket_get_name_done");

    match emitter.target.arch {
        Arch::AArch64 => {
            emitter.instruction(&format!("cbz x1, {}", false_label));           // a null pointer means the lookup failed
        ...
        Arch::X86_64 => {
            emitter.instruction("test rax, rax");                               // a null pointer means the lookup failed
            emitter.instruction(&format!("jz {}", false_label));                // box false when the lookup failed
