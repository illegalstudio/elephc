//! Purpose:
//! Emits PHP `gethostbyaddr` calls.
//! Reverse-resolves an IPv4 address to a host name.
//!
//! Called from:
//! - `crate::codegen_support::builtins::io::emit()`.
//!
//! Key details:
//! - Evaluates the address argument into the string result registers and
//!   delegates to `__rt_gethostbyaddr`. The helper returns a null pointer for a
//!   malformed address, which is boxed as PHP `false`; a found host name (or
//!   the unchanged address when no record exists) is boxed as a string.
...
pub fn emit(
    ...
    emitter.comment("gethostbyaddr()");
    emit_expr(&args[0], emitter, ctx, data);
    abi::emit_call_label(emitter, "__rt_gethostbyaddr");
...
fn box_string_or_false(emitter: &mut Emitter, ctx: &mut Context) {
    let false_label = ctx.next_label("gethostbyaddr_false");
    let done_label = ctx.next_label("gethostbyaddr_done");

    match emitter.target.arch {
        Arch::AArch64 => {
            emitter.instruction(&format!("cbz x1, {}", false_label));           // a null pointer means a malformed address
        ...
        Arch::X86_64 => {
            emitter.instruction("test rax, rax");                               // a null pointer means a malformed address
            emitter.instruction(&format!("jz {}", false_label));                // box false for a malformed address
