...
pub fn emit(
    ...
    match &val_ty {
        ...
        PhpType::Mixed | PhpType::Union(_) | PhpType::Array(_) | PhpType::AssocArray { .. } | PhpType::Object(_) => {
            ...
            if release_after_refcounted_push {
                crate::codegen_support::emit_release_pushed_refcounted_temp_after_array_push(emitter, &val_ty);
            }
        }
        PhpType::Callable => {
            // -- push callable descriptor pointer onto array as a plain 8-byte scalar --
            emitter.instruction("mov x1, x0");                                  // move callable descriptor pointer value to x1
            emitter.instruction("mov x0, x9");                                  // move outer array pointer to x0
            emitter.instruction("bl __rt_array_push_int");                      // append callable descriptor bits as a plain scalar slot
        }
        _ => {}
    }
...
fn update_array_push_arg_type(arg: &Expr, elem_ty: &PhpType, ctx: &mut Context) {
    if let crate::parser::ast::ExprKind::Variable(name) = &arg.kind {
        ...
        ctx.update_var_type_and_ownership(
            name,
            updated_ty.clone(),
            crate::codegen_support::context::HeapOwnership::local_owner_for_type(&updated_ty),
        );
    }
}
