...
fn emit_isset_arg(
    ...
    if let ExprKind::ArrayAccess { array, index } = &arg.kind {
        let array_ty = crate::codegen_support::functions::infer_contextual_type(array, ctx);
        if crate::codegen_support::expr::arrays::type_is_array_access_object(&array_ty, ctx) {
            crate::codegen_support::expr::arrays::emit_array_access_offset_exists(
...
fn emit_loaded_result_isset(ty: &PhpType, emitter: &mut Emitter) {
    match ty.codegen_repr() {
        ...
        PhpType::Int | PhpType::Bool if crate::codegen_support::sentinels::null_repr_is_tagged() => {
