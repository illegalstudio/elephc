    ...
    F: FnMut(&RuntimeCallableCase, Option<&PhpType>, &mut Emitter, &mut Context, &mut DataSection),
{
    ...
    let callback_ty = crate::codegen_support::functions::infer_contextual_type(callback, ctx).codegen_repr();
    match callback_ty {
        PhpType::Array(elem_ty) if matches!(elem_ty.codegen_repr(), PhpType::Mixed) => {
    ...
    ),
{
    ...
    emit_mixed_selector_slots(var_name, emitter, ctx, data);
    crate::codegen_support::expr::emit_expr(array, emitter, ctx, data);
    ...
    F: FnMut(&RuntimeCallableCase, Option<&PhpType>, &mut Emitter, &mut Context, &mut DataSection),
{
    ...
    let static_cases =
        crate::codegen_support::callable_dispatch::runtime_public_static_method_cases(ctx, data);
    crate::codegen_support::expr::emit_expr(callback, emitter, ctx, data);
...
fn emit_mixed_selector_slots(
    ...
    crate::codegen_support::expr::emit_expr(&receiver, emitter, ctx, data);
    emit_unbox_mixed_result(emitter);
    emit_push_mixed_unbox_payload(emitter);

    let method = callable_array_slot_expr(var_name, 1);
    crate::codegen_support::expr::emit_expr(&method, emitter, ctx, data);
    emit_unbox_mixed_result(emitter);
    emit_push_mixed_unbox_payload(emitter);
}
...
fn emit_string_selector_slots(
    ...
    crate::codegen_support::expr::emit_expr(&class, emitter, ctx, data);
