...
use crate::codegen_support::expr::calls::args;
...
use crate::codegen_support::callable_invoker_args::{
    emit_box_invoker_arg_clone_as_mixed, emit_branch_if_mixed_arg_tag,
    emit_call_user_func_array_invalid_mixed_args_abort,
    emit_clone_assoc_array_for_invoker_with_value_type, emit_clone_indexed_array_for_invoker,
    emit_clone_runtime_mixed_invoker_arg_as_mixed, INVOKER_ARG_REF_CELL_TAG,
};
...
pub fn emit(
        ...
        (&args[0].kind, &args[1].kind)
    {
        match lookup_function(ctx, name) {
            Some(FunctionLookup::Extern(extern_name)) => {
                return Some(crate::codegen_support::ffi::emit_extern_call(
                    &extern_name,
                    elems,
                    args[0].span,
                    emitter,
            ...
            Some(FunctionLookup::Builtin(builtin_name)) => {
                if let Some(ret_ty) = crate::codegen_support::builtins::emit_builtin_call(
                    &builtin_name,
                    elems,
                    args[0].span,
                    emitter,
    ...
    if save_concat_before_args {
        crate::codegen_support::expr::save_concat_offset_before_nested_call(emitter, ctx);
    }
    ...
    let direct_fcc_function =
        crate::codegen_support::callables::direct_first_class_function_sig(&args[0], ctx);
    let precomputed_sig = direct_fcc_function
        .as_ref()
        .map(|(_, sig)| sig.clone())
        .or_else(|| crate::codegen_support::callables::callable_sig(&args[0], ctx));
    ...
    let ret_ty = if let Some(sig) = sig {
        if sig.ref_params.iter().any(|is_ref| *is_ref) {
            ...
            let inferred_arg_array_ty =
                crate::codegen_support::functions::infer_contextual_type(&args[1], ctx);
...
fn emit_descriptor_invoker_call_user_func_array_expr(
    ...
    let sig = crate::codegen_support::callables::callable_sig(callback, ctx);
...
fn emit_descriptor_invoker_indexed_arg_array_for_call_user_func_array(
    ...
    crate::codegen_support::arrays::emit_array_value_type_stamp(
        emitter,
    ...
    for (index, elem) in elems.iter().enumerate() {
        ...
        let boxed_iterable = crate::codegen_support::emit_box_iterable_value_for_mixed_container(
            emitter,
            &mut ty,
        );
        if !matches!(ty, PhpType::Mixed | PhpType::Union(_)) {
            crate::codegen_support::emit_box_current_expr_value_as_mixed_for_container(emitter, elem, &ty);
...
fn emit_box_current_ref_arg_address_for_invoker(
    ...
    crate::codegen_support::emit_box_runtime_payload_as_mixed(
        emitter,
        marker_tag_reg,
        ref_cell_reg,
        source_tag_reg,
...
fn variable_runtime_value_tag(var_name: &str, ctx: &Context) -> u8 {
    ctx.variables
        .get(var_name)
        .map(|var| crate::codegen_support::runtime_value_tag(&var.ty.codegen_repr()))
        .unwrap_or_else(|| crate::codegen_support::runtime_value_tag(&PhpType::Int))
}
...
fn emit_spread_callback_call_from_array_expr(
    ...
    if concat_saved_before_args {
        ...
        crate::codegen_support::expr::restore_concat_offset_after_nested_call(emitter, ctx, &ret_ty);
    } else {
        crate::codegen_support::expr::restore_concat_offset_after_nested_call(emitter, ctx, &ret_ty);
        abi::emit_release_temporary_stack(emitter, overflow_bytes);
        abi::emit_release_temporary_stack(emitter, emitted_args.source_temp_bytes);
...
fn push_loaded_invoker_ref_cell_value_arg(
    ...
    let release_mixed_after_coerce = target_ty.is_some_and(|target_ty| {
        !matches!(target_ty.codegen_repr(), PhpType::Mixed | PhpType::Union(_))
            && crate::codegen_support::expr::can_coerce_result_to_type(&PhpType::Mixed, target_ty)
    });
...
fn emit_box_loaded_invoker_ref_cell_value_as_mixed(emitter: &mut Emitter, ctx: &mut Context) {
    ...
    crate::codegen_support::emit_box_runtime_payload_as_mixed(emitter, tag_reg, lo_reg, hi_reg);
}
...
fn emit_loaded_mixed_array_callback_call(
    ...
    emit_branch_if_mixed_arg_tag(
        tag_reg,
        crate::codegen_support::runtime_value_tag(&indexed_ty),
        &indexed_label,
        emitter,
    );
    emit_branch_if_mixed_arg_tag(
        tag_reg,
        crate::codegen_support::runtime_value_tag(&assoc_ty),
        &assoc_label,
        emitter,
    );
...
fn emit_call_descriptor_array_invoker_with_label(
    ...
    if concat_saved_before_args {
        ...
        crate::codegen_support::expr::restore_concat_offset_after_nested_call(
            emitter,
            ctx,
            &PhpType::Mixed,
        );
...
fn emit_normalized_invoker_arg_mixed(
    ...
    match arr_ty {
        ...
        PhpType::Mixed | PhpType::Union(_) => {
            emit_clone_runtime_mixed_invoker_arg_as_mixed(
                dest_reg,
                emitter,
                &mut |prefix| ctx.next_label(prefix),
                data,
            );
            PhpType::Mixed
        }
...
fn emit_loaded_indexed_array_unknown_callback_call(
    ...
    for case in cases {
            ...
            emit_case_descriptor_for_invoker(case, descriptor_source, call_reg, emitter)
        {
            ...
            crate::codegen_support::emit_box_current_value_as_mixed(emitter, &case_ret_ty.codegen_repr());
        }
        abi::emit_jump(emitter, &done_label);
        emitter.label(&next_case);
    }
    ...
    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &fallback_ret_ty.codegen_repr());
...
fn unknown_callback_register_arg_capacity(target: crate::codegen_support::platform::Target, elem_ty: &PhpType) -> usize {
    match elem_ty.codegen_repr() {
        PhpType::Float => 8,
