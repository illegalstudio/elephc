...
//!
//! Called from:
//! - `crate::codegen_support::builtins::spl::emit()`
//!
//! Key details:
...
use crate::codegen_support::builtins::arrays::callback_env;
use crate::codegen_support::builtins::arrays::call_user_func_array::{
    self, LoadedArraySource,
};
use crate::codegen_support::builtins::arrays::receiver_call_args;
use crate::codegen_support::builtins::arrays::runtime_callable_array_callback;
...
use crate::codegen_support::stmt::{emit_iterable_object_loop, emit_iterator_loop};
...
pub fn emit(
    ...
    let (captures, sig, callback_slot_kind, descriptor_arg_prefix) = if runtime_string_callback {
        ...
        let direct_fcc_function =
            crate::codegen_support::callables::direct_first_class_function_sig(&args[1], ctx);
        let precomputed_sig = direct_fcc_function
            .as_ref()
            .map(|(_, sig)| sig.clone())
            .or_else(|| crate::codegen_support::callables::callable_sig(&args[1], ctx));
    ...
    let callback_arg_source = match callback_args_expr(args) {
        ...
        {
            let arg_array_ty = crate::codegen_support::functions::infer_contextual_type(expr, ctx);
        ...
        Some(_) => {
            let (arg_expr, arg_ty) = match callback_args {
                ...
                CallbackArgsExpr::Evaluated { expr, .. } => {
                    (expr.clone(), crate::codegen_support::functions::infer_contextual_type(expr, ctx))
                }
            };
...
fn emit_callback_invocation(
    ...
    } = callback_arg_source
    {
        ...
        crate::codegen_support::expr::coerce_to_truthiness(emitter, ctx, &dynamic_ret_ty);
    ...
    crate::codegen_support::expr::coerce_to_truthiness(emitter, ctx, ret_ty);
    iterator_common::emit_increment_saved_count_at_offset(count_offset, emitter);
    emit_branch_if_callback_false(emitter, loop_end);
}
