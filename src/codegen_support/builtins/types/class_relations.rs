...
use crate::codegen_support::expr::arrays::{
    emit_assoc_array_literal, emit_empty_assoc_array_literal,
};
use crate::codegen_support::expr::emit_expr;
use crate::codegen_support::{abi, emit_box_current_value_as_mixed};
...
fn class_uses(target: &ClassLikeTarget, ctx: &Context) -> Vec<String> {
    match target {
        ClassLikeTarget::Class(class_name) => lookup_class(ctx, class_name)
            ...
            .unwrap_or_default(),
        ClassLikeTarget::Trait(trait_name) => crate::codegen_support::declared_trait_uses(trait_name),
