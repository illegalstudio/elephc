//! Purpose:
//! Emits PHP `md5($string, $binary = false)` calls.
//! Routes the data string and the optional `$binary` flag into the `__rt_md5`
...
//!   the `__rt_md5` register contract (data ptr/len + flag in AArch64 x5 /
//!   x86_64 r10).
...
/// Lowers the PHP `md5($string, $binary = false)` call.
///
...
/// elephc-crypto function pointer, and calls `__rt_md5`. Returns `PhpType::Str`.
///
...
pub fn emit(
    ...
    abi::emit_call_label(emitter, "__rt_md5");                                  // call the target-aware runtime helper that hashes through elephc-crypto and returns the PHP string
    Some(PhpType::Str)
}
