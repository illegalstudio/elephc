...
//!
//! Called from:
//! - `crate::codegen_support::builtins::strings::hash::emit()` and
//!   `crate::codegen_support::builtins::strings::hash_hmac::emit()` (each publishes the
//!   fn pointers immediately before its `__rt_hash`/`__rt_hash_hmac` call).
//! - `crate::codegen_support::runtime::strings::hash::emit_hash()` and
//!   `crate::codegen_support::runtime::strings::hash_hmac::emit_hash_hmac()` (emit the
...
//!   `crate::codegen_support::builtins::math::clamp`. The messages live in the fixed
