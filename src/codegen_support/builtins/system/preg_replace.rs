...
pub fn emit(
    ...
    match emitter.target.arch {
        ...
        Arch::X86_64 => {
            emit_expr(&args[2], emitter, ctx, data);
            crate::codegen_support::abi::emit_push_reg_pair(emitter, "rax", "rdx");     // push subject ptr and len
            emit_expr(&args[1], emitter, ctx, data);
            crate::codegen_support::abi::emit_push_reg_pair(emitter, "rax", "rdx");     // push replacement ptr and len
            emit_expr(&args[0], emitter, ctx, data);
            ...
            crate::codegen_support::abi::emit_pop_reg_pair(emitter, "rdx", "rcx");      // pop replacement ptr/len into the next SysV integer argument registers
            crate::codegen_support::abi::emit_pop_reg_pair(emitter, "r8", "r9");        // pop subject ptr/len into the remaining SysV integer argument registers
            crate::codegen_support::abi::emit_call_label(emitter, "__rt_preg_replace"); // regex replace → rax=result ptr, rdx=result len
        }
    }
