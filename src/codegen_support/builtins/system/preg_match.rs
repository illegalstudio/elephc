...
pub fn emit(
    ...
    match emitter.target.arch {
        ...
        Arch::X86_64 => {
            ...
            crate::codegen_support::abi::emit_pop_reg_pair(emitter, "rdx", "rcx");      // pop subject ptr/len into the remaining SysV argument registers
            if let Some(matches_arg) = args.get(2) {
                crate::codegen_support::abi::emit_call_label(emitter, "__rt_preg_match_capture"); // regex match → rax=match flag, rdx=matches array
                emit_store_matches_arg(emitter, ctx, matches_arg);
            } else {
                crate::codegen_support::abi::emit_call_label(emitter, "__rt_preg_match"); // regex match → rax=1 if matched, 0 if not
            }
        }
    }
