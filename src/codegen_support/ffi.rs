...
use crate::codegen_support::builtins::callable_lookup::{lookup_function, FunctionLookup};
use crate::codegen_support::context::{
    Context, DeferredExternCallbackTrampoline, HeapOwnership,
};
use crate::codegen_support::data_section::DataSection;
use crate::codegen_support::emit::Emitter;
use crate::codegen_support::expr::{can_coerce_result_to_type, coerce_result_to_type, emit_expr};
...
pub fn emit_extern_call(
    ...
    let regular_param_count =
        crate::codegen_support::expr::calls::args::regular_param_count(Some(&call_sig), args.len());
    let normalized = if crate::codegen_support::expr::calls::args::has_named_args(args) {
        crate::codegen_support::expr::calls::args::preevaluate_named_call_args_to_temps(
            &call_sig,
            args,
            call_span,
            regular_param_count,
        ...
        crate::codegen_support::expr::calls::args::normalize_named_call_args_with_checks(
            &call_sig,
            args,
            regular_param_count,
        )
    };
    crate::codegen_support::expr::calls::args::emit_spread_length_checks(
    ...
    crate::codegen_support::expr::restore_concat_offset_after_nested_call(emitter, ctx, &nested_return_ty);
    ...
    for (i, arg) in args.iter().enumerate() {
        ...
        if can_coerce_result_to_type(&actual_ty, &param_ty) {
            if should_release_owned_mixed_after_extern_arg_coerce(arg, &actual_ty, &param_ty) {
                ...
                crate::codegen_support::expr::calls::args::release_preserved_mixed_after_arg_coercion(
                    emitter,
                    &param_ty,
                );
            } else {
...
fn emit_extern_callable_arg(
    ...
    match &arg.kind {
        ...
        _ => {
            let precomputed_sig = crate::codegen_support::callables::callable_sig(arg, ctx);
...
fn expr_result_needs_retain_for_extern_callback_slot(arg: &Expr) -> bool {
    !matches!(
        crate::codegen_support::expr::expr_result_heap_ownership(arg),
        HeapOwnership::Owned
    )
}
...
fn should_release_owned_mixed_after_extern_arg_coerce(
    ...
    matches!(source_repr, PhpType::Mixed | PhpType::Union(_))
        ...
        && (crate::codegen_support::expr::expr_result_heap_ownership(arg) == HeapOwnership::Owned
            || matches!(
                arg.kind,
                ExprKind::BinaryOp {
