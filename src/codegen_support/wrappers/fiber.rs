...
//!
//! Called from:
//! - `crate::codegen` and `crate::codegen_support::driver_support` after deferred wrappers are registered.
//!
//! Key details:
...
use crate::codegen_support::{abi, callable_descriptor, runtime};
use crate::types::PhpType;

use super::DeferredFiberWrapper;
    ...
    match ty {
        ...
        _ => {
            emitter.instruction(&format!(
                ...
                slot_offset
            )); // spill the descriptor-captured payload for the final call
...
fn box_wrapper_return(emitter: &mut Emitter, return_ty: PhpType) {
    ...
    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &return_ty);
}
