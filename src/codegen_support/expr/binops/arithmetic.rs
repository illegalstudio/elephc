...
use crate::codegen_support::context::HeapOwnership;
...
fn emit_mixed_numeric_binop(
    ...
    if right_was_boxed {
        crate::codegen_support::emit_box_current_value_as_mixed(emitter, right_ty);
    }
    abi::emit_push_reg(emitter, abi::int_result_reg(emitter));
    load_saved_numeric_operand(emitter, left_stack_ty, 16);
    if left_was_boxed {
        crate::codegen_support::emit_box_current_value_as_mixed(emitter, left_stack_ty);
    }
    match emitter.target.arch {
        Arch::AArch64 => {
