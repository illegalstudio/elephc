...
pub(crate) fn emit_new_dynamic(
    ...
    crate::codegen_support::expr::emit_expr(name_expr, emitter, ctx, data);
...
/// `crate::codegen_support::callable_dispatch`), which would otherwise reference an unemitted symbol.
...
pub(super) fn emit_boxed_null(emitter: &mut Emitter) {
    ...
    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &PhpType::Void);
}
...
pub(super) fn box_nullable_result(result_ty: &PhpType, emitter: &mut Emitter) {
    if !matches!(result_ty.codegen_repr(), PhpType::Mixed) {
        crate::codegen_support::emit_box_current_value_as_mixed(emitter, result_ty);
    }
}
...
pub(crate) fn push_magic_property_name_arg(
    ...
    crate::codegen_support::abi::emit_symbol_address(emitter, ptr_reg, &label); // materialize the magic-property name string address for the active target ABI
    crate::codegen_support::abi::emit_load_int_immediate(emitter, len_reg, len as i64); // materialize the magic-property name length for the active target ABI
    crate::codegen_support::abi::emit_push_reg_pair(emitter, ptr_reg, len_reg); // push the magic-property name argument pair onto the temporary call stack
}
