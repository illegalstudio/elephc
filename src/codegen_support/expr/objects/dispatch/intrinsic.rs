...
fn emit_fiber_start_intrinsic(
    ...
    let max_arg_off = crate::codegen_support::runtime::FIBER_USER_ARG_MAX_OFFSET;
    let skip_label = ctx.next_label("fiber_start_args_done");
    let supplied_arg_count = assignments
        .len()
        .min(crate::codegen_support::runtime::FIBER_START_ARGS_MAX as usize);
    match emitter.target.arch {
        Arch::AArch64 => {
            ...
            for i in 0..supplied_arg_count {
                ...
                let off = crate::codegen_support::runtime::FIBER_START_ARGS_OFFSET + (i as i32) * 8;
    ...
    match emitter.target.arch {
        Arch::AArch64 => {
            ...
            emitter.instruction(&format!("str x9, [x0, #{}]", crate::codegen_support::runtime::FIBER_START_ARG_COUNT_OFFSET)); // record start() arity for descriptor-backed Fiber invokers
        }
        Arch::X86_64 => {
            emitter.instruction(&format!("mov QWORD PTR [rdi + {}], {}", crate::codegen_support::runtime::FIBER_START_ARG_COUNT_OFFSET, supplied_arg_count)); // record start() arity for descriptor-backed Fiber invokers
        }
    }
    abi::emit_call_label(
        emitter,
