...
use crate::codegen_support::builtins::arrays::{callback_env, runtime_callable_array_callback};
use crate::codegen_support::callable_dispatch::RuntimeCallableCase;
use crate::codegen_support::{abi, runtime_value_tag};
use crate::codegen_support::context::{Context, HeapOwnership};
use crate::codegen_support::data_section::DataSection;
use crate::codegen_support::emit::Emitter;
use crate::codegen_support::expr::calls::args as call_args;
use crate::codegen_support::platform::Arch;
use crate::codegen_support::{NULL_SENTINEL, UNINITIALIZED_TYPED_PROPERTY_SENTINEL};
...
pub(super) fn emit_new_object_core(
    ...
    for i in 0..num_props {
        if let Some(default_expr) = &class_info.defaults[i] {
            ...
            let prop_ty = if let Some(assoc_ty) =
                crate::codegen_support::expr::arrays::emit_array_literal_as_assoc_target(
                    &default_expr,
                    &expected_ty,
                    emitter,
                    ctx,
    ...
    if run_constructor && class_info.methods.contains_key("__construct") {
        ...
        let assignments = crate::codegen_support::abi::build_outgoing_arg_assignments_for_target(
            emitter.target,
            &arg_types,
            1,
        );
        let overflow_bytes =
            crate::codegen_support::abi::materialize_outgoing_args(emitter, &assignments);
...
fn emit_new_spl_array_storage_object(
    ...
    let keys_ty = crate::codegen_support::builtins::arrays::array_keys::emit_loaded_keys(
        &source_ty,
        emitter,
        ctx,
    )
    ...
    load_storage_source_from_stack(emitter);
    let values_ty = crate::codegen_support::builtins::arrays::array_values::emit_loaded_values(
        &source_ty,
        emitter,
        ctx,
        data,
...
fn emit_store_callback_filter_adapter_descriptor_at_stack_offset(
    ...
    crate::codegen_support::callable_descriptor::emit_load_descriptor_address_with_meta(
        emitter,
        data,
        ...
        crate::codegen_support::callable_descriptor::CallableDescriptorInvocation::new(
            crate::codegen_support::callable_descriptor::CallableDescriptorShape::CallbackAdapter,
        ),
    );
...
fn emit_callback_filter_callable_arg(
    ...
    let captures = crate::codegen_support::callables::callable_captures(&capture_source, ctx);
...
fn callback_filter_target_arg_types(callback_expr: &Expr, ctx: &Context) -> Vec<PhpType> {
    let sig = match &callback_expr.kind {
        ...
        _ => crate::codegen_support::callables::callable_sig(callback_expr, ctx),
    };
    sig.map(|sig| {
        sig.params
            .into_iter()
