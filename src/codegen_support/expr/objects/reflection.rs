...
//!
//! Called from:
//! - `crate::codegen_support::expr::objects::allocation::emit_new_object()`
//!
//! Key details:
...
pub(super) fn emit_new_reflection_owner(
    ...
    if let Some(reflected_name) = metadata.reflected_name.as_deref() {
        crate::codegen_support::reflection::emit_set_string_property(
            emitter,
            data,
            reflected_name,
...
fn overwrite_attrs_property(
    ...
    crate::codegen_support::reflection::emit_reflection_attribute_array(
        attr_names,
        attr_args,
        emitter,
        ctx,
...
fn class_name_arg(expr: &Expr, ctx: &Context) -> Option<String> {
    match &expr.kind {
        ExprKind::StringLiteral(name) => crate::codegen_support::reflection::resolve_class_name(
            &ctx.classes,
            name,
        )
        .map(str::to_string),
...
fn resolve_static_receiver_class(receiver: &StaticReceiver, ctx: &Context) -> Option<String> {
    match receiver {
        StaticReceiver::Named(name) => crate::codegen_support::reflection::resolve_class_name(
            &ctx.classes,
            &name.as_canonical(),
        )
        .map(str::to_string),
