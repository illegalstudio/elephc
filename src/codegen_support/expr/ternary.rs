...
use crate::codegen_support::{abi, functions};
...
fn coerce_branch_result(
    ...
    if matches!(result_ty, PhpType::Mixed | PhpType::Union(_)) {
        crate::codegen_support::emit_box_current_value_as_mixed(emitter, branch_ty);
    ...
    } else if crate::codegen_support::expr::can_coerce_result_to_type(branch_ty, result_ty) {
        let release_mixed_after_coerce = !matches!(result_ty, PhpType::Mixed | PhpType::Union(_))
            && crate::codegen_support::stmt::helpers::should_release_owned_mixed_after_coerce(
                branch_expr,
                branch_ty,
                result_ty,
            );
        ...
        if release_mixed_after_coerce {
            crate::codegen_support::stmt::helpers::release_preserved_mixed_after_coercion(
                emitter,
                result_ty,
            );
        }
