...
pub(in crate::codegen_support::expr) fn emit_null_coalesce(
    value: &Expr,
    default: &Expr,
    emitter: &mut Emitter,
    ctx: &mut Context,
    ...
    let default_ty = crate::codegen_support::functions::infer_contextual_type(default, ctx);
    let result_ty = widen_codegen_type(&val_ty, &default_ty);

    if matches!(val_ty, PhpType::Int) && crate::codegen_support::sentinels::null_repr_is_tagged() {
    ...
    if matches!(val_ty, PhpType::TaggedScalar) {
        crate::codegen_support::sentinels::emit_branch_if_tagged_scalar_not_null(emitter, &use_value_label);
