...
//!
//! Called from:
//! - `crate::codegen_support::expr::compare`
//!
//! Key details:
...
pub(in crate::codegen_support::expr) fn emit_strict_compare(
    left: &Expr,
    op: &BinOp,
    right: &Expr,
    emitter: &mut Emitter,
    ...
    let types_match = match (&lt_peek, &rt_peek) {
        ...
        (Some(l), Some(r))
            if crate::codegen_support::sentinels::null_repr_is_tagged()
            ...
            || matches!(rt, PhpType::Mixed | PhpType::Union(_) | PhpType::TaggedScalar)
        {
            ...
            match &lt {
                PhpType::Float => {
                    ...
                    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &lt);  // box the left float operand so mixed comparison can inspect its runtime tag
                ...
                PhpType::Str => {
                    ...
                    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &lt);  // box the left string payload so mixed comparison can inspect its runtime tag
                    match emitter.target.arch {
                        crate::codegen_support::platform::Arch::AArch64 => {
                            emitter.instruction("str x0, [sp, #16]");           // replace the old left comparison slot with the boxed left mixed pointer
                        }
                ...
                PhpType::TaggedScalar => {
                    ...
                    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &lt);  // box the left tagged scalar so mixed comparison can inspect its runtime tag
                    match emitter.target.arch {
                        crate::codegen_support::platform::Arch::AArch64 => {
                            emitter.instruction("str x0, [sp, #16]");           // replace the old left comparison slot with the boxed left mixed pointer
                        }
                ...
                _ => {
                    ...
                    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &lt);  // box the left operand when it is not already mixed
                    match emitter.target.arch {
                        crate::codegen_support::platform::Arch::AArch64 => {
                            emitter.instruction("str x0, [sp, #16]");           // replace the old left comparison slot with the boxed left mixed pointer
                        }
            ...
            crate::codegen_support::emit_box_current_value_as_mixed(emitter, &rt);          // box the right operand when it is not already mixed
