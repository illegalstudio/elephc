...
//!
//! Called from:
//! - `crate::codegen_support::expr` and statement assignment emitters
//!
//! Key details:
...
fn coerce_to_string_inner(
    ...
    match ty {
        ...
        PhpType::TaggedScalar => {
            ...
            crate::codegen_support::sentinels::emit_branch_if_tagged_scalar_null(emitter, &null_label);
...
pub fn coerce_null_to_zero(emitter: &mut Emitter, ty: &PhpType) {
    ...
    } else if *ty == PhpType::Int && crate::codegen_support::sentinels::null_repr_is_tagged() {
