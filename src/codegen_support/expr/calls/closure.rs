...
use crate::codegen_support::context::{Context, DeferredClosure};
    ...
    fn collect_return_types(
        ...
        match &stmt.kind {
            StmtKind::Return(Some(expr)) => {
                return_types.push(crate::codegen_support::functions::infer_local_type_with_ctx(
                    expr,
                    sig,
                    capture_ctx,
                ));
...
pub(super) fn emit_closure(
    ...
    super::descriptor_value::emit_callable_descriptor_value(
        &closure_label,
        None,
        crate::codegen_support::callable_descriptor::CALLABLE_DESC_KIND_CLOSURE,
        &sig,
        &capture_types,
        &hidden_params,
        crate::codegen_support::callable_descriptor::CallableDescriptorInvocation::new(
            crate::codegen_support::callable_descriptor::CallableDescriptorShape::Closure,
        ),
        emitter,
        ctx,
        data,
...
pub(super) fn emit_closure_call(
        ...
        .map(str::to_string)
    {
            ...
            .is_some_and(|class_info| class_info.methods.contains_key("__invoke"))
        {
            ...
            return crate::codegen_support::expr::objects::emit_method_call(
        ...
        match &target {
            CallableTarget::Function(name) => {
                ...
                if ctx.extern_functions.contains_key(name_str) {
                    return crate::codegen_support::ffi::emit_extern_call(
                        name_str, args_exprs, span, emitter, ctx, data,
                    );
                }
                if let Some(ty) = crate::codegen_support::builtins::emit_builtin_call(
                    name_str, args_exprs, span, emitter, ctx, data,
                ) {
                    return ty;
                }
            ...
            CallableTarget::StaticMethod { receiver, method } => {
                ...
                if matches!(receiver, StaticReceiver::Named(_) | StaticReceiver::Static) {
                    return crate::codegen_support::expr::objects::emit_static_method_call(
    ...
    if ctx.ref_params.contains(var) {
        crate::codegen_support::abi::load_at_offset(emitter, call_reg, var_offset);     // load the by-reference callable slot address into the nested-call scratch register
        crate::codegen_support::abi::emit_load_from_address(emitter, call_reg, call_reg, 0);
    } else {
        crate::codegen_support::abi::load_at_offset(emitter, call_reg, var_offset);     // load the callable descriptor into the nested-call scratch register
    }
    ...
    crate::codegen_support::abi::emit_push_reg(emitter, call_reg);

    let assignments =
        crate::codegen_support::abi::build_outgoing_arg_assignments_for_target(emitter.target, &arg_types, 0);

    crate::codegen_support::abi::emit_pop_reg(emitter, call_reg);
...
fn emit_descriptor_invoker_variable_call(
    ...
    let var_offset = var_info.stack_offset;
    if ctx.ref_params.contains(var) {
        crate::codegen_support::abi::load_at_offset(
            emitter,
            crate::codegen_support::abi::int_result_reg(emitter),
            var_offset,
        );
        crate::codegen_support::abi::emit_load_from_address(
            emitter,
            crate::codegen_support::abi::int_result_reg(emitter),
            crate::codegen_support::abi::int_result_reg(emitter),
            0,
    ...
    crate::codegen_support::abi::emit_push_reg(emitter, crate::codegen_support::abi::int_result_reg(emitter)); // preserve the callable descriptor while building variable-call arguments
    ...
    crate::codegen_support::builtins::arrays::call_user_func_array::emit_call_descriptor_array_invoker(
        crate::codegen_support::builtins::arrays::call_user_func_array::LoadedArraySource::TemporaryStackSlot(0),
        &arr_ty,
        call_reg,
