...
//!
//! Called from:
//! - `crate::codegen_support::expr::calls::emit_runtime_callable_array_call()`
//! - `crate::codegen_support::expr::calls::emit_callable_array_literal_call()`
//! - `crate::codegen_support::expr::calls::emit_callable_array_variable_call()`
//!
//! Key details:
...
pub(super) fn emit_literal_call(
    ...
    let callee_ty = crate::codegen_support::functions::infer_contextual_type(callee, ctx).codegen_repr();
    match callee_ty {
        PhpType::Array(elem_ty) if matches!(elem_ty.codegen_repr(), PhpType::Mixed) => {
...
fn emit_mixed_variable_call(
    ...
    let instance_cases =
        crate::codegen_support::callable_dispatch::runtime_public_instance_method_cases(ctx, data);
    let static_cases =
        crate::codegen_support::callable_dispatch::runtime_public_static_method_cases(ctx, data);
...
fn emit_instance_literal_case_call(
    ...
    let arr_ty = if let Some(arg_array) = single_spread_inner(args) {
        let arg_array_ty = crate::codegen_support::functions::infer_contextual_type(arg_array, ctx);
