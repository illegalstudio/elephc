...
//!
//! Called from:
//! - `crate::codegen_support::expr::calls::indirect`
//!
//! Key details:
...
use crate::codegen_support::builtins::arrays::descriptor_arg_builder;
...
fn emit_descriptor_prefix_expr_as_mixed_hash(
    ...
    let Some(prefix_expr) = prefix_expr else {
        crate::codegen_support::expr::arrays::emit_empty_assoc_array_literal(
            PhpType::Mixed,
            PhpType::Mixed,
            emitter,
        );
    ...
    match prefix_ty {
        PhpType::AssocArray { .. } => {
            crate::codegen_support::callable_invoker_args::emit_clone_assoc_array_for_invoker(
...
fn emit_saved_object_prefix_numeric_entry(object_stack_offset: usize, emitter: &mut Emitter) {
    ...
    abi::emit_load_int_immediate(
        emitter,
        tag_reg,
        crate::codegen_support::runtime_value_tag(&PhpType::Object(String::new())) as i64,
    );
    crate::codegen_support::emit_box_runtime_payload_as_mixed(emitter, tag_reg, object_reg, zero_reg);
...
fn emit_mixed_prefix_as_hash_or_abort(
    ...
    crate::codegen_support::callable_invoker_args::emit_clone_indexed_array_for_invoker_with_runtime_tag(
        abi::int_result_reg(emitter),
        emitter,
    );
    ...
    abi::emit_load_temporary_stack_slot(emitter, abi::int_result_reg(emitter), 0);
    crate::codegen_support::callable_invoker_args::emit_clone_assoc_array_for_invoker(abi::int_result_reg(emitter), emitter);
...
fn emit_mixed_invoker_hash_value(
    ...
    let boxed_iterable =
        crate::codegen_support::emit_box_iterable_value_for_mixed_container(emitter, &mut value_ty);
...
fn emit_hash_set_current_mixed_named_suffix(
    ...
    match emitter.target.arch {
        Arch::AArch64 => {
            ...
            abi::emit_load_int_immediate(emitter, "x5", crate::codegen_support::runtime_value_tag(&PhpType::Mixed) as i64);
        ...
        Arch::X86_64 => {
            ...
            abi::emit_load_int_immediate(emitter, "r9", crate::codegen_support::runtime_value_tag(&PhpType::Mixed) as i64);
