...
pub(super) fn first_class_callable_sig(target: &CallableTarget, ctx: &Context) -> Option<FunctionSig> {
    let sig = match target {
        ...
        CallableTarget::Method { object, method } => {
            ...
            let class_name = crate::codegen_support::functions::singular_object_class(&object_ty)?;
            ctx.classes
                .get(class_name)
    ...
    match &object.kind {
        ExprKind::Variable(name) => {
            let ty = ctx
                ...
                .unwrap_or_else(|| crate::codegen_support::functions::infer_contextual_type(object, ctx));
            Some((name.clone(), ty))
        }
        ExprKind::This => ctx
            .variables
        ...
        _ => {
            ...
            let receiver_static_ty = crate::codegen_support::functions::infer_contextual_type(object, ctx);
            let receiver_ty = crate::codegen_support::expr::emit_expr(object, emitter, ctx, data);
...
fn descriptor_invocation_for_target(
    ...
    match target {
        ...
        CallableTarget::Method { object, method } => {
            ...
            let receiver_name = crate::codegen_support::functions::singular_object_class(
                &crate::codegen_support::functions::infer_contextual_type(receiver_object, ctx),
            )
            .map(str::to_string);
            CallableDescriptorInvocation::method(
...
pub(super) fn emit_first_class_callable(
    ...
    super::descriptor_value::emit_callable_descriptor_value(
        &wrapper_label,
        None,
        crate::codegen_support::callable_descriptor::CALLABLE_DESC_KIND_FIRST_CLASS,
        &sig,
        &captures,
        &hidden_params,
