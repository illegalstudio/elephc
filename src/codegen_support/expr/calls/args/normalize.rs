...
fn preevaluate_named_spread_args_to_temps(
    ...
    crate::codegen_support::stmt::emit_assign_stmt(&prefix_name, &prefix_expr, emitter, ctx, data);
        ...
        match &arg.kind {
            ...
            _ => {
                ...
                if is_ref || is_side_effect_free_literal(arg) {
                    ...
                    crate::codegen_support::stmt::emit_assign_stmt(&temp_name, arg, emitter, ctx, data);
...
fn preevaluate_named_value_if_needed(
    ...
    crate::codegen_support::stmt::emit_assign_stmt(&temp_name, value, emitter, ctx, data);
    ...
    call_args::expand_static_assoc_spread_args(args)
        ...
        .map(|arg| match &arg.kind {
            ExprKind::Spread(inner) => matches!(
                crate::codegen_support::functions::infer_contextual_type(inner, ctx),
                PhpType::AssocArray { .. }
            ),
            _ => false,
        })
