...
use crate::codegen_support::{abi, context::Context, data_section::DataSection, platform::Arch};
...
fn push_loaded_mixed_hash_value_arg(
    ...
    emit_branch_if_hash_value_tag(
        raw_tag_reg,
        crate::codegen_support::runtime_value_tag(&PhpType::Mixed),
        &nested_probe_label,
        emitter,
    );
...
fn emit_box_raw_invoker_ref_cell_value_as_mixed(
    ...
    crate::codegen_support::emit_box_runtime_payload_as_mixed(emitter, tag_scratch, lo_reg, hi_reg);
}
...
fn materialize_hash_value_to_result(emitter: &mut Emitter, source_elem_ty: &PhpType) {
    match emitter.target.arch {
        crate::codegen_support::platform::Arch::AArch64 => match source_elem_ty.codegen_repr() {
            ...
            PhpType::Mixed | PhpType::Union(_) => {
                crate::codegen_support::emit_box_runtime_payload_as_mixed(emitter, "x3", "x1", "x2");
            }
            _ => {
                emitter.instruction("mov x0, x1");                              // move the hash pointer payload into the standard result register
            }
        },
        crate::codegen_support::platform::Arch::X86_64 => match source_elem_ty.codegen_repr() {
            PhpType::Int | PhpType::Bool => {
            ...
            PhpType::Mixed | PhpType::Union(_) => {
                crate::codegen_support::emit_box_runtime_payload_as_mixed(emitter, "rcx", "rdi", "rsi");
            }
            _ => {
