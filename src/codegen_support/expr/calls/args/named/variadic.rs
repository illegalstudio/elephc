...
pub(super) fn emit_variadic_array_arg_from_sources(
    ...
    let (capacity_reg, elem_size_reg, peek_reg, len_reg) = match emitter.target.arch {
        crate::codegen_support::platform::Arch::AArch64 => ("x0", "x1", "x9", "x10"),
        crate::codegen_support::platform::Arch::X86_64 => ("rdi", "rsi", "r11", "r10"),
    };
    ...
    for (idx, source) in variadic_sources.iter().enumerate() {
        ...
        {
            crate::codegen_support::emit_box_current_value_as_mixed(emitter, &elem_ty);
        ...
        if idx == 0 {
            crate::codegen_support::arrays::emit_array_value_type_stamp(emitter, peek_reg, &elem_ty);
        }
...
fn emit_prefix_tail_into_variadic_hash(
    ...
    let zero_reg = match emitter.target.arch {
        crate::codegen_support::platform::Arch::AArch64 => "xzr",
        crate::codegen_support::platform::Arch::X86_64 => "0",
    };
    ...
    abi::emit_load_int_immediate(
        emitter,
        value_tag_reg,
        crate::codegen_support::runtime_value_tag(&elem_ty) as i64,
    );
...
fn emit_variadic_assoc_arg_from_sources(
    ...
    abi::emit_load_int_immediate(
        emitter,
        tag_reg,
        crate::codegen_support::runtime_value_tag(&container_elem_ty) as i64,
    );
