...
//!
//! Called from:
//! - `crate::codegen_support::expr::arrays`
//!
//! Key details:
...
pub(crate) fn emit_array_literal(
    ...
    for (i, elem) in elems.iter().enumerate() {
        ...
        let boxed_iterable =
            crate::codegen_support::emit_box_iterable_value_for_mixed_container(emitter, &mut ty);
            ...
            match &ty {
                ...
                _ => {
                    if ty.is_refcounted() {
                        ...
                        crate::codegen_support::emit_release_pushed_refcounted_temp_after_array_push(emitter, &ty); // drop the codegen's owning reference now that the array holds its own
                    } else {
                        emitter.instruction("mov x1, x0");                      // x1 = value to push
                        emitter.instruction("mov x0, x9");                      // x0 = array pointer
                        emitter.instruction("bl __rt_array_push_int");          // push value onto array
...
fn infer_indexed_literal_element_type(elems: &[Expr], ctx: &Context) -> PhpType {
    ...
    for (i, elem) in elems.iter().enumerate() {
        let next_ty = match &elem.kind {
            ExprKind::Spread(inner) => match crate::codegen_support::functions::infer_contextual_type(inner, ctx) {
                PhpType::Array(inner_ty) => *inner_ty,
                _ => PhpType::Mixed,
            },
            _ => crate::codegen_support::functions::infer_contextual_type(elem, ctx),
        };
