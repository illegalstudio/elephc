...
use crate::codegen_support::emit_box_runtime_payload_as_mixed;
...
pub(crate) fn emit_array_access_with_loaded_base(
    ...
    if let Some(val_ty) = assoc_value_ty {
        ...
        if boxed_assoc_base {
            ...
            match emitter.target.arch {
                Arch::AArch64 => {
                    ...
                    } else if crate::codegen_support::sentinels::null_repr_is_tagged()
        ...
        let tagged_int_result = crate::codegen_support::sentinels::null_repr_is_tagged()
            && matches!(val_ty, PhpType::Int)
            && !box_assoc_result;
