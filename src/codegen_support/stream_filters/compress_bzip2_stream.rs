//! Purpose:
//! Emits the shared bzip2 read-transform body used by stream-filter lowering.
//! Slurps the compressed payload, decompresses it through libbz2, writes the
//! plain bytes to a temp file, then `dup2`s that fd over the source descriptor.
//!
//! Called from:
//! - `crate::codegen_support::stream_filters::bzip2` for `bzip2.decompress`.
//!
//! Key details:
