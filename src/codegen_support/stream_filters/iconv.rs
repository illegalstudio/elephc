//! Purpose:
//! Emits the `convert.iconv.<from>/<to>` read-transform stream filter body for
//! EIR lowering.
//!
//! Called from:
//! - `crate::codegen::lower_inst::builtins::io` when lowering iconv stream-filter
//!   attachments.
//!
//! Key details:
    ...
    F: FnMut(&str) -> String,
{
    ...
    emitter.instruction(&format!(                                               // pass the iconv source buffer to the conversion helper
        "mov r10, 0x{:x}",
