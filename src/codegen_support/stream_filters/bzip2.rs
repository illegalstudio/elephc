...
//! (read-direction) stream filter helpers for EIR lowering.
//!
//! Called from:
//! - `crate::codegen::lower_inst::builtins::io` when lowering `stream_filter_append`
//!   or `stream_filter_prepend` for `"bzip2.compress"` or `"bzip2.decompress"`.
//!
//! Key details:
...
pub(crate) fn emit_compress_x86_64(
    ...
    emitter.instruction(&format!(                                               // pass the decompression input buffer to the native filter
