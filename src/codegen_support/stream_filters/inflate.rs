//! Purpose:
//! Emits the `zlib.inflate` read-direction stream filter body for EIR lowering.
//!
//! Called from:
//! - `crate::codegen::lower_inst::builtins::io` when lowering `stream_filter_append`
//!   or `stream_filter_prepend` for the `"zlib.inflate"` literal filter.
//!
//! Key details:
    ...
    F: FnMut(&str) -> String,
{
    ...
    emitter.instruction(&format!(                                               // pass the inflate input buffer to the native filter
