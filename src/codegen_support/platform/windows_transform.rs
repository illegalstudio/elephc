...
"#;
    ...
    fn test_full_windows_runtime_has_conservative_unwind_coverage() {
        ...
        // Count the independently unwindable regions AFTER the same promotion
        // pass used by `transform_for_windows`. A raw textual prologue nested in
        // a live RBP frame cannot receive a distinct PE function-table record;
        // an independently entered internal frame is given a synthetic local
        // region first and must be described or rejected just like a global one.
        let promoted = promote_independent_frame_regions(&raw);
        let promoted_lines = promoted.lines().collect::<Vec<_>>();
        for (index, line) in promoted_lines.iter().enumerate() {
            let Some(symbol) = frame_region_symbol(line.trim()) else {
                continue;
            };
            if !symbol.starts_with("__rt_")
                && !symbol.starts_with(".L__elephc_unwind_region_")
            {
                continue;
            }
            let end = function_segment_end(&promoted_lines, index);
            let segment = &promoted_lines[index..end];
        ...
        eprintln!(
            "Windows runtime unwind audit: {runtime_functions} functions, {leaf_functions} true leaves, {functions_requiring_unwind} requiring unwind state; {described_runtime_frames}/{canonical_frames} canonical frame regions described, {rejected_canonical_frames} regions explicitly rejected {rejection_counts:?}: {rejected_symbols:?}"
        );
        assert!(canonical_frames > 0, "runtime audit fixture has no canonical frames");
        assert!(
            !rejection_counts.is_empty(),
            "the conservative runtime audit must exercise its explicit rejection path"
        );
        assert!(
            described_runtime_frames + rejected_canonical_frames >= canonical_frames,
            "every canonical region must be described or explicitly rejected: {described_runtime_frames} described, {rejected_canonical_frames} rejected, {canonical_frames} canonical"
        );
