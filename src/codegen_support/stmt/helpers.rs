...
//!
//! Called from:
//! - `crate::codegen_support::stmt` submodules
//!
//! Key details:
...
pub(crate) fn should_release_owned_mixed_after_coerce(
    ...
    matches!(source_ty, PhpType::Mixed | PhpType::Union(_))
        && crate::codegen_support::expr::can_coerce_result_to_type(source_ty, target_ty)
...
pub(super) fn indexed_array_runtime_value_tag(ty: &PhpType) -> i64 {
    crate::codegen_support::runtime_value_tag(ty) as i64
}
