...
use crate::codegen_support::expr::{coerce_result_to_type, emit_expr, objects};
...
pub(crate) fn emit_property_assign_stmt(
    ...
    if let Some((current, default)) =
        crate::codegen_support::stmt::null_coalesce_property_target(object, property, value)
    {
    ...
    if let Some(target_ty) = &declared_target_ty {
        ...
        } else if crate::codegen_support::expr::can_coerce_result_to_type(&val_ty, target_ty) {
...
fn is_stdclass_receiver(object: &Expr, ctx: &Context) -> bool {
    ...
    crate::codegen_support::functions::singular_object_class(&obj_ty)
...
fn nullable_object_class(object: &Expr, ctx: &Context) -> Option<String> {
    ...
    crate::codegen_support::functions::singular_object_class(&obj_ty).map(str::to_string)
}
...
fn declared_property_type(object: &Expr, property: &str, ctx: &Context) -> Option<PhpType> {
    let obj_ty = crate::codegen_support::functions::infer_contextual_type(object, ctx);
    let class_name = crate::codegen_support::functions::singular_object_class(&obj_ty)?;
