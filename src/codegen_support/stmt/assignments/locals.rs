...
//!
//! Called from:
//! - `crate::codegen_support::stmt::assignments`
//!
//! Key details:
    ...
    match &value.kind {
        ...
        _ => {
            if ty == PhpType::Callable {
                if let Some(sig) = crate::codegen_support::callables::callable_sig(value, ctx) {
                    ctx.closure_sigs.insert(name.to_string(), sig);
                    ctx.runtime_callable_vars.insert(name.to_string());
                } else {
                    ctx.closure_sigs.remove(name);
                ...
                if let Some(sig) = crate::codegen_support::callables::callable_array_sig(value, ctx) {
                    ctx.closure_sigs.insert(name.to_string(), sig);
                    ctx.runtime_callable_vars.insert(name.to_string());
                } else {
                    ctx.closure_sigs.remove(name);
...
fn update_fiber_start_metadata(name: &str, value: &Expr, ty: &PhpType, ctx: &mut Context) {
    if let Some(sig) = crate::codegen_support::fiber_sigs::fiber_start_sig_for_expr(value, ctx) {
...
fn specialize_callable_array_assignment_type(
    ...
    if is_callable_array_type(ty)
        ...
        || crate::codegen_support::callables::callable_array_sig(value, ctx).is_none()
    {
        return None;
    }
    Some(match ty {
...
fn emit_indexed_literal_as_assoc_target(
    ...
    if elems.is_empty() {
        return crate::codegen_support::expr::arrays::emit_empty_assoc_array_literal(
            *target_key_ty.clone(),
    ...
    let pairs: Vec<(Expr, Expr)> = elems
        ...
        .collect();
    crate::codegen_support::expr::arrays::emit_assoc_array_literal(&pairs, emitter, ctx, data)
}
...
fn emit_branch_if_result_non_null(ty: &PhpType, keep_label: &str, emitter: &mut Emitter) {
    if matches!(ty, PhpType::TaggedScalar) {
        crate::codegen_support::sentinels::emit_branch_if_tagged_scalar_not_null(emitter, keep_label);
        return;
    }
    if matches!(ty, PhpType::Int) && crate::codegen_support::sentinels::null_repr_is_tagged() {
