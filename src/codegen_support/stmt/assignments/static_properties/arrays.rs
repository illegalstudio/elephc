//! Purpose:
//! Lowers static property array mutation and append lowering.
//! Works with static property symbols and class metadata instead of local frame slots.
//!
//! Called from:
//! - `crate::codegen_support::stmt::assignments::static_properties`
//!
//! Key details:
...
pub(crate) fn emit_static_property_array_push_stmt(
    ...
    let boxed_iterable =
        crate::codegen_support::emit_box_iterable_value_for_mixed_container(emitter, &mut val_ty);
    if !boxed_iterable
...
pub(crate) fn emit_static_property_array_assign_stmt(
    ...
    if let Some((current, default)) =
        crate::codegen_support::stmt::null_coalesce_static_property_array_target(
        ...
        )
    {
        ...
        if current_ty != PhpType::Void {
            ...
            crate::codegen_support::stmt::emit_branch_if_result_non_null(
                &current_ty,
                &keep_label,
                emitter,
            );
    ...
    if crate::codegen_support::expr::arrays::type_is_array_access_object(
        &crate::codegen_support::functions::infer_contextual_type(&array_access_receiver, ctx),
        ctx,
    ) {
    ...
    match emitter.target.arch {
        Arch::AArch64 => match val_ty {
            ...
            PhpType::Mixed | PhpType::Array(_) | PhpType::AssocArray { .. } | PhpType::Object(_) => {
                ...
                crate::codegen_support::emit_release_pushed_refcounted_temp_after_array_push(emitter, val_ty); // drop the codegen's owning reference now that the static array holds its own
            }
