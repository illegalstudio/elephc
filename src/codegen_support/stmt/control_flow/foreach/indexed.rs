//! Purpose:
//! Lowers foreach over indexed arrays and runtime Mixed indexed iterables.
...
pub(crate) fn emit_indexed_foreach(
    ...
    if let Some(kv) = key_var {
        if let Some(kvar) = ctx.variables.get(kv) {
            ...
            crate::codegen_support::abi::store_at_offset_scratch(emitter, "x0", k_offset, "x10");
...
pub(crate) fn emit_indexed_foreach_runtime_mixed(
    ...
    if value_by_ref {
        ...
        if !ctx.ref_params.contains(value_var) {
            ...
            crate::codegen_support::abi::load_at_offset_scratch(emitter, "x0", val_offset, "x10");
...
fn emit_indexed_foreach_linux_x86_64(
    ...
    let stack_size = 48 + if value_by_ref { 16 } else { 0 } + if saved_ref_offset.is_some() { 16 } else { 0 };
    crate::codegen_support::abi::emit_push_reg(emitter, "rax");                              // preserve the indexed-array pointer across the foreach loop state setup
    emitter.instruction("mov r10, QWORD PTR [rax]");                            // load the indexed-array logical length before entering the foreach loop
    crate::codegen_support::abi::emit_push_reg(emitter, "r10");                              // preserve the indexed-array logical length in a dedicated foreach loop stack slot
    emitter.instruction("xor r10, r10");                                        // materialize the initial foreach loop index value 0 in a scratch register
    crate::codegen_support::abi::emit_push_reg(emitter, "r10");                              // preserve the current foreach loop index in a dedicated loop stack slot
    ...
    if let Some(kv) = key_var {
        if let Some(kvar) = ctx.variables.get(kv) {
            let k_offset = kvar.stack_offset;
            crate::codegen_support::abi::store_at_offset_scratch(emitter, "rax", k_offset, "r10"); // store the current foreach loop index into the loop key variable slot
            ctx.update_var_type_and_ownership(kv, PhpType::Int, HeapOwnership::NonHeap);
        } else {
            emitter.comment(&format!("WARNING: undefined foreach key variable ${}", kv));
        }
...
fn emit_indexed_foreach_runtime_mixed_linux_x86_64(
    ...
    if value_by_ref {
        ...
        if !ctx.ref_params.contains(value_var) {
            ...
            crate::codegen_support::abi::load_at_offset_scratch(emitter, "rax", val_offset, "r10");
...
fn store_runtime_mixed_index_key_aarch64(
    ...
    crate::codegen_support::abi::load_at_offset_scratch(emitter, "x0", offset, "x10");
...
fn store_runtime_mixed_index_key_x86_64(
    ...
    crate::codegen_support::abi::load_at_offset_scratch(emitter, "rax", offset, "r10");
