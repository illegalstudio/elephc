...
use crate::codegen_support::expr::objects::dispatch::{
...
fn emit_current_as_saved_iterator_receiver(emitter: &mut Emitter) -> PhpType {
    ...
    crate::codegen_support::abi::emit_incref_if_refcounted(
        emitter,
...
fn store_iterator_typed_result(
    ...
    crate::codegen_support::abi::store_at_offset_scratch(
        emitter,
...
fn store_iterator_mixed_result(
    ...
    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &result_ty.codegen_repr());
    match emitter.target.arch {
        Arch::AArch64 => {
        ...
        Arch::X86_64 => {
            crate::codegen_support::abi::emit_push_reg(emitter, "rax");                 // preserve the freshly returned mixed value across previous-value cleanup
            ...
            crate::codegen_support::abi::emit_pop_reg(emitter, "rax");                  // restore the new mixed value after cleanup
...
fn normalize_iterator_mixed_slot(
    ...
    crate::codegen_support::emit_box_current_value_as_mixed(emitter, &old_ty);
...
fn cleanup_replaced_iterator_slot(
    ...
    match old_ty {
        PhpType::Str => {
            ...
            crate::codegen_support::abi::load_at_offset_scratch(
                emitter,
                result_reg,
                old_offset,
                scratch_reg,
        ...
        ty if ty.is_refcounted() => {
            ...
            crate::codegen_support::abi::emit_decref_if_refcounted(emitter, ty);
        }
        _ => {}
    }
}
...
fn release_saved_iterator_receiver(release_ty: &PhpType, emitter: &mut Emitter) {
    ...
    crate::codegen_support::abi::emit_decref_if_refcounted(emitter, release_ty);
}
