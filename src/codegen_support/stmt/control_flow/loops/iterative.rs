...
use crate::codegen_support::context::{Context, LoopLabels};
...
pub(crate) fn emit_do_while_stmt(
    ...
    crate::codegen_support::expr::coerce_to_truthiness(emitter, ctx, &cond_ty);
    crate::codegen_support::abi::emit_branch_if_int_result_nonzero(emitter, &loop_start);
...
pub(crate) fn emit_while_stmt(
    ...
    let cond_ty = emit_expr(condition, emitter, ctx, data);
    crate::codegen_support::expr::coerce_to_truthiness(emitter, ctx, &cond_ty);
    crate::codegen_support::abi::emit_branch_if_int_result_zero(emitter, &loop_end);
    ...
    crate::codegen_support::abi::emit_jump(emitter, &loop_start);                       // unconditional branch back to loop start
    emitter.label(&loop_end);
}
