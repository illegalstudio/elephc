...
fn rewrite_date_procedural_alias(name: &str, args: &[Expr]) -> Option<ExprKind> {
    ...
    match bare.as_str() {
        ...
        "date_create" if args.len() <= 1 => Some(new_object("DateTime")),
        "date_create_immutable" if args.len() <= 1 => Some(new_object("DateTimeImmutable")),
        ...
        // baked from (see crates/elephc-tz), matching PHP's timelib version string.
        // Keep this in lockstep with that crate's data/*.data regeneration.
        "timezone_version_get" if args.is_empty() => {
            Some(ExprKind::StringLiteral("2026.1".to_string()))
        }
