...
fn ini_bitmask_atom_value(atom: &str, php_version: PhpVersion) -> i32 {
    ...
    if atom == "E_ALL" {
        return php_version.all_error_levels() as i32;
    }
    if atom.starts_with("E_") {
        if let Some(elephc_builtin_contract::ConstValue::Int(value)) =
            elephc_builtin_contract::lookup_constant(atom).map(|constant| constant.value)
        {
            return value as i32;
        }
    }
