...
impl NativeLock {
    ...
    pub fn from_manifest(manifest: &ManifestDocument) -> Result<Self, NativeError> {
        ...
        for (name, selected) in manifest.dependencies() {
            ...
            ensure_dependencies_declared(name, version.dependencies, manifest)?;
    ...
    pub fn validate_current(&self, manifest: &ManifestDocument) -> Result<(), NativeError> {
        ...
        if self != &expected {
            return Err(NativeError::new(
                ...
                "native lock is missing or stale",
            ));
        }
        Ok(())
    }
...
/// Fails closed until every catalog dependency is explicitly declared by the project.
fn ensure_dependencies_declared(
    package: &str,
    dependencies: &[&str],
    ...
    for dependency in dependencies {
        if !manifest.dependencies().contains_key(*dependency) {
            return Err(NativeError::new(
                ...
                format!(
                    "native package '{package}' depends on undeclared native package '{dependency}'"
                ),
            )
            .with_recovery(format!("elephc native add {dependency}")));
        }
    }
    Ok(())
}
...
mod tests {
    ...
    /// Verifies catalog dependencies cannot be silently omitted by manifest-only installation.
    #[test]
    fn undeclared_catalog_dependencies_fail_closed() {
        let manifest = manifest();
        let error =
            ensure_dependencies_declared("future-package", &["zlib"], &manifest).unwrap_err();
        assert_eq!(error.kind, NativeErrorKind::Manifest);
        assert!(error.message.contains("undeclared native package 'zlib'"));
        assert_eq!(error.recovery.as_deref(), Some("elephc native add zlib"));
    }
}
