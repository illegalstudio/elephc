...
/// Test-only. The compilation path builds this surface with `build::pdo_declarations`; this text
/// is the oracle that build is checked against, and `cfg(test)` is what keeps that true — if a
/// production path ever reads the PHP again, the crate stops compiling instead of silently
/// re-introducing 6,700 lines of tokenizing and parsing into every PDO compile.
#[cfg(test)]
...
final class __ElephcPDOSqliteBlobStream {
    ...
    public function stream_read(int $count): string {
        ...
        $_chunk = __elephc_ptr_read_string(elephc_pdo_blob_data_ptr(), $_length);
...
class PDO {
    ...
    public function __construct(string $dsn, ?string $username = null, #[\SensitiveParameter] ?string $password = null, ?array $options = null) {
        ...
        if ($this->conn < 0) {
            ...
            $_sqlstate = elephc_pdo_last_open_sqlstate();
            $_nativeCode = elephc_pdo_last_open_native_code();
            if ($_sqlstate === "") {
                $_sqlstate = (str_starts_with($_dsn, "sqlite:") || str_starts_with($_dsn, "oci:")) ? "HY000" : "08006";
            }
            $_nativeInfo = $_nativeCode == 0 ? null : $_nativeCode;
            throw PDOException::__elephcFromErrorInfo("SQLSTATE[" . $_sqlstate . "]: " . $_openMsg, [$_sqlstate, $_nativeInfo, $_openMsg]);
        }
        ...
        if (str_starts_with($_dsn, "mysql:")) {
            ...
            foreach ([0, 1, 1004] as $_ociIntAttribute) {
                if (isset($this->attributes[$_ociIntAttribute])) {
                    $_ociValue = $_ociIntAttribute == 0
                        ? ($this->attrBoolValue($this->attributes[$_ociIntAttribute]) ? 1 : 0)
                        : $this->attrIntValue($this->attributes[$_ociIntAttribute]);
                    elephc_pdo_oci_set_attribute_int($this->conn, $_ociIntAttribute, $_ociValue);
                }
            }
            foreach ([1000, 1001, 1002, 1003] as $_ociTextAttribute) {
                if (isset($this->attributes[$_ociTextAttribute])) {
    ...
    public function setAttribute(int $attribute, $value): bool {
        ...
        } elseif ($attribute == 1 && $_driver === "oci") {
            return elephc_pdo_oci_set_attribute_int($this->conn, 1, $this->attrIntValue($value)) === 1;
        } elseif ($attribute == 20) {
            if ($_driver === "dblib") {
        ...
        } elseif (($attribute == 1000 || $attribute == 1001 || $attribute == 1002 || $attribute == 1003) && $_driver === "oci") {
    ...
    public function getAttribute(int $attribute): mixed {
        ...
        if (($attribute == 0 || $attribute == 1 || $attribute == 1004) && elephc_pdo_driver_name($this->conn) === "oci") {
            ...
            if ($_ociValue >= 0) {
                return $attribute == 0 ? ($_ociValue === 1) : $_ociValue;
            }
        }
    ...
    public function prepare(string $query, array $options = []): PDOStatement|bool {
        ...
        } elseif ($_driver === "oci" && array_key_exists(1, $options)) {
            $_prefetchOverride = $this->attrIntValue($options[1]);
        }
        ...
        if ($_driver === "sqlsrv" && $_scrollable) {
            $_simple = $_simple | 2;
            if (array_key_exists(1003, $options)) {
                $_sqlsrvCursorType = $this->attrIntValue($options[1003]);
                if ($_sqlsrvCursorType !== 1 && $_sqlsrvCursorType !== 2
                    && $_sqlsrvCursorType !== 3 && $_sqlsrvCursorType !== 42) {
                    $this->failCode("IMSSP", "An invalid statement option was designated.");
                    return false;
                }
                $_simple = $_simple | ($_sqlsrvCursorType << 8);
            }
        } elseif ($_driver === "sqlsrv" && array_key_exists(1003, $options)) {
            $this->failCode("IMSSP", "The cursor type must be scrollable to use a scroll type.");
            return false;
        }
        if ($_driver === "sqlsrv") {
    ...
    public function quote(string $string, int $type = 2): string|false {
        ...
        if ($_driver === "cubrid") {
            ...
            return __elephc_ptr_read_string(elephc_pdo_blob_data_ptr(), $_length);
        }
...
// defer value lookup until access time. The compiler recognizes PDOStatement's internal
// construction/refresh calls while keeping both hooks private to user code, matching the
// non-public construction and mutation surface exposed by php-src's internal row object.
...
class PDOStatement implements IteratorAggregate {
    ...
    public function bindParam($parameter, mixed &$variable, int $type = 2, int $maxLength = 0, mixed $driverOptions = null): bool {
    ...
    // Copies completed native input/output buffers back into the durable reference
    // cells captured by bindParam(). Scalar outputs intentionally remain strings,
    // matching the PDO_OCI and CLI driver post-execute callbacks.
    private function syncOutputParameters(): void {
        $_refCount = count($this->boundParamRefIndexes);
        for ($_ri = 0; $_ri < $_refCount; $_ri++) {
            $_boundIndex = (int) $this->boundParamRefIndexes[$_ri];
            $_slot = (int) $this->boundParams[$_boundIndex];
            $_length = elephc_pdo_output_data($this->stmt, $_slot);
            if ($_length == -3) {
                continue;
            }
            $_setter = $this->boundParamRefSetters[$_ri];
            if (is_callable($_setter)) {
                callable $_typedSetter = $_setter;
                if ($_length == -2) {
                    call_user_func_array($_typedSetter, [null]);
                } else {
                    $_bytes = "";
                    if ($_length > 0) {
                        $_bytes = __elephc_ptr_read_string(elephc_pdo_blob_data_ptr(), $_length);
                    }
                    if (elephc_pdo_output_is_lob($this->stmt, $_slot) != 0) {
                        ...
                        call_user_func_array($_typedSetter, [$_bytes]);
                    }
                }
            }
        }
    ...
    public function execute(?array $params = null): bool {
        ...
        if ($_bindError !== "") {
            ...
            $this->executed = false;
            if ($_bindError === "__elephc_pdo_driver_error") {
                $this->failCode(elephc_pdo_stmt_sqlstate($this->stmt), elephc_pdo_stmt_errmsg($this->stmt));
                return false;
            }
        ...
        // The optional drivers below populate their bridge-owned column metadata only
        // when they execute. Run them once here, then inspect the materialized rowset
        // shape and cache a first row exactly as the ordinary SELECT prefetch path does.
        if (elephc_pdo_driver_name($this->conn) === "dblib"
            || elephc_pdo_driver_name($this->conn) === "firebird"
            || elephc_pdo_driver_name($this->conn) === "cubrid"
            || elephc_pdo_driver_name($this->conn) === "odbc"
            || elephc_pdo_driver_name($this->conn) === "informix"
            || elephc_pdo_driver_name($this->conn) === "ibm"
            || elephc_pdo_driver_name($this->conn) === "sqlsrv"
    ...
    private function columnValue(int $index): mixed {
        ...
        $_out = "";
        if ($_len > 0) {
            $_out = __elephc_ptr_read_string(elephc_pdo_column_data_ptr($this->stmt, $index), $_len);
        }
    ...
    public function fetch(int $mode = 0, int $cursorOrientation = 0, int $cursorOffset = 0): mixed {
        ...
        if ($_base == 8) {
            ...
            //
            // The internal PDO class classifier is lowered against the program's AOT metadata,
            // so hydrateClassOrStd() can select stdClass before attempting dynamic allocation.
            //
    ...
    // (zend_lookup_class() failing, pdo_stmt.c:805-829). The null test remains defensive;
    // hydrateClassOrStd() normally rejects unknown names with the AOT class classifier first.
    //
    // The object lives HERE, in a PARAMETER, rather than at the call site in a local, and
    // that placement is load-bearing: routing a dynamic allocation result through a caller LOCAL
    ...
    // Keeps the dynamic-new result in a callee parameter until hydration is complete. Passing the
    // object inline is required here: storing this Mixed result in the caller before the call loses
    // its concrete runtime class identity in the current EIR local representation.
    private function hydrateClassOrStdWithoutConstructor(mixed $object, string $class, int $start, int $count): mixed {
        if ($object === null) {
            return $this->assignColumnsFrom(new stdClass(), $start, $count);
        }
        $_object = $this->assignColumnsFrom($object, $start, $count);
        if (__elephc_class_has_constructor($class)) {
            call_user_func_array([$_object, "__construct"], $this->fetchCtorArgs);
        } elseif (count($this->fetchCtorArgs) != 0) {
            throw new Error("Class " . $class . " does not have a constructor, so you cannot pass any constructor arguments");
    ...
    private function hydrateClassOrStd(string $class, int $start, int $count): mixed {
        if (__elephc_pdo_statement_class_status($class) == 0) {
            return $this->assignColumnsFrom(new stdClass(), $start, $count);
        }
        if ($this->fetchPropsLate) {
            return $this->assignColumnsFromOrStd(new $class(...$this->fetchCtorArgs), $start, $count);
        }
        return $this->hydrateClassOrStdWithoutConstructor(
            __elephc_new_without_constructor($class),
            $class,
            $start,
            $count
        );
    ...
    public function getColumnMeta(int $column): array|false {
        ...
        if ($_driver === "informix") {
            $_informixFlags = [];
            $_informixFlagBits = elephc_pdo_column_flags($this->stmt, $column);
        ...
        if ($_driver === "oci") {
            $_ociFlags = [];
            $_ociFlagBits = elephc_pdo_oci_column_flags($this->stmt, $column);
            if (($_ociFlagBits & 1) !== 0) {
                $_ociFlags[] = "nullable";
            } else {
                $_ociFlags[] = "not_null";
            ...
            $_ociType = elephc_pdo_column_native_type($this->stmt, $column);
            return [
                "oci:decl_type" => $_ociType,
                "native_type" => $_ociType,
                "pdo_type" => elephc_pdo_oci_column_pdo_type($this->stmt, $column),
                "scale" => elephc_pdo_oci_column_scale($this->stmt, $column),
                "flags" => $_ociFlags,
            ];
        }
        if ($_driver === "dblib") {
...
namespace Pdo {
    ...
    class Sqlite extends \PDO {
        ...
        // only for PHP 8.5 and later. The baseline PHP 8.4 template intentionally ends
