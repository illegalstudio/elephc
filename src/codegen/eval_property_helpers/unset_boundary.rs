...
//! - Its internal unset helper consumes only the five borrowed words through the runtime ABI.
...
pub(super) fn emit(module: &Module, emitter: &mut Emitter) {
    ...
    if (emitter.target.platform, emitter.target.arch) == (Platform::Windows, Arch::X86_64) {
        ...
        for index in 4..6 {
            abi::load_from_caller_stack(emitter, scratch, 48 + (index - 4) * 8);
            abi::store_at_offset(emitter, scratch, (index + 1) * 8);
        }
    } else {
        for index in 0..6 {
    ...
    fn native_unset_boundary_contains_throws_and_versions_its_output_on_all_targets() {
        for name in ["macos-aarch64", "ios-arm64", "ios-sim-arm64", "linux-aarch64", "linux-x86_64", "windows-x86_64"] {
            ...
            if target.platform == Platform::Windows {
                assert!(asm.contains("mov r10, QWORD PTR [rbp + 48]"), "{name}: fifth C word must skip the shadow space");
                assert!(asm.contains("mov r10, QWORD PTR [rbp + 56]"), "{name}: sixth C word must follow the fifth C word");
                for (offset, register) in [(8, "rdi"), (16, "rsi"), (24, "rdx"), (32, "rcx"), (40, "r8")] {
                    assert!(
                        asm.contains(&format!("mov {register}, QWORD PTR [rbp - {offset}]")),
                        "{name}: internal helper argument at offset {offset} must use the runtime ABI"
                    );
                }
            }
        }
