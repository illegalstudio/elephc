...
//! `__rt_hash_update`, `__rt_hash_final`, `__rt_hash_copy`) backing PHP's
//! `hash_init`/`hash_update`/`hash_final`/`hash_copy`. Each calls the elephc-crypto
//! incremental C ABI through a fail-closed function-pointer slot.
//!
//! Called from:
...
//!   boxed as a Mixed cell (tag 9) exactly like `fopen` boxes a file descriptor.
//!   `hash_update`/`hash_final`/`hash_copy` receive the already-unboxed raw handle
//!   (the emitter uses `emit_stream_fd_arg`).
