...
pub fn emit_hash_new(emitter: &mut Emitter) {
    ...
    // -- calculate total size: 24 + capacity * 40 --
    emitter.instruction("mov x9, #40");                                         // entry size = 40 bytes
    ...
    emitter.instruction("add x11, x0, #24");                                    // x11 = base of entries region
    emitter.instruction("mov x12, #40");                                        // x12 = entry size
    ...
    emitter.instruction("add x11, x11, #40");                                   // advance to next entry
