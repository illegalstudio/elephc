...
//!   kind 2 = HashContext (elephc_crypto_free).
...
pub fn emit_mixed_free_deep(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov x9, #0x40000000");                                 // load the synthetic user-wrapper handle threshold into a scratch register

    emitter.instruction("cmp x0, x9");                                          // synthetic handles are managed by the wrapper layer

    emitter.instruction("b.hs __rt_mixed_free_deep_box");                       // skip close for synthetic handles
    ...
    emitter.instruction("bl __rt_hash_ctx_free");                               // free an unfinalized HashContext through the indirect crypto slot
...
fn emit_mixed_free_deep_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("cmp rdi, 0x40000000");                                 // synthetic user-wrapper handle threshold

    emitter.instruction("jae __rt_mixed_free_deep_box");                        // synthetic handles are managed by the wrapper layer
    ...
    emitter.instruction("call __rt_hash_ctx_free");                             // free an unfinalized HashContext through the indirect crypto slot
