...
pub fn emit_gc_collect_cycles(emitter: &mut Emitter) {
    ...
    emitter.instruction("ldr x10, [x12, x10]");                                    // load the runtime metadata / length word for this property slot
    emitter.instruction("cmp x10, #4");                                            // was this property last written with an indexed array?
    emitter.instruction("b.eq __rt_gc_collect_cycles_count_object_child");         // count nested array property pointers
    emitter.instruction("cmp x10, #5");                                            // was this property last written with an associative array?
