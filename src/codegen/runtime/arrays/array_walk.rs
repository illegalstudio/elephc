//! Purpose:
//! Emits the `__rt_array_walk`, `__rt_array_walk_loop` runtime helper assembly for array walk.
...
pub fn emit_array_walk(emitter: &mut Emitter) {
    ...
    emitter.instruction("cbz x9, __rt_array_walk_call");                        // keep legacy one-argument callback ABI when no environment is present
    ...
    emitter.label("__rt_array_walk_call");
