//! Purpose:
//! Emits the `__rt_array_diff_refcounted`, `__rt_array_new` runtime helper assembly for array diff refcounted.
...
pub fn emit_array_diff_refcounted(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_array_diff_refcounted_linux_x86_64(emitter);
        return;
    }
...
fn emit_array_diff_refcounted_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("push rbp");                                            // preserve the caller frame pointer before reserving refcounted array-diff spill slots
    emitter.instruction("mov rbp, rsp");                                        // establish a stable frame base for both inputs, the result array, and the outer loop index
    emitter.instruction("sub rsp, 32");                                         // reserve aligned spill slots for the refcounted array-diff bookkeeping
    ...
    emitter.label("__rt_array_diff_ref_outer_x86");
    ...
    emitter.instruction("jge __rt_array_diff_ref_done_x86");                    // finish once every payload slot from the first input indexed array has been examined
    ...
    emitter.label("__rt_array_diff_ref_inner_x86");
    emitter.instruction("cmp r9, r11");                                         // compare the inner scan index against the second input indexed-array logical length
    emitter.instruction("jge __rt_array_diff_ref_not_found_x86");               // append the candidate payload once the full second input indexed array has been scanned without a match
    ...
    emitter.instruction("je __rt_array_diff_ref_found_x86");                    // skip appending when the candidate payload already exists in the second input indexed array
    ...
    emitter.instruction("jmp __rt_array_diff_ref_inner_x86");                   // continue scanning the second input indexed array for the current candidate payload

    emitter.label("__rt_array_diff_ref_not_found_x86");
    emitter.instruction("mov rdi, QWORD PTR [rbp - 24]");                       // reload the result indexed-array pointer before appending the unmatched candidate payload
    emitter.instruction("mov rsi, r8");                                         // place the unmatched candidate refcounted payload in the second x86_64 append helper argument register
    emitter.instruction("call __rt_array_push_refcounted");                     // append the retained unmatched candidate payload into the result indexed array
    ...
    emitter.label("__rt_array_diff_ref_found_x86");
    ...
    emitter.instruction("jmp __rt_array_diff_ref_outer_x86");                   // continue examining refcounted payloads from the first input indexed array

    emitter.label("__rt_array_diff_ref_done_x86");
    ...
    emitter.instruction("add rsp, 32");                                         // release the refcounted array-diff spill slots before returning
    emitter.instruction("pop rbp");                                             // restore the caller frame pointer before returning to the caller
    emitter.instruction("ret");                                                 // return the refcounted array-diff result indexed-array pointer in rax
}
