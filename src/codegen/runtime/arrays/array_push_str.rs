...
pub fn emit_array_push_str(emitter: &mut Emitter) {
    ...
    emitter.instruction("stp x1, x2, [sp, #0]");                                // save string ptr and len

    emitter.instruction("bl __rt_array_grow");                                  // double array capacity → x0 = new array

    emitter.instruction("ldp x1, x2, [sp, #0]");                                // restore string ptr and len
    emitter.instruction("ldr x9, [x0]");                                        // reload length from new array
    emitter.instruction("lsl x10, x9, #4");                                     // x10 = length * 16
    emitter.instruction("add x10, x0, x10");                                    // x10 = array base + offset
    emitter.instruction("add x10, x10, #24");                                   // x10 = skip header
    emitter.instruction("str x1, [x10]");                                       // store string pointer
    emitter.instruction("str x2, [x10, #8]");                                   // store string length
    emitter.instruction("add x9, x9, #1");                                      // length += 1
