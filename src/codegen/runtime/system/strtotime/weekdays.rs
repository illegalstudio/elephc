...
fn emit_weekdays_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov rcx, r10");                                        // rcx = end pointer
    emitter.instruction("sub rcx, QWORD PTR [rsp + 112]");                     // rcx = remaining = end - cursor_after_lc
    emitter.instruction("mov r8, 16");                                         // cap window to lc16 size
