...
pub(crate) fn emit_function_variant_dispatcher(
    ...
    match emitter.target.arch {
        Arch::AArch64 => {
            emitter.instruction(&format!("cbz {}, {}", target_reg, fail_label)); // abort if no include branch has loaded this conditional function
            emitter.instruction(&format!("br {}", target_reg));                 // tail-dispatch to the loaded function variant without changing arguments
        }
        Arch::X86_64 => {
            emitter.instruction(&format!("test {}, {}", target_reg, target_reg)); // abort if no include branch has loaded this conditional function
