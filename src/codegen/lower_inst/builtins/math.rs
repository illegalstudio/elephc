...
pub(crate) fn lower_pi(ctx: &mut FunctionContext<'_>, inst: &Instruction) -> Result<()> {
    ...
    match ctx.emitter.target.arch {
        ...
        Arch::X86_64 => {
            ctx.emitter.instruction(                                            // load the M_PI floating constant into the floating result register
...
fn emit_throw_value_error_x86_64(
    ...
    ctx.emitter.instruction(                                                    // load ValueError's runtime class id for this program
...
fn load_float_literal_to_reg(ctx: &mut FunctionContext<'_>, reg: &str, value: f64) {
    ...
    match ctx.emitter.target.arch {
        ...
        Arch::X86_64 => {
            ctx.emitter.instruction(                                            // load the floating-point comparison constant through the symbol scratch register
...
fn lower_float_rounding_builtin(
    ...
    match ctx.emitter.target.arch {
        ...
        Arch::X86_64 => {
            ctx.emitter.instruction(                                            // round the floating-point argument with the builtin's direction
...
fn emit_int_abs_for_result(
    ...
    match ctx.emitter.target.arch {
        Arch::AArch64 => {
            ctx.emitter.instruction(                                            // negative inputs need the overflow-checked negation
                &format!("tbnz {}, #63, {}", result_reg, negative_label)
            );                                                                  // negative inputs need the overflow-checked negation
        }
        Arch::X86_64 => {
            ctx.emitter.instruction(                                            // inspect the sign of the integer operand
