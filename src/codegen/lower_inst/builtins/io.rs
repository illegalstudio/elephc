...
///
/// Used to decide whether a stream address can select a crypto transport before
/// emitting the handshake-on-connect path. Reading the literal here keeps the
/// decision aligned with `stream_socket_client_requirements`, which inspects the
/// same literal on the AST side: skipping the path is what lets a program with a
/// plain `tcp://` address avoid linking the TLS bridge at all.
...
///
/// A literal address answers definitively. Anything else -- a variable, a
/// concatenation, a function result -- could carry any scheme at run time, so the
/// handshake path must be emitted.
...
///
/// php reads the ini setting; elephc has no ini layer for it yet, so the default
/// stands in. Naming it keeps the one place that would have to change.
const DEFAULT_SOCKET_TIMEOUT_SECONDS: u32 = 60;

/// Bounds the handshake that `stream_socket_client()` runs on a crypto transport.
///
/// Without this the handshake blocks forever against a peer that completes the TCP
/// connect and then says nothing: rustls waits on a read that never arrives, and
/// the descriptor carries no timeout. Measured against php 8.5.6 with a peer that
/// accepts and stays silent -- php returns `false` after `default_socket_timeout`
/// (10.0s with the ini set to 10), elephc never returned at all. A hung client also
/// costs far more than a failing one: it takes a whole CI shard down with it.
///
/// php gets this for free because the stream carries `default_socket_timeout` as
/// its read timeout, and `php_stream_xport_crypto_enable` negotiates through the
/// stream. The descriptor is already staged in the attach frame at `[sp]`, so the
/// timeout is applied from there and register clobbering is irrelevant. A plain
/// file descriptor makes `setsockopt` fail, which the helper reports as 0 and this
/// path ignores -- the handshake is what needs bounding, not the syscall.
fn emit_crypto_handshake_deadline(ctx: &mut FunctionContext<'_>) {
    match ctx.emitter.target.arch {
        Arch::AArch64 => {
            ctx.emitter.instruction("ldr x0, [sp, #0]");                        // the descriptor staged for the attach
            ctx.emitter
                .instruction(&format!("mov x1, #{}", DEFAULT_SOCKET_TIMEOUT_SECONDS)); // php's default_socket_timeout
            ctx.emitter.instruction("mov x2, #0");                              // no sub-second component
            abi::emit_call_label(ctx.emitter, "__rt_stream_set_timeout");
        }
        Arch::X86_64 => {
            ctx.emitter.instruction("mov rdi, QWORD PTR [rsp + 0]");            // the descriptor staged for the attach
            ctx.emitter
                .instruction(&format!("mov rsi, {}", DEFAULT_SOCKET_TIMEOUT_SECONDS)); // php's default_socket_timeout
            ctx.emitter.instruction("mov rdx, 0");                              // no sub-second component
            abi::emit_call_label(ctx.emitter, "__rt_stream_set_timeout");
        }
    }
}

/// Emits the AArch64 handshake-on-connect path for `stream_socket_client()`.
///
/// php-src registers `ssl`, `sslv3`, `tls` and `tlsv1.0`..`tlsv1.3` as crypto
/// transports whose factory sets `sslsock->enable_on_connect = 1`
/// (`php_openssl_ssl_socket_factory`, ext/openssl/xp_ssl.c), so the TLS handshake
/// runs *inside* `stream_socket_client()` and a failed handshake makes the call
/// return `false`. Without this the function handed back a working plaintext
/// socket for `tls://`, silently downgrading every such connection.
///
/// Entry: the integer result register holds the connected descriptor, and the
/// caller's 16-byte scratch frame still holds the address pointer at `[sp, #0]`
/// and its length at `[sp, #8]`. That frame is reused to carry the descriptor
/// across the attach, so it must stay live until this returns. Exit: the result
/// register holds the descriptor to expose, or -1 when the handshake failed.
///
/// Unlike `stream_socket_enable_crypto()`, which reports 0 for "call me again"
/// on a non-blocking stream, this path treats any non-positive status as failure:
/// `stream_socket_client()` connects blocking, so a handshake that has not
/// completed by the time the bridge returns never will -- a peer that closes
/// mid-handshake surfaces as 0, and handing back a stream there would be the very
/// plaintext downgrade this path exists to prevent.
fn emit_stream_client_crypto_on_connect_aarch64(ctx: &mut FunctionContext<'_>) {
    ...
    ctx.emitter.instruction("cmp x0, #0");                                      // did the connect itself fail?
    ctx.emitter.instruction(&format!("b.lt {}", plain));                        // nothing to negotiate on a failed connect
    ctx.emitter.instruction("mov x3, x0");                                      // hold the descriptor while the address is probed
    ctx.emitter.instruction("ldr x0, [sp, #0]");                                // address pointer
    ctx.emitter.instruction("ldr x1, [sp, #8]");                                // address byte length
    ctx.emitter.instruction("str x3, [sp, #0]");                                // the descriptor replaces the pointer in the scratch slot
    abi::emit_call_label(ctx.emitter, "__rt_addr_tls_crypto_method");
    ctx.emitter.instruction(&format!("cbz x0, {}", restore));                   // zero means a plain transport
    ctx.emitter.instruction("mov x4, x0");                                      // preserve the scheme's PHP crypto method
    ...
    ctx.emitter.instruction("str x9, [sp, #0]");                                // descriptor slot expected by the shared attach path
    ctx.emitter.instruction("str x4, [sp, #8]");                                // the transport pins the crypto method
    ctx.emitter.instruction("mov x9, #1");                                      // the method is always explicit here
    ctx.emitter.instruction("str x9, [sp, #16]");                               // the bridge rejects an absent method outright
    ctx.emitter.instruction("str xzr, [sp, #24]");                              // no session is being resumed
    ...
    ctx.emitter.instruction("cmp x0, #0");                                      // 1 completed, 0 still handshaking, negative terminal
    ctx.emitter.instruction(&format!("b.le {}", failed));                       // only a completed handshake may hand back a stream
    ctx.emitter.label(&restore);
    ctx.emitter.instruction("ldr x0, [sp, #0]");                                // hand back the descriptor, not the probe result
    ctx.emitter.instruction(&format!("b {}", plain));                           // join the common exit
    ctx.emitter.label(&failed);
    ctx.emitter.instruction("ldr x0, [sp, #0]");                                // close the socket the failed session no longer owns
    ctx.emitter.bl_c("close");                                                  // php does not leak the descriptor on failure
    ctx.emitter.instruction("mov x0, #-1");                                     // the false sentinel understood by the result boxing
    ctx.emitter.label(&plain);
}

/// Emits the x86_64 handshake-on-connect path for `stream_socket_client()`.
///
/// Mirrors `emit_stream_client_crypto_on_connect_aarch64`; see it for the php-src
/// reference and the scratch-frame contract.
fn emit_stream_client_crypto_on_connect_x86_64(ctx: &mut FunctionContext<'_>) {
    ...
    ctx.emitter.instruction("cmp rax, 0");                                      // did the connect itself fail?
    ctx.emitter.instruction(&format!("jl {}", plain));                          // nothing to negotiate on a failed connect
    ctx.emitter.instruction("mov rcx, rax");                                    // hold the descriptor while the address is probed
    ctx.emitter.instruction("mov rdi, QWORD PTR [rsp + 0]");                    // address pointer
    ctx.emitter.instruction("mov rsi, QWORD PTR [rsp + 8]");                    // address byte length
    ctx.emitter.instruction("mov QWORD PTR [rsp + 0], rcx");                    // the descriptor replaces the pointer in the scratch slot
    abi::emit_call_label(ctx.emitter, "__rt_addr_tls_crypto_method");
    ctx.emitter.instruction("test rax, rax");                                   // zero means a plain transport
    ctx.emitter.instruction(&format!("jz {}", restore));                        // leave the socket in the clear
    ctx.emitter.instruction("mov r8, rax");                                     // preserve the scheme's PHP crypto method
    abi::emit_reserve_temporary_stack(ctx.emitter, 32);
    ctx.emitter.instruction("mov r9, QWORD PTR [rsp + 32]");                    // reload the descriptor from the caller scratch frame
    ctx.emitter.instruction("mov QWORD PTR [rsp + 0], r9");                     // descriptor slot expected by the shared attach path
    ctx.emitter.instruction("mov QWORD PTR [rsp + 8], r8");                     // the transport pins the crypto method
    ctx.emitter.instruction("mov QWORD PTR [rsp + 16], 1");                     // the bridge rejects an absent method outright
    ctx.emitter.instruction("mov QWORD PTR [rsp + 24], 0");                     // no session is being resumed
    emit_crypto_handshake_deadline(ctx);
    lower_stream_socket_enable_crypto_attach_x86_64(ctx, &attached);
    ctx.emitter.label(&attached);
    ctx.emitter.instruction("cmp rax, 0");                                      // 1 completed, 0 still handshaking, negative terminal
    ctx.emitter.instruction(&format!("jle {}", failed));                        // only a completed handshake may hand back a stream
    ctx.emitter.label(&restore);
    ctx.emitter.instruction("mov rax, QWORD PTR [rsp + 0]");                    // hand back the descriptor, not the probe result
    ctx.emitter.instruction(&format!("jmp {}", plain));                         // join the common exit
    ctx.emitter.label(&failed);
    ctx.emitter.instruction("mov rdi, QWORD PTR [rsp + 0]");                    // close the socket the failed session no longer owns
    ctx.emitter.emit_call_c("close");                                           // php does not leak the descriptor on failure
    ctx.emitter.instruction("mov rax, -1");                                     // the false sentinel understood by the result boxing
    ctx.emitter.label(&plain);
}
...
pub(crate) fn lower_fclose(ctx: &mut FunctionContext<'_>, inst: &Instruction) -> Result<()> {
    ...
    if ctx.emitter.target.platform != Platform::Windows {
        if matches!(ctx.emitter.target.arch, Arch::X86_64) {
            // The descriptor lives in rax here, and this helper reads it from rdi.
            // Staging it looks redundant because rdi usually still holds the fd from
            // the TLS teardown's own map lookup -- but only while that teardown finds
            // no session to close. When it does find one it loads the session handle
            // into rdi and calls the bridge, and the close leaves rdi holding whatever
            // it pleases. `__rt_user_filter_release_fd` then indexes its instance
            // table with that value instead of a descriptor, which is how closing a
            // TLS stream came to fault on linux-x86_64 while closing a plain one --
            // and every other fd-taking call in this function stages rdi explicitly.
            ctx.emitter.instruction("mov rdi, rax");                            // pass the stream fd to the user-filter release helper
        }
        abi::emit_call_label(ctx.emitter, "__rt_user_filter_release_fd");
    }
    match ctx.emitter.target.arch {
...
pub(crate) fn lower_fsockopen(ctx: &mut FunctionContext<'_>, inst: &Instruction) -> Result<()> {
    ...
    match ctx.emitter.target.arch {
        Arch::AArch64 => {
            ...
            ctx.emitter.instruction("sub sp, sp, #16");                         // scratch frame the crypto-on-connect helper reads
            ctx.emitter.instruction("str x0, [sp, #0]");                        // hostname pointer, which carries any crypto scheme
            ctx.emitter.instruction("str x1, [sp, #8]");                        // hostname byte length
        }
        Arch::X86_64 => {
            load_string_to_result(ctx, host, "fsockopen host")?;
            abi::emit_push_reg_pair(ctx.emitter, "rax", "rdx");
            ...
            ctx.emitter.instruction("sub rsp, 16");                             // scratch frame the crypto-on-connect helper reads
            ctx.emitter.instruction("mov QWORD PTR [rsp + 0], rdi");            // hostname pointer, which carries any crypto scheme
            ctx.emitter.instruction("mov QWORD PTR [rsp + 8], rsi");            // hostname byte length
        }
    }
    abi::emit_call_label(ctx.emitter, "__rt_fsockopen");
    // php accepts a crypto scheme in the hostname and hands back an encrypted
    // stream, because fsockopen and stream_socket_client share one transport
    // factory. The connect happens inside __rt_fsockopen, so the handshake is
    // driven here, on the hostname the caller passed.
    let may_negotiate = address_may_select_tls(ctx, host);
    if may_negotiate {
        // rustls needs a server name to build the client session, and the crypto
        // helper reads it from the per-descriptor connect-host table. The
        // stream_socket_client lowering fills that table itself; fsockopen reaches
        // the socket through __rt_fsockopen, so it must record the hostname here or
        // the attach fails and the connect reports false.
        match ctx.emitter.target.arch {
            Arch::AArch64 => {
                ctx.emitter.instruction("ldr x1, [sp, #0]");                    // hostname pointer
                ctx.emitter.instruction("ldr x2, [sp, #8]");                    // hostname byte length
            }
            Arch::X86_64 => {
                ctx.emitter.instruction("mov rdi, rax");                        // connected descriptor
                ctx.emitter.instruction("mov rsi, QWORD PTR [rsp + 0]");        // hostname pointer
                ctx.emitter.instruction("mov rdx, QWORD PTR [rsp + 8]");        // hostname byte length
            }
        }
        abi::emit_call_label(ctx.emitter, "__rt_stash_connect_host");
    }
    match ctx.emitter.target.arch {
        Arch::AArch64 => {
            ...
            ctx.emitter.instruction("add sp, sp, #16");                         // release the hostname scratch frame
        }
        Arch::X86_64 => {
            if may_negotiate {
                emit_stream_client_crypto_on_connect_x86_64(ctx);
            }
            ctx.emitter.instruction("add rsp, 16");                             // release the hostname scratch frame
        }
    }
...
pub(crate) fn lower_sys_get_temp_dir(
    ...
    // php resolves the directory once, in php_get_temporary_directory: TMPDIR
    // first, minus exactly one trailing slash, then P_tmpdir verbatim. tmpfile()
    // reads it through the same helper, so the two can never disagree about where
    // temporary files belong.
    abi::emit_call_label(ctx.emitter, "__rt_php_temp_dir");
    store_if_result(ctx, inst)
}
...
fn lower_stream_socket_enable_crypto_attach_aarch64(
    ...
    ctx.emitter.instruction("sxtw x0, w0");                                     // the bridge returns i32: bits 32-63 of x0 are undefined, so a
                                                                                // negative status would read as a large positive 64-bit value
...
fn lower_stream_socket_enable_crypto_attach_x86_64(
    ...
    ctx.emitter.instruction("movsxd rax, eax");                                 // the bridge returns i32: the upper half of rax is undefined, so a
                                                                                // negative status would read as a large positive 64-bit value
