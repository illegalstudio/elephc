...
fn lower_angle_conversion(
    ...
    match ctx.emitter.target.arch {
        ...
        Arch::X86_64 => {
            ctx.emitter.instruction(                                            // load the angle conversion multiplier from the data section
                &format!("movsd xmm1, QWORD PTR [{}]", scratch)
            );                                                                  // load the angle conversion multiplier from the data section
