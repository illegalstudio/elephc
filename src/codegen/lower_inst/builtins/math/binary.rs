...
fn emit_intdiv_overflow_throw(ctx: &mut FunctionContext<'_>) {
    ...
    match ctx.emitter.target.arch {
        Arch::AArch64 => {
            ...
            abi::emit_store_reg_to_symbol(ctx.emitter, "x0", "_exc_value", 0);  // publish the active ArithmeticError object
        ...
        Arch::X86_64 => {
            ...
            abi::emit_store_reg_to_symbol(ctx.emitter, "rax", "_exc_value", 0); // publish the active ArithmeticError object
