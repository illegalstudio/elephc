...
fn lower_hash_append_aarch64(
    ...
    // The next key is computed in one place, `__rt_hash_append`, which also refuses to go past
    // `PHP_INT_MAX` the way PHP does (#1315); an inline copy of that scan used to wrap instead.
    materialize_hash_value_aarch64(ctx, value, value_ty, storage_value_ty)?;
    ctx.emitter.instruction("mov x1, x3");                                      // pass the materialized value low word to hash_append
    ctx.emitter.instruction("mov x2, x4");                                      // pass the materialized value high word to hash_append
    abi::emit_load_int_immediate(ctx.emitter, "x3", hash_set_value_tag(value_ty, storage_value_ty));
    ctx.load_value_to_reg(hash, "x0")?;
...
fn lower_hash_append_x86_64(
    ...
    // See `lower_hash_append_aarch64`: the next key comes from `__rt_hash_append` only.
    materialize_hash_value_x86_64(ctx, value, value_ty, storage_value_ty)?;
    ctx.emitter.instruction("mov rsi, rcx");                                    // pass the materialized value low word to hash_append
    ctx.emitter.instruction("mov rdx, r8");                                     // pass the materialized value high word to hash_append
    abi::emit_load_int_immediate(ctx.emitter, "rcx", hash_set_value_tag(value_ty, storage_value_ty));
    ctx.load_value_to_reg(hash, "rdi")?;
    abi::emit_call_label(ctx.emitter, "__rt_hash_append");
    Ok(())
}
