...
fn declared_mixed_property_get_candidates(
    ctx: &FunctionContext<'_>,
    inst: &Instruction,
) -> Result<Vec<MixedPropertyCandidate>> {
    let mut candidates = Vec::new();
...
fn emit_runtime_stdclass_get_for_stacked_name(
    ctx: &mut FunctionContext<'_>,
    inst: &Instruction,
    object_stack_offset: usize,
...
fn load_property_store_value_to_result(
    ...
    if can_convert_indexed_array_to_mixed_property(&value_ty, slot_ty) {
        ...
        // Give the conversion helper an owned candidate. Its COW split consumes
        // that retain while leaving the SSA source untouched, and the returned
        // unique array transfers directly into the property slot.
    ...
    if can_store_assoc_array_as_mixed_property(&value_ty, slot_ty) {
        ...
        // Retain before a possible COW conversion so `PropSet` never consumes
        // the SSA source. The retained value itself is the property owner when
        // the hash already stores Mixed entries.
        abi::emit_incref_if_refcounted(ctx.emitter, &loaded_ty);
