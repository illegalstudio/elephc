...
fn emit_reflection_attribute_new_x86_64(
    emitter: &mut Emitter,
    layout: &ReflectionAttributeLayout,
) {
    ...
    emitter.instruction("sub rsp, 48");                                         // reserve slots for inputs, object, and unboxed args
    ...
    emitter.instruction("mov QWORD PTR [rbp - 32], rax");                       // save the unboxed ReflectionAttribute object pointer
    ...
    emitter.instruction("mov rdi, QWORD PTR [rbp - 32]");                       // move the ReflectionAttribute object pointer into the Mixed payload
...
fn emit_set_name_property_x86_64(emitter: &mut Emitter, layout: &ReflectionAttributeLayout) {
    ...
    emitter.instruction("mov r10, QWORD PTR [rbp - 32]");                       // reload the ReflectionAttribute object pointer
...
fn emit_set_args_property_x86_64(
    ...
    emitter.instruction("mov QWORD PTR [rbp - 40], rdi");                       // save the unboxed argument array across incref
    ...
    emitter.instruction("mov rdi, QWORD PTR [rbp - 40]");                       // reload the retained argument array payload
    emitter.instruction("mov r10, QWORD PTR [rbp - 32]");                       // reload the ReflectionAttribute object pointer
