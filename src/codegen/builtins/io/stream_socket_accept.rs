...
fn emit_store_peer_name(arg: &Expr, emitter: &mut Emitter, ctx: &mut Context) {
    ...
    match emitter.target.arch {
        ...
        Arch::X86_64 => {
            ...
            emitter.instruction("lea r9, [rip + _accept_peer_ptr]");            // address of the stashed-pointer global
            emitter.instruction("mov r10, QWORD PTR [r9]");                     // load the stashed peer address pointer
            emitter.instruction("lea r9, [rip + _accept_peer_len]");            // address of the stashed-length global
