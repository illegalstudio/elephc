...
fn runtime_cache_prologue() -> Vec<Stmt> {
    ...
    vec![
        ...
        //
        // THE DENOMINATOR IS ALL THREE COUNTERS, the same `reqs` the blacklist ratio below
        // uses: php-src's internal `misses` includes blacklist misses, and the `misses` it
        // REPORTS has them subtracted back out, so reconstructing `reqs` from the reported
        // figures means adding `blacklist_misses` back. VERIFIED on reference PHP 8.5.10 with
        // hits=5 misses=2 blacklist_misses=1 reporting 62.5 — which is 5*100/8, not 5*100/7.
        s_assign(
            "__elephc_rt_lookups",
            e_binop(
...
pub(crate) fn compile_file_decl(enabled: bool, manifest_paths: Expr) -> Stmt {
    ...
    // entry, and php-src serves one without reopening the file when `validate_timestamps=0`.
    // MEASURED: compile, `unlink()`, compile again — reference answers `true, true`.
    //
    // A file that cannot be OPENED warns twice, `Failed to open stream: <strerror>` then
    // `Failed opening ... for inclusion`, and only the bridge holds the `io::Error` that names
    // the real reason. This body used to word the warnings itself and could only guess from
    // `realpath()`: it answered a mode-`0000` file in silence, and called a file under a
    // directory without search permission missing where reference says `Permission denied`.
    // MEASURED, both. A file that opens but does not parse still answers `false` quietly — the
    // pinned divergence where reference throws.
    //
    // `$filename`, not `$path`, in both arms: the warning names the path as the caller spelled
    // it, as reference does, and the bridge canonicalizes it for the cache key itself.
    body.extend(path_normalization_stmts_with_fallback(vec![s_return(e_binop(
