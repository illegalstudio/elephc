...
pub(crate) fn link(
    ...
    if target.platform == Platform::Windows {
        ...
        // in by the PDO and image bridges. advapi32 resolves the ACL family
        // (`OpenThreadToken`/`ImpersonateSelf`/`RevertToSelf`/
        // `GetFileSecurityW`/`MapGenericMask`/`AccessCheck`) the php-parity
        // `is_readable()`/`is_writable()` check emits unconditionally
        // (`codegen_support::runtime::win32::shims_fs::emit_win_acl_helpers`).
        ld_cmd.args([
            "-lkernel32",
            "-lmsvcrt",
            "-lwinmm",
