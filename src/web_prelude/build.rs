...
/// `elephc_web_handle_uncaught_exception` — transcribed from the PHP form.
fn decl_extern_elephc_web_handle_uncaught_exception() -> Stmt {
    extern_fn("elephc_web_handle_uncaught_exception", "elephc_web")
        .returns(CType::Void)
        .build()
}
                ...
                vec![
                    s_if(
                        ...
                        vec![s_if(
                            ...
                            vec![s_expr(e_method_call(e_var("__elephc_h"), "close", vec![]))],
                            vec![],
                            None,
                        )],
                        vec![],
...
fn decl_fn_session_create_id() -> Stmt {
    function("session_create_id")
        ...
        .body(vec![
            ...
            // An empty result is entropy failure whatever the prefix was, so the `false` return sits
            // OUTSIDE the prefix test: nesting it under `$prefix !== ''` let an empty generated id
            // escape as a string for the default empty prefix.
            s_if(
                e_binop(e_var("__elephc_created_id"), BinOp::StrictEq, e_str("")),
                vec![
                    s_if(
...
pub(crate) fn web_declarations(
    ...
    internal_declarations(|| {
        let mut declarations: Program = vec![
            ...
            decl_extern_elephc_web_handle_uncaught_exception(),
